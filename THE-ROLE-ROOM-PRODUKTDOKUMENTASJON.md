...
| Spilltest & Swift-runtime | Swift Package for iPad/RealityKit (samme delsett som Unity/Godot, paritetstest); spillet sender scene-hendelser (enter/exit/choice/death/complete) med hashet prosjekt-token til et alltid-204-inntak uten PII; Spilltest-fane på scenekortet med økter, drop-off, median tid og valgfordeling per build; hjem-KPI «Spilltest» | ✅ Fase 8e (alle planer; gating kommer i 8g) |
| Lesning & referansebilde | «Les opp scenen» med én stemme per taler (nettleser-TTS gratis, KI-stemmer på Pro/Studio), EN/NB, stemmecast fra karakterprofil; «Generer referansebilde» på storyboardet fra scenekort + lokasjon + plattformens visuelle retning (DALL·E), lagret i objektlager som ramme, 10 per prosjekt per dag | ✅ Fase 8f (lesning alle planer; KI-stemmer og referansebilde Pro/Studio) |
| Salgsklar (Fase 8g) | Prosjektmaler (tomt / demo-eventyr / WFU-utdrag), plan-features `ci_evidence` og `playtest_telemetry` på Studio, rate-limit på mutasjoner, `maxProjects`, revisjons-retensjon, studio-guide og API-referanse, oppdatert landingskort | ✅ Fase 8g |
