...
export interface VerifyOptions {
   ...
   * Kladden får navnet reknaren_restore_verify_<tilfeldig>.
   */
...
export async function verifyBackup(opts: VerifyOptions): Promise<VerifyResult> {
  ...
  const scratchName = `reknaren_restore_verify_${randomBytes(6).toString('hex')}`;
  ...
  try {
    ...
    try {
      ...
      const migrations = await scratch.query(`SELECT COUNT(*)::INT AS n FROM _reknaren_migrations`);
      ...
      if (manifest.storageArchive) {
        extractedStorage = await mkdtemp(join(tmpdir(), 'reknaren-restore-verify-'));
