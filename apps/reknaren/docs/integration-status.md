...
| **Altinn / Skatteetaten (MVA-melding-innsending)** | **Kodet, men ikke aktiv — venter på ID-porten-legitimasjon + scopes** | **🔑 Bekreftet av Skatteetaten (MVA-produksjon, 2026-08-19): MVA-melding — både validering OG innsending — går KUN via ID-porten (pålogget person m/ MVA-fullmakt representerer virksomheten), IKKE Maskinporten.** API-laget bruker de frie funksjonene `validateMvaMeldingWithToken` og `submitMvaMeldingWithToken` (`src/integrations/vat-submission.ts`) med ID-porten-access-tokenet fra sesjonen. `validate()` POST-er mva-melding-XML til grensesnittstøtte (`…/api/mva/grensesnittstoette/mva-melding/valider`); `submit()` veksler ID-porten-token → Altinn-token (`exchange/id-porten`) → Altinn 3-instansflyt (app **prod `skd/mva-melding-innsending-v1`**, **tt02 `skd/mva-melding-innsending-test`**). Scopes på **ID-porten-klienten**: `openid`, `skatteetaten:mvameldingvalidering` (validering, test+prod), `altinn:instances.read` + `altinn:instances.write` (innsending). `IDPORTEN_SCOPES` = alle fire. Uten ID-porten-legitimasjon: submit/validate-endepunktene svarer 503 (`INTEGRATION_UNAVAILABLE`) / 401 (`IDPORTEN_LOGIN_REQUIRED`); `/api/integrations/status` → `altinn.mode='awaiting_idporten', active=false`. `SkatteetatenVatSubmissionClient` (Maskinporten-veien) er **@deprecated for MVA** og kalles ikke fra endepunktet; beholdt for test/bakoverkompatibilitet. **NB:** innsendings-konvoluttens eksakte schema + dataType-navn bekreftes mot tt02 ved første kjøring; `validate()` er fasit på XML-en underveis. |
