 ...
 * og registrerer dem i _reknaren_migrations. Kjørte migrasjoner endres aldri —
...
export async function runMigrations(databaseUrl: string): Promise<string[]> {
  ...
  try {
    await client.query(`
      CREATE TABLE IF NOT EXISTS _reknaren_migrations (
    ...
    for (const file of files) {
      ...
      const existing = await client.query(
        'SELECT sha256 FROM _reknaren_migrations WHERE filename = $1',
        [file],
      );
      ...
      try {
        ...
        await client.query(
          'INSERT INTO _reknaren_migrations (filename, sha256) VALUES ($1, $2)',
          [file, sha256],
        );
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  const url =
    process.env.DATABASE_URL ?? 'postgres://reknaren:reknaren_dev@localhost:5432/reknaren_dev';
  runMigrations(url)
    .then((applied) => {
      console.log(
        applied.length
