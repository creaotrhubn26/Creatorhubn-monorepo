...
import {
  ...
  type MockupCinematicSceneStyle,
  type MockupTextSlot,
...
function loadImage(src: string): Promise<HTMLImageElement> {
  ...
  const p = materializeMockupAsset(src).then((resolved) => new Promise<HTMLImageElement>((resolve, reject) => {
    const img = new Image();
    // Bundlede ramme-assets, data-URL-er og private blob-URL-er er canvas-trygge.
...
const _videoSafeSrc = (p: string): string => {
  try {
 ...
 */
function composeSceneShot(
  img: HTMLImageElement,
  typeAnim: import("./mockupStudioModel").TypeAnimCfg | undefined,
  t?: number,
  ...
  if (!x) return img;
  x.drawImage(img, 0, 0);
  const st = typedState(typeAnim.text, t ?? 1, {
    payoff: typeAnim.payoff,
    correct: typeAnim.correct,
  });
  const kbTop = drawOnScreenKeyboard(x, W, H, st.pressed);
  drawField(x, W, H, st, kbTop - 0.13, {
    style: typeAnim.field,
    placeholder: typeAnim.placeholder,
  });
  if (typeAnim.keyPop && st.next && st.next !== " " && !st.done)
    drawKeyPop(x, W, H, st.next, st.sub, 0.58);
  return cv;
}
...
async function drawScene(
  ...
  const W = doc.canvas.w,
    H = doc.canvas.h;
  try {
    drawFitted(ctx, await loadImage(sc.src), 0, 0, W, H, "cover");
  } catch {
    /* behold */
  }
  ...
  try {
    ...
    drawImageQuad(
      ctx,
      composeSceneShot(shotImg, doc.canvas.scene?.typeAnim, t),
      quad,
    );
  } catch {
    /* behold svart skjerm */
...
function drawDecor(ctx: CanvasRenderingContext2D, doc: MockupDoc): void {
  ...
  const W = c.w,
    H = c.h;
  const light = c.background === "light";
  const a1 = hexToRgb(c.accent),
    a2 = hexToRgb(c.accent2);
  const rgb1 = `${a1.r},${a1.g},${a1.b}`,
    rgb2 = `${a2.r},${a2.g},${a2.b}`;
  const radial = (
    cx: number,
    cy: number,
  ...
  if (decor === "orbs") {
    ...
    ctx.beginPath();
    ctx.arc(W * 1.0, H * 0.1, W * 0.28, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = `rgba(${rgb2},${light ? 0.09 : 0.12})`;
    ctx.beginPath();
    ctx.arc(W * 0.0, H * 0.96, W * 0.22, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = `rgba(${rgb1},${light ? 0.22 : 0.3})`;
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.moveTo(W * 0.055, H * 0.42);
    ctx.lineTo(W * 0.055, H * 0.42 + 110);
    ctx.stroke();
    ctx.restore();
  } else if (decor === "rings") {
    ...
    ctx.lineWidth = 2;
    for (let r = W * 0.07; r < W * 0.95; r += W * 0.085) {
      ctx.beginPath();
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let x = 0; x <= W; x += 12) {
        const y =
          yBase + Math.sin((x / W) * Math.PI * 3 + i * 0.6) * (H * 0.03);
 ...
 */
async function renderDeviceFrameLayer(
  doc: MockupDoc,
  dev: MockupDeviceSlot,
  spec: (typeof DEVICE_FRAMES)[keyof typeof DEVICE_FRAMES],
  ...
  const sx = spec.screen.x * natW,
    sy = spec.screen.y * natH,
    sw = spec.screen.w * natW,
    sh = spec.screen.h * natH,
    r = spec.radius * natW;
  octx.save();
  ...
  if (
    dev.cleanStatusBar &&
    (dev.variant === "iphone" || dev.variant === "android")
  ) {
...
function drawStatusBar(
  ...
  try {
    const strip = ctx.getImageData(
      Math.round(sx + sw * 0.3),
      Math.round(sy + barH * 0.3),
      Math.max(1, Math.round(sw * 0.4)),
      1,
    ).data;
    let lum = 0;
  ...
  rx -= bw;
  ctx.strokeStyle = ink;
  ctx.lineWidth = Math.max(1, barH * 0.05);
  ...
  rx -= barH * 0.9;
  for (let i = 0; i < 4; i++) {
    const bhh = barH * (0.18 + i * 0.11);
    ctx.fillRect(rx + i * barH * 0.16, cy + barH * 0.25 - bhh, barH * 0.1, bhh);
  }
  ctx.restore();
}
...
function drawReflection(
  ...
  tctx.scale(1, -1); // vertikal speiling
...
async function drawDevice(
  ...
  if (dev.threeD && is3dVariant(dev.variant) && webglAvailable()) {
    try {
      ...
      const typeKey = typeArg
        ? `${typeArg.text.length}:${typeArg.progress.toFixed(3)}:${typeArg.keyPop ? 1 : 0}:${ta!.field ?? ""}:${ta!.payoff ? 1 : 0}:${ta!.correct ? 1 : 0}`
        : "";
      ...
      if (!baked) {
        ...
        const px = Math.max(
          256,
          Math.round(w * 2 * Math.max(1, Math.min(2, zoom))),
        );
        // Keyframe-graf: overstyr rot/zoom fra kurvene ved global playhead (gt).
        const kf = dev.threeD.kf;
        const kfv = (prop: string, def: number) =>
          kf && gt != null ? (sampleKf(kf[prop], gt) ?? def) : def;
        baked = await render3dDevice({
          variant: dev.variant,
          shot: dev.image,
          rotX: kfv("rotX", dev.threeD.rotX),
          rotY: kfv("rotY", dev.threeD.rotY),
          rotZ: kfv("rotZ", dev.threeD.rotZ),
          light: dev.threeD.light,
          zoom: kfv("zoom", zoom),
          kbLayout: dev.threeD.kbLayout,
          w: px,
          h: Math.round(px * (h / w)),
          type: typeArg,
        });
  ...
  if (dev.checklistContent?.animate || dev.dashboardContent?.animate) {
    const screen = {
      x: dev.x + spec.screen.x * w,
      y: dev.y + spec.screen.y * h,
      w: spec.screen.w * w,
      h: spec.screen.h * h,
    };
    if (dev.checklistContent?.animate)
      drawDeviceChecklist(ctx, doc, dev, screen, t);
    if (dev.dashboardContent?.animate)
      drawDeviceDashboard(ctx, doc, dev, screen, t);
  }
...
async function drawWatch(
  ...
  roundRectPath(ctx, bandX, y - h * 0.05, bandW, h * 0.22, bandW * 0.16);
  ctx.fill();
  roundRectPath(ctx, bandX, y + h * 0.83, bandW, h * 0.22, bandW * 0.16);
  ctx.fill();
  ctx.restore();
  ...
  ctx.fillStyle = grad;
  roundRectPath(ctx, x, y, w, h, bodyR);
  ctx.fill();
  ctx.restore();
...
function drawImageSlot(
  ctx: CanvasRenderingContext2D,
  im: import("./mockupStudioModel").MockupImageSlot,
  ...
  if (im.shadow) {
    ctx.save();
    ctx.shadowColor = "rgba(0,0,0,0.4)";
    ctx.shadowBlur = Math.max(12, im.w * 0.05);
    ctx.shadowOffsetY = im.h * 0.03;
    ctx.fillStyle = "#000";
...
/** Transparent subject-compositor. Beholder hele silhuetten og matcher den til scenens
 * bakkekontakt, rimlys, miljøkontrast og perspektiv uten å bake bakgrunn inn i assetet. */
function drawGeneratedFigure(
  ctx: CanvasRenderingContext2D,
  im: MockupImageSlot,
  source: CanvasImageSource,
  sw: number,
  ...
  if (!sw || !sh || alpha <= 0.001) return;
  const comp = im.figureGeneration?.compositing ?? {
    contactShadow: 0.72, rimLight: 0.36, ambientMatch: 0.28,
    depthBlur: 0, perspective: 0, groundOffset: 0.93,
  };
  const scale = Math.min(im.w / sw, im.h / sh);
  const rw = sw * scale, rh = sh * scale;
  const dx = im.x + (im.w - rw) / 2, dy = im.y + (im.h - rh) / 2;
  const groundY = im.y + im.h * comp.groundOffset;

  ctx.save();
  ctx.globalAlpha *= alpha;
  if (comp.contactShadow > 0) {
    const shadow = ctx.createRadialGradient(im.x + im.w / 2, groundY, 0, im.x + im.w / 2, groundY, im.w * 0.3);
    shadow.addColorStop(0, `rgba(5,12,20,${0.32 * comp.contactShadow})`);
    shadow.addColorStop(0.62, `rgba(5,12,20,${0.13 * comp.contactShadow})`);
    shadow.addColorStop(1, "rgba(5,12,20,0)");
    ctx.save();
    ctx.scale(1, 0.24);
    ctx.fillStyle = shadow;
    ctx.beginPath();
    ctx.arc(im.x + im.w / 2, groundY / 0.24, im.w * 0.3, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  ...
  const cx = im.x + im.w / 2;
  ctx.translate(cx, 0);
  ctx.transform(1, 0, comp.perspective * 0.16, 1, -cx, 0);
  ctx.filter = `blur(${Math.max(0, comp.depthBlur * 2).toFixed(2)}px) saturate(${(1 + comp.ambientMatch * 0.08).toFixed(2)}) contrast(${(1 + comp.ambientMatch * 0.05).toFixed(2)})`;
  if (comp.rimLight > 0) {
    ctx.shadowColor = hexToRgba(brandAccent, Math.min(0.85, comp.rimLight));
    ctx.shadowBlur = Math.max(2, im.w * 0.04 * comp.rimLight);
  }
...
function solveIK2(
  ...
  const maxReach = upperLen + lowerLen,
    minReach = Math.abs(upperLen - lowerLen);
  const dist = Math.max(
    minReach + 0.01,
    Math.min(maxReach - 0.01, Math.hypot(dx, dy)),
  );
  ...
  const cosA =
    ...
    (2 * upperLen * dist);
  const elbowAngle =
    baseAngle + bendSign * Math.acos(Math.max(-1, Math.min(1, cosA)));
  return {
...
function applyCinematicSceneGrade(
  ctx: CanvasRenderingContext2D,
  w: number,
  h: number,
  style?: MockupCinematicSceneStyle,
): void {
  if (!style || style.renderQuality !== "cinematic") return;
  const primary = /^#[0-9a-f]{6}$/i.test(style.primary)
    ? style.primary
    : "#102A43";
  const accent = /^#[0-9a-f]{6}$/i.test(style.accent)
    ? style.accent
    : "#2CB67D";
  ctx.save();
  ...
  key.addColorStop(0, "#FFFFFF42");
  key.addColorStop(0.55, accent + "12");
  key.addColorStop(1, accent + "00");
  ctx.fillStyle = key;
  ctx.fillRect(0, 0, w, h);

  const brandSpill = ctx.createLinearGradient(0, h, w, 0);
  brandSpill.addColorStop(0, primary + "22");
  brandSpill.addColorStop(0.58, primary + "00");
  brandSpill.addColorStop(1, accent + "16");
  ctx.fillStyle = brandSpill;
  ctx.fillRect(0, 0, w, h);
  ...
  vignette.addColorStop(0, "rgba(7,12,28,0)");
  vignette.addColorStop(0.72, "rgba(7,12,28,0.02)");
  vignette.addColorStop(1, "rgba(7,12,28,0.16)");
  ctx.fillStyle = vignette;
  ctx.fillRect(0, 0, w, h);
  ctx.restore();
}

export function drawOfficeBackdrop(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  w: number,
  h: number,
  sceneStyle?: MockupCinematicSceneStyle,
): void {
  ctx.save();
  ctx.translate(x, y);
  ...
  withWallDepth(ctx, () => {
    ...
    ctx.beginPath();
    ctx.arc(doorX + doorW - 10, doorY + doorH * 0.52, 3, 0, Math.PI * 2);
    ctx.fill();
  });
  ...
  withWallDepth(ctx, () => {
    const dipX = w * 0.32,
      ...
      dipH = h * 0.13;
    ctx.fillStyle = "#c9963b";
    roundRectPath(ctx, dipX - 3, dipY - 3, dipW + 6, dipH + 6, 3);
    ctx.fill();
    ctx.fillStyle = "#fdfbf6";
    roundRectPath(ctx, dipX, dipY, dipW, dipH, 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(0,0,0,0.1)";
    ctx.lineWidth = 1.5;
    [0.3, 0.48, 0.66].forEach((f) => {
      ctx.beginPath();
      ctx.moveTo(dipX + dipW * 0.15, dipY + dipH * f);
      ctx.lineTo(dipX + dipW * 0.85, dipY + dipH * f);
      ctx.stroke();
    });
  });
  ...
  ctx.strokeStyle = "#c9cfd9";
  ctx.lineWidth = 2;
  roundRectPath(ctx, shelfX, shelfY, shelfW, shelfH, 4);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(shelfX, shelfY + shelfH * 0.5);
  ctx.lineTo(shelfX + shelfW, shelfY + shelfH * 0.5);
  ctx.stroke();
  ...
  ctx.beginPath();
  ctx.ellipse(rugX, rugY, w * 0.28, h * 0.07, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "rgba(197,150,90,0.3)";
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.ellipse(rugX, rugY, w * 0.28, h * 0.07, 0, 0, Math.PI * 2);
  ctx.stroke();
  ...
  ctx.beginPath();
  ctx.ellipse(w * 0.5, h * 0.03, w * 0.05, h * 0.012, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  withWallDepth(ctx, () => {
    ...
    ctx.lineTo(clockX, clockY - clockR * 0.55);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(clockX, clockY);
    ctx.lineTo(clockX + clockR * 0.4, clockY + clockR * 0.15);
    ctx.stroke();

    const crossX = w * 0.15,
      crossY = h * 0.16,
      crossS = Math.min(w, h) * 0.045;
  ...
  roundRectPath(ctx, deskX, deskY, deskW, deskH, 8);
  ctx.fill();
  ctx.fillStyle = "#c9963b";
  ...
  const monW = deskW * 0.3,
    monH = h * 0.1,
    monX = deskX + deskW * 0.08,
    monY = deskY - monH;
  ctx.fillStyle = "#8a8f9a";
  ctx.fillRect(monX + monW * 0.4, deskY - 4, 3, 4); // fot
  ctx.fillStyle = "#2b3348";
  roundRectPath(ctx, monX, monY, monW, monH, 4);
  ctx.fill();
  ctx.fillStyle = "#fdfbf6";
  roundRectPath(ctx, monX + 3, monY + 3, monW - 6, monH - 10, 2);
  ctx.fill();
  ctx.fillStyle = "#1b294b";
  ctx.fillRect(monX + 3, monY + 3, monW - 6, (monH - 10) * 0.3); // rapport-hode
  ctx.fillStyle = "rgba(0,0,0,0.18)";
  [0.55, 0.72, 0.89].forEach((f) =>
    ctx.fillRect(monX + 6, monY + 3 + (monH - 10) * f, monW - 12, 2),
  ); // rapport-tekstlinjer
  ctx.fillStyle = "#dfe4ec";
  roundRectPath(ctx, deskX + deskW * 0.12, deskY - 8, deskW * 0.34, 8, 3);
  ctx.fill(); // tastatur, flatt på pulten
  ...
  withWallDepth(ctx, () => {
    ...
    ctx.strokeStyle = "#c9cfd9";
    ctx.lineWidth = 2;
    roundRectPath(ctx, eyeX, eyeY, eyeW, eyeH, 2);
    ctx.fill();
    ctx.stroke();
  ...
  ctx.strokeStyle = "#c9cfd9";
  ctx.lineWidth = 2;
  roundRectPath(ctx, cabX, cabY, cabW, cabH, 4);
  ctx.fill();
  ctx.stroke();
  ctx.strokeStyle = "#c9cfd9";
  ctx.beginPath();
  ctx.moveTo(cabX, cabY + cabH * 0.5);
  ctx.lineTo(cabX + cabW, cabY + cabH * 0.5);
  ctx.stroke();
  ctx.fillStyle = "#8a8f9a";
  ...
  const potX = w * 0.88,
    potY = h * 0.68;
  ctx.fillStyle = "#c9963b";
  roundRectPath(ctx, potX - 16, potY, 32, 26, 4);
  ctx.fill();
  ...
  [
    ...
    ctx.beginPath();
    ctx.ellipse(potX + dx, potY + dy, 12, 20, dx * 0.03, 0, Math.PI * 2);
    ctx.fill();
  });

  applyCinematicSceneGrade(ctx, w, h, sceneStyle);
  ctx.restore();
}
...
export function drawWaitingRoomBackdrop(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  w: number,
  ...
  roundRectPath(ctx, deskX, deskY + deskH * 0.25, deskW, deskH * 0.75, 6);
  ctx.fill(); // front-panel
  ctx.fillStyle = "#f5f6f8";
  roundRectPath(ctx, deskX - 6, deskY, deskW + 12, deskH * 0.3, 5);
  ctx.fill(); // disk-overheng
  ctx.fillStyle = "#3a4256";
  ctx.beginPath();
  ctx.arc(deskX + deskW * 0.82, deskY + deskH * 0.14, 5, 0, Math.PI * 2);
  ctx.fill(); // bjelle
  ctx.fillRect(deskX + deskW * 0.82 - 1.5, deskY + deskH * 0.14 - 8, 3, 5);

  // Veggklokke over resepsjonen.
  withWallDepth(ctx, () => {
    const clockX = deskX + deskW / 2,
      clockY = h * 0.16,
      clockR = Math.min(w, h) * 0.035;
    ctx.fillStyle = "#ffffff";
    ctx.strokeStyle = "#c9cfd9";
  ...
  const benchX = w * 0.04,
    ...
    seatH = h * 0.045;
  ctx.fillStyle = "#dfe4ec";
  roundRectPath(ctx, benchX, benchY - backH, benchW, backH, 8);
  ctx.fill(); // rygg (lav, hele benken)
  ...
  roundRectPath(ctx, benchX, benchY, benchW, seatH, 6);
  ctx.fill(); // sete (hele benken, ett stykke)
  ctx.strokeStyle = "#8a8f9a";
  ctx.lineWidth = 3;
  for (let i = 1; i < 3; i++) {
    // arm-skiller mellom de 3 sitteplassene
    const ax = benchX + (benchW / 3) * i;
    ctx.beginPath();
    ctx.moveTo(ax, benchY - backH * 0.5);
    ctx.lineTo(ax, benchY + seatH);
    ctx.stroke();
  }
  ctx.fillStyle = "#8a8f9a"; // bein
  [0.06, 0.94].forEach((f) => {
    ctx.fillRect(benchX + benchW * f - 2, benchY + seatH, 4, h * 0.05);
  });
  ...
  const tblX = w * 0.42,
    tblY = h * 0.68;
  ctx.fillStyle = "#c9963b";
  roundRectPath(ctx, tblX - 34, tblY, 68, 8, 3);
  ctx.fill();
  ctx.fillStyle = "#8a8f9a";
  ctx.fillRect(tblX - 30, tblY + 8, 4, 22);
  ctx.fillRect(tblX + 26, tblY + 8, 4, 22);
  ...
  const coolX = w * 0.35,
    coolY = h * 0.5;
  ctx.fillStyle = "#c9cfd9";
  roundRectPath(ctx, coolX, coolY, 22, 38, 5);
  ctx.fill();
  ctx.fillStyle = "rgba(150,190,220,0.6)";
  ctx.beginPath();
  ctx.arc(coolX + 11, coolY + 10, 8, 0, Math.PI * 2);
  ctx.fill(); // vannflaske
  ctx.fillStyle = "#8a8f9a";
  ...
  ctx.lineTo(rackX, rackY + h * 0.16);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(rackX - 14, rackY);
  ctx.lineTo(rackX + 14, rackY);
  ctx.stroke();
  [
    ["#e0546a", -14],
    ["#3a4256", 6],
    ...
    roundRectPath(ctx, rackX + (dx as number), rackY + 4, 16, 26, 6);
    ctx.fill();
  });
  ...
  withWallDepth(ctx, () => {
    const noteX = w * 0.28,
      ...
      noteH = h * 0.12;
    ctx.fillStyle = "#c9963b";
    roundRectPath(ctx, noteX - 3, noteY - 3, noteW + 6, noteH + 6, 3);
    ctx.fill();
    ctx.fillStyle = "#e8ddc4";
    roundRectPath(ctx, noteX, noteY, noteW, noteH, 2);
    ctx.fill();
    [
      ["#e0546a", 0.2, 0.22],
      ["#2563eb", 0.55, 0.35],
      ...
      ctx.save();
      ctx.translate(
        noteX + noteW * (fx as number),
        noteY + noteH * (fy as number),
      );
      ctx.rotate(-0.08);
...
export function drawPersonLaptop(
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  w: number,
  ...
  const sleeveColor = outfit === "legefrakk" ? "#f5f6f8" : shirt;
  const hairStyle = style?.hairStyle || "kort";
  const presentation = style?.presentation || "neutral";
  const ageRange = style?.ageRange || "adult";
  const faceShape = style?.faceShape || "balanced";
  const accessory = style?.accessory || "ingen";
  const scenario = style?.scenario || "laptop";
  const cinematic =
    style?.renderQuality === "cinematic" ||
    style?.artDirection === "cinematic-3d-v1";
  const highFidelity = style?.renderQuality === "high" || cinematic;
  const NW = 600,
    NH = 760; // normalisert tegne-boks, skalert til (w,h) m/ bevart aspekt, sentrert
  ...
  const shade = (base: string, amt: number): string =>
    mixHex(base, amt < 0 ? "#000000" : "#ffffff", Math.abs(amt));

  if (cinematic) {
    // Myk trepunkts-lyssetting rundt fallback-riggen gir mer filmatisk volum
    // også når den generative bildeleverandøren er utilgjengelig.
    const aura = ctx.createRadialGradient(260, 310, 24, 300, 360, 310);
    aura.addColorStop(0, "rgba(255,246,224,0.34)");
    aura.addColorStop(0.58, "rgba(255,255,255,0.09)");
    aura.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = aura;
    ctx.beginPath();
    ctx.ellipse(300, 360, 292, 340, 0, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  ctx.beginPath();
  ctx.ellipse(300, 700, 210, 28, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  if (scenario === "laptop") {
    ...
    roundRectPath(ctx, 180, 300, 240, 260, 40);
    ctx.fill();
  }
  ...
  ctx.rotate((pose.leanX * Math.PI) / 180);
  ctx.translate(-300, -560);

  const shirtGrad = ctx.createLinearGradient(195, 350, 195, 570);
  shirtGrad.addColorStop(0, shade(shirt, 0.1));
  if (cinematic) shirtGrad.addColorStop(0.42, shade(shirt, 0.03));
  shirtGrad.addColorStop(1, shade(shirt, -0.2));
  ctx.fillStyle = shirtGrad;
  roundRectPath(ctx, 195, 350, 210, 220, 60);
  ctx.fill();
  if (highFidelity) {
    ctx.strokeStyle = shade(shirt, -0.26);
    ctx.globalAlpha = 0.2;
    ctx.lineWidth = 2;
    roundRectPath(ctx, 195, 350, 210, 220, 60);
    ctx.stroke();
  ...
  const drawStethoscope = (): void => {
    ...
    ctx.beginPath();
    ctx.moveTo(278, 358);
    ctx.quadraticCurveTo(300, 400, 300, 430);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(322, 358);
    ctx.quadraticCurveTo(300, 400, 300, 430);
  ...
  if (outfit === "hettegenser") {
    ...
    ctx.fillStyle = shirtGrad;
    roundRectPath(ctx, 240, 328, 120, 55, 26);
    ctx.fill();
    ...
    [0, 1, 2].forEach((i) => {
      ctx.beginPath();
      ctx.arc(300, 400 + i * 28, 3, 0, Math.PI * 2);
      ctx.fill();
    });
    ...
    ctx.fill();
    if (highFidelity) {
      // Stoff-sømmer, brystlomme og mansjettdetaljer gjør frakken lesbar også i liten størrelse.
      ctx.strokeStyle = "rgba(108,126,145,0.34)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(257, 386);
      ctx.lineTo(247, 548);
      ...
      ctx.lineTo(353, 548);
      ctx.stroke();
      ctx.fillStyle = "rgba(216,225,232,0.9)";
      roundRectPath(ctx, 214, 438, 46, 34, 5);
      ctx.fill();
      ctx.strokeStyle = "rgba(108,126,145,0.26)";
      roundRectPath(ctx, 214, 438, 46, 34, 5);
      ctx.stroke();
      [420, 458, 496].forEach((buttonY) => {
        ctx.fillStyle = "#c8d2dc";
        ctx.beginPath();
        ctx.arc(335, buttonY, 2.6, 0, Math.PI * 2);
        ctx.fill();
      });
    }
    drawStethoscope();
    ...
    ctx.beginPath();
    ctx.ellipse(300, 358, 26, 12, 0, 0, Math.PI);
    ctx.fill();
    ctx.fillStyle = "#ffffff";
    roundRectPath(ctx, 320, 420, 26, 20, 4);
    ctx.fill();
    ctx.strokeStyle = "#e0546a";
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(333, 425);
    ctx.lineTo(333, 435);
  ...
  if (accessory === "id-kort") {
    ...
    ctx.stroke();
    ctx.fillStyle = "#ffffff";
    roundRectPath(ctx, 288, 440, 24, 32, 3);
    ctx.fill();
    ctx.fillStyle = "#dfe3ea";
    roundRectPath(ctx, 293, 446, 14, 12, 2);
    ctx.fill();
  }
  ...
  ctx.beginPath();
  ctx.ellipse(213, 356, 15, 11, -0.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(387, 356, 15, 11, 0.3, 0, Math.PI * 2);
  ctx.fill();
  ...
  roundRectPath(ctx, 282, 335, 36, 34, 10);
  ctx.fill();
  ...
  if (highFidelity) {
    ...
    ctx.beginPath();
    const earOffset =
      faceShape === "soft" ? 65 : faceShape === "angular" ? 61 : 63;
    ctx.ellipse(300 - earOffset, 300, 13, 20, -0.08, 0, Math.PI * 2);
    ctx.ellipse(300 + earOffset, 300, 13, 20, 0.08, 0, Math.PI * 2);
    ctx.fill();
  }

  const [faceRx, faceRy] = {
    soft: cinematic ? [72, 72] : [69, 68],
    balanced: cinematic ? [70, 76] : [68, 70],
    angular: cinematic ? [65, 80] : [64, 74],
  }[faceShape];

  const skinGrad = ctx.createRadialGradient(278, 274, 8, 310, 316, 86);
  skinGrad.addColorStop(0, shade(skin, cinematic ? 0.24 : 0.14));
  if (cinematic) skinGrad.addColorStop(0.55, skin);
  skinGrad.addColorStop(1, shade(skin, cinematic ? -0.16 : -0.08));
  ctx.fillStyle = skinGrad;
  if (cinematic) {
    ctx.shadowColor = "rgba(42,30,27,0.22)";
    ctx.shadowBlur = 18;
    ctx.shadowOffsetY = 10;
  }
  ctx.beginPath();
  ctx.ellipse(300, 300, faceRx, faceRy, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.shadowColor = "transparent";
  ctx.shadowBlur = 0;
  ctx.shadowOffsetY = 0;
  if (cinematic) {
    const faceLight = ctx.createRadialGradient(270, 276, 2, 280, 292, 54);
    faceLight.addColorStop(0, "rgba(255,255,255,0.34)");
    faceLight.addColorStop(1, "rgba(255,255,255,0)");
    ctx.fillStyle = faceLight;
    ctx.beginPath();
    ctx.ellipse(283, 292, 52, 62, -0.12, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(255,230,214,0.42)";
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(300, 302, 67, -1.3, 1.15);
    ctx.stroke();
  }
  ...
  if (hairStyle !== "bald") {
    ...
    ctx.lineTo(374, 300);
    ctx.arc(300, 300, 74, 0, Math.PI, true);
    ctx.closePath();
    ctx.fill();
    if (highFidelity) {
      ...
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.arc(298, 270, 58, Math.PI * 1.12, Math.PI * 1.63);
      ctx.stroke();
    }
    if (cinematic) {
      // Separate hårstrå-grupper og kaldt rimlys gir en tydelig 3D-silhuett.
      ctx.strokeStyle = shade(hair, 0.28);
      ctx.lineWidth = 7;
      ctx.lineCap = "round";
      [-38, -18, 4, 26, 44].forEach((dx, index) => {
        ctx.beginPath();
        ctx.moveTo(300 + dx, 226 + Math.abs(dx) * 0.18);
        ctx.quadraticCurveTo(
          300 + dx * 1.08,
          248 + index * 2,
          300 + dx * 1.18,
          271 + Math.abs(dx) * 0.12,
        );
        ctx.stroke();
      });
      ctx.strokeStyle = "rgba(139,210,224,0.42)";
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.arc(300, 280, 72, Math.PI * 1.55, Math.PI * 1.93);
      ctx.stroke();
    }
    if (presentation === "female" && hairStyle === "kort") {
      ctx.strokeStyle = hairGrad;
      ctx.lineWidth = 18;
      ctx.beginPath();
      ctx.moveTo(242, 265);
      ctx.quadraticCurveTo(230, 302, 246, 342);
      ctx.moveTo(358, 265);
      ctx.quadraticCurveTo(370, 302, 354, 342);
      ctx.stroke();
    }
    if (hairStyle === "buffert") {
      ctx.beginPath();
      ctx.arc(300, 224, 22, 0, Math.PI * 2);
      ctx.fill();
    } else if (hairStyle === "krøller" || hairStyle === "coily") {
      [
        [242, 252],
        [270, 220],
        [300, 210],
        ...
        ctx.beginPath();
        ctx.arc(cx, cy, hairStyle === "coily" ? 15 : 17, 0, Math.PI * 2);
        ctx.fill();
      });
    }
  }
  ...
  if (outfit === "sykepleier") {
    ctx.fillStyle = "#ffffff";
    ctx.beginPath();
    ctx.arc(300, 224, 34, Math.PI, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#e0546a";
  ...
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(270, bry + 2);
  ctx.lineTo(286, bry);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(314, bry);
  ctx.lineTo(330, bry + 2);
  ctx.stroke();

  // Øyne: skalert av eyeSize, flates ut mot blink=1 (lukket) — enkel, lesbar blunk på flate ikon-prikker.
  const eyeRx = (cinematic ? 7.2 : 5) * pose.eyeSize;
  const eyeRy = eyeRx * (1 - pose.blink);
  if (highFidelity && eyeRy > 1.4) {
    [278, 322].forEach((eyeX) => {
      ctx.fillStyle = "rgba(255,255,255,0.94)";
      ctx.beginPath();
      ctx.ellipse(eyeX, 302, eyeRx + 2.2, eyeRy * 0.78, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = shade(hair, 0.16);
      if (cinematic) {
        ...
        iris.addColorStop(0, shade(accent, 0.35));
        iris.addColorStop(0.5, shade(accent, -0.12));
        iris.addColorStop(1, shade(hair, -0.08));
        ctx.fillStyle = iris;
      }
      ctx.beginPath();
      ctx.arc(eyeX, 302, Math.max(2.4, eyeRx * 0.7), 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = shade(hair, -0.35);
      ctx.beginPath();
      ctx.arc(eyeX, 302, Math.max(1.2, eyeRx * 0.34), 0, Math.PI * 2);
      ctx.fill();
      if (cinematic) {
        ctx.strokeStyle = "rgba(65,40,35,0.34)";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(eyeX, 300, eyeRx + 2, Math.PI * 1.08, Math.PI * 1.92);
        ctx.stroke();
      }
    });
  } else {
    ctx.fillStyle = hair;
    ctx.beginPath();
    ctx.ellipse(278, 302, eyeRx, Math.max(0.6, eyeRy), 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(322, 302, eyeRx, Math.max(0.6, eyeRy), 0, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  if (eyeRy > 1.5) {
    ...
    ctx.beginPath();
    ctx.arc(276, 300, Math.min(1.6, eyeRx * 0.28), 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(320, 300, Math.min(1.6, eyeRx * 0.28), 0, Math.PI * 2);
    ctx.fill();
  }
  if (highFidelity) {
    ...
    ctx.beginPath();
    ctx.moveTo(301, 300);
    ctx.quadraticCurveTo(296, 313, 303, 316);
    ctx.stroke();
    ctx.fillStyle = "rgba(207,112,105,0.16)";
    ctx.beginPath();
    ctx.ellipse(267, 318, 14, 7, -0.1, 0, Math.PI * 2);
    ctx.ellipse(333, 318, 14, 7, 0.1, 0, Math.PI * 2);
    ctx.fill();
    ctx.globalAlpha = 1;
  }
  if (ageRange === "mature") {
    ...
    ctx.beginPath();
    ctx.moveTo(264, 310);
    ctx.quadraticCurveTo(278, 316, 290, 311);
    ctx.moveTo(310, 311);
    ctx.quadraticCurveTo(322, 316, 336, 310);
    ctx.moveTo(286, 326);
    ctx.quadraticCurveTo(280, 338, 282, 346);
    ctx.moveTo(314, 326);
    ctx.quadraticCurveTo(320, 338, 318, 346);
    ctx.stroke();
    ctx.globalAlpha = 1;
  }
  if (presentation === "male" && highFidelity) {
    ctx.strokeStyle = shade(skin, -0.24);
    ctx.globalAlpha = 0.2;
    ctx.lineWidth = 5;
    ctx.beginPath();
    ctx.arc(300, 304, faceRx - 5, 0.18 * Math.PI, 0.82 * Math.PI);
    ctx.stroke();
    ctx.globalAlpha = 1;
  }
  ...
  ctx.stroke();
  if (cinematic && accessory !== "munnbind") {
    ctx.strokeStyle = "rgba(133,69,68,0.7)";
    ctx.lineWidth = 2.4;
    ctx.beginPath();
    ctx.moveTo(288, 319);
    ctx.quadraticCurveTo(300, 322 + pose.mouthCurve * 3, 312, 319);
    ctx.stroke();
    ctx.strokeStyle = "rgba(255,239,232,0.55)";
    ctx.lineWidth = 1.2;
    ctx.beginPath();
    ctx.moveTo(291, 316);
    ctx.quadraticCurveTo(300, 314, 309, 316);
    ctx.stroke();
  }
  // Munnbind: dekker nese/munn/hake, tegnes OVER munnen + strikk mot ørene.
  if (accessory === "munnbind") {
    ctx.fillStyle = "#eaf1f5";
    roundRectPath(ctx, 268, 300, 64, 42, 16);
    ctx.fill();
  ...
  if (accessory === "hodetelefoner") {
    ...
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.arc(300, 262, 78, Math.PI * 1.08, Math.PI * 1.92);
    ctx.stroke();
    ctx.fillStyle = "#2a2f3d";
    ctx.beginPath();
    ctx.ellipse(230, 302, 10, 17, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(370, 302, 10, 17, 0, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore(); // hode
  ...
  if (scenario !== "laptop") {
    ...
    const drawLeg = (hipX: number, phaseOffset: number): void => {
      const angle =
        scenario === "walk"
          ? Math.sin(pose.legSwing * Math.PI * 2 + phaseOffset) *
            ((18 * Math.PI) / 180)
          : 0;
      ctx.save();
      ctx.translate(hipX, 572);
      ctx.rotate(angle);
      ctx.translate(-hipX, -572);
      ctx.fillStyle = pantsColor;
      roundRectPath(ctx, hipX - 13, 572, 26, 128, 13);
      ctx.fill();
      ctx.fillStyle = "#20242c";
      roundRectPath(ctx, hipX - 15, 686, 30, 18, 8);
      ctx.fill(); // sko
      ctx.restore();
    };
    drawLeg(270, 0);
  ...
  if (scenario === "laptop") {
    ...
    ctx.fillStyle = deskGrad;
    roundRectPath(ctx, 60, 560, 480, 30, 10);
    ctx.fill();
    ...
    roundRectPath(ctx, 220, 470, 160, 100, 10);
    ctx.fill();
    ...
    const lineW = lineBase.map((base, i) =>
      Math.max(
        20,
        Math.min(
          96,
          base + Math.sin(pose.screenActivity * Math.PI * 2 + i * 1.1) * 10,
        ),
      ),
    );
    ...
    roundRectPath(ctx, 205, 568, 190, 14, 6);
    ctx.fill();
  }
  ...
  const shoulderL = { x: 240, y: 430 },
    shoulderR = { x: 360, y: 430 };
  ...
  if (scenario === "laptop") {
    ...
    const ARM_REACH = 118;
    const angleR =
      Math.sin(pose.legSwing * Math.PI * 2) * ((20 * Math.PI) / 180);
    const angleL =
      Math.sin(pose.legSwing * Math.PI * 2 + Math.PI) * ((20 * Math.PI) / 180);
    handL = {
      x: shoulderL.x + Math.sin(angleL) * ARM_REACH,
      y: shoulderL.y + Math.cos(angleL) * ARM_REACH,
    };
    handR = {
      x: shoulderR.x + Math.sin(angleR) * ARM_REACH,
      y: shoulderR.y + Math.cos(angleR) * ARM_REACH,
    };
  } else {
    handL = { x: 190, y: 500 + sw };
    handR = { x: 410, y: 500 - sw }; // 'stand' — hender ved siden, tydelig bredere enn skuldrene
  }
  handL.x += pose.handTargetLeftX || 0;
  handL.y += pose.handTargetLeftY || 0;
  handR.x += pose.handTargetRightX || 0;
  handR.y += pose.handTargetRightY || 0;
  const UPPER_ARM = 78,
    LOWER_ARM = 82; // ~samme total rekkevidde som tidligere kurve
  ...
  if (highFidelity) {
    ctx.strokeStyle = "rgba(25,35,48,0.15)";
    ctx.lineWidth = 28;
    ctx.lineCap = "round";
    ctx.lineJoin = "round";
    ctx.beginPath();
    ctx.moveTo(shoulderL.x, shoulderL.y + 3);
    ctx.lineTo(elbowL.x, elbowL.y + 3);
    ctx.lineTo(handL.x, handL.y + 3);
    ctx.moveTo(shoulderR.x, shoulderR.y + 3);
    ctx.lineTo(elbowR.x, elbowR.y + 3);
    ctx.lineTo(handR.x, handR.y + 3);
    ctx.stroke();
  }
  ctx.strokeStyle = highFidelity ? shade(sleeveColor, -0.02) : sleeveColor;
  ctx.lineWidth = highFidelity ? 24 : 21;
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ctx.beginPath();
  ...
  const drawHand = (
    ...
    if (scenario === "laptop") {
      ...
      if (highFidelity) {
        ...
        [-1, 0, 1].forEach((finger) => {
          ctx.beginPath();
          ctx.moveTo(h.x + finger * 7 - 2, h.y + 13);
          ctx.lineTo(h.x + finger * 7 + 3, h.y + 13);
          ctx.stroke();
        });
        ctx.globalAlpha = 1;
      }
    ...
    const dx = h.x - elbow.x,
      dy = h.y - elbow.y;
    const len = Math.hypot(dx, dy) || 1;
    const ux = dx / len,
      uy = dy / len; // enhetsvektor utover fra albuen
    ctx.fillStyle = skin;
    ctx.save();
  ...
  if (scenario === "phone") {
    ...
    roundRectPath(ctx, handR.x - 13, handR.y - 30, 26, 46, 6);
    ctx.fill();
    ctx.fillStyle = "#3a4256";
    roundRectPath(ctx, handR.x - 10, handR.y - 26, 20, 34, 3);
    ctx.fill();
  }
...
function chatReactionAt(
  chat: import("./mockupStudioModel").ChatTypeConfig,
  localT: number,
): { blinkPulse: number; attentive: number } {
  const cps = CHAT_TYPE_SPEEDS[chat.speed] ?? CHAT_TYPE_SPEEDS.normal;
  const durs = chat.turns.map((turn) => chatTurnDuration(turn, cps));
  const total =
    durs.reduce((s, d, i) => s + d + (i > 0 ? CHAT_TURN_GAP : 0), 0) || 1;
...
function laggedKfv(
  ...
  return offsets.reduce(
    (s, f, i) =>
      s + (sampleKf(kfs, Math.max(0, t - f * dtNorm)) ?? def) * weights[i],
    0,
  );
}
...
function drawChatBubble(
  ...
  lines.forEach((l, i) =>
    ctx.fillText(l, bx + padX, top + labelH + padY + i * lh),
  );
  ...
  if (opts.meta) {
    ...
    ctx.fillText(
      opts.delivered ? `${opts.meta}  ·  Levert ✓✓` : opts.meta,
      metaX,
      bottom + metaFs * 0.35,
    );
    ctx.textAlign = "left";
 ...
 */
function turnPhase(
  turn: import("./mockupStudioModel").ChatTurn,
  cps: number,
  sec: number,
): ChatPhase {
  const dotsLen = 0.6;
  ...
  if (sec < dotsLen + qLen) {
    ...
    return {
      mode: "question",
      qVisible: turn.question.slice(0, Math.round(turn.question.length * p)),
      qDone: p >= 1,
      composeVisible: "",
      sent: false,
    };
  ...
  if (!turn.reply)
    return {
      mode: "question",
      qVisible: turn.question,
      qDone: true,
      composeVisible: "",
      sent: false,
    };
  const pauseLen = 0.3,
    composeStart = dotsLen + qLen + pauseLen;
  if (sec < composeStart)
    return {
      mode: "question",
      qVisible: turn.question,
  ...
  if (sec < composeStart + replyLen) {
    ...
    return {
      ...
      qVisible: turn.question,
      qDone: true,
      composeVisible: turn.reply.slice(0, Math.round(turn.reply.length * p)),
      sent: false,
    };
  }
  return {
...
function drawChatType(
  ...
  const total =
    ...
    CHAT_TURN_GAP * Math.max(0, chat.turns.length - 1);
  const globalSec = localT * total;
  let acc = 0,
    activeIdx = chat.turns.length - 1,
    localSec = durations[durations.length - 1];
  for (let i = 0; i < chat.turns.length; i++) {
    if (globalSec < acc + durations[i] || i === chat.turns.length - 1) {
      activeIdx = i;
      localSec = Math.max(0, globalSec - acc);
      break;
    }
  ...
  ctx.textAlign = "left";
  ctx.fillText(
    chat.label ?? "PreVisit-assistent",
    iconCx + iconR + w * 0.03,
    iconCy,
  );

  // Fast meldingsfelt nederst — «man skriver i et skrivefelt» (kun for ekte fram-og-tilbake-samtaler).
  const inputH = isChat ? h * 0.115 : 0,
    inputY = y + h - inputH;
  if (isChat) {
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(x, inputY, w, inputH);
    ctx.strokeStyle = "rgba(0,0,0,0.08)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(x, inputY);
    ctx.lineTo(x + w, inputY);
    ctx.stroke();
    const inputPillH = inputH * 0.6,
      inputPillY = inputY + (inputH - inputPillH) / 2;
    ...
    const drawX =
      itw > inputPillW - iPadX * 2
        ? x + w * 0.06 + iPadX - (itw - (inputPillW - iPadX * 2))
        : x + w * 0.06 + iPadX;
    ...
    if (composing) {
      ...
      ctx.fillRect(
        drawX + itw + 2,
        inputPillY + inputPillH * 0.22,
        Math.max(2, ifs * 0.09),
        inputPillH * 0.56,
      );
    }
    ctx.restore();
    const sendCx = x + w * 0.06 + inputPillW + w * 0.02 + sendR,
      sendCy = inputY + inputH / 2;
    ctx.fillStyle = composing ? accent : "#d7d9de";
    ctx.beginPath();
    ctx.arc(sendCx, sendCy, sendR, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#ffffff";
    ctx.beginPath();
  ...
  const contentX = x + w * 0.06,
    contentW = w * 0.88;
  const contentTop = headerY + appHeaderH + h * 0.035,
    contentBottom = inputY - h * 0.02;
  const fs = Math.round(w * 0.032);
  const rowGap = h * 0.045,
    dotsH = h * 0.12;
  const baseMin = 30; // avledet tidsstempel-basis (bare visuell — ikke ekte klokke)
  const stampFor = (i: number, isReply: boolean) =>
    `14:${String(baseMin + i * 3 + (isReply ? 1 : 0)).padStart(2, "0")}`;
  ...
  for (let i = 0; i < activeIdx; i++) {
    ...
    stackH +=
      measureBubbleH(ctx, t.question, contentW, fs, fontFamily, false, true) +
      rowGap;
    if (t.reply)
      stackH +=
        measureBubbleH(ctx, t.reply, contentW, fs, fontFamily, false, true) +
        rowGap;
  }
  ...
  if (phase.mode === "done" && chat.turns[activeIdx].reply)
    stackH +=
      ...
      measureBubbleH(
        ctx,
        chat.turns[activeIdx].reply as string,
        contentW,
        fs,
        fontFamily,
        false,
  ...
  ctx.save();
  ctx.beginPath();
  ctx.rect(x, contentTop, w, contentBottom - contentTop);
  ctx.clip();
  ...
  for (let i = 0; i < activeIdx; i++) {
    const t = chat.turns[i];
    cy =
      drawChatBubble(ctx, t.question, contentX, cy, contentW, fs, fontFamily, {
        align: "left",
        fill: "#ffffff",
        ink: CHAT_INK,
        boxW: contentW,
        meta: stampFor(i, false),
      }) + rowGap;
    if (t.reply)
      cy =
        drawChatBubble(ctx, t.reply, contentX, cy, contentW, fs, fontFamily, {
          align: "right",
          fill: CHAT_NAVY,
          ink: CHAT_NAVY_TEXT,
          boxW: contentW,
          meta: stampFor(i, true),
          delivered: true,
        }) + rowGap;
  }
  if (phase.mode === "dots") {
...
export function deviceScreenRect(dev: MockupDeviceSlot): {
  x: number;
  y: number;
  w: number;
  h: number;
...
export function annRect(
  doc: MockupDoc,
  a: MockupAnnotation,
): { x: number; y: number; w: number; h: number } {
  const dev = a.deviceId
    ? doc.devices.find((d) => d.id === a.deviceId)
    : undefined;
  return dev
    ? deviceScreenRect(dev)
...
function drawMarker(
  ...
  ctx.fill();
  ctx.strokeStyle = doc.canvas.accent;
  ctx.lineWidth = Math.max(3, s.w * 0.006);
  roundRectPath(ctx, x, y, w, h, r);
  ctx.stroke();
  ctx.restore();
}
...
function drawCallout(
  ...
  const cxp =
    side === "left" ? lx + chipW : side === "right" ? lx : lx + chipW / 2;
  const cyp =
    side === "top" ? ly + chipH : side === "bottom" ? ly : ly + chipH / 2;
  // leder-linje anker → chip
  ctx.strokeStyle = doc.canvas.accent;
  ctx.lineWidth = Math.max(2, pinR * 0.16);
  ctx.beginPath();
  ctx.moveTo(ax, ay);
  ctx.lineTo(cxp, cyp);
  ctx.stroke();
  ...
  ctx.beginPath();
  ctx.arc(ax, ay, pinR, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "#fff";
  ctx.lineWidth = Math.max(2, pinR * 0.16);
  ctx.stroke();
  ...
  // chip
  ctx.fillStyle = "#ffffff";
  ctx.strokeStyle = "rgba(20,30,40,0.14)";
  ctx.lineWidth = 1.5;
  roundRectPath(ctx, lx, ly, chipW, chipH, chipH / 2);
  ctx.fill();
  ctx.stroke();
  const dotX = lx + pinR * 1.1 + dotR;
  ctx.fillStyle = doc.canvas.accent;
  ctx.beginPath();
  ctx.arc(dotX, ly + chipH / 2, dotR, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#fff";
  ctx.font = `700 ${Math.round(dotR * 1.3)}px system-ui`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
...
function drawConnectorLine(
  ...
  ctx.fillStyle = glow;
  ctx.beginPath();
  ctx.arc(qx, qy, dotR * 3.2, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = col;
  ctx.beginPath();
...
function drawIconPill(
  ...
  ctx.shadowColor = "rgba(20,30,45,0.16)";
  ...
  ctx.fillStyle = doc.canvas.accent;
  ctx.beginPath();
  ctx.arc(icx, cy, iconR, 0, Math.PI * 2);
  ctx.fill();
  if (isIconId(a.glyph)) {
    const pulse = a.glyphPulse
      ? 1 + 0.14 * Math.sin((t ?? 0) * Math.PI * 6)
      : 1;
    drawIcon(
      ctx,
      a.glyph,
...
function drawCardSteps(
  ...
  const points = content.steps.map((s, i) => ({
    ...s,
    cx: im.x + (n === 1 ? 300 : margin + (usable * i) / (n - 1)) * sf,
  }));
  const navy = doc.canvas.accent,
    gold = doc.canvas.accent2;
  ctx.save();
  points.slice(0, -1).forEach((s, i) => {
    ...
    ctx.beginPath();
    ctx.moveTo(s.cx + 15 * sf, cy);
    ctx.lineTo(next.cx - 15 * sf, cy);
    ctx.stroke();
  });
  points.forEach((s, i) => {
    ...
    withReveal(ctx, rev, s.cx, cy, () => {
      if (isIconId(s.icon)) {
        ...
        ctx.beginPath();
        ctx.arc(s.cx, cy, 15 * sf, 0, Math.PI * 2);
        ctx.fill();
        const pulse = s.iconPulse
          ? 1 + 0.14 * Math.sin((t ?? 0) * Math.PI * 6)
          : 1;
        drawIcon(ctx, s.icon!, s.cx, cy, 10 * sf * pulse, s.iconColor ?? navy);
      } else {
        const fill =
          s.state === "todo" ? "#ffffff" : s.state === "active" ? gold : navy;
        ctx.fillStyle = fill;
        ctx.strokeStyle = s.state === "todo" ? "#d1d5db" : fill;
        ctx.lineWidth = Math.max(1, 2 * sf);
        ctx.beginPath();
        ctx.arc(s.cx, cy, 15 * sf, 0, Math.PI * 2);
        ctx.fill();
        ...
        if (s.state === "done") {
          ...
          ctx.lineJoin = "round";
          ctx.beginPath();
          ctx.moveTo(s.cx - 6 * sf, cy + 5 * sf);
          ctx.lineTo(s.cx - 1 * sf, cy + 10 * sf);
          ctx.lineTo(s.cx + 8 * sf, cy - 5 * sf);
          ctx.stroke();
        }
      }
      ctx.fillStyle = "#6b7280";
      ctx.font = `${13 * sf}px -apple-system, system-ui, sans-serif`;
      ctx.textAlign = "center";
...
function drawInfoCardRows(
  ...
  const headerH = 84,
    rowH = 78,
    highlightH = content.highlightRow ? 64 : 0;
  ...
  content.rows.forEach((r, i) => {
    ...
    withReveal(ctx, rev, im.x + im.w / 2, cy, () => {
      ...
      ctx.beginPath();
      ctx.arc(im.x + 46 * sf, cy, 19 * sf, 0, Math.PI * 2);
      ctx.fill();
      if (isIconId(r.icon)) {
        const pulse = r.iconPulse
          ? 1 + 0.14 * Math.sin((t ?? 0) * Math.PI * 6)
          : 1;
        drawIcon(
          ctx,
        ...
        ctx.font = `${18 * sf}px -apple-system, system-ui, sans-serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
      ...
      ctx.fillText(
        r.quote ? `„${r.value}"` : r.value,
        im.x + 80 * sf,
        cy + 16 * sf,
      );
    });
    ctx.strokeStyle = "#eef0f3";
    ctx.lineWidth = Math.max(1, sf);
    ctx.beginPath();
    ctx.moveTo(im.x, y + rowH * sf - sf);
    ctx.lineTo(im.x + im.w, y + rowH * sf - sf);
    ctx.stroke();
    y += rowH * sf;
  });
  if (content.highlightRow) {
    ...
    const rev =
      t != null ? revealFor("callout", content.rows.length, total, t) : null;
    withReveal(ctx, rev, im.x + im.w / 2, cy, () => {
      ...
      if (isIconId(hr.icon)) {
        ...
        ctx.font = `${17 * sf}px -apple-system, system-ui, sans-serif`;
        ctx.fillStyle = "#000";
        ctx.fillText(hr.icon, im.x + 52 * sf, cy + 6 * sf);
      }
      ctx.fillStyle = navy;
      ctx.font = `700 ${16 * sf}px -apple-system, system-ui, sans-serif`;
  ...
  content.fields.forEach((f, i) => {
    ...
    withReveal(ctx, rev, im.x + im.w / 2, cy, () => {
      ...
      if (hasIcon) {
        ...
        ctx.beginPath();
        ctx.arc(im.x + 40 * sf, cy + 5 * sf, 18 * sf, 0, Math.PI * 2);
        ctx.fill();
        const pulse = f.iconPulse
          ? 1 + 0.14 * Math.sin((t ?? 0) * Math.PI * 6)
          : 1;
        drawIcon(
          ctx,
          ...
          cy + 5 * sf,
          10 * sf * pulse,
          f.iconColor ?? doc.canvas.accent,
        );
      }
      ctx.textAlign = "left";
      ctx.textBaseline = "alphabetic";
      ctx.fillStyle = "#171a1f";
      ctx.font = `600 ${17 * sf}px -apple-system, system-ui, sans-serif`;
      ctx.fillText(f.q, im.x + tx, cy - 6 * sf);
      ctx.fillStyle = "#8a8f98";
      ctx.font = `${13 * sf}px -apple-system, system-ui, sans-serif`;
      ctx.fillText(f.sub, im.x + tx, cy + 16 * sf);
      ctx.fillStyle = doc.canvas.accent2;
      ctx.font = `${20 * sf}px -apple-system, system-ui, sans-serif`;
      ctx.textAlign = "center";
...
function coverFit(
  ...
  if (imgAspect > targetAspect) {
    ...
    sx = Math.round((srcW - cropW) * 0.5);
  } else {
    cropH = Math.round(srcW / targetAspect);
    sy = Math.round((srcH - cropH) * 0.5);
  }
...
function drawDeviceChecklist(
  ...
  const srcW = PREVISIT_SCREEN_W.phone,
    srcH = 1160;
  ...
  ctx.save();
  roundRectPath(ctx, screen.x, screen.y, screen.w, screen.h, 0);
  ctx.clip();
    ...
    withReveal(ctx, rev, cx, cy, () => {
      if (isIconId(it.icon)) {
        ...
        drawIcon(
          ...
          cy,
          10 * scale * pulse,
          it.iconColor ?? primary,
        );
      } else {
        const fill = it.done ? primary : "#ffffff";
        ctx.fillStyle = fill;
        ctx.strokeStyle = it.done ? primary : "#d1d5db";
        ...
        if (it.done) {
          ...
          ctx.lineJoin = "round";
          ctx.beginPath();
          ctx.moveTo(lx(28), ly(y + 5));
          ctx.lineTo(lx(33), ly(y + 10));
          ctx.lineTo(lx(42), ly(y - 5));
          ctx.stroke();
        }
      }
      ctx.fillStyle = it.done ? "#171a1f" : "#9aa0a8";
      ctx.font = `${it.done ? 600 : 500} ${17 * scale}px -apple-system, system-ui, sans-serif`;
      ctx.textAlign = "left";
      ctx.textBaseline = "alphabetic";
...
function drawDeviceDashboard(
  ...
  const srcW = PREVISIT_SCREEN_W.dashboard,
    srcH = 620;
  const { sx, sy, scale } = coverFit(srcW, srcH, screen.w, screen.h);
  const lx = (localX: number) => screen.x + (localX - sx) * scale;
  const ly = (localY: number) => screen.y + (localY - sy) * scale;
  ...
  content.fields.forEach((f, i) => {
    ...
    withReveal(ctx, rev, cx, cy, () => {
      ...
      ctx.fillStyle = "#171a1f";
      ctx.font = `600 ${17 * scale}px -apple-system, system-ui, sans-serif`;
...
function drawLoupe(
  ...
  a: MockupAnnotation,
  scale: number,
  sampleSource: CanvasImageSource,
): void {
  const s = annRect(doc, a);
  const fx = s.x + a.fx * s.w,
    fy = s.y + a.fy * s.h;
  const W = doc.canvas.w,
    H = doc.canvas.h;
  const R = a.radius ?? 150;
  const lx = (a.lensX ?? 0.86) * W,
    ly = (a.lensY ?? 0.82) * H;
  ...
  ctx.beginPath();
  ctx.arc(lx, ly, R + R * 0.028, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
}
...
function drawAnnotations(
  ...
  markers.forEach((a, i) => {
    const c = annCenter(doc, a);
    withReveal(
      ctx,
      a.noReveal || t == null
        ? null
        : revealFor("marker", i, markers.length, t, doc.motion, reducedMotion),
      c.x,
      c.y,
      () => drawMarker(ctx, doc, a),
    );
  });
  callouts.forEach((a, i) => {
    ...
    withReveal(
      ...
      c.x,
      c.y,
      () => drawCallout(ctx, doc, a),
    );
  });
  loupes.forEach((a, i) => {
    const lx = (a.lensX ?? 0.86) * doc.canvas.w,
      ly = (a.lensY ?? 0.82) * doc.canvas.h;
    withReveal(
      ctx,
      a.noReveal || t == null
        ? null
        : revealFor("loupe", i, loupes.length, t, doc.motion, reducedMotion),
      lx,
      ly,
      () => drawLoupe(ctx, doc, a, scale, sampleSource),
    );
  });
...
function drawMindmap(ctx: CanvasRenderingContext2D, doc: MockupDoc): void {
  ...
  try {
    (
      ctx as CanvasRenderingContext2D & { letterSpacing?: string }
    ).letterSpacing = `${Math.round(W * 0.0015)}px`;
  } catch {
    /* noop */
  }
  ...
  level1.forEach((node, i) => {
    ...
    if (m > 0) {
      ...
      kids.forEach((k, j) => {
        ...
        const kx = cx + Math.cos(aa) * R2,
          ky = cy + Math.sin(aa) * R2;
        ...
        roundedPill(ctx, kx, ky, k.label, l2font, W * 0.011, H * 0.012);
        ctx.fill();
        ctx.strokeStyle = hexToRgba(c.accent2, 0.7);
        ctx.lineWidth = Math.max(1.5, W * 0.0011);
        roundedPill(ctx, kx, ky, k.label, l2font, W * 0.011, H * 0.012);
        ctx.stroke();
        ctx.fillStyle = ink;
        ctx.font = l2font;
        ctx.fillText(k.label, kx, ky + 1);
      });
    }
  });
  ...
  level1.forEach((node, i) => {
    ...
    roundedPill(ctx, p.x, p.y, node.label, l1font, W * 0.014, H * 0.016);
    ctx.fill();
  ...
  ctx.font = rootFont;
  ctx.fillText(tree.label, cx, cy + 1);
  void rp;
  ctx.restore();
}
...
export async function rasterizeMockup(
  doc: MockupDoc,
  scale = 1,
  opts?: {
    skipAnnotations?: boolean;
    anim?: { t: number; reducedMotion?: boolean };
    transparent?: boolean;
    videoTime?: number;
  },
): Promise<HTMLCanvasElement> {
  ...
  ctx.scale(scale, scale);
  const t = opts?.anim?.t;
  const reducedMotion = opts?.anim?.reducedMotion === true;
  // Global kamera-transform under avspilling: push-in (jevn zoom) + zoom-punch (beat-puls).
  if (
    !reducedMotion &&
    t != null &&
    (doc.canvas.pushIn || (doc.canvas.beatPunch && doc.canvas.bpm))
  ) {
    let z = 1;
    ...
    ctx.translate(doc.canvas.w / 2, doc.canvas.h / 2);
    ctx.scale(z, z);
    ctx.translate(-doc.canvas.w / 2, -doc.canvas.h / 2);
  }
  ...
  const revealOf = (
    ...
    if (tl) {
      const lt = clipLocalT(tl, ref, "reveal", t);
      if (lt != null)
        return revealFromLocal(
          kind,
          lt,
          doc.motion,
          reducedMotion,
    ...
    return revealFor(kind, i, total, t, doc.motion, reducedMotion);
  };
  ...
  if (doc.canvas.scene?.id) {
    await drawScene(
      ctx,
      doc,
      doc.canvas.scene?.typeAnim?.text ? typeTFor("scene") : (t ?? undefined),
    );
    doc.texts.forEach((tx) => drawText(ctx, doc, tx));
    await drawLogo(ctx, doc.canvas);
    if (!opts?.skipAnnotations)
      drawAnnotations(ctx, doc, scale, canvas, t, reducedMotion);
  ...
  if (doc.mindmap && doc.mindmap.trim()) {
    const rev =
      t != null
        ? revealFor("mindmap", 0, 1, t, doc.motion, reducedMotion)
        : null;
    withReveal(ctx, rev, doc.canvas.w / 2, doc.canvas.h * 0.52, () =>
      drawMindmap(ctx, doc),
    );
    await drawLogo(ctx, doc.canvas);
    applyWarmth(ctx, canvas, doc.canvas.warmth);
  ...
  if (doc.images?.length) {
    ...
    const chatSourceImg = !chatSourceDev
      ? doc.images.find((im) => im.chatType?.turns?.length)
      : undefined;
    ...
    const isBackdropIllustration = (
      v: MockupImageSlot["illustration"],
    ): boolean => v === "office-backdrop" || v === "waiting-room-backdrop";
    const drawOrder = doc.images
      .map((im, i) => ({ im, i }))
      .sort(
        (a, b) =>
          (isBackdropIllustration(a.im.illustration) ? 0 : 1) -
          (isBackdropIllustration(b.im.illustration) ? 0 : 1),
      );
    for (const { im: im0, i } of drawOrder) {
      ...
      const kfv = (prop: string, def: number): number =>
        im0.kf && t != null ? (sampleKf(im0.kf[prop], t) ?? def) : def;
      ...
      if (isBackdropIllustration(im.illustration)) {
        const drawFn =
          im.illustration === "waiting-room-backdrop"
            ? drawWaitingRoomBackdrop
            : drawOfficeBackdrop;
        if (rev)
          withReveal(ctx, rev, im.x + im.w / 2, im.y + im.h / 2, () =>
            drawFn(ctx, im.x, im.y, im.w, im.h, im.sceneStyle),
          );
        else drawFn(ctx, im.x, im.y, im.w, im.h, im.sceneStyle);
        ctx.restore();
        continue;
      }
      if (im.illustration === "person-laptop") {
        // Når brukerens kreditt-gatede 3D-generering er ferdig, er rasterassetet
        // primærkilden. Hele slotten beholder transform/reveal-motion; den
        // redigerbare Canvas-riggen under brukes ved planlagt/feilet generering.
        if (
          im.figureGeneration?.status === "generated" &&
          (im.figureGeneration.renderMode ?? "generated-raster") !== "editable-rig" &&
          im.mediaProvenance?.source === "generated" &&
          im.image
        ) {
          const mode = im.figureGeneration.renderMode ?? "generated-raster";
          const frames = mode === "sprite-sequence" && im.sprite?.frames.length
            ? im.sprite.frames
            : [im.image];
          const phase = opts?.videoTime != null ? opts.videoTime * (im.sprite?.fps || 1) : 0;
          const frameIndex = Math.floor(phase) % frames.length;
          const nextIndex = (frameIndex + 1) % frames.length;
          const blend = mode === "sprite-sequence" && im.sprite?.interpolation === "crossfade"
            ? phase - Math.floor(phase)
            : 0;
          const generated = await loadImage(frames[frameIndex]).catch(() => null);
          const nextGenerated = blend > 0.001
            ? await loadImage(frames[nextIndex]).catch(() => null)
            : null;
          if (generated) {
            const drawGenerated = () => {
              drawGeneratedFigure(ctx, im, generated, generated.naturalWidth, generated.naturalHeight, doc.canvas.accent, 1 - blend);
              if (nextGenerated) drawGeneratedFigure(ctx, im, nextGenerated, nextGenerated.naturalWidth, nextGenerated.naturalHeight, doc.canvas.accent, blend);
            };
            if (rev)
              withReveal(
                ctx,
        ...
        const reaction =
          chatSource?.chatType && t != null
            ? chatReactionAt(chatSource.chatType, typeTFor(chatSource.id) ?? 0)
            : null;
        ...
        const pose: PersonRigPose = {
          ...
          blink: kfv(
            "blink",
            reaction?.blinkPulse ? 1 : (vt * 0.33) % 3 < 0.12 ? 1 : 0,
          ),
          ...
          handTargetLeftX: kfv("handTargetLeftX", 0),
          handTargetLeftY: kfv("handTargetLeftY", 0),
          handTargetRightX: kfv("handTargetRightX", 0),
          handTargetRightY: kfv("handTargetRightY", 0),
        };
        if (rev)
          withReveal(ctx, rev, im.x + im.w / 2, im.y + im.h / 2, () =>
            drawPersonLaptop(ctx, im.x, im.y, im.w, im.h, pose, im.personStyle),
          );
        else
          drawPersonLaptop(ctx, im.x, im.y, im.w, im.h, pose, im.personStyle);
        ctx.restore();
      ...
      if (im.sprite?.frames?.length) {
        const idx =
          opts?.videoTime != null
            ? Math.floor(opts.videoTime * im.sprite.fps) %
              im.sprite.frames.length
            : 0;
        try {
          const img = await loadImage(im.sprite.frames[idx]);
          source = img;
          sw = img.naturalWidth;
      ...
      if (!source && opts?.videoTime != null && im.video) {
        try {
          const v = await loadVideo(im.video);
          await seekVideo(v, opts.videoTime);
          source = v;
          sw = v.videoWidth;
          sh = v.videoHeight;
        } catch {
      ...
      if (rev)
        withReveal(ctx, rev, im.x + im.w / 2, im.y + im.h / 2, () =>
          drawImageSlot(ctx, im, src, cw, ch),
        );
      ...
      if (im.chatType?.turns?.length && (!rev || rev.alpha > 0.7)) {
        drawChatType(
          ...
          doc,
          im.chatType,
          { x: im.x, y: im.y, w: im.w, h: im.h, radius: im.radius },
          typeTFor(im.id) ?? 1,
        );
      }
  ...
  for (let i = 0; i < doc.devices.length; i++) {
    ...
    await drawDevice(
      ...
      doc,
      dev,
      dev.typeAnim?.text ? typeTFor(dev.id) : (t ?? undefined),
      t ?? undefined,
    );
    if (dev.chatType?.turns?.length && (!rev || rev.alpha > 0.7)) {
      const sr = deviceScreenRect(dev);
      const radius =
        ((DEVICE_FRAMES as Record<string, { radius: number }>)[dev.variant]
          ?.radius ?? 0.04) * dev.w;
      drawChatType(
        ctx,
        doc,
  ...
  doc.texts.forEach((tx, i) => {
    ...
    if (
      ...
      tx.priceText &&
      /^\d+$/.test(tx.priceText) &&
      tx.text.endsWith(tx.priceText)
    ) {
      const prog = rev ? rev.p : 1;
      drawTx = {
        ...tx,
        text: `${tx.baseText} · ${Math.round(Number(tx.priceText) * prog)}`,
      };
    }
    withReveal(
      ctx,
      rev,
      tx.x + tx.w / 2,
      tx.y + measureTextHeight(tx) / 2,
      () => drawText(ctx, doc, drawTx),
    );
  });
  await drawLogo(ctx, doc.canvas);
  if (!opts?.skipAnnotations)
...
export async function renderMotionFrames(
  ...
  for (let f = 0; f < total; f++) {
    ...
    frames.push(
      await rasterizeMockup(doc, scale, {
        anim: { t },
        videoTime: f / cfg.fps,
      }),
    );
...
export async function rasterizeToPngDataUrl(
  doc: MockupDoc,
  scale = 1,
  opts?: { transparent?: boolean; format?: "png" | "webp" },
): Promise<string> {
...
export async function rasterizeLayers(
  ...
  if (bg.ctx) {
    fillBackground(bg.ctx, doc);
    await drawBgImage(bg.ctx, doc);
    drawDecor(bg.ctx, doc);
  }
  ...
  for (const dev of doc.devices) {
    ...
    out.push({
      name: VARIANT_LABEL[dev.variant] ?? dev.variant,
      canvas: c.canvas,
    });
  }
  for (const t of doc.texts) {
    ...
    const label =
      (t.text || "Tekst").replace(/\s+/g, " ").trim().slice(0, 24) || "Tekst";
