...
async function hydrateSource(src?: string): Promise<string | undefined> {
  if (!src?.startsWith("mockup-cloud-file:")) return src;
  return blobToDataUrl(await downloadMockupAsset(src.slice("mockup-cloud-file:".length)));
}
export async function hydrateMockupProject(doc: MockupDoc): Promise<MockupDoc> {
  const out = structuredClone(doc);
  for (const device of out.devices) device.image = await hydrateSource(device.image);
  for (const image of out.images ?? []) image.image = (await hydrateSource(image.image)) || image.image;
  if (out.canvas.logo?.image) out.canvas.logo.image = (await hydrateSource(out.canvas.logo.image)) || out.canvas.logo.image;
  return out;
}
