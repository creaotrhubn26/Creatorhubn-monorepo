...
function windowFor(
  ...
  switch (kind) {
    case "device":
      return [
        Math.min(0.5, 0.02 + i * 0.09),
        Math.min(0.6, 0.02 + i * 0.09 + 0.34),
      ];
    case "text":
      return [
        Math.min(0.6, 0.24 + i * 0.06),
        Math.min(0.8, 0.24 + i * 0.06 + 0.28),
      ];
    case "marker":
      return [0.36, 0.56];
    case "callout": {
...
export function revealFromLocal(
  kind: RevealKind,
  local: number,
  motion?: MockupBrandMotionSpec,
  reducedMotion = false,
  ...
  local = clamp01(local);
  if (reducedMotion && motion?.reducedMotion === "fade") {
    return { p: local, alpha: clamp01(local * 1.2), ty: 0, scale: 1 };
  }

  if (motion) {
    const baseEased = alreadyEased
      ? local
    ...
    const eased =
      pop && motion.overshoot > 0
        ? baseEased + Math.sin(Math.PI * local) * motion.overshoot
        : baseEased;
    const scaleWeight =
      kind === "callout" ||
      kind === "loupe" ||
      ...
      kind === "mindmap"
        ? 1.5
        : 1;
    const travelWeight = kind === "device" ? 1 : kind === "text" ? 0.7 : 0.55;
    return {
      p: local,
      alpha: clamp01(local * 1.2),
      ty: motion.revealDistance * travelWeight * (1 - baseEased),
      scale: 1 - motion.revealScale * scaleWeight * (1 - eased),
    };
  }
  ...
  return {
    p: local,
    alpha,
    ty: rise * (1 - easeOutCubic(local)),
    scale: 0.98 + 0.02 * easeOutCubic(local),
  };
}
