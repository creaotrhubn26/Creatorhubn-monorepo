...
export function clipLocalT(
  ...
  const local = Math.max(
    ...
    Math.min(
      1,
      (tGlobal * tl.duration - clip.start) / Math.max(0.01, clip.len),
    ),
  );
...
export function deriveTimeline(doc: MockupDoc): MockupTimeline {
  ...
  doc.devices.forEach((d, i) => {
    ...
    if (d.typeAnim?.text && d.threeD) {
      ...
      clips.push({
        id: `type_${d.id}`,
        label: `skriv: ${d.typeAnim.text.slice(0, 16)}`,
        track: 1,
        start: t + 0.6,
        len,
        kind: "type",
    ...
    if (d.chatType?.turns?.length) {
      clips.push({
        id: `chat_${d.id}`,
        label: `chat: ${d.chatType.turns[0]?.question.slice(0, 16) ?? ""}`,
        track: 4,
        start: devStart + devLen + 0.15,
        len: chatClipDuration(d.chatType),
        kind: "type",
        ref: d.id,
        ease: "linear",
      });
  ...
  if (doc.canvas.scene?.typeAnim?.text) {
    ...
    clips.push({
      id: "type_scene",
      label: `skriv: ${doc.canvas.scene.typeAnim.text.slice(0, 16)}`,
      track: 1,
      start: t + 0.4,
      len,
      kind: "type",
  ...
  (doc.images ?? []).forEach((im, i) => {
    ...
    if (im.chatType?.turns?.length) {
      clips.push({
        id: `chat_${im.id}`,
        label: `chat: ${im.chatType.turns[0]?.question.slice(0, 16) ?? ""}`,
        track: 4,
        start: revStart + revLen + 0.15,
        len: chatClipDuration(im.chatType),
        kind: "type",
        ref: im.id,
        ease: "linear",
      });
    ...
    if (
      im.illustration === "person-laptop" &&
      im.kf &&
      Object.keys(im.kf).length
    ) {
...
export const TYPE_PRESETS: { id: string; label: string; cfg: TypeAnimCfg }[] = [
  ...
  },
  {
    id: "url",
    label: "Åpne side",
    cfg: { text: "creatorhubn.com", field: "url", payoff: true },
  },
  {
    id: "code",
    label: "Skriv kode",
    cfg: {
      text: "const leads = await api.fetch();",
      field: "code",
      keyPop: true,
    },
  },
  {
    id: "terminal",
    label: "Terminal",
    cfg: { text: "npm run deploy", field: "terminal", payoff: true },
  },
  {
    id: "form",
    label: "Skriv i felt",
    cfg: {
      text: "daniel@creatorhubn.com",
      field: "plain",
      placeholder: "E-post",
      correct: true,
    },
...
export type MockupTypographyId =
  | "moderne"
  | "editorial"
  | "teknisk"
  | "geometrisk"
...
> = {
  ...
  teknisk: {
    label: "Teknisk",
    display: 'Menlo, "SF Mono", monospace',
...
export function resolveBaseBg(canvas: MockupCanvasSpec): string {
  ...
  switch (canvas.background) {
    case "light":
      return "#f4f5f7";
    case "brand":
      return mixHex(canvas.accent, "#0a0b10", 0.82); // dyp merkevare-tone
    case "dark":
    default:
      return "#0f1117";
  }
...
export type MockupBrandMotionProfile =
  | "calm-precise"
  | "warm-human"
  | "bold-editorial"
  | "energetic-social";

/** Persisted motion art direction resolved from the project's brand guide. */
export interface MockupBrandMotionSpec {
  profile: MockupBrandMotionProfile;
...
export interface MockupDoc {
  ...
  /** Brandstyrt timing/bevegelse. Mangler den, brukes den eldre standardmotionen. */
  motion?: MockupBrandMotionSpec;
...
export function deviceHeight(
  slot: Pick<MockupDeviceSlot, "variant" | "w">,
): number {
...
export type MockupAnnotationKind =
  | "callout"
  | "loupe"
  | "marker"
  | "step"
...
export function makeAnnotation(
  ...
  if (kind === "callout")
    return { ...base, n, label: "Ny funksjon", side: "right" };
  if (kind === "loupe")
    return {
      ...base,
      fx: 0.5,
  ...
  if (kind === "marker")
    return { ...base, fx: 0.4, fy: 0.35, fw: 0.22, fh: 0.14 };
  if (kind === "step")
    return { ...base, deviceId: undefined, fx: 0.035, fy: 0.035, n };
  if (kind === "pill")
    return {
      ...base,
      deviceId: undefined,
...
export function makeDevice(
  variant: MockupDeviceVariant,
  partial: Partial<MockupDeviceSlot> = {},
): MockupDeviceSlot {
...
export interface MockupCinematicSceneStyle {
...
export interface MockupImageSlot {
  ...
  sprite?: {
    frames: string[];
    fps: number;
    interpolation?: "hold" | "crossfade";
    labels?: string[];
    packageId?: string;
    layerManifest?: MockupFigureLayer[];
  };
  x: number;
  y: number;
  w: number;
  ...
  /** Helhetlig brand-, lys- og materialretning for prosedurale bakgrunner. */
  ...
  /** Beskrivende tekst og asset-opphav for konseptillustrasjoner. */
  altText?: string;
  mediaProvenance?: {
    source: "deterministic-procedural" | "generated" | "uploaded";
    disclosure?: string;
  ...
  /** Genereringsoppskrift for en original kinematisk 3D-figur. Role Room fyller
   *  denne ut; Mockup Studio kan deretter generere ett ekte rasterasset. Inntil
   *  genereringen er ferdig brukes person-laptop-riggen som redigerbar fallback. */
  figureGeneration?: MockupFigureGenerationSpec;
...
export interface PersonStyle {
  /** Renderer-kvalitet. Eldre dokumenter uten felt beholder standardutseendet. */
  renderQuality?: "standard" | "high" | "cinematic";
  artDirection?: "editorial-vector-v2" | "cinematic-3d-v1";
  presentation?: "female" | "male" | "neutral";
  ageRange?: "young-adult" | "adult" | "mature";
  faceShape?: "soft" | "balanced" | "angular";
  skin?: string;
  hair?: string;
  /** Klær-farge. */
  shirt?: string;
  ...
  hairStyle?: "kort" | "buffert" | "krøller" | "coily" | "bald";
...
export interface MockupFigureAppearance {
  ageRange: "young-adult" | "adult" | "mature";
  skinTone: string;
  hairColor: string;
  hairStyle: "short" | "volume" | "curly" | "coily" | "bald";
  faceShape: "soft" | "balanced" | "angular";
  customDirection?: string;
}

export type MockupFigureRenderMode =
  | "editable-rig"
  | "generated-raster"
  | "sprite-sequence";
export type MockupFigurePoseId =
  | "neutral"
  | "presenting"
  | "listening"
  | "pointing"
  | "walking";
export type MockupFigureExpressionId =
  | "calm"
  | "warm"
  | "focused"
  | "surprised";
export type MockupFigureMasterView = "front" | "three-quarter" | "profile";

export interface MockupFigureLayer {
  id: string;
  label: string;
  z: number;
}

export interface MockupFigureCompositing {
...
export interface MockupFigureQaCheck {
  id: string;
  passed: boolean;
  detail: string;
  score?: number;
...
export interface MockupFigurePixelAudit {
  width: number;
  height: number;
  transparentRatio: number;
  visibleRatio: number;
  brandPixelRatio: number;
  touchesEdge: boolean;
  passed: boolean;
  checks: MockupFigureQaCheck[];
}

export interface MockupFigureSemanticAudit {
  score: number;
  model?: string;
  checks: MockupFigureQaCheck[];
  summary: string;
...
export interface MockupFigureVisualQa {
  status: "pending" | "passed" | "warning" | "failed";
  score: number;
  checkedAt: string;
  pixel?: MockupFigurePixelAudit;
  semantic?: MockupFigureSemanticAudit;
  checks: MockupFigureQaCheck[];
}

export interface MockupFigureVariant {
  id: string;
  label: string;
  kind: "master-view" | "pose" | "expression" | "sprite-frame";
  image: string;
  assetHash?: string;
  ...
  generatedAt: string;
  providerMode?: "text-generation" | "reference-edit";
  qa?: MockupFigureVisualQa;
}

export interface MockupFigureCharacterMaster {
  id: string;
  consistencyKey: string;
  approvedView: MockupFigureMasterView;
  views: Partial<Record<MockupFigureMasterView, MockupFigureVariant>>;
  locks: {
    face: boolean;
    hair: boolean;
    outfit: boolean;
...
export interface MockupFigureGenerationSpec {
  qualityTarget: "cinematic-feature-animation";
  renderMode?: MockupFigureRenderMode;
  presentation: "female" | "male" | "neutral";
  ...
  pipelineSkills?: Record<string, {
    version: string;
    executionKey: string;
    decisions: Record<string, unknown>;
  }>;
  status: "planned" | "generating" | "generated" | "failed";
  provider: "gpt-image-2" | "fal-flux-pro-1.1";
  consistencyStrategy?: "reference-edit" | "provider-seed";
  prompt: string;
  ...
  poseId?: MockupFigurePoseId;
  expressionId?: MockupFigureExpressionId;
  characterMaster?: MockupFigureCharacterMaster;
  variants?: MockupFigureVariant[];
  compositing?: MockupFigureCompositing;
  visualQa?: MockupFigureVisualQa;
  assetHash?: string;
  generatedAt?: string;
  error?: string;
}

export const PERSON_SCENARIO_LABELS: Record<
  NonNullable<PersonStyle["scenario"]>,
  string
> = {
  laptop: "Ved laptop",
  stand: "Stående",
...
export const PERSON_OUTFIT_LABELS: Record<
  NonNullable<PersonStyle["outfit"]>,
  string
> = {
  genser: "Genser",
  skjorte: "Skjorte",
  hettegenser: "Hettegenser",
  legefrakk: "Legefrakk",
...
export const BACKDROP_ANCHORS: Record<
  "office-backdrop" | "waiting-room-backdrop",
  BackdropAnchor[]
> = {
  "office-backdrop": [
...
export const PERSON_ROLE_PRESETS: {
  id: string;
  label: string;
  style: PersonStyle;
}[] = [
  {
    id: "pasient",
    label: "Pasient",
    style: { outfit: "genser", shirt: "#1b294b", accessory: "ingen" },
  },
  {
    id: "lege",
    label: "Lege",
    style: { outfit: "legefrakk", shirt: "#ffffff", accessory: "id-kort" },
  },
  {
    id: "sykepleier",
    label: "Sykepleier",
    style: { outfit: "sykepleier", shirt: "#5fb8c9", accessory: "id-kort" },
  },
];
export const PERSON_HAIR_LABELS: Record<
  NonNullable<PersonStyle["hairStyle"]>,
  string
> = {
  kort: "Kort",
  buffert: "Buffert",
...
export const PERSON_ACCESSORY_LABELS: Record<
  NonNullable<PersonStyle["accessory"]>,
  string
> = {
  ingen: "Ingen",
  briller: "Briller",
...
export interface PersonRigPose {
  ...
  tears: number;
  /** Manuelle IK-målforskyvninger i figurens lokale piksler. Albuen løses automatisk. */
  handTargetLeftX: number;
  handTargetLeftY: number;
  handTargetRightX: number;
  handTargetRightY: number;
}
...
export const PERSON_RIG_PROPS: {
  id: keyof PersonRigPose;
  label: string;
  min: number;
  max: number;
}[] = [
  ...
  { id: "handTargetLeftX", label: "IK venstre hånd · X", min: -120, max: 120 },
  { id: "handTargetLeftY", label: "IK venstre hånd · Y", min: -120, max: 120 },
  { id: "handTargetRightX", label: "IK høyre hånd · X", min: -120, max: 120 },
  { id: "handTargetRightY", label: "IK høyre hånd · Y", min: -120, max: 120 },
];
...
export const EXPRESSION_PRESETS: {
  id: string;
  label: string;
  values: Partial<PersonRigPose>;
}[] = [
  {
    id: "noytral",
    label: "Nøytral",
    values: { mouthCurve: 0, browRaise: 0, eyeSize: 1, tears: 0 },
  },
  {
    id: "glad",
    label: "Glad",
    values: { mouthCurve: 1, browRaise: 0.3, eyeSize: 1, tears: 0 },
  },
  {
    id: "fokusert",
    label: "Fokusert",
    values: { mouthCurve: 0.1, browRaise: -0.3, eyeSize: 0.85, tears: 0 },
  },
  {
    id: "frustrert",
    label: "Frustrert",
    values: { mouthCurve: -0.6, browRaise: -0.6, eyeSize: 0.9, tears: 0.3 },
  },
  {
    id: "overrasket",
    label: "Overrasket",
    values: { mouthCurve: 0.2, browRaise: 1, eyeSize: 1.5, tears: 0 },
  },
];
...
export const IMAGE_TRANSFORM_PROPS: {
  id: string;
  label: string;
  min: number;
  max: number;
...
export const CHAT_TYPE_SPEEDS: Record<ChatTypeSpeed, number> = {
  slow: 7,
  normal: 13,
  fast: 20,
};
export const CHAT_TYPE_SPEED_LABELS: Record<ChatTypeSpeed, string> = {
  slow: "Sakte",
  normal: "Normal",
  fast: "Rask",
};
...
function chatClipDuration(chat: ChatTypeConfig): number {
  ...
  return chat.turns.reduce(
    (sum, turn, i) =>
      sum + chatTurnDuration(turn, cps) + (i > 0 ? CHAT_TURN_GAP : 0),
    0,
  );
}
...
export function makeImage(
  image: string,
  partial: Partial<MockupImageSlot> = {},
): MockupImageSlot {
  return {
    id: uid("img"),
    image,
...
const bounds = (W: number, _H: number, o: GridOpts) => {
  const m = o.margin ?? Math.round(W * 0.037),
    gap = o.gap ?? Math.round(W * 0.019);
...
export const gridCells: LayoutFn = (n, W, H, opts = {}) => {
  ...
  return Array.from({ length: n }, (_, i) => {
    const c = i % cols,
      r = Math.floor(i / cols);
    return {
      x: Math.round(m + c * (cw + gap)),
      y: Math.round(top + r * (ch + gap)),
      w: Math.round(cw),
      h: Math.round(ch),
    };
  });
...
export const columnCells: LayoutFn = (n, W, H, opts = {}) => {
  if (n <= 0) return [];
  const { m, gap, top, bottom } = bounds(W, H, opts);
  const ch = (H - top - bottom - gap * (n - 1)) / n,
    cw = W - 2 * m;
  return Array.from({ length: n }, (_, i) => ({
    x: Math.round(m),
    y: Math.round(top + i * (ch + gap)),
    w: Math.round(cw),
    h: Math.round(ch),
  }));
};
...
export const heroCells: LayoutFn = (n, W, H, opts = {}) => {
  ...
  const { m, gap, top, bottom } = bounds(W, H, opts);
  const heroW = Math.round((W - 2 * m - gap) * 0.62),
    restW = W - 2 * m - gap - heroW;
  const rest = n - 1,
    rch = (H - top - bottom - gap * (rest - 1)) / rest;
  const cells: LayoutCell[] = [{ x: m, y: top, w: heroW, h: H - top - bottom }];
  for (let i = 0; i < rest; i++)
    cells.push({
      x: Math.round(m + heroW + gap),
      y: Math.round(top + i * (rch + gap)),
      w: Math.round(restW),
      h: Math.round(rch),
    });
  return cells;
...
export const bentoCells: LayoutFn = (n, W, H, opts = {}) => {
  ...
  const maxB = Math.max(...cells.map((c) => c.y + c.h)),
    avail = H - bottom;
  if (maxB > top) {
    const f = (avail - top) / (maxB - top);
    cells.forEach((c) => {
      c.y = Math.round(top + (c.y - top) * f);
      c.h = Math.round(c.h * f);
    });
  }
  return cells;
};
...
export const circleCells: LayoutFn = (n, W, H, opts = {}) => {
  ...
  const cx = W / 2,
    cy = H / 2,
    R = Math.min(W, H) * 0.33,
    size = Math.min(W, H) * (opts.cols ? 0.2 : 0.24);
  return Array.from({ length: n }, (_, i) => {
    const a = -Math.PI / 2 + (i / n) * Math.PI * 2;
    return {
      x: Math.round(cx + R * Math.cos(a) - size / 2),
      y: Math.round(cy + R * Math.sin(a) - size / 2),
      w: Math.round(size),
      h: Math.round(size),
      rotation: Math.round(((a * 180) / Math.PI + 90) / 4),
    };
  });
};
...
export const diagonalCells: LayoutFn = (n, W, H, opts = {}) => {
  ...
  return Array.from({ length: n }, (_, i) => ({
    x: Math.round(m + i * stepX),
    y: Math.round(m + i * stepY),
    w: Math.round(size),
    h: Math.round(size),
    rotation: ((i * 47) % 9) - 4,
  }));
...
export const colsForAspect = (w: number, h: number): number =>
  w > h * 1.25 ? 3 : 2;
...
export function makeText(
  role: MockupTextRole,
  partial: Partial<MockupTextSlot> = {},
): MockupTextSlot {
...
export const PURPOSE_CATEGORIES: {
  id: string;
  label: string;
  categories: MockupTemplateCategory[];
}[] = [
  {
    id: "product",
    label: "Presentere et produkt",
    categories: ["produktoversikt", "funksjonslansering"],
  },
  {
    id: "feature",
    label: "Selge en funksjon",
    categories: ["funksjonslansering", "salgspitch"],
  },
  {
    id: "offer",
    label: "Oppsummere et tilbud",
    categories: ["salgspitch", "nokkeltall"],
  },
  {
    id: "case",
    label: "Vise en kundecase",
    categories: ["kundecase", "produktoversikt"],
  },
  {
    id: "launch",
    label: "Annonsere en lansering",
    categories: ["funksjonslansering", "produktoversikt"],
  },
];
...
export const MOCKUP_TEMPLATES: MockupTemplate[] = [
  {
    id: "hero_mac_phone_dark",
    name: "Hero — Mac + iPhone",
    category: "produktoversikt",
    variant: "dark",
    devices: 2,
    description:
      "Overskrift til venstre, MacBook med iPhone-overlapp til høyre. Mørk.",
    build: () => {
      const t = ink("dark");
      return doc(
        "Produktoversikt",
        ...
        [
          ...
          makeText("body", {
            text: "Kort verdiløfte i én til to setninger. Bytt ut teksten og skjermbildene.",
            x: 120,
            y: 560,
            w: 520,
            color: t.body,
  ...
  {
    id: "hero_mac_phone_light",
    name: "Hero — Mac + iPhone (lys)",
    category: "produktoversikt",
    variant: "light",
    devices: 2,
    ...
    build: () => {
      ...
      return doc(
        "Produktoversikt",
        "hero_mac_phone_light",
        baseCanvas({ background: "light", bgStyle: "clean" }),
        [
          makeDevice("macbook", { x: 700, y: 250, w: 820, shadow: true }),
          makeDevice("iphone", {
            x: 1210,
        ...
        [
          ...
          makeText("body", {
            text: "Kort verdiløfte i én til to setninger.",
            x: 120,
            y: 560,
            w: 520,
            color: t.body,
  ...
  {
    id: "feature_trio_dark",
    name: "Funksjoner — iPad + iPhone",
    category: "funksjonslansering",
    variant: "dark",
    devices: 2,
    description:
      "Sentrert overskrift over to enheter, med tre funksjons-punkter.",
    build: () => {
      const t = ink("dark");
      return doc(
        "Funksjonslansering",
        "feature_trio_dark",
        baseCanvas({ accent: "#a78bfa", accent2: "#22d3ee" }),
        [
          makeDevice("ipad", {
            x: 470,
            y: 380,
    ...
    build: () => {
      ...
      return doc(
        ...
        [
          ...
          makeText("body", {
            text: "Ett verktøy fra første møte til signert kontrakt.",
            x: 120,
            y: 560,
            w: 440,
            color: t.body,
  ...
  {
    id: "stats_dark",
    name: "Nøkkeltall — iPhone",
    category: "nokkeltall",
    variant: "dark",
    devices: 1,
    description:
      "iPhone til høyre, tre store nøkkeltall i accent-farger til venstre.",
    build: () => {
      const t = ink("dark");
      return doc(
        "Nøkkeltall",
        "stats_dark",
        baseCanvas({ typography: "geometrisk" }),
        [
          makeDevice("iphone", {
            x: 1120,
            y: 250,
    ...
    build: () => {
      ...
      return doc(
        ...
        [
          ...
          makeText("body", {
            text: "— Kari Nordmann, driftsleder",
            x: 120,
            y: 560,
            w: 500,
            color: t.body,
  ...
  },
  {
    ...
    devices: 2,
    description:
      "Apple Watch og iPhone side ved side — for helse/aktivitet-apper.",
    build: () => {
      const t = ink("dark");
      return doc(
        "Produktoversikt",
        "watch_focus_dark",
        baseCanvas({ accent: "#22c55e", accent2: "#22d3ee" }),
        [
          makeDevice("iphone", {
            x: 760,
            y: 330,
        ...
        [
          ...
          makeText("body", {
            text: "iPhone og Apple Watch, sømløst synkronisert.",
            x: 120,
            y: 540,
            w: 440,
            color: t.body,
  ...
  {
    id: "feature_launch_light",
    name: "Lansering — iPad (lys)",
    category: "funksjonslansering",
    variant: "light",
    devices: 1,
    ...
    build: () => {
      ...
      return doc(
        "Funksjonslansering",
        "feature_launch_light",
        baseCanvas({ background: "light", bgStyle: "gradient" }),
  ...
  {
    id: "chat_hook_laptop",
    name: "Chat-hook — MacBook",
    category: "kundecase",
    variant: "light",
    devices: 1,
    description:
      "Sitat-drevet hook: en flerrunders chat-samtale (hode + spørsmål/svar-bobler, auto-scroll) som skriver seg selv inn — prikker → tegn-for-tegn — inni en ekte MacBook-mock. Overskrift/brødtekst til venstre. God for bruker-/kunde-sitat-annonser (se chatType på enheten for fart-presets og flere runder).",
    build: () => {
      const t = ink("light");
      return doc(
        "Chat-hook",
          ...
          makeDevice("macbook", {
            ...
            chatType: {
              ...
              turns: [
                {
                  question:
                    'Skriv spørsmålet som skal "skrives" inn på skjermen…',
                  reply: "Skriv svaret her (valgfritt)…",
                },
              ],
            },
          }),
  ...
  {
    id: "previsit_campaign_1",
    name: "PreVisit kampanje — foto + UI-kort",
    category: "kampanje",
    variant: "light",
    devices: 0,
    description:
      "Kvadratisk (1080×1080) sosial annonse: nummerert steg-badge, MedSide-logo, serif-overskrift m/ gull-aksent, foto-plassholder + rundet UI-kort koblet med en prikket linje, bunn-pill med kort forklaring. Bytt ut foto-plassholderne med ekte bilder.",
    build: () => {
      const canvas = baseCanvas({
        w: 1080,
        h: 1080,
        accent: MEDSIDE_COLORS.navy,
        accent2: MEDSIDE_COLORS.gold,
        background: "light",
        bgColor: MEDSIDE_COLORS.cream,
        bgStyle: "clean",
        decor: "arc",
        typography: "editorial",
      ...
      return {
        ...
        images: [
          ...
          makeImage(
            placeholderImage(
              "PASIENT-FOTO",
              MEDSIDE_COLORS.cream,
              MEDSIDE_COLORS.navy,
            ),
            { x: 0, y: 420, w: 540, h: 660, radius: 0 },
          ),
          makeImage(
            placeholderImage(
              "LEGE-FOTO",
            ...
            { x: 540, y: 420, w: 540, h: 660, radius: 0 },
          ),
          makeImage(placeholderImage("", "#ffffff", "#ffffff"), {
            x: 538,
            y: 420,
            w: 4,
            h: 660,
          ...
          makeImage(
            ...
            ),
            {
              ...
              h: 340,
              radius: 20,
              cardContent: DEFAULT_PREVISIT_CARD_CONTENT,
            },
          ),
        ],
        annotations: [
          ...
          // bunn-pills, én pr. foto (ikke én sentrert pill).
          {
            ...
            fy: 0.945,
            glyph: "⌂",
            label: "Pasienten fyller ut",
            label2: "– hjemme i ro og fred",
          },
          {
            id: uid("ann"),
            kind: "pill",
            ...
            fy: 0.945,
            glyph: "✓",
            label: "Klinikeren er forberedt",
            label2: "– før pasienten kommer",
          },
        ],
      };
    },
  ...
  {
    id: "previsit_campaign_2",
    name: "PreVisit kampanje — foto + skjema-kort",
    category: "kampanje",
    variant: "light",
    devices: 0,
    description:
      "Kvadratisk (1080×1080): ett foto-plassholder, flytende «skjema»-kort (spørsmålsliste m/ piler + CTA), badge 2, én bunn-pill.",
    build: () => {
      const canvas = baseCanvas({
        w: 1080,
        h: 1080,
      ...
      return {
        ...
        images: [
          makeImage(
            ...
            { x: 0, y: 420, w: 1080, h: 660, radius: 0 },
          ),
          makeImage(
  ...
  {
    id: "previsit_campaign_3",
    name: "PreVisit kampanje — historie → anamnese",
    category: "kampanje",
    variant: "light",
    devices: 0,
    description:
      "Kvadratisk (1080×1080): halvt foto-plassholder + bredt «strukturert anamnese»-kort m/ gull-utheving på siste rad, badge 3, sentrert bunn-pill.",
    build: () => {
      const canvas = baseCanvas({
        w: 1080,
        h: 1080,
      ...
      const cardContent: PreVisitInfoCardContent = {
        ...
        rows: [
          { icon: "cross", label: "Aktuelt", value: "Hodepine, tiltakende" },
          {
            icon: "clipboard",
            label: "Anamnese",
            value: "Ingen tidligere hodepine",
          },
          { icon: "pill", label: "Legemidler", value: "Paracetamol ved behov" },
          {
            icon: "people",
            label: "Relevant historikk",
            value: "Migrene i familien",
          },
        ],
      ...
      return {
        ...
        annotations: [
          ...
          },
          {
            ...
            fy: 0.945,
            glyph: "✓",
            label: "Bedre forberedelser",
            label2: "– bedre konsultasjoner",
          },
        ],
      };
    },
  ...
  {
    id: "previsit_campaign_4",
    name: "PreVisit kampanje — mottatt-kort",
    category: "kampanje",
    variant: "light",
    devices: 0,
    description:
      "Kvadratisk (1080×1080): ett foto-plassholder (kliniker), «PreVisit mottatt ✓»-kort m/ nøkkeldata, badge 4, sentrert bunn-pill.",
    build: () => {
      const canvas = baseCanvas({
        w: 1080,
        h: 1080,
      ...
      const cardContent: PreVisitInfoCardContent = {
        title: "PreVisit mottatt",
        checkHeader: true,
        rows: [
          {
            icon: "cross",
            label: "Hovedproblem",
            value: "Hodepine og svimmelhet",
          },
  ...
  {
    id: "previsit_campaign_5",
    name: "PreVisit kampanje — sammendrag",
    category: "kampanje",
    variant: "light",
    devices: 0,
    description:
      "Kvadratisk (1080×1080): to foto-plassholdere (pasient+kliniker), «PreVisit sammendrag»-kort m/ sitat + tidsstempel, badge 5, tre kompakte bunn-pills.",
    build: () => {
      const canvas = baseCanvas({
        w: 1080,
        h: 1080,
        ...
        rows: [
          {
            icon: "people",
            label: "Pasienten",
            value: "Kari Nordmann · 10:30",
          },
          { icon: "cross", label: "Hovedårsak", value: "Vedvarende tretthet" },
          {
            icon: "warning",
            label: "Relevante forhold",
            value: "Ingen kjente allergier",
          },
          {
            icon: "chat",
            label: "Pasientens egne notater",
            value: "Har vært slapp i 2 uker",
            quote: true,
          },
        ],
  ...
  {
    id: "previsit_campaign_6",
    name: "PreVisit kampanje — telefon + laptop",
    category: "kampanje",
    variant: "light",
    devices: 2,
    description:
      "Kvadratisk (1080×1080), avslutnings-annonse uten badge/foto: telefon + laptop-skjermer viser produktet, tre bunn-pills (Trygt/Enkelt/Klar til bruk).",
    build: () => {
      const canvas = baseCanvas({
        w: 1080,
        h: 1080,
      ...
      const questionContent: PreVisitQuestionContent = {
        ...
        total: 10,
        question:
          "Hva er grunnen til at du bestilte time i dag? Fortell gjerne hva som har skjedd og hva som gjorde at du tok kontakt nå.",
        answer: "Jeg har hatt vondt i hodet i noen uker.",
      };
      const noteContent: PreVisitNoteContent = {
        patientLine:
          "Kari Nordmann · født 12.05.1968 · tlf +47 900 00 000 · time 18.08.2026, 10:30",
        paragraphs: [
        ...
        footerNote:
          "Når du har limt notatet inn i journalen, marker det som hentet – da slettes samtalen og notatet.",
        chatLog: [
          {
            question: "Hva er grunnen til at du bestilte time i dag?",
            answer: "Jeg har hatt vondt i hodet i noen uker.",
          },
          {
            question: "Kan du fortelle om det du opplever?",
            answer:
              "Vondt i hodet nesten hver dag. Våkner med det. Er også kvalm på morgenen.",
          },
          {
            question: "Hvordan påvirker det deg i hverdagen?",
            answer: "Vanskelig å jobbe.",
          },
          {
            question: "Hvordan har det utviklet seg?",
            answer:
              "Blir gradvis verre. Begynte helt smått i bakhodet og har blitt mer og mer.",
          },
        ],
      };
      const base = doc(
        ...
        [
          makeDevice("iphone", {
            ...
            y: 420,
            w: 230,
            image: previsitQuestionScreenImage(
              questionContent,
              canvas.accent,
              canvas.accent2,
            ),
        ...
        [
          ...
          makeText("body", {
            text: "Pasienten forteller før timen. Du møter forberedt.",
            x: 70,
            y: 260,
            w: 900,
            size: 22,
...
export function previsitUiCardImage(
  ...
  const steps = content.steps.map((s, i) => ({
    ...s,
    cx: n === 1 ? 300 : margin + (usable * i) / (n - 1),
  }));
...
export function previsitInfoCardImage(opts: {
  ...
  const highlightSvg = highlightRow
    ? (() => {
        ...
        const svg = animateRows
          ? ""
          : `<rect x="24" y="${y + 8}" width="${W - 48}" height="${highlightH - 16}" rx="14" fill="${accent}" fill-opacity="0.14"/>
  ...
  const footerSvg = footer
    ? `<text x="${W / 2}" y="${H - 14}" font-family="-apple-system,system-ui,sans-serif" font-size="12" fill="#9aa0a8" text-anchor="middle">${escXml(footer)}</text>`
    : "";
...
export function previsitFormListCardImage(
  fields: { q: string; sub: string; icon?: string; iconColor?: string }[],
  buttonText: string,
  primary: string,
  accent: string,
  animateRows?: boolean,
...
export function previsitPhoneScreenImage(
  content: PreVisitChecklistContent,
  primary: string,
): string {
  const { items, animate } = content;
  const rowsSvg = animate
    ? ""
    : items
        .map((it, i) => {
          const y = 210 + i * 84;
      ...
      <text x="64" y="${y + 6}" font-family="-apple-system,system-ui,sans-serif" font-size="17" font-weight="600" fill="#171a1f">${escXml(it.label)}</text>`;
          ...
          const mark = it.done
            ? `<path d="M28 ${y + 5} l5 5 l9 -10" stroke="#fff" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"/>`
            : "";
...
export function previsitDashboardScreenImage(
  content: PreVisitDashboardContent,
  primary: string,
  accent: string,
): string {
  ...
  const fieldsSvg = animate
    ? ""
    : fields
        .map(
          (f, i) => `
    <text x="${col(i)}" y="${row(i)}" font-family="-apple-system,system-ui,sans-serif" font-size="14" fill="#8a8f98">${escXml(f.label)}</text>
    <text x="${col(i)}" y="${row(i) + 26}" font-family="-apple-system,system-ui,sans-serif" font-size="17" font-weight="600" fill="#171a1f">${escXml(f.value)}</text>`,
        )
        .join("");
...
export function previsitQuestionScreenImage(
  content: PreVisitQuestionContent,
  primary: string,
  accent: string,
): string {
  ...
  const qSvg = qLines
    .map(
      (l, i) =>
        `<text x="60" y="${252 + i * 28}" font-family="-apple-system,system-ui,sans-serif" font-size="19" fill="#171a1f">${escXml(l)}</text>`,
    )
    .join("");
  ...
  const aSvg = aLines
    .map(
      (l, i) =>
        `<text x="52" y="${answerY + 42 + i * 26}" font-family="-apple-system,system-ui,sans-serif" font-size="16" fill="#6b7280">${escXml(l)}</text>`,
    )
    .join("");
...
export function previsitNoteScreenImage(
  content: PreVisitNoteContent,
  primary: string,
  accent: string,
): string {
    ...
    ? chatLog!
        .map((qa) => {
          ...
          const qBlock = qLines
            .map(
              (l, i) =>
                `<text x="64" y="${y + i * 21}" font-family="-apple-system,system-ui,sans-serif" font-size="13" font-weight="700" fill="#171a1f">${escXml(l)}</text>`,
            )
            .join("");
          ...
          const aBlock = aLines
            .map(
              (l, i) =>
                `<text x="64" y="${y + i * 21}" font-family="-apple-system,system-ui,sans-serif" font-size="13" fill="#3c4453">${escXml(l)}</text>`,
            )
            .join("");
    ...
    : paragraphs
        .map((p) => {
          ...
          const block = lines
            .map(
              (l, i) =>
                `<text x="64" y="${y + i * 24}" font-family="-apple-system,system-ui,sans-serif" font-size="14" fill="#3c4453">${escXml(l)}</text>`,
            )
            .join("");
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="880" height="620">
    ...
    ${wrapByChars(footerNote, 70)
      .slice(0, 2)
      .map(
        (l, i) =>
          `<text x="60" y="${170 + boxH + (wrapByChars(footerNote, 70).length > 1 ? 33 : 53) + i * 18}" font-family="-apple-system,system-ui,sans-serif" font-size="12" fill="#6b7280">${escXml(l)}</text>`,
      )
      .join("")}
...
const ROLE_LABEL: Record<MockupTextRole, string> = {
  eyebrow: "Etikett",
  title: "Overskrift",
  body: "Brødtekst",
  tag: "Liten tekst",
...
export function orientationGroup(
  v: MockupDeviceVariant,
): MockupDeviceVariant[] {
  const landscape: MockupDeviceVariant[] = [
    "macbook",
    "ipad_landscape",
    "browser",
  ];
  const portrait: MockupDeviceVariant[] = [
    "iphone",
    "ipad",
    "watch",
    "android",
  ...
  doc.devices.forEach((d) => {
    if (d.slotId)
      slots.push({
        id: d.slotId,
        kind: "device",
        label: DEV_LABEL[d.variant],
        zone: { x: d.x, y: d.y, w: d.w, rotation: d.rotation },
        allowedVariants: orientationGroup(d.variant),
      });
  });
  doc.texts.forEach((t) => {
    if (t.slotId)
...
export function applyLayout(doc: MockupDoc, id: LayoutVariantId): MockupDoc {
  ...
  const zoneOf = (slotId?: string) =>
    doc.slots!.find((s) => s.id === slotId)?.zone;
  const devices = doc.devices.map((d) => {
    ...
    return id === "mirror"
      ? {
          ...
          x: W - z.x - d.w,
          y: z.y,
          rotation: z.rotation != null ? -z.rotation : d.rotation,
        }
...
export function switchTemplate(
  ...
  next.devices.forEach((nd) => {
    ...
    if (idx < 0) {
      idx = oldDevices.findIndex((od) =>
        orientationGroup(od.variant).includes(nd.variant),
      );
      good = idx >= 0;
    }
    if (idx < 0 && oldDevices.length) {
      idx = 0;
      good = false;
    } // kryss-orientering → må justeres
    if (idx >= 0) {
  ...
  oldDevices.forEach((od) =>
    replaced.push(`${DEV_LABEL[od.variant]}-skjermbilde`),
  );
  ...
  next.texts.forEach((nt) => {
    ...
    if (idx >= 0) {
      const ot = oldTexts.splice(idx, 1)[0];
      nt.text = ot.text;
      nt.uppercase = ot.uppercase;
      kept.push(ROLE_LABEL[nt.role]);
    }
  });
  oldTexts.forEach((ot) =>
    dropped.push(`${ROLE_LABEL[ot.role]}: «${ot.text.slice(0, 20)}»`),
  );
...
function estTextHeight(
  ...
  const wrapped = Math.max(
    hard,
    Math.ceil((t.text.length * t.size * 0.5) / Math.max(1, w)),
  );
...
function reflowAuto(doc: MockupDoc, fmt: MockupFormat): MockupDoc {
  ...
  if (stack) {
    ...
    const totalW =
      rawW.reduce((a, b) => a + b, 0) +
      gap * Math.max(0, doc.devices.length - 1);
    const shrink = totalW > deviceZone.w ? deviceZone.w / totalW : 1;
    const w2 = rawW.map((w) => w * shrink);
    const rowW =
      w2.reduce((a, b) => a + b, 0) +
      gap * shrink * Math.max(0, doc.devices.length - 1);
...
export function currentFormatId(doc: MockupDoc): string | null {
  return (
    MOCKUP_FORMATS.find((f) => f.w === doc.canvas.w && f.h === doc.canvas.h)
      ?.id ?? null
  );
}
...
function captureLayout(doc: MockupDoc): Record<string, SlotPlacement> {
  ...
  doc.devices.forEach((d) => {
    if (d.slotId)
      out[d.slotId] = { x: d.x, y: d.y, w: d.w, rotation: d.rotation };
  });
  doc.texts.forEach((t) => {
    if (t.slotId) out[t.slotId] = { x: t.x, y: t.y, w: t.w, size: t.size };
  });
  // Frie elementer (uten slot): id-nøklet så per-format-layout kan gjenopprette dem eksakt.
  (doc.images ?? []).forEach((im) => {
    out[`img:${im.id}`] = { x: im.x, y: im.y, w: im.w, h: im.h };
  });
  doc.texts.forEach((t) => {
    if (!t.slotId)
      out[`txt:${t.id}`] = { x: t.x, y: t.y, w: t.w, size: t.size };
  });
  return out;
}
...
export function applyFormat(doc: MockupDoc, fmt: MockupFormat): MockupDoc {
  ...
  if (doc.images?.length) {
    const sx = fmt.w / doc.canvas.w,
      sy = fmt.h / doc.canvas.h;
    next = {
      ...next,
      images: doc.images.map((im) => ({
        ...im,
        x: Math.round(im.x * sx),
        y: Math.round(im.y * sy),
        w: Math.round(im.w * sx),
        h: Math.round(im.h * sy),
      })),
    };
  }
  ...
  if (saved) {
    next = {
      ...
      devices: next.devices.map((d) => {
        ...
        return p
          ? {
              ...
              y: p.y,
              w: p.w ?? d.w,
              rotation: p.rotation ?? d.rotation,
            }
          : d;
      }),
      texts: next.texts.map((t) => {
        ...
        return p
          ? { ...t, x: p.x, y: p.y, w: p.w ?? t.w, size: p.size ?? t.size }
          : t;
      }),
      ...
      images: (next.images ?? []).map((im) => {
        ...
        return p
          ? { ...im, x: p.x, y: p.y, w: p.w ?? im.w, h: p.h ?? im.h }
          : im;
      }),
    };
  }
...
export function saveFormatLayout(doc: MockupDoc): MockupDoc {
  ...
  return {
    ...doc,
    formatLayouts: { ...(doc.formatLayouts ?? {}), [fid]: captureLayout(doc) },
  };
}
...
export function buildMindmapDoc(
  mermaid: string,
  opts?: {
    accent?: string;
    accent2?: string;
  ...
  const doc: MockupDoc = {
    ...base,
    id: uid("doc"),
    name: opts?.name || "Produkt-mind map",
    devices: [],
    texts: [],
    slots: [],
    annotations: [],
...
export function duplicateProject(id: string): MockupDoc | null {
  ...
  if (!p) return null;
  const clone: MockupDoc = {
    ...(JSON.parse(JSON.stringify(p)) as MockupDoc),
    id: uid("doc"),
    name: `${p.name} (kopi)`,
    status: "draft",
    updatedAt: Date.now(),
...
export function saveBrandKit(
  ...
  const kit: MockupBrandKit = {
    id: uid("bk"),
    name: name.trim() || "Merkevare",
    accent: canvas.accent,
    accent2: canvas.accent2,
    background: canvas.background,
    bgStyle: canvas.bgStyle,
  ...
  try {
    localStorage.setItem(
      BRANDKITS_KEY,
      JSON.stringify(
        [kit, ...listBrandKits().filter((k) => k.name !== kit.name)].slice(
          0,
          20,
        ),
      ),
...
export function deleteBrandKit(id: string): void {
  try {
    localStorage.setItem(
      BRANDKITS_KEY,
      JSON.stringify(listBrandKits().filter((k) => k.id !== id)),
    );
  } catch {
    /* ignore */
  }
...
export function saveKit(
  ...
  try {
    ...
    return {
      ok: false,
      error:
        "Kunne ikke lagre kit (for stort? slett noen kits eller skjermbilder).",
    };
  }
}
...
export function deleteKit(id: string): void {
  try {
    localStorage.setItem(
      KITS_KEY,
      JSON.stringify(listKits().filter((k) => k.id !== id)),
    );
  } catch {
    /* ignore */
  }
...
export function saveVersion(
  ...
  const v: MockupVersion = {
    id: uid("ver"),
    name: name.trim() || `Versjon ${listVersions().length + 1}`,
    at: Date.now(),
    doc: JSON.parse(JSON.stringify(doc)) as MockupDoc,
  };
  try {
    localStorage.setItem(
      VERSIONS_KEY,
      JSON.stringify([v, ...listVersions()].slice(0, 20)),
    );
    return { ok: true };
  } catch {
...
export function addExport(name: string, format: string, path: string): void {
  ...
  try {
    localStorage.setItem(
      EXPORTS_KEY,
      JSON.stringify([rec, ...listExports()].slice(0, 40)),
    );
  } catch {
    /* ignore */
  }
...
export const DESIGN_PRESETS: MockupDesignPreset[] = [
  // ── Mørke ──────────────────────────────────────────────────────────────
  {
    id: "midnatt",
    label: "Midnatt",
    blurb: "Kjølig cyan-glød, ren og moderne",
    templateId: "hero_mac_phone_dark",
    typography: "moderne",
    decor: "orbs",
    background: "brand",
    bgStyle: "gradient",
  ...
  },
  {
    id: "aurora",
    label: "Aurora",
    blurb: "Mesh-gradient i teal og fiolett",
    templateId: "hero_mac_phone_dark",
    typography: "geometrisk",
    decor: "mesh",
    background: "brand",
    bgStyle: "atmospheric",
    accent: "#2dd4bf",
  ...
  },
  {
    id: "neon-pitch",
    label: "Neon",
    blurb: "Magenta-cyan salgspitch med punch",
    templateId: "sales_pitch_dark",
    typography: "geometrisk",
    decor: "mesh",
    background: "dark",
    bgStyle: "atmospheric",
  ...
  },
  {
    id: "kobber-stats",
    label: "Kobber",
    blurb: "Varm amber, redaksjonelle tall",
    templateId: "stats_dark",
    typography: "editorial",
    decor: "shapes",
    background: "dark",
    bgStyle: "gradient",
  ...
  },
  {
    id: "skog-feature",
    label: "Skog",
    blurb: "Grønt rutenett, rolig og teknisk",
    templateId: "feature_trio_dark",
    typography: "moderne",
    decor: "grid",
    background: "dark",
    bgStyle: "gradient",
  ...
  },
  {
    id: "grafitt-watch",
    label: "Grafitt",
    blurb: "Nøktern mono-teknisk på grafitt",
    templateId: "watch_focus_dark",
    typography: "teknisk",
    decor: "grid",
    background: "dark",
    bgStyle: "clean",
  ...
  },
  {
    id: "rubin",
    label: "Rubin",
    blurb: "Rosa-oransje glød, energisk",
    templateId: "stats_dark",
    typography: "geometrisk",
    decor: "orbs",
    background: "brand",
  ...
  },
  {
    id: "mono",
    label: "Mono",
    blurb: "Minimalistisk, ren og tekst-drevet",
    templateId: "sales_pitch_dark",
    typography: "teknisk",
    decor: "none",
    background: "dark",
  ...
  },
  {
    id: "indigo-hero",
    label: "Indigo",
    blurb: "Dyp indigo-mesh, produktfokus",
    templateId: "hero_mac_phone_dark",
    typography: "moderne",
    decor: "mesh",
    background: "brand",
  ...
  },
  {
    id: "ember",
    label: "Ember",
    blurb: "Glødende oransje-rød hero",
    templateId: "hero_mac_phone_dark",
    typography: "geometrisk",
    decor: "orbs",
    background: "brand",
  ...
  },
  {
    id: "nordlys",
    label: "Nordlys",
    blurb: "Grønn-fiolett mesh, nøkkeltall",
    templateId: "stats_dark",
    typography: "geometrisk",
    decor: "mesh",
    background: "brand",
  ...
  },
  {
    id: "karbon",
    label: "Karbon",
    blurb: "Teknisk mørk med rutenett",
    templateId: "feature_trio_dark",
    typography: "teknisk",
    decor: "grid",
    background: "dark",
  ...
  // ── Modige / kreative (nye virkemidler) ──────────────────────────────────
  {
    id: "plakat",
    label: "Plakat",
    blurb: "Diagonalt fargebånd, plakat-look",
    templateId: "hero_mac_phone_dark",
    typography: "geometrisk",
    decor: "band",
    background: "brand",
  ...
  },
  {
    id: "radar",
    label: "Radar",
    blurb: "Konsentriske ringer, teknisk puls",
    templateId: "hero_mac_phone_dark",
    typography: "teknisk",
    decor: "rings",
    background: "dark",
  ...
  },
  {
    id: "rytme",
    label: "Rytme",
    blurb: "Flytende bølger, rolig og elegant",
    templateId: "hero_mac_phone_dark",
    typography: "moderne",
    decor: "waves",
    background: "brand",
  ...
  },
  {
    id: "rampelys",
    label: "Rampelys",
    blurb: "Dramatisk spotlight på tallene",
    templateId: "stats_dark",
    typography: "editorial",
    decor: "spotlight",
    background: "dark",
  ...
  },
  {
    id: "volt",
    label: "Volt",
    blurb: "Elektriske striper, maks energi",
    templateId: "stats_dark",
    typography: "geometrisk",
    decor: "stripes",
    background: "dark",
  ...
  },
  {
    id: "puls",
    label: "Puls",
    blurb: "Radar-ringer i rosa, energisk",
    templateId: "stats_dark",
    typography: "geometrisk",
    decor: "rings",
    background: "brand",
  ...
  // ── Lyse ───────────────────────────────────────────────────────────────
  {
    id: "kritt",
    label: "Kritt",
    blurb: "Lys og luftig med indigo-orber",
    templateId: "hero_mac_phone_light",
    typography: "moderne",
    decor: "orbs",
    background: "light",
    bgStyle: "gradient",
  ...
  },
  {
    id: "redaksjon",
    label: "Redaksjon",
    blurb: "Serif-tittel, ren kundecase",
    templateId: "case_study_light",
    typography: "editorial",
    decor: "none",
    background: "light",
    bgStyle: "clean",
  ...
  },
  {
    id: "pastell",
    label: "Pastell",
    blurb: "Myk rosa-fiolett lansering",
    templateId: "feature_launch_light",
    typography: "geometrisk",
    decor: "mesh",
    background: "light",
    bgStyle: "gradient",
  ...
  },
  {
    id: "sitrus",
    label: "Sitrus",
    blurb: "Friske former i oransje og rosa",
    templateId: "feature_launch_light",
    typography: "moderne",
    decor: "shapes",
    background: "light",
  ...
  },
  {
    id: "luft",
    label: "Luft",
    blurb: "Nøytralt rutenett, blå aksent",
    templateId: "hero_mac_phone_light",
    typography: "teknisk",
    decor: "grid",
    background: "light",
  ...
  },
  {
    id: "lavendel",
    label: "Lavendel",
    blurb: "Myk lilla serif-kundecase",
    templateId: "case_study_light",
    typography: "editorial",
    decor: "orbs",
    background: "light",
  ...
  },
  {
    id: "prisme",
    label: "Prisme",
    blurb: "Lyst fargebånd, moderne plakat",
    templateId: "hero_mac_phone_light",
    typography: "moderne",
    decor: "band",
    background: "light",
  ...
  },
  {
    id: "fest",
    label: "Fest",
    blurb: "Konfetti-lansering, leken",
    templateId: "feature_launch_light",
    typography: "geometrisk",
    decor: "confetti",
    background: "light",
  ...
  },
  {
    id: "trykk",
    label: "Trykk",
    blurb: "Halvtone-raster, retro redaksjon",
    templateId: "case_study_light",
    typography: "teknisk",
    decor: "halftone",
    background: "light",
  ...
  },
  {
    id: "donning",
    label: "Dønning",
    blurb: "Rolige bølger, seriøs kundecase",
    templateId: "case_study_light",
    typography: "editorial",
    decor: "waves",
    background: "light",
