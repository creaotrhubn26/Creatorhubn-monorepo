...
const CINEMATIC_RUNTIME: string[] = [
  ...
  '    window.__paCamera = (move, x, y, w, h) => { const b = document.body; if (!b) return; const ox = (x + w / 2) + (window.scrollX || 0), oy = (y + h / 2) + (window.scrollY || 0); b.style.transformOrigin = ox + "px " + oy + "px"; const set = (tr, ms) => { b.style.transition = "transform " + (ms || 1100) + "ms cubic-bezier(.4,0,.2,1)"; b.style.transform = tr; }; if (move === "zoom_out") { b.style.transition = "none"; b.style.transform = "scale(1.2)"; requestAnimationFrame(() => set("none", 1200)); } else if (move === "pan_left") { set("scale(1.12) translateX(3.5%)"); } else if (move === "pan_right") { set("scale(1.12) translateX(-3.5%)"); } else if (move === "section_snap") { set("scale(1.04)", 700); } else if (move === "parallax") { set("scale(1.14) translate(2%,1%)"); requestAnimationFrame(() => setTimeout(() => set("scale(1.2) translate(-2%,-1%)", 2600), 60)); } else { set("scale(1.18)"); } };',
...
export function buildAutonomousScript(project: DemoProject, dwellsMs: number[]): string {
  ...
  project.scenes.forEach((s, i) => {
    ...
    const CAM_MOVES = ['push_in', 'pan_right', 'parallax', 'zoom_out', 'pan_left', 'section_snap'];
