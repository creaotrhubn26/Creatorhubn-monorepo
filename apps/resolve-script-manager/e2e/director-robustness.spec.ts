...
test.describe("claudeProxyService støtter AbortSignal", () => {
  ...
  test("fetch får signal videresendt", () => {
    // Implementasjonen kombinerer nå ekstern opts.signal + intern timeout-AbortController
    // (unngår listener-lekkasje, tillater intern timeout uavhengig av ekstern cancel) —
    // fetch får ctrl.signal, og opts.signal lytter til abort for å trigge ctrl.abort().
    // Testet var mot en eldre, enklere «send opts.signal rett til fetch»-form.
