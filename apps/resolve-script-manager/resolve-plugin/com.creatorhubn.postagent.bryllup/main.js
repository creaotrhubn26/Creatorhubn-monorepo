...
async function chatRunScript(scriptId, params) {
    ...
    if (scriptId === 'generate_broll' && String(p.mode).toLowerCase() === 'generate') {
        return { refused: 'generering koster kreditter — kjøres fra ✨-seksjonen i panelet m/ bekreftelse' };
    }
    if (scriptId === 'selects_builder' && ['edl', 'build'].includes(String(p.mode).toLowerCase())) {
        dry = true; // eksport/bygging utføres i panelet
    }
    if (scriptId === 'disk_cleanup' && String(p.mode).toLowerCase() === 'clean') {
        dry = true; // sletting utføres i panelet m/ confirm
    }
