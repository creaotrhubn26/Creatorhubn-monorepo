...
pub async fn generate_broll_clip(
    ...
    if prompt.is_empty() {
        return Err("prompt kreves for å generere klipp".into());
    }
    if no_people {
    ...
    let out = cmd
        .output()
        .map_err(|e| format!("higgsfield generate: {}", e))?;
    if let Some(p) = _tmp_start.as_ref() {
        let _ = std::fs::remove_file(p);
    } // rydd anker-ramme
    let stdout = String::from_utf8_lossy(&out.stdout);
    let stderr = String::from_utf8_lossy(&out.stderr);
    let url = extract_url(&stdout, &stderr).ok_or_else(|| {
        let tail: String = format!("{}{}", stdout, stderr)
            .chars()
            .rev()
            .take(300)
            .collect::<String>()
...
pub async fn generate_broll_clip_fal(
    ...
    let res = match resolution.as_str() {
        "480p" | "720p" | "1080p" => resolution.as_str(),
        _ => "720p",
    };
    ...
    let sub_json: serde_json::Value = sub
        .json()
        .await
        .map_err(|e| format!("submit-svar: {}", e))?;
    let response_url = sub_json
        .get("responseUrl")
        .and_then(|v| v.as_str())
        .ok_or("fikk ingen responseUrl fra serveren")?
        .to_string();
    ...
    for _ in 0..48 {
        ...
        let p = client
            .post(format!("{}/ai/generate-video/poll", base))
            .header("Authorization", format!("Bearer {}", bearer))
        ...
        if !p.status().is_success() {
            let s = p.status();
            let t = p.text().await.unwrap_or_default();
            return Err(format!(
                "poll {}: {}",
                s,
...
fn decode_data_url(data_url: &str, dir: &std::path::Path, stem: &str) -> Option<PathBuf> {
    ...
    let ext = if header.contains("jpeg") || header.contains("jpg") {
        "jpg"
    } else if header.contains("mpeg") || header.contains("mp3") {
        "mp3"
    } else if header.contains("wav") {
        "wav"
...
fn resolve_media_to_local(
    ...
    if input.starts_with("http") {
        ...
        let ok = Command::new("/usr/bin/curl")
            .args(["-sL", "-o", &path.to_string_lossy(), input])
            .status()
            .map(|s| s.success())
            .unwrap_or(false);
        if ok && path.metadata().map(|m| m.len() > 100).unwrap_or(false) {
            return Some(path);
        }
        return None;
    }
...
pub async fn generate_presenter_clip(
    ...
    let img_path = resolve_media_to_local(
        &presenter_image,
        &dir,
        &format!("{}._presenter", safe_scene),
        "png",
    )
    ...
    let dur = ffmpeg
        .as_ref()
        .and_then(|f| probe_duration_secs(f, &audio_path))
        .unwrap_or(6)
        .clamp(4, 15);
    // 'fast'-modus støtter kun 480p/720p → bruk 'std' for 1080p/4k.
    let mode = if res == "1080p" || res == "4k" {
        "std"
    } else {
        "std"
    };
    ...
    let out = Command::new(&hf)
        ...
        .output()
        .map_err(|e| format!("higgsfield presenter: {}", e))?;
    // Rydd temp-innputt.
    ...
    let url = extract_url(&stdout, &stderr).ok_or_else(|| {
        format!(
            "Fikk ingen resultat-URL fra Higgsfield: {}",
...
fn resolve_role_room_auth(app: &AppHandle) -> Result<(String, String), String> {
    ...
    if bearer.is_empty() {
        return Err(
            "Ikke logget inn til The Role Room (RR_BEARER_TOKEN mangler). Logg inn fra Settings."
                .into(),
        );
    }
...
pub async fn list_broll_jobs(
    ...
    let client = reqwest::Client::new();
    let r = client
        .get(format!("{}/ai/generate-video/jobs", base))
        .query(&[("projectId", project_id.as_str())])
        .header("Authorization", format!("Bearer {}", bearer))
        .send()
        .await
        .map_err(|e| format!("jobb-liste feilet: {}", e))?;
    ...
    let j: serde_json::Value = r
        .json()
        .await
        .map_err(|e| format!("jobb-liste-svar: {}", e))?;
    Ok(j.get("jobs")
