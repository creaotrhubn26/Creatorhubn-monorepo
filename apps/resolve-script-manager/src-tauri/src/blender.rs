...
fn scripts_dir(app: &AppHandle) -> Option<PathBuf> {
    if let Ok(res) = app.path().resource_dir() {
        for sub in ["scripts/blender", "_up_/scripts/blender"] {
            let p = res.join(sub);
            if p.join("render-device-scene.py").is_file() {
                return Some(p);
            }
        }
    }
    let dev = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("scripts")
        .join("blender");
    if dev.join("render-device-scene.py").is_file() {
        Some(dev)
    } else {
        None
    }
...
pub async fn render_blender_cinematic(
    ...
    let blender = find_blender()
        .ok_or("Blender ikke funnet. Installer Blender 3.6+ (brew install --cask blender).")?;
    ...
    let shot_path: Option<PathBuf> = shot.as_deref().and_then(|s| {
        if s.starts_with("data:") {
            decode_shot(s, &work)
        } else if !s.is_empty() && PathBuf::from(s).is_file() {
            Some(PathBuf::from(s))
        } else {
            None
        }
    ...
    let screendir: Option<PathBuf> = match type_text.as_deref() {
        Some(t) if !t.trim().is_empty() => {
            let py = find_python_pil()
                .ok_or("Skrive-animasjon krever python3 med Pillow (pip3 install pillow).")?;
            ...
            let mut a: Vec<String> = vec![
                ...
                n.to_string(),
                "--out".into(),
                sd.to_string_lossy().into(),
            ];
            if let Some(sp) = &shot_path {
                a.push("--base".into());
                a.push(sp.to_string_lossy().into());
            }
            if device != "macbook" {
                a.push("--osk".into());
            } // on-screen-tastatur for telefon/tablet
            if key_pop.unwrap_or(false) {
                a.push("--keypop".into());
            }
            let ok = Command::new(&py)
                ...
                .map(|s| s.success())
                .unwrap_or(false);
            if ok && sd.join("screen_0001.png").is_file() {
                Some(sd)
            } else {
                None
            }
    ...
    let layout = if kb_layout.as_deref() == Some("windows") {
        "windows"
    } else {
        "mac"
    };
        ...
        match type_text.as_deref() {
            Some(t) if !t.trim().is_empty() => find_python_pil().and_then(|py| {
                ...
                let ok = Command::new(&py)
                    ...
                    .map(|s| s.success())
                    .unwrap_or(false);
                if ok && dd.join("deck_0001.png").is_file() {
                    Some(dd)
                } else {
                    None
                }
    ...
    let mut args: Vec<String> = vec![
        ...
        device.into(),
        "--rotx".into(),
        rot_x.unwrap_or(0.0).to_string(),
        "--roty".into(),
        rot_y.unwrap_or(0.0).to_string(),
        "--rotz".into(),
        rot_z.unwrap_or(0.0).to_string(),
    ];
    if let Some(sp) = &shot_path {
        args.push("--shot".into());
    ...
    if !out.status.success() {
        ...
        return Err(format!(
            "Blender-render feilet: {}",
            err.lines().last().unwrap_or("ukjent")
        ));
    }
    ...
    let ok = Command::new(&ffmpeg)
        .args([
            ...
            "24",
            "-i",
            &work.join("frame_%04d.png").to_string_lossy(),
            "-vf",
