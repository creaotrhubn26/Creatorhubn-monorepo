...
async fn stream_child(
    ...
    let status = child
        .wait()
        .await
        .map_err(|e| format!("prosess feilet: {e}"))?;
    if let Some(state) = app.try_state::<RunningScriptsState>() {
...
pub async fn setup_playwright(app: AppHandle) -> Result<RunSummary, String> {
    ...
    let _ = app.emit(
        "script-event",
        json!({ "type": "started", "runId": run_id, "scriptId": "setup_playwright" }),
    );
    ...
    if code1 != 0 {
        return Err(format!(
            "npm install playwright feilet (exit {code1}). Er node/npm installert?"
        ));
    }
    ...
    if system_chrome_present() {
        ...
        let _ = app.emit(
            "script-event",
            json!({ "type": "log", "message": "Laster ned Chromium… (~150 MB)", "runId": run_id }),
        );
    ...
    let _ = app.emit(
        "script-event",
        json!({ "type": "finished", "runId": run_id, "succeeded": succeeded }),
    );
    Ok(RunSummary {
        run_id,
        script_id: "setup_playwright".into(),
        exit_code: Some(code2),
        succeeded,
        events: vec![],
        started_at,
...
#[tauri::command]
pub async fn run_playwright_demo(
    app: AppHandle,
    script_code: String,
    ...
    let _ = app.emit(
        "script-event",
        json!({ "type": "started", "runId": run_id, "scriptId": "run_playwright_demo" }),
    );
    ...
    if let Some(ref vp) = video_path {
        let _ = app.emit(
            "script-event",
            json!({ "type": "video", "path": vp, "runId": run_id }),
        );
    }
    ...
    Ok(RunSummary {
        run_id,
        script_id: "run_playwright_demo".into(),
        exit_code: Some(code),
        succeeded,
        events: vec![],
        started_at,
...
pub async fn playwright_capture_shots(app: AppHandle, url: String) -> Result<Value, String> {
    ...
    let txt = std::fs::read_to_string(&out_file)
        .map_err(|e| format!("Kunne ikke lese shots.json: {e}"))?;
...
async fn ig_capture_frames(
    ...
    std::fs::write(&script, IG_CAPTURE_MJS)
        .map_err(|e| format!("kunne ikke skrive capture-skript: {e}"))?;
...
fn ig_fade_in_vf(entrance: &str, alpha: bool) -> Option<String> {
    if entrance.is_empty() || entrance == "none" {
        return None;
    }
    let d = IG_ENTRANCE_DUR;
    Some(if alpha {
        format!("fade=t=in:st=0:d={d:.3}:alpha=1")
    } else {
        format!("fade=t=in:st=0:d={d:.3}")
    })
}
...
pub async fn render_infographic(
    ...
    let (work, _safe) = ig_capture_frames(
        &app,
        &html,
        &name,
        frames,
    ...
    let mut args: Vec<String> = vec![
        ...
        format!("{fps}"),
        "-i".into(),
        format!("{}/f%03d.png", work.to_string_lossy()),
    ];
    if let Some(vf) = ig_combine_vf(
        ig_fade_in_vf(&entrance, true),
        ig_fade_vf(exit_sec.unwrap_or(0.0), real_dur, true),
    ) {
        args.push("-vf".into());
        args.push(vf);
    }
    ...
    std::fs::rename(&tmp_file, &out_file).map_err(|e| {
        let _ = std::fs::remove_file(&tmp_file);
        format!("kunne ikke ferdigstille cache-fil: {e}")
    })?;
...
pub async fn export_infographic(
    ...
    let frames = if is_still {
        ...
        ((duration_sec.max(1.0)) * fps).round().clamp(8.0, 3600.0) as i64
    };
    // Reell klipp-lengde etter clamp — fade-timing bruker denne (ikke duration_sec).
    let real_dur = frames as f64 / fps;
    let (work, safe) = ig_capture_frames(
        &app,
        &html,
        &name,
        frames,
    ...
    let fade_alpha = ig_combine_vf(
        ig_fade_in_vf(&entrance, true),
        ig_fade_vf(ex, real_dur, true),
    );
    let fade_black = ig_combine_vf(
        ig_fade_in_vf(&entrance, false),
        ig_fade_vf(ex, real_dur, false),
    );
    ...
    let (out_file, args): (std::path::PathBuf, Vec<String>) = match format.as_str() {
        "png" => {
            ...
            std::fs::copy(work.join("f000.png"), &out)
                .map_err(|e| format!("kunne ikke lagre PNG: {e}"))?;
            return Ok(out.to_string_lossy().to_string());
        }
        "mp4" => {
            let out = out_dir.join(format!("{safe}.mp4"));
            let vf = match &fade_black {
                Some(f) => format!("{f},format=yuv420p"),
                None => "format=yuv420p".into(),
            };
            (
                out.clone(),
                vec![
        ...
        "gif" => {
            ...
            (
                ...
                vec![
                    ...
                    "-vf".into(),
                    format!(
                        "{pre}split[s0][s1];[s0]palettegen=reserve_transparent=1[p];[s1][p]paletteuse=alpha_threshold=128"
                    ),
                    out.to_string_lossy().to_string(),
                ],
            )
    ...
    if !metadata.is_empty() {
        if let Some(out) = args.pop() {
            ...
            args.push(format!("title={metadata}"));
            args.push("-metadata".into());
            args.push("comment=Laget med Post Agent Infographic Studio".into());
            args.push(out);
        }
    }
    let status = Command::new(&ffmpeg)
