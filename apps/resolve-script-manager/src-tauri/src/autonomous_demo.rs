            ...
            if r.status().is_success() {
                if let Ok(bytes) = r.bytes().await {
                    if std::fs::write(&mp3, &bytes).is_ok() {
                        ...
                        if ok {
                            return Ok(TtsResult {
                                path: m4a.to_string_lossy().to_string(),
                                duration_sec: probe_duration(&ffprobe, &m4a),
                            });
                        }
                    }
                }
...
pub async fn tts_from_audio(
    ...
    if !ok {
        return Err("ffmpeg konvertering feilet".into());
    }
    Ok(TtsResult {
        path: m4a.to_string_lossy().to_string(),
        duration_sec: probe_duration(&ffprobe, &m4a),
...
fn run_ff(ffmpeg: &PathBuf, args: &[String], what: &str) -> Result<(), String> {
    let st = Command::new(ffmpeg)
        .args(args)
        .status()
        .map_err(|e| format!("ffmpeg {what} spawn: {e}"))?;
    if !st.success() {
        return Err(format!("ffmpeg {what} feilet"));
    }
    Ok(())
}
...
pub async fn mux_demo_video(
    ...
    std::fs::create_dir_all(&out_dir)
        .map_err(|e| format!("kunne ikke lage ~/Movies/Post Agent: {e}"))?;
    let base: String = out_name
        .unwrap_or_else(|| "autonom-demo".into())
        .chars()
        .map(|c| {
    ...
    if segments.is_empty() {
        ...
        filter.push_str(&format!(
            "amix=inputs={}:dropout_transition=0:normalize=0[mix]",
            segments.len()
        ));
        args.extend([
            "-filter_complex".into(),
    ...
    if let Some(b64) = fin.frame_png.as_ref() {
        ...
        let (fx, fy, fw, fh) = (
            fin.frame_x.unwrap_or(80),
            fin.frame_y.unwrap_or(70),
            fin.frame_w.unwrap_or(1120),
            fin.frame_h.unwrap_or(700),
        );
        ...
        run_ff(
            ...
            &[
                ...
                "-i".into(),
                frame_png.to_string_lossy().to_string(),
                "-filter_complex".into(),
                format!("[1:v]null[bg];[0:v]scale={fw}:{fh}[v];[bg][v]overlay={fx}:{fy}[out]"),
                "-map".into(),
                "[out]".into(),
                "-map".into(),
                "0:a?".into(),
                ...
                "-c:a".into(),
                "aac".into(),
                framed.to_string_lossy().to_string(),
            ],
            "ramme",
        )?;
        current = framed;
    ...
    if has_intro || has_outro {
        ...
        if let Some(b64) = fin.intro_png.as_ref() {
            let p = work.join("intro.png");
            write_b64_png(b64, &p)?;
            let seg = work.join("intro.mp4");
            png_to_segment(&ffmpeg, &p, fin.intro_sec.unwrap_or(2.6), &seg)?;
            list.push(seg);
        }
        list.push(current.clone());
        if let Some(b64) = fin.outro_png.as_ref() {
            let p = work.join("outro.png");
            write_b64_png(b64, &p)?;
            let seg = work.join("outro.mp4");
            png_to_segment(&ffmpeg, &p, fin.outro_sec.unwrap_or(2.6), &seg)?;
            list.push(seg);
        }
    ...
    run_ff(
        ...
        &[
            ...
            current.to_string_lossy().to_string(),
            "-vf".into(),
            "eq=contrast=1.04:saturation=1.07,vignette=angle=PI/6".into(),
            "-c:v".into(),
            "libx264".into(),
            "-preset".into(),
            "veryfast".into(),
            ...
            "-movflags".into(),
            "+faststart".into(),
            final_out.to_string_lossy().to_string(),
        ],
        "grade+faststart",
    )
    .or_else(|_| {
