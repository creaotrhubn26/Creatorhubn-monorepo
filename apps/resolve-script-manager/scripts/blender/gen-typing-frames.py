...
def state_at(t):
    """Returnerer (typed, pressed) — skrevet substring + tasten som trykkes nå."""
    n = len(TEXT)
    if n == 0:
        return '', None
    pos = max(0.0, min(1.0, t)) * n
    count = max(0, min(n, math.floor(pos + 0.5)))
    pressed = TEXT[count - 1] if (t < 1 and count > 0) else None
    return TEXT[:count], pressed
...
for i in range(1, FRAMES + 1):
    ...
    im = base.copy()
    d = ImageDraw.Draw(im)
    typed, pressed = state_at(t)
    pad = int(W * 0.05)
    fieldh = int(fh * 1.6)
    if OSK:
        kb_top = draw_keyboard(d, W, H, pressed)
