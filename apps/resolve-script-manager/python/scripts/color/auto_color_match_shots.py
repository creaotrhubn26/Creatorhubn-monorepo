...
def run(params: dict[str, Any], dry_run: bool) -> None:
    ...
    if dry_run:
        bridge.result({
            ...
            "method": "signalstats Y-mean → eq brightness-adjustment",
        })
        return
    ...
    for i, p in enumerate(picks):
        ...
        measurements.append({"pickIndex": p.get("index", i), "y": y_mean})
        if (i + 1) % 5 == 0:
            bridge.progress(i + 1, len(picks), f"Målt {i+1}/{len(picks)}")

    # Bestem target Y — median er mer robust enn mean (skjermer mot outliers)
    sorted_y = sorted(m["y"] for m in measurements)
    target_y = sorted_y[len(sorted_y) // 2]
    spread = max(sorted_y) - min(sorted_y)
    bridge.log(f"Target Y={target_y:.1f}, baseline-spread={spread:.1f}")

    # Beregn brightness-adjust per pick. ffmpeg eq=brightness går fra -1.0 til
    # +1.0 hvor 0 = ingen endring. Konverter Y-delta (range 0-255) til
    # brightness-justering med konservativ scaling for å unngå clipping.
    adjustments = []
    for m in measurements:
        delta_y = target_y - m["y"]
        # 1 Y-enhet ≈ 0.004 brightness-units i ffmpeg eq
        brightness = max(-0.4, min(0.4, delta_y / 255.0 * 1.6))
        # Gamma-korrigering hjelper når shot er svært under/overexponert
        gamma = 1.0
    ...
    bridge.result({
        ...
        "method": "Y-mean median targeting via ffmpeg eq filter",
    })
