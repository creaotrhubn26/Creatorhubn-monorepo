...
# Trygt valgfri import av brand-vignett-helper (build skal ikke knekke om fila flyttes).
try:
...
BUDGET = {"prelude": .08, "arrival": .12, "ceremony": .21, "portraits": .13,
          ...
          # Norsk/vestlig bryllups-bue (norsk-bryllup-highlight-skillen):
          "establishing": .05, "prep": .13, "recessional": .07, "sendoff": .14}
...
CH_MOOD = {"prelude":"quiet","arrival":"energy","ceremony":"quiet","portraits":"quiet",
           ...
           # Norsk/vestlig: far-first-look (prep) er stille/emo; recessional+sendoff er energi
           "establishing":"quiet","prep":"quiet","recessional":"energy","sendoff":"energy"}
...
def build_in_resolve(source_video, picks, timeline_name, src_fps, video_only=True):
    ...
    vignette_path = (os.environ.get("BRAND_VIGNETTE_PATH") or "").strip()
    if vignette_path and apply_vignette_to_timeline is not None:
        try:
            vignette_result = apply_vignette_to_timeline(conn, tl, vignette_path)
    ...
    return {"timeline": name, "placed": len(placed) if isinstance(placed, list) else 0,
            "videoOnly": video_only, "brandVignette": vignette_result}
