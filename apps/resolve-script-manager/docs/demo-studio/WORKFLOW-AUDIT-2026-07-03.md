...
- [x] **G23 · Ingen PII-sladding i shots/guider/vision-kall** — *V1 FIKSET 2026-07-03: nytt `demo_pii_inject.js` maskerer skjemafelt-verdier (input/textarea, passord egne prikker) + e-postadresser + telefon-lignende siffersekvenser (8+ sifre; priser/årstall slipper) i synlig tekst UNDER hvert html2canvas-skudd, og gjenoppretter etterpå — dekker scan-thumbnails, skjermbilder og økt-shots (alt som går til Claude vision, delte guider og sky). Funksjonstestet i ekte Chromium (12/12 sjekker inkl. restore). GJENSTÅR (v2): video-OPPTAK (getDisplayMedia/native) og tekst brent i bilder (krever OCR-sladding).* · output-tillit · `demo_pii_inject.js`, `demo_capture.rs`, shot/scan/session-inject
