...
- [ ] **G8 · `continueMode='assisted'` + per-scene-modus er blindvei** · opptak · `demoStudioModel.ts:171-174`, `ScriptBuilderView.tsx:332-335`, `GuidedRecorderView.tsx:108`, `DemoStudioShell.tsx:1555` — valgbar i UI, men ingen kode leser `scene.continueMode` (kun prosjekt-nivå); shell viser hardkodet «continueMode: manual». Fiks: fjern dropdown eller implementer assisted (=verify-per-steg). **Est: S**
...
- [ ] **G13 · Overlays sendes men droppes i video-render** · eksport · `ExportView.tsx:288-297` vs `mockup-polish-pro.mts:46-51` — per-scene hotspot/cursor/ripple/autoZoom bygges og sendes, men `MockupConfig` har ikke feltet → forsvinner stille. UI er ærlig («Kommer», `ExportView.tsx:56-57`). Fiks: burn-in i mockup-polish-pro. **Est: M**
- [ ] **G14 · Guide: iframe-fallback + `startScrollPct` ignorert + hotspots i viewport-%** · eksport · `demoStudioExports.ts:577-579, 519, 573-595, 507` — frame-blokkerte sider gir svart boks; scroll-posisjon legges i steps-data men brukes aldri i guide-JS; hotspots driver ved scroll. Fiks: alltid-thumbnail/DOM-snapshot + scroll i render + DOM-forankring. **Est: M/L**
- [ ] **G15 · Ett-prosjekt-fellen** · alle · `demoStudioModel.ts:1270-1277`, `demoStudioStore.ts:136-143`, `DemoStudioShell.tsx:1942` — kun `last`-peker leses; «Lag ny video» gjør forrige prosjekt utilgjengelig for alltid (aldri ryddet → forsterker G1-kvoten). Fiks: prosjektliste-UI + opprydding. **Est: M**
...
- [ ] **G17 · Blank iframe-preview uten feilmelding** · preview · `DemoStudioShell.tsx:1330,1928` — X-Frame-Options-sider rendrer stille blankt; bruker tror verktøyet er ødelagt. Fiks: frame-blokk-deteksjon + «kjør skann»-hint. **Est: S**
- [ ] **G18 · URL-bytte invaliderer ikke avledet state** · alle · `DemoStudioShell.tsx:848-850, 297-304, 260` — ny URL i topbaren beholder `scanShots`/forståelse fra forrige nettsted; stale-sjekk finnes kun i `runOneClickDemo`. Fiks: invalider på URL-endring. **Est: S**
...
- [ ] **G21 · Native opptak gir null UI-feedback** · opptak · `GuidedRecorderView.tsx:106,130-148,200-204,240-244` — blokkerer 2–120 s uten «Recording»-badge; «Mark as Done» auto-starter neste opptak usynlig. Fiks: sett rec-state rundt native-kall. **Est: S**
