 ...
 * Uten nøkkel (`LEDGERLY_STRIPE_SECRET_KEY`) er integrasjonen ærlig inaktiv:
  ...
  async listPaidInvoices(sinceUnix?: number): Promise<StripePaidInvoice[]> {
    if (!this.hasApiKey) {
      throw new StripeAuthError(
        'Stripe er ikke konfigurert (LEDGERLY_STRIPE_SECRET_KEY mangler). Inntektssynk er ikke aktiv.',
      );
    }
