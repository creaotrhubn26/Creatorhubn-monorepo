  ...
  async fetchLegalText(refID: string): Promise<LegalTextResult> {
    ...
    if (!this.hasApiKey) {
      ...
      throw new LovdataAuthError(
        'Lovdata API-nøkkel (X-API-Key) er ikke konfigurert. Per-paragraf lovtekst-oppslag er ikke aktivt — sett LEDGERLY_LOVDATA_API_KEY. Åpne bulk-datasett kan hentes uten nøkkel via listPublicDatasets().',
      );
    }
