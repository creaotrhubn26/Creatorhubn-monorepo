...
struct CameraAssistView: View {
    ...
    private var viewfinderCard: some View {
        ...
        .onAppear {
            // Sømløst: gjenoppta kamera-tilkobling hvis vi har en lagret IP.
            model.camera.resumeIfNeeded()
        }
        .onChange(of: scenePhase) { _, phase in
            // Kom appen tilbake i forgrunn? Gjenoppta tilkoblingen.
            if phase == .active { model.camera.resumeIfNeeded() }
        }
    }
    ...
    private var cameraCard: some View {
        ...
        if state.connected {
            ...
            VStack(alignment: .leading, spacing: Theme.spacingSM) {
                ...
                Text("Kameraet sover ofte for å spare strøm. Vekk det (halvtrykk lukker) — vi kobler til automatisk. Tips: skru av «Auto avslåing» for stabil økt.")
                    .font(.caption)
                    .foregroundStyle(Theme.textSecondary)
                Button("Avbryt") { model.camera.disconnect() }
                    .font(.caption.weight(.semibold))
                    .foregroundStyle(Theme.textSecondary)
            }
            .card(elevated: true)
