...
extension APIClient {
    ...
    func sendMoteEtterarbeid(selskap: String, tekst: String,
                             ...
                             meetingAt: Date? = nil, requestId: UUID,
                             projectId: String) async throws -> EtterarbeidDTO {
    ...
    func hentMoteOppgaver(projectId: String) async throws -> [MoteOppgaveDTO] {
        struct Resp: Decodable { let oppgaver: [MoteOppgaveDTO] }
        var comps = URLComponents(string: "/api/leadgrid/oppgaver")!
        comps.queryItems = [URLQueryItem(name: "projectId", value: projectId)]
        let r: Resp = try await _get(comps.string ?? "/api/leadgrid/oppgaver")
        return r.oppgaver
    }
    ...
    func settMoteOppgaveStatus(id: String, ferdig: Bool,
                               projectId: String) async throws {
        struct Body: Encodable { let status: String; let projectId: String }
        let data = try JSONEncoder().encode(
            Body(status: ferdig ? "done" : "open", projectId: projectId))
        _ = try await _request("/api/leadgrid/oppgaver/\(id)",
                               method: "PATCH", body: data)
    }

    /// Mål & behov (ugated): hent/lagre per selskap.
    func hentMoteMaal(selskap: String, leadId: String? = nil,
                      projectId: String) async throws -> MoteMaalDTO {
        var comps = URLComponents(string: "/api/leadgrid/moter/maal")!
        comps.queryItems = [
            URLQueryItem(name: "selskap", value: selskap),
            URLQueryItem(name: "leadId", value: leadId),
            URLQueryItem(name: "projectId", value: projectId),
        ]
        return try await _get(comps.string ?? "/api/leadgrid/moter/maal")
    }

    func lagreMoteMaal(selskap: String, maal: String, behov: [String],
                       leadId: String? = nil, projectId: String) async throws {
        struct Body: Encodable {
            let selskap: String
            let maal: String
            let behov: [String]
    ...
    func oppdaterMote(leadId: String, tidspunkt: Date? = nil,
                      ...
                      note: String? = nil, projectId: String,
                      organizationId: String? = nil) async throws {
        struct Body: Encodable {
            let datetime: String?
            ...
            enum CodingKeys: String, CodingKey {
                case datetime, durationMinutes, meetingStatus, note, projectId
    ...
    func hentMoteBrief(selskap: String, orgnr: String? = nil,
                       ...
                       leadStatus: String? = nil, leadId: String? = nil,
                       projectId: String) async throws -> MoteBriefDTO {
        struct Body: Encodable {
            let selskap: String
            let orgnr: String?
            let kontakt: String?
