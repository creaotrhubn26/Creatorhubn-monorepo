...
struct MinProfilSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .task(id: appState.activeLeadgridProjectId) {
                dorsalgMeg = nil
                guard EntitlementStore.shared.erRenDorsalgOrg,
    ...
    private var statisticsGrid: some View {
        if EntitlementStore.shared.erRenDorsalgOrg {
            dorsalgStatisticsGrid
        } else {
            leadStatisticsGrid
        }
    }

    private var dorsalgStatisticsGrid: some View {
        let m = dorsalgMeg
        ...
        return LazyVGrid(columns: [GridItem(.adaptive(minimum: 150), spacing: 12)], spacing: 12) {
            metricCard(title: "Dører i dag", value: "\(m?.iDag ?? 0)", icon: "door.left.hand.open", color: LBrand.purpleLight)
            metricCard(title: "Denne uka", value: "\(m?.denneUka ?? 0)", icon: "calendar", color: LBrand.blue)
            metricCard(title: "Vunnet", value: "\(m?.vunnet ?? 0)", icon: "trophy.fill", color: LBrand.green)
            metricCard(title: "Hit-rate", value: hitRate, icon: "percent", color: LBrand.orange)
        }
        .accessibilityIdentifier("profile-dorsalg-stats")
    }

    private var leadStatisticsGrid: some View {
