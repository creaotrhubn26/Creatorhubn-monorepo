...
// MARK: - DropPinShape + GlowHalo (lokal kopi)
//
// LeadPinView.swift definerer disse internt — vi kopierer dem som
// file-private structs slik at OversiktView ikke trenger å eksponere
// dem på tvers av filer. Identisk geometri/farger som LeadPinView for
// pixel-paritet på kart-thumbnail (LeadsInAreaCard).

fileprivate struct DropPinShape: Shape {
...
fileprivate struct GlowHalo: View {
    let color: Color
    var body: some View {
        ZStack {
...
// MARK: - Placeholder-modeller for Momentum/Forecast
//
// Disse to typene representerer pipeline-helse-metrikker som vises i header
// + KPI-kort. Backend-endepunktene `/leadgrid/momentum/today` og
// `/leadgrid/forecasting/pipeline` finnes ennå ikke i prod (planlagt mig 327
// kom aldri inn på main). Inntil da kjører `refresh()` med statisk mock.
// Når backend er klar, bytt `refresh()` til å kalle de ekte API-metodene.

struct LeadgridMomentum: Sendable {
    let callsToday: Int
    let emailsToday: Int
...
struct LeadgridForecast: Sendable {
...
struct OversiktView: View {
    ...
    private func refresh() async {
        loading = true
        defer { loading = false }
        // TODO: Bytt ut når backend `/leadgrid/momentum/today` +
        // `/leadgrid/forecasting/pipeline` er klar (mig 327 ikke på main).
        let mom = LeadgridMomentum(callsToday: 14, emailsToday: 22,
                                   meetingsToday: 3, visitsToday: 7)
        let fc = LeadgridForecast(predictedRevenueMid: 1_240_000)
        await MainActor.run {
            self.momentum = mom
            self.forecast = fc
...
private struct MiniPin: View {
    ...
    var body: some View {
        ZStack {
            if isHot {
                GlowHalo(color: Brand.red)
                    .scaleEffect(0.6)
            } else if isWarm {
                GlowHalo(color: Brand.orange)
                    .scaleEffect(0.6)
            }
            ZStack {
                DropPinShape()
            ...
            .shadow(color: isHot ? Brand.red.opacity(0.65) : .black.opacity(0.4),
