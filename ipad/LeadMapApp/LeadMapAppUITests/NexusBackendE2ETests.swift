...
final class NexusBackendE2ETests: XCTestCase {
    ...
    func testNexusListerNotaterFraEkteBackend() throws {
        ...
        // Nexus ligger under «Flere funksjoner» i sidepanelet, og den
        // seksjonen er sammenklappet ved oppstart.
        let fane = app.buttons.matching(
            NSPredicate(format: "label == %@ OR label == %@", "Nexus", "Canvas")
        ).firstMatch
        if !fane.waitForExistence(timeout: 40) {
            let flere = app.buttons["Flere funksjoner"].firstMatch
            if flere.waitForExistence(timeout: 10) { flere.tap() }
                else { app.staticTexts["Flere funksjoner"].firstMatch.tap() }
        }
        XCTAssertTrue(fane.waitForExistence(timeout: 20), "Fant ikke Nexus-fanen i sidepanelet.")
        fane.tap()
