...
final class CanvasPersistenceTests: XCTestCase {
    func testRealtimeAuthCloseRequiresReauthentication() {
        XCTAssertTrue(CanvasRealtime.kreverNyInnlogging(
            closeCode: .policyViolation,
            closeReason: Data("auth_required".utf8)))
        XCTAssertTrue(CanvasRealtime.kreverNyInnlogging(
            closeCode: .policyViolation,
            closeReason: Data("session_revoked".utf8)))
        XCTAssertFalse(CanvasRealtime.kreverNyInnlogging(
            closeCode: .policyViolation,
            closeReason: Data("access_revoked".utf8)))
        XCTAssertFalse(CanvasRealtime.kreverNyInnlogging(
            closeCode: .goingAway,
            closeReason: Data("session_revoked".utf8)))
    }

    func testPairingAuthorityChangeRequiresFreshPairingCode() {
        let error = PairExchangeError(code: "pairing_authority_changed")
        XCTAssertTrue(error.requiresNewPairing)
        XCTAssertTrue(error.localizedDescription.contains("generer en ny kode"))
    }
    ...
    func testQueuedOrgASaveCannotSendAfterSwitchToOrgB() async {
        let coordinator = CanvasSaveCoordinator()
        let captured = CanvasTenantSnapshot(
            scope: "daniel@example.no|org-a",
            organizationId: "org-a")
        var currentScope: String? = "daniel@example.no|org-a"
        var sentOrganizations: [String] = []

        coordinator.enqueue(noteID: "note-a") {
            try? await Task.sleep(nanoseconds: 30_000_000)
        }
        coordinator.enqueue(noteID: "note-a") {
            if captured.canSend(currentScope: currentScope) {
                sentOrganizations.append(captured.organizationId)
            }
        }

        currentScope = "daniel@example.no|org-b"
        await coordinator.wait(for: "note-a")

        XCTAssertEqual(captured.organizationId, "org-a")
        XCTAssertTrue(sentOrganizations.isEmpty)
    }

    func testSlowOrgATaskResponseCannotReplaceFastOrgBResponse() async {
        var currentOrganizationId: String? = "org-a"
        var renderedOrganizationIds: [String] = []

        let slowOrgA = Task { @MainActor in
            try? await Task.sleep(nanoseconds: 30_000_000)
            if MoteOppgaveLoadScope.canApply(
                requestedOrganizationId: "org-a",
                currentOrganizationId: currentOrganizationId
            ) {
                renderedOrganizationIds.append("org-a")
            }
        }

        currentOrganizationId = "org-b"
        let fastOrgB = Task { @MainActor in
            if MoteOppgaveLoadScope.canApply(
                requestedOrganizationId: "org-b",
                currentOrganizationId: currentOrganizationId
            ) {
                renderedOrganizationIds.append("org-b")
            }
        }

        await fastOrgB.value
        await slowOrgA.value

        XCTAssertEqual(renderedOrganizationIds, ["org-b"])
    }
