...
final class AIStoryboardStudioUITests: XCTestCase {
    ...
    @MainActor
    func testAIStudioDemoShowsContextPromptInspectorAndCostGate() throws {
        XCUIDevice.shared.orientation = .landscapeLeft
        let app = XCUIApplication()
        // Debug-only deterministic harness: the workflow is rendered by the
        // real app target, but no provider call or model credit is possible.
        app.launchEnvironment["SB_AI_VIDEO_DEMO"] = "1"
        app.launch()

        XCTAssertTrue(app.navigationBars["AI Studio · Shot 3B"].waitForExistence(timeout: 15))
        XCTAssertTrue(app.staticTexts["Animer shot"].waitForExistence(timeout: 8))
        let preflightButton = app.buttons["storyboard.ai.video.preflight"]
        XCTAssertTrue(preflightButton.waitForExistence(timeout: 5),
                      "Betalt animasjon må gå via en eksplisitt forhåndssjekk")

        for _ in 0..<5 where !app.staticTexts["Regissørens intensjon"].exists {
            app.swipeUp()
        }
        ...
        XCTAssertTrue(app.staticTexts["Preflight valid"].waitForExistence(timeout: 8),
        ...
        let inspectorDone = app.navigationBars["Prompt Inspector"].buttons["Ferdig"]
        XCTAssertTrue(inspectorDone.waitForExistence(timeout: 5))
        inspectorDone.tap()
        ...
        let imageButton = app.buttons["storyboard.ai.image.generate"]
        for _ in 0..<8 where !imageButton.isHittable { app.swipeUp() }
        XCTAssertTrue(imageButton.waitForExistence(timeout: 5))
        XCTAssertTrue(imageButton.isHittable)
        XCTAssertLessThanOrEqual(
            imageButton.frame.maxX,
            app.frame.maxX + 1,
            "Regissør- og stilkontrollene flyter utenfor høyre skjermkant")

        for _ in 0..<6 where !preflightButton.isHittable { app.swipeDown() }
        XCTAssertTrue(app.staticTexts["Animer shot"].waitForExistence(timeout: 8))
        XCTAssertTrue(preflightButton.waitForExistence(timeout: 5))
        XCTAssertTrue(preflightButton.isHittable)
