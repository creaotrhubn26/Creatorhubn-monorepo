...
struct AIStoryboardStudioView: View {
    ...
    @State private var pendingVideoPreflight: StoryboardVideoPreflight?
    @State private var pendingVideoPrompt = ""
    @State private var showVideoConfirmation = false
    ...
    var body: some View {
            ...
            GeometryReader { proxy in
                ScrollView {
                    VStack(spacing: 22) {
                        ...
                        contextPanel
                        Group {
                            if proxy.size.width >= 1_500 {
                                let controlsWidth = min(420, proxy.size.width * 0.4)
                                let previewWidth = max(
                                    320,
                                    proxy.size.width - controlsWidth - 70)
                                HStack(alignment: .top, spacing: 22) {
                                    previewPanel.frame(width: previewWidth)
                                    controlsPanel.frame(width: controlsWidth)
                                }
                            } else {
                                VStack(spacing: 22) {
                                    previewPanel
        ...
        .confirmationDialog(
            "Start betalt animasjon?",
            isPresented: $showVideoConfirmation,
            titleVisibility: .visible
        ) {
            if let preflight = pendingVideoPreflight {
                Button(videoConfig?.billingMultiplier == 0
                       ? "Start animasjon · inkludert"
                       : String(format: "Start animasjon · maks $%.2f",
                                preflight.estimatedCostUSD
                                    * (videoConfig?.billingMultiplier ?? 1))) {
                    submitVideo(preflight)
                }
            }
            Button("Avbryt", role: .cancel) {
                pendingVideoPreflight = nil
                pendingVideoPrompt = ""
            }
        } message: {
            if let preflight = pendingVideoPreflight {
                Text("\(preflight.provider) · \(preflight.duration) sek. Kilde og prompt låses med fingerprint før jobben sendes.")
            }
        }
        #if DEBUG
        .onAppear {
    ...
    private var previewPanel: some View {
        ...
        .accessibilityIdentifier("storyboard.ai.preview.panel")
    }
    ...
    private var controlsPanel: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            .buttonStyle(.plain)
            .accessibilityIdentifier("storyboard.ai.image.generate")
    ...
    private var hasStoredSourceImage: Bool {
        isSimulatorDemo || frame?.imageUrl != nil
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if let videoConfig {
                if !videoConfig.enabled || !videoConfig.allowed {
                    ...
                    Button { preflightVideo() } label: {
                        HStack {
                            if isGeneratingVideo { ProgressView().tint(.white) }
                            Image(systemName: "wand.and.stars")
                            Text(isGeneratingVideo ? "Kontrollerer kilde og kostnad …"
                                 : selectedVideoModelID == "auto"
                                    ? "Forhåndssjekk billigste modell"
                                    : "Forhåndssjekk valgt modell")
                                .fontWeight(.bold)
                            Spacer()
                    ...
                    .buttonStyle(.plain)
                    .accessibilityIdentifier("storyboard.ai.video.preflight")
    ...
    private func preflightVideo() {
        guard let scene, let frame, let context = activeContext,
              let model = activeVideoModel, canGenerateVideo else { return }
        isGeneratingVideo = true
        errorMessage = nil
        successMessage = nil
        pendingVideoPreflight = nil
        pendingVideoPrompt = generationPrompt
        videoStatusMessage = "Kontrollerer kilde, prompt og pris hos \(model.label) …"
        let requestedDuration = videoDuration
        let prompt = generationPrompt
        Task {
            do {
                let preflight = try await RoleRoomAPIClient.shared.preflightStoryboardVideo(
                    projectId: projectId,
                    sceneId: scene.id,
                    frameId: frame.id,
                    ...
                    context: context)
                pendingVideoPreflight = preflight
                videoStatusMessage = videoConfig?.billingMultiplier == 0
                    ? "Forhåndssjekk bestått · inkludert"
                    : String(format: "Forhåndssjekk bestått · maks $%.2f",
                             preflight.estimatedCostUSD
                                * (videoConfig?.billingMultiplier ?? 1))
                showVideoConfirmation = true
            } catch is CancellationError {
                videoStatusMessage = "Forhåndssjekken ble avbrutt."
            } catch {
                errorMessage = error.localizedDescription
    ...
    private func submitVideo(_ preflight: StoryboardVideoPreflight) {
        guard let frame, let context = activeContext,
              !isGeneratingVideo,
              preflight.compilationFingerprint.count >= 8,
              preflight.sourceFingerprint.count >= 8 else { return }
        isGeneratingVideo = true
        errorMessage = nil
        successMessage = nil
        showVideoConfirmation = false
        let prompt = pendingVideoPrompt
        pendingVideoPreflight = nil
        videoStatusMessage = "Sender bekreftet jobb til \(preflight.provider) …"
        Task {
            do {
                var job = try await RoleRoomAPIClient.shared.submitStoryboardVideo(
                    projectId: projectId,
                    prompt: prompt,
                    context: context,
                    preflight: preflight)
                ...
                player.play()
                if videoConfig?.billingMultiplier == 0 {
                    videoStatusMessage = "Video lagret · \(preflight.duration) s · inkludert"
                } else {
                    let charge = preflight.estimatedCostUSD
                        * (videoConfig?.billingMultiplier ?? 1)
                    videoStatusMessage = "Video lagret · \(preflight.duration) s · belastning ca. $\(String(format: "%.2f", charge))"
                }
    ...
    private func openVideoNow(_ version: AIVideoVersion) async {
        ...
        do {
            guard let scene, let frame else {
                throw SyncError.malformed("aktivt shot")
            }
            let storyboardId = try await RoleRoomAPIClient.shared.storyboardRecordID(
                projectId: projectId,
                sceneId: scene.id,
                frameId: frame.id,
                title: "\(scene.heading) · Shot \(frame.shotNumber)")
    ...
    private func demoPayload() -> [String: Any] {
        ...
        return [
            ...
            "inspector": [
                ...
                "model": [
                    "id": selectedModelID,
                    "label": mode == "storyboard-image" ? "GPT Image 2" : "Seedance 2 Fast",
                    "provider": mode == "storyboard-image" ? "OpenAI" : "fal.ai / ByteDance",
                ],
            ],
        ]
    }
