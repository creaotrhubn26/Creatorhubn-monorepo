#!/usr/bin/env node
/**
 * Meta App Review screencast: Page Public Content Access.
 *
 * Records one video showing the /admin/page-public-content-app-review-demo
 * page being driven end-to-end:
 *   1. Producer pastes a public Norwegian production-company Page ID
 *   2. Click "Fetch public profile" → backend calls /v21.0/{page-id} with
 *      App Access Token, returns name/category/fan_count/about
 *   3. Click "Fetch public posts" → backend calls /v21.0/{page-id}/posts
 *      with engagement summaries
 *
 * Output: recordings/page-public-content-demo-<ts>.webm
 *
 * Env (read from backend/.env.page-public-content.demo.local if present):
 ...
 *   DEMO_PAGE_ID                 Norwegian production-company Page ID
 */
...
const ENV_FILE = path.resolve(REPO_ROOT, 'backend/.env.page-public-content.demo.local');
  ...
  await page.addStyleTag({
    content: `
      ...
      #ar-title {
        ...
        background: radial-gradient(circle at 50% 45%, rgba(7, 89, 133, 0.95), rgba(2, 6, 23, 0.98));
      ...
      #ar-title h2 { font-size: 22px; font-weight: 500; margin: 0 0 8px; color: #bae6fd; }
      #ar-title p  { font-size: 18px; font-weight: 400; max-width: 780px; color: #e0f2fe; margin: 24px 16px 0; line-height: 1.55; }
      .ar-spot {
        outline: 3px solid #0ea5e9 !important;
        outline-offset: 6px;
        border-radius: 12px !important;
        box-shadow: 0 0 0 8px rgba(14, 165, 233, 0.32) !important;
        animation: arPulse 1.8s ease-in-out infinite;
      }
      @keyframes arPulse {
        0%,100% { box-shadow: 0 0 0 8px rgba(14,165,233,0.32) !important; }
        50%     { box-shadow: 0 0 0 16px rgba(14,165,233,0.12) !important; }
      }
    `,
  });
}
...
async function runDemo(page, env) {
  let pageUrl = appendBypassToken(
    `${env.APP_BASE_URL}/admin/page-public-content-app-review-demo`,
  ...
  await showTitleCard(page, {
    ...
    title: 'Page Public Content Access — Discovery for Casting',
    body: 'Demonstrating end-to-end use of Graph API GET /v21.0/{page-id} + /v21.0/{page-id}/posts with the App Access Token. The Role Room surfaces public production-company posts to casting directors who want to track active calls and announcements.',
  });
  await beat(page, 4500);
  await hideTitleCard(page);
  await beat(page, 600);

  await showCaption(page, 'Step 1', 'Producer pastes a public Norwegian production-company Page ID');
  await spotlight(page, '[data-testid="page-id-input"]');
  await beat(page, 1200);
  await page.locator('[data-testid="page-id-input"]').fill(env.DEMO_PAGE_ID);
  await beat(page, 1600);

  await showCaption(page, 'Step 2', 'Backend calls GET /v21.0/{page-id} with App Access Token — public fields only');
  await spotlight(page, '[data-testid="fetch-profile-button"]');
  await beat(page, 1000);
  await page.locator('[data-testid="fetch-profile-button"]').click();
  await page.locator('[data-testid="profile-card"], [data-testid="profile-err"]').first().waitFor({ timeout: 20_000 });
  await spotlight(page, '[data-testid="profile-result"]');
  await beat(page, 3500);

  await showCaption(page, 'Step 3', 'Backend calls GET /v21.0/{page-id}/posts — reactions + comments are summary-counts only');
  await spotlight(page, '[data-testid="fetch-posts-button"]');
  await beat(page, 1000);
  await page.locator('[data-testid="fetch-posts-button"]').click();
  await page.locator('[data-testid="post-card"], [data-testid="posts-err"]').first().waitFor({ timeout: 20_000 });
  await spotlight(page, '[data-testid="posts-result"]');
  ...
  await showTitleCard(page, {
    subtitle: 'The Role Room · Meta App Review',
    title: 'Page Public Content Access — End of Demo',
    body: 'Read-only access to public Page fields and posts. No private data, no user-level read. App Access Token only. The Role Room surfaces these public posts to casting directors as discovery hints — never stores personal data.',
  });
  await beat(page, 3500);
}
...
async function main() {
  ...
  try {
    ...
    if (stats.length) {
      ...
      const stableName = `page-public-content-demo-${ts}.webm`;
