#!/usr/bin/env node
/**
 * Meta App Review screencast: Page Public Metadata Access.
 *
 * Records one video showing the /admin/page-public-metadata-app-review-demo
 * page being driven end-to-end:
 *   1. Producer pastes a production-team Page ID
 *   2. Click "Fetch metadata" → backend calls /v21.0/{page-id} with rich field-set
 *   3. Field-grid renders: name, category, founded, location, hours,
 *      contact, payment_options, parking, transit, ...
 *
 * Output: recordings/page-metadata-demo-<ts>.webm
 *
 * Env (read from backend/.env.page-metadata.demo.local if present):
 *   APP_BASE_URL                 default: https://creatorhub-backend-rtbl.onrender.com
 *   WHATSAPP_DEMO_BYPASS_TOKEN   bypass-token (same as other demos)
 *   DEMO_PAGE_ID                 Page ID to inspect
 */
...
const ENV_FILE = path.resolve(REPO_ROOT, 'backend/.env.page-metadata.demo.local');
...
async function installStyles(page) {
  await page.addStyleTag({
    content: `
      ...
      #ar-title p  { font-size: 18px; font-weight: 400; max-width: 780px; color: #dcfce7; margin: 24px 16px 0; line-height: 1.55; }
      .ar-spot {
...
async function runDemo(page, env) {
  ...
  await showTitleCard(page, {
    ...
    title: 'Page Public Metadata Access — Partner Discovery',
    body: 'Demonstrating end-to-end use of Graph API GET /v21.0/{page-id} with a richer field-set (founded, mission, location, hours, payment_options, ...). The Role Room surfaces these public-only fields so casting directors can filter the partner-discovery feed.',
  });
  await beat(page, 4500);
  await hideTitleCard(page);
  await beat(page, 600);

  await showCaption(page, 'Step 1', 'Producer pastes a production-team Page ID');
  await spotlight(page, '[data-testid="page-id-input"]');
  await beat(page, 1200);
  await page.locator('[data-testid="page-id-input"]').fill(env.DEMO_PAGE_ID);
  await beat(page, 1500);

  await showCaption(page, 'Step 2', 'Backend calls GET /v21.0/{page-id} with the richer field-set');
  await spotlight(page, '[data-testid="fetch-metadata-button"]');
  await beat(page, 1000);
  await page.locator('[data-testid="fetch-metadata-button"]').click();
  await page.locator('[data-testid="field-name"], [data-testid="metadata-err"]').first().waitFor({ timeout: 20_000 });
  await spotlight(page, '[data-testid="metadata-result"]');
  await beat(page, 4500);
  ...
  await showTitleCard(page, {
    subtitle: 'The Role Room · Meta App Review',
    title: 'Page Public Metadata Access — End of Demo',
    body: 'Read-only access to richer public metadata fields. All fields are explicitly public on Meta; no private data, no user-token impersonation, no write operations.',
  });
...
async function main() {
  ...
  try {
    ...
    if (stats.length) {
      ...
      const stableName = `page-metadata-demo-${ts}.webm`;
