...
const LEGACY_LEDGER_BASELINE_ID = "creatorhub-production-ledger-2026-09-10-v4";
...
async function runSelfTest() {
  ...
  const expectedLegacyAppliedFilenames = [
    ...
    "0506_role_room_storage_ownership_billing.sql",
  ];
  ...
  assert.deepEqual(
    ...
    "the versioned production baseline must contain exactly the 26 explicit ledger-only tombstones",
  );
  const productionInventory = await loadMigrationInventory(
    DEFAULT_MIGRATIONS_DIR,
  );
  assert.equal(productionInventory.legacyAppliedFilenames.size, 26);
  assert.deepEqual(
