#!/usr/bin/env node
/**
 * Meta App Review screencast: leads_retrieval.
 *
 * Records one video showing the /admin/leads-retrieval-app-review-demo
 * page being driven end-to-end:
 *   1. Producer enters Page-scoped token + Page ID
 *   2. Click "List lead-gen forms" → /{page-id}/leadgen_forms
 *   3. Click "Pull leads" on a form → /{form-id}/leads with field_data
 *
 * Output: recordings/leads-retrieval-demo-<ts>.webm
 *
 * Env (read from backend/.env.leads-retrieval.demo.local if present):
 *   APP_BASE_URL                 default: https://creatorhub-backend-rtbl.onrender.com
 *   WHATSAPP_DEMO_BYPASS_TOKEN   bypass-token
 *   DEMO_PAGE_ID                 Page ID
 *   DEMO_PAGE_ACCESS_TOKEN       Page-scoped token with leads_retrieval
 */
  ...
  await page.addStyleTag({
    content: `
      ...
      #ar-title {
        ...
        background: radial-gradient(circle at 50% 45%, rgba(22, 78, 99, 0.95), rgba(2, 6, 23, 0.98));
      ...
      .ar-spot {
        ...
        box-shadow: 0 0 0 8px rgba(6, 182, 212, 0.32) !important;
        animation: arPulse 1.8s ease-in-out infinite;
      }
      @keyframes arPulse {
        0%,100% { box-shadow: 0 0 0 8px rgba(6,182,212,0.32) !important; }
        50%     { box-shadow: 0 0 0 16px rgba(6,182,212,0.12) !important; }
      }
    `,
  });
}
...
async function runDemo(page, env) {
  ...
  await showTitleCard(page, {
    ...
    title: 'leads_retrieval — Casting-Call Lead Ingest',
    body: 'Demonstrating end-to-end use of /v21.0/{page-id}/leadgen_forms + /v21.0/{form-id}/leads. Lead Ads sign-ups for casting calls are ingested automatically into the casting pipeline — saving casting directors hours of manual Meta Business Suite exports.',
  });
  ...
  await showCaption(page, 'Step 2', 'Backend calls GET /v21.0/{page-id}/leadgen_forms — list all Lead Ads forms on the Page');
  await spotlight(page, '[data-testid="list-forms-button"]');
  await beat(page, 1000);
  await page.locator('[data-testid="list-forms-button"]').click();
  await page.locator('[data-testid="form-card"], [data-testid="forms-err"]').first().waitFor({ timeout: 25_000 });
  await spotlight(page, '[data-testid="forms-result"]');
  await beat(page, 4500);

  // Step 3: pull leads from the first form
  await showCaption(page, 'Step 3', 'Click "Pull leads" on a form → GET /v21.0/{form-id}/leads with field_data');
  await spotlight(page, '[data-testid="pull-leads-0"]');
  await beat(page, 1200);
  await page.locator('[data-testid="pull-leads-0"]').click().catch(() => {});
  await page.locator('[data-testid="lead-card"], [data-testid="leads-err"]').first().waitFor({ timeout: 25_000 }).catch(() => {});
  await spotlight(page, '[data-testid="leads-result"]');
  await beat(page, 4500);
...
async function main() {
  ...
  try {
    ...
    if (stats.length) {
      ...
      const stableName = `leads-retrieval-demo-${ts}.webm`;
