...
class BiRefNetMatte:
    """BiRefNet-forgrunns-matting. Injiser ``session`` i tester; bruk ``from_onnx``
    i produksjon."""

    def __init__(self, session: Any, input_name: str, size: int = 1024) -> None:
        self._session = session
        self._input_name = input_name
        self._size = int(size)

    @classmethod
    def from_onnx(cls, onnx_path: str, size: int = 1024) -> "BiRefNetMatte":
        ...
        return cls(sess, sess.get_inputs()[0].name, size)

    def _preprocess(self, image_bgr: np.ndarray) -> np.ndarray:
        rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
        r = cv2.resize(rgb, (self._size, self._size), interpolation=cv2.INTER_LINEAR)
    ...
    def matte(self, image_bgr: np.ndarray) -> np.ndarray:
        ...
        pred = self._to_2d(outs[-1] if isinstance(outs, (list, tuple)) else outs)
        # logits → sannsynlighet (BiRefNet-eksporter varierer: noen har alt sigmoid)
