...
// ============================================================================
// COUPLE ENDPOINTS — Projects, Dashboard, Vendors for logged-in couples
// ============================================================================

// GET /api/couples/projects — Get projects for the logged-in couple
app.get('/api/couples/projects', async (req, res) => {
  try {
    ...
    // Find projects where the couple is the client (by client_email)
    const result = await pool.query(`
...
// GET /api/couples/projects/:projectId — Get single project detail for couple
app.get('/api/couples/projects/:projectId', async (req, res) => {
  try {
    ...
    // Verify the couple has access (is the client)
    if (coupleEmail && r.client_email && r.client_email.toLowerCase() !== coupleEmail.toLowerCase()) {
      return res.status(403).json({ error: 'Ingen tilgang til dette prosjektet' });
    }
...
// GET /api/couples/dashboard — Dashboard data for logged-in couple
app.get('/api/couples/dashboard', async (req, res) => {
  try {
    ...
    // Get vendors linked to these projects (project.user_id -> users.id -> users.email -> vendors.email)
    const vendorUserIds = [...new Set(projectsResult.rows.map(p => p.user_id).filter(Boolean))];
    let vendors: any[] = [];
    if (vendorUserIds.length > 0) {
      const vendorResult = await pool.query(`
        ...
        WHERE u.id = ANY($1)
      `, [vendorUserIds]);
      vendors = vendorResult.rows.map(v => ({
        id: v.id,
        userId: v.user_id,
...
// GET /api/couples/vendors — Get vendors assigned to the couple's projects
  ...
  try {
    ...
    return {
      addresses: (data.adresser || []).map((a: any) => ({
        ...
        coordinates: { lat: a.representasjonspunkt?.lat || 0, lng: a.representasjonspunkt?.lon || 0 },
...
function calculateTravelInfo(from: { lat: number; lng: number }, to: { lat: number; lng: number }) {
  const R = 6371; // km
...
app.get('/api/evendi/weather-location/:coupleId', async (req, res) => {
  try {
    ...
    if (cpResult.rowCount === 0) return res.status(404).json({ error: 'Brudepar ikke funnet' });
    ...
    const timelineData = tlResult.rowCount ? (tlResult.rows[0].timeline_data || {}) : {};
        ...
        if (tlResult.rowCount) {
          await pool.query(
            `UPDATE wedding_timelines SET timeline_data = COALESCE(timeline_data, '{}')::jsonb || $1::jsonb, updated_at = NOW()
             WHERE id = $2`,
            [JSON.stringify({ venueCoordinates: venueCoords, venueMunicipality: addr.municipality, venueCounty: addr.county }),
             tlResult.rows[0]?.id || '']
          ).catch(() => {});
        }
      }
    ...
    let weather = null;
    try {
      const yrData = await fetchYrWeatherBridge(venueCoords.lat, venueCoords.lng);
      ...
      weather = {
        ...
        daily: timeseries.filter((_: any, i: number) => i % 6 === 0).slice(0, 7).map((t: any) => ({
          time: t.time,
          temperature: t.data?.instant?.details?.air_temperature,
          windSpeed: t.data?.instant?.details?.wind_speed,
      ...
      if (couple.wedding_date) {
        const weddingDateStr = new Date(couple.wedding_date).toISOString().split('T')[0];
        const weddingDayEntries = timeseries.filter((t: any) => t.time?.startsWith(weddingDateStr));
        if (weddingDayEntries.length > 0) {
          weather.weddingDayForecast = {
            ...
            avgTemperature: Math.round(weddingDayEntries.reduce((s: number, t: any) =>
              s + (t.data?.instant?.details?.air_temperature || 0), 0) / weddingDayEntries.length * 10) / 10,
            maxPrecipitation: Math.max(...weddingDayEntries.map((t: any) =>
          ...
          if (maxPrecip > 2) tips.push('☂️ Regn forventet — ha paraply og Plan B for uteseremoni');
          if (maxPrecip > 0 && maxPrecip <= 2) tips.push('🌦️ Noe nedbør mulig — vurder regntelt');
          if (avgTemp < 5) tips.push('🧥 Kaldt — sørg for varme sjal/kåper til gjestene');
          if (avgTemp > 25) tips.push('☀️ Varmt — server kalde drikker og ha skygge tilgjengelig');
          if (avgTemp >= 15 && avgTemp <= 25 && maxPrecip < 1) tips.push('✨ Perfekt vær for utendørs bryllup!');
        }
      }
    } catch (err) {
    ...
    const eventsWithWeather = schedEvents.rows.map((evt: any) => {
      if (!weather?.hourly?.length || !evt.time) return { ...evt, weather: null };
      ...
      const closest = weather.hourly.reduce((best: any, h: any) => {
        ...
        return Math.abs(hHour - eventHour) < Math.abs(new Date(best.time).getHours() - eventHour) ? h : best;
      }, weather.hourly[0]);
...
app.post('/api/evendi/weather-location/:coupleId/venue', async (req, res) => {
  try {
    ...
    if (!lat || !lng) {
      ...
      if (!query) return res.status(400).json({ error: 'Oppgi stedsnavn, adresse, eller koordinater' });

      const kartResult = await searchKartverketAddress(query);
      if (kartResult.addresses.length === 0) {
        return res.status(404).json({ error: 'Fant ingen adresser for søket', query });
      }
      return res.json({
    ...
    if (tl.rowCount) {
      await pool.query(
        `UPDATE wedding_timelines SET 
          ...
          updated_at = NOW()
         WHERE id = $3`,
        [venueName || address, JSON.stringify({
...
app.get('/api/evendi/weather-location/:coupleId/travel', async (req, res) => {
  try {
    ...
    const cp = await pool.query('SELECT wedding_venue FROM couple_profiles WHERE id = $1', [coupleId]);
    const tl = await pool.query(
    ...
    const venueCoords = timelineData.venueCoordinates || { lat: 59.9139, lng: 10.7522 };
    ...
    if (isNaN(fromLat) || isNaN(fromLng)) {
      if (fromCity) {
        ...
        return res.status(400).json({ error: 'Oppgi fromLat/fromLng eller fromCity' });
      }
    }
    ...
    res.json({
      ...
      venue: { name: cp.rows[0]?.wedding_venue || 'Bryllupssted', coordinates: venueCoords, weather: venueWeather },
    });
  } catch (error) {
...
app.get('/api/evendi/weather-location/:coupleId/event-weather', async (req, res) => {
  try {
    ...
    const dayEntries = timeseries.filter((t: any) => t.time?.startsWith(targetDate));

    // Match each event to its closest weather entry
    const eventsWithWeather = events.rows.map((evt: any) => {
      if (!evt.time || dayEntries.length === 0) return { ...evt, weather: null };
      ...
      const closest = dayEntries.reduce((best: any, entry: any) => {
        ...
        const bestMinutes = bestDate.getUTCHours() * 60 + bestDate.getUTCMinutes();
        return Math.abs(entryMinutes - eventMinutes) < Math.abs(bestMinutes - eventMinutes) ? entry : best;
      }, dayEntries[0]);
      ...
      return {
        ...
        weather: {
          ...
          symbol: closest.data?.next_1_hours?.summary?.symbol_code || closest.data?.next_6_hours?.summary?.symbol_code,
        ...
        weatherTip: (() => {
          ...
          if (precip > 2) return '☂️ Regn — flytt innendørs eller ha regntelt klart';
          if (wind > 10) return '💨 Sterk vind — sikre dekorasjoner og løse gjenstander';
          if (temp < 5) return '🧥 Kaldt — ha varmedrikker og pledd tilgjengelig';
          if (temp > 28) return '☀️ Veldig varmt — server kalde drikker og ha skygge';
    ...
    res.json({
      ...
      dailySummary: dayEntries.length > 0 ? {
        avgTemperature: Math.round(dayEntries.reduce((s: number, t: any) =>
          s + (t.data?.instant?.details?.air_temperature || 0), 0) / dayEntries.length * 10) / 10,
        maxPrecipitation: Math.max(0, ...dayEntries.map((t: any) =>
          t.data?.next_1_hours?.details?.precipitation_amount || 0)),
        maxWind: Math.max(0, ...dayEntries.map((t: any) =>
          t.data?.instant?.details?.wind_speed || 0)),
      } : null,
    });
  } catch (error) {
...
app.post('/api/evendi/weather-location/sync-from-project/:projectId', async (req, res) => {
  try {
    ...
    const proj = await pool.query('SELECT client_email FROM legacy.projects WHERE id = $1', [projectId]);
    if (proj.rowCount === 0) return res.status(404).json({ error: 'Prosjekt ikke funnet' });

    const cp = await pool.query('SELECT id FROM couple_profiles WHERE email = $1', [proj.rows[0].client_email]);
    if (cp.rowCount === 0) return res.status(404).json({ error: 'Ingen brudepar koblet til prosjektet' });

    const coupleId = cp.rows[0].id;

    // Update couple profile venue
    if (venueName) {
      await pool.query('UPDATE couple_profiles SET wedding_venue = $1, updated_at = NOW() WHERE id = $2',
        [venueName, coupleId]);
    }

    // Update timeline with location intelligence
    const tl = await pool.query('SELECT id FROM wedding_timelines WHERE project_id = $1 LIMIT 1', [projectId]);
    if (tl.rowCount) {
    ...
    console.log(`📍 Location bridge: synced project ${projectId.substring(0, 8)} → couple ${coupleId.substring(0, 8)}`);
    res.json({ synced: true, coupleId, projectId });
  } catch (error) {
    console.error('Location sync-from-project error:', error);
    res.status(500).json({ error: 'Kunne ikke synkronisere stedsinformasjon' });
  }
});
...
app.get('/api/evendi/conversations', async (req, res) => {
  try {
    ...
    if (!vendorResult.rows.length) {
      return res.status(404).json({ error: 'Ingen vendor-profil funnet for denne brukeren' });
    }
...
app.post('/api/evendi/conversations/:id/messages', async (req, res) => {
  try {
    ...
    const conversationId = req.params.id;
    const { body } = req.body;
    if (!body?.trim()) return res.status(400).json({ error: 'Meldingstekst er påkrevd' });
...
async function getVendorFromSession(req: any, res: any): Promise<{id: string, business_name: string} | null> {
  const token = req.headers.authorization?.replace('Bearer ', '');
  const session = token ? activeSessions.get(token) : null;
  if (!session) { res.status(401).json({ error: 'Ikke autentisert' }); return null; }
  const result = await pool.query(
    'SELECT id, business_name FROM vendors WHERE LOWER(email) = LOWER($1) LIMIT 1',
    [session.email]
  );
  if (!result.rows.length) { res.status(404).json({ error: 'Ingen vendor-profil funnet' }); return null; }
...
app.post('/api/evendi/offers', async (req, res) => {
  try {
    ...
    const { coupleId, conversationId, title, message, validUntil, items } = req.body;
    if (!coupleId || !title || !items?.length) {
      return res.status(400).json({ error: 'coupleId, title og minst 1 linje er påkrevd' });
    }
    ...
    const offerResult = await pool.query(`
      ...
      RETURNING *
    `, [vendor.id, coupleId, conversationId || null, title, message || null, totalAmount, validUntil || null]);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      `, [offer.id, item.productId || null, item.title, item.description || null, item.quantity, item.unitPrice, lineTotal, i]);
    }
    ...
    if (conversationId) {
      ...
      await pool.query(`
        INSERT INTO messages (conversation_id, sender_type, sender_id, body)
        VALUES ($1, 'vendor', $2, $3)
      `, [conversationId, vendor.id, `📋 Nytt tilbud sendt: "${title}" — ${formattedAmount} NOK`]);
      await pool.query(`
...
app.patch('/api/evendi/offers/:id', async (req, res) => {
  try {
    ...
    // Verify ownership
    const check = await pool.query('SELECT id FROM vendor_offers WHERE id = $1 AND vendor_id = $2', [offerId, vendor.id]);
    if (!check.rows.length) return res.status(403).json({ error: 'Ingen tilgang til dette tilbudet' });
    ...
    if (validUntil !== undefined) { updates.push(`valid_until = $${idx++}`); values.push(validUntil); }
    ...
    if (items?.length) {
      await pool.query('DELETE FROM vendor_offer_items WHERE offer_id = $1', [offerId]);
      let totalAmount = 0;
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        const lineTotal = item.quantity * item.unitPrice;
        ...
        await pool.query(`
          INSERT INTO vendor_offer_items (offer_id, product_id, title, description, quantity, unit_price, line_total, sort_order)
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        `, [offerId, item.productId || null, item.title, item.description || null, item.quantity, item.unitPrice, lineTotal, i]);
      }
      await pool.query('UPDATE vendor_offers SET total_amount = $1 WHERE id = $2', [totalAmount, offerId]);
    }
...
app.patch('/api/evendi/contracts/:id', async (req, res) => {
  try {
    ...
    const check = await pool.query('SELECT id FROM couple_vendor_contracts WHERE id = $1 AND vendor_id = $2', [contractId, vendor.id]);
    if (!check.rows.length) return res.status(403).json({ error: 'Ingen tilgang' });
...
// ============================================================
// Universal CRM API – /api/universal-crm/*
// Used by UniversalCRMDashboard.tsx on the frontend
// ============================================================

/**
 * GET /api/universal-crm/customers
 * List customers with optional filters: profession, search, status
 */
app.get('/api/universal-crm/customers', async (req, res) => {
  try {
    const { profession, search, status, limit = '100', offset = '0' } = req.query as Record<string, string>;
...
app.post('/api/universal-crm/customers', async (req, res) => {
  try {
    const {
      name, email, phone, company, profession, projectType,
      budget, status, tags, notes, source, customFields,
    } = req.body;
    ...
    const result = await pool.query(
      ...
      [
        ...
        profession || null, projectType || null,
        budget || null, status || 'lead',
        tags || null, notes || null, source || null,
...
app.get('/api/universal-crm/customers/:id', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM crm_customers WHERE id = $1', [req.params.id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Customer not found' });
    }
    const r = result.rows[0];
...
app.put('/api/universal-crm/customers/:id', async (req, res) => {
  try {
    ...
    const fieldMap: Record<string, string> = {
      ...
      profession: 'profession', projectType: 'project_type',
    ...
    for (const [jsKey, dbCol] of Object.entries(fieldMap)) {
      if (updates[jsKey] !== undefined) {
        ...
        params.push(dbCol === 'custom_fields' ? JSON.stringify(updates[jsKey]) : updates[jsKey]);
      }
    }
...
app.delete('/api/universal-crm/customers/:id', async (req, res) => {
  try {
    const result = await pool.query('DELETE FROM crm_customers WHERE id = $1 RETURNING id', [req.params.id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Customer not found' });
    }
...
app.get('/api/universal-crm/stats', async (req, res) => {
  try {
    ...
    byStatus.rows.forEach((r: any) => { statusMap[r.status || 'unknown'] = parseInt(r.count); });

    const professionMap: Record<string, number> = {};
    byProfession.rows.forEach((r: any) => { professionMap[r.profession || 'unknown'] = parseInt(r.count); });
...
app.get('/api/universal-crm/deals', async (req, res) => {
  try {
    const { customer_id, stage, limit = '50' } = req.query as Record<string, string>;
    let where = 'WHERE 1=1';
    const params: any[] = [];
    let idx = 1;
    if (customer_id) { where += ` AND customer_id = $${idx++}`; params.push(customer_id); }
    if (stage) { where += ` AND stage = $${idx++}`; params.push(stage); }
...
app.post('/api/universal-crm/deals', async (req, res) => {
  try {
    const { customerId, customer_id: cid, title, value, currency, stage, probability, expectedCloseDate, expected_close_date: ecd, assignedTo, assigned_to: ato, serviceType, service_type: st, notes } = req.body;
    ...
    const result = await pool.query(
      ...
      [customer_id || null, title, value || 0, currency || 'NOK', stage || 'prospecting', probability || 50, expected_close_date || null, assigned_to || null, service_type || null, notes || null]
    );
...
app.put('/api/universal-crm/deals/:id', async (req, res) => {
  try {
    const allowed = ['customer_id', 'title', 'value', 'currency', 'stage', 'probability', 'expected_close_date', 'assigned_to', 'service_type', 'notes'];
    ...
    const result = await pool.query(`UPDATE crm_deals SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING *`, params);
...
app.get('/api/universal-crm/activities', async (req, res) => {
  try {
    const { customer_id, deal_id, type, limit = '50' } = req.query as Record<string, string>;
    let where = 'WHERE 1=1';
    const params: any[] = [];
    let idx = 1;
    if (customer_id) { where += ` AND customer_id = $${idx++}`; params.push(customer_id); }
    if (deal_id) { where += ` AND deal_id = $${idx++}`; params.push(deal_id); }
...
app.post('/api/universal-crm/activities', async (req, res) => {
  try {
    const { customerId, customer_id: cid, dealId, deal_id: did, type, subject, description, scheduledAt, scheduled_at: sa, direction, outcome } = req.body;
    const customer_id = customerId || cid || null;
    ...
    const result = await pool.query(
      ...
      [customer_id || null, deal_id || null, type, subject, description || null, scheduled_at || null, direction || null, outcome || null]
    );
...
app.get('/api/universal-crm/tasks', async (req, res) => {
  try {
    const { customer_id, status, assigned_to, limit = '50' } = req.query as Record<string, string>;
    let where = 'WHERE 1=1';
    const params: any[] = [];
    let idx = 1;
    if (customer_id) { where += ` AND customer_id = $${idx++}`; params.push(customer_id); }
...
app.post('/api/universal-crm/tasks', async (req, res) => {
  try {
    const { customerId, customer_id: cid, dealId, deal_id: did, title, description, assignedTo, assigned_to: ato, priority, status, dueDate, due_date: dd } = req.body;
    const customer_id = customerId || cid || null;
    const deal_id = dealId || did || null;
    const assigned_to = assignedTo || ato || null;
    ...
    const result = await pool.query(
      `INSERT INTO crm_tasks (id, customer_id, deal_id, title, description, assigned_to, priority, status, due_date, created_at, updated_at)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8, now(), now()) RETURNING *`,
      [customer_id || null, deal_id || null, title, description || null, assigned_to || null, priority || 'medium', status || 'pending', due_date || null]
    );
...
app.put('/api/universal-crm/tasks/:id', async (req, res) => {
  try {
    const allowed = ['customer_id', 'deal_id', 'title', 'description', 'assigned_to', 'priority', 'status', 'due_date', 'completed_at'];
    const setClauses = ['updated_at = now()'];
    const params: any[] = [];
    let idx = 1;
    for (const key of allowed) {
    ...
    params.push(req.params.id);
    const result = await pool.query(`UPDATE crm_tasks SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING *`, params);
...
app.get('/api/universal-crm/pipeline-stages', async (req, res) => {
  try {
    const rows = await pool.query('SELECT * FROM crm_pipeline_stages WHERE is_active = true ORDER BY position ASC');
...
app.post('/api/universal-crm/pipeline-stages', async (req, res) => {
  try {
    ...
    const maxPos = await pool.query('SELECT COALESCE(max(position), 0) + 1 as next FROM crm_pipeline_stages');
    const result = await pool.query(
      ...
      [name, description || null, position || maxPos.rows[0].next, color || '#3B82F6']
    );
...
app.get('/api/universal-crm/email-templates', async (req, res) => {
  try {
    const rows = await pool.query('SELECT * FROM crm_email_templates WHERE is_active = true ORDER BY name ASC');
