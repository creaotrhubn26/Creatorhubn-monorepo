/**
 * Game billing routes — plan-katalog, abonnement, tester-invites,
 * admin-settings og Stripe checkout/webhook for spillstudio (Story Graph).
 *
 * Speiler dance-billing-routes.ts. Mountes under /api/game/billing.
 * Tillegg: GET /me → { subscription, plan, active } (effektiv plan, solo-fallback).
 */
...
import * as svc from './game-billing-service.js';
...
export interface CreateGameBillingRouterDeps {
  activeSessions?: Map<string, SessionData>;
}

export function createGameBillingRouter(
  pool: Pool,
  deps: CreateGameBillingRouterDeps = {},
): ExpressRouter {
  const router = Router();
  ...
  // Effektiv plan for innlogget bruker (abonnement → plan, ellers solo).
  router.get('/me', auth, async (req, res) => {
    const { userId } = req as AuthedRequest;
    res.json({ success: true, data: await svc.resolveEffectivePlan(pool, userId) });
  });
