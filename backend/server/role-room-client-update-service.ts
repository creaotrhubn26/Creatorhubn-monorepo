    ...
    .map(
      (h) => `
        <tr>
          ...
          <td style="padding:6px 0;color:#2a3d56;font-size:14px;font-weight:600">${escapeHtml(h.value)}</td>
        </tr>`,
    )
    .join('');

  const noteBlock = digest.producerNote
    ? `<div style="background:#f7f9ff;border-radius:8px;padding:14px 16px;margin:16px 0;color:#2b2553;font-size:14px;line-height:1.6">${escapeHtml(digest.producerNote)}</div>`
    : '';

  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;line-height:1.6;color:#2a3d56">
      <h2 style="color:#3e3180;margin:0 0 16px">${escapeHtml(digest.headline)}</h2>
      ...
      <p>
        <a href="${portalLink}"
           style="display:inline-block;background:linear-gradient(135deg,#3e3180,#32127a);color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:600">
