...
export function setupProToolsCompanionRoutes(deps: ProToolsCompanionDeps): void {
  ...
  // Browser og companion bruker nå samme rollebaserte AWS-objekttabell,
  // checksum-kontrakt og multipart/resume-motor.
  ...
  // POST /api/protools/sessions/:id/bounce/complete — verifiser objekt og opprett review-versjon.
  ...
  app.post("/api/protools/sessions/:id/bounce/complete", async (req, res) => {
    ...
    try {
      ...
      return res.status(error?.message?.includes("verification") || error?.message?.includes("mismatch") ? 422 : 503)
        .json({ error: error?.message || "bounce_verification_failed" });
    }
    ...
    try {
      ...
      if (reviewId) {
        ...
        const privateFileUrl = `/api/audio-versions/${versionId}/media`;
        const version = await client.query(
          ...
          [versionId, reviewId, strOrNull(req.body?.versionLabel, 80) || `Mix V${versionNumber}`, versionNumber, fileName || storageObject.display_name, privateFileUrl,
      ...
      const bounce = await client.query(
        `INSERT INTO protools_companion_bounces
           (session_id,file_name,file_url,storage_key,size_bytes,duration_seconds,review_version_id,client_event_id,content_fingerprint,
            storage_object_id,checksum_sha256)
         VALUES ($1::uuid,$2,$3,$4,$5,$6,$7,$8,$9,$10::uuid,$11) RETURNING id`,
        [sess.id, fileName || storageObject.display_name, versionId ? `/api/audio-versions/${versionId}/media` : null,
         storageObject.object_key, Number(storageObject.size_bytes), numOrNull(req.body?.durationSeconds), versionId,
         clientEventId, contentFingerprint, storageObject.id, storageObject.checksum_sha256],
      );
      await client.query(
