...
const CANVAS_SESSION_PATH = /^\/api\/leadgrid\/canvas(?:\/|$)/i;
...
export function leadgridWriteNeedsAuthoritativeSession(
  ...
  const isCanvasSessionRequest = CANVAS_SESSION_PATH.test(path);
  if (
    !WRITE_METHODS.has(method) &&
    !(READ_METHODS.has(method) && isCanvasSessionRequest)
  ) return false;
  ...
  if (isCanvasSessionRequest) return true;
...
export function createLeadgridAuthoritativeWriteMiddleware(input: {
  ...
  return async (req, res, next) => {
    ...
    const canvasRequest = CANVAS_SESSION_PATH.test(requestPath(req));
