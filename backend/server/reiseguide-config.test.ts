...
describe("reiseguide-config", () => {
  it("bruker SenseAid-spesifikke variabelnavn", () => {
    ...
    expect(SENSEAID_ENV.elevenLabsApiKey).toBe("SENSEAID_ELEVENLABS_API_KEY");
  });
  ...
  it("feiler med variabelnavnet når nøkkelen mangler, uten fallback til delte nøkler", () => {
    const env = { ELEVENLABS_API_KEY: "delt-nokkel", SENSEAID_SONIOX_API_KEY: "" };
    expect(() => requireSenseAidEnv(SENSEAID_ENV.elevenLabsApiKey, env)).toThrow(MissingSenseAidEnvError);
  ...
  it("rapporterer status uten å lekke verdier", () => {
    ...
    expect(status).toEqual({
      ...
      SENSEAID_ELEVENLABS_API_KEY: false,
