  ...
  app.get("/api/role-room/ig-business-basic/profile", async (req, res) => {
    ...
    try {
      const upstream = await fetch(
        `${META_GRAPH_BASE}/${encodeURIComponent(igUserId)}?${params.toString()}`,
      );
...
<style>
  *{box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}
  ...
  button{display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,#5d76cb,#ec4899);color:#fff;border:none;padding:14px 22px;border-radius:10px;font-weight:700;font-size:15px;cursor:pointer;margin-top:14px;transition:transform .12s}
  ...
  .badge{display:inline-block;background:linear-gradient(135deg,rgba(93, 118, 203,.2),rgba(236,72,153,.2));color:#c4b5fd;padding:4px 10px;border-radius:99px;font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}
  ...
  .profile-card img{width:84px;height:84px;border-radius:50%;object-fit:cover;border:2px solid #5d76cb;background:#1e293b}
  ...
  .acct-pill{display:inline-block;background:rgba(93, 118, 203,.18);color:#c4b5fd;padding:3px 10px;border-radius:99px;font-size:11px;font-weight:700;margin-top:8px}
</style></head>
<body>
...
function renderProfile(payload) {
  ...
  const img = p.profilePictureUrl
    ...
    : '<div style="width:84px;height:84px;border-radius:50%;background:#1e293b;border:2px solid #5d76cb"></div>';
