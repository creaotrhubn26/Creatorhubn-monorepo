/**
 * meeting-notes-routes.ts
 *
 * 7 endpoints for møtenotat-system (AI-prosessering + writing-assist
 * + CRUD + Google Docs-backup).
 *
 *   POST /api/meeting-notes/ai-process       — AI sammendrag/full
 *   POST /api/meeting-notes/writing-assist   — sanntid skrive-assist
 *   GET  /api/meeting-notes                  — list per bruker/prosjekt
 *   GET  /api/meeting-notes/:meetingId       — detalj
 *   POST /api/meeting-notes                  — opprett
 *   PUT  /api/meeting-notes/:meetingId       — oppdater
 *   POST /api/meeting-notes/google-backup    — sync til Drive + NotebookLM
 *
 * Tung dep-overflate fordi modulen kombinerer AI-orchestrering med
 * DB-persistens + Google Drive-sync + NotebookLM-integrasjon.
 */
...
export interface MeetingNotesRoutesDeps {
  ...
  resolveMeetingNotesProjectContext: (projectId?: string | null) => Promise<any>;
  ensureMeetingNotesCompatibilitySchema: () => Promise<void>;
  normalizeMeetingNotesPayload: (payload: any, creatorId: string) => Promise<any>;
  mapMeetingNotesRecord: (row: any) => any;
  buildLocalMeetingNotesAiResult: (args: any) => any;
  requestMeetingNotesAiResult: (args: any) => Promise<any>;
  buildLocalMeetingWritingAssist: (args: any) => any;
  requestMeetingWritingAssist: (args: any) => Promise<any>;
  syncMeetingNotesToCustomerDrive: (row: any, creatorId: string) => Promise<any>;
  syncNotebookLmWorkspaceForMeetingNote: (pool: Pool, row: any, creatorId: string) => Promise<any>;
  syncMeetingNotesLifecycleArtifacts: (...args: any[]) => Promise<any>;
}

export function setupMeetingNotesRoutes(deps: MeetingNotesRoutesDeps): void {
  const {
    app,
    pool,
    getUserIdFromAuth,
