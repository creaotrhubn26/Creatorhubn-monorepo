/**
 * Game team routes (spillstudio) — mountes under /api/game/teams og /api/game/invites.
 *
 * Auth-modell:
...
import * as svc from './game-team-service.js';
...
export interface CreateGameTeamRouterDeps {
  activeSessions?: Map<string, SessionData>;
}

export function createGameTeamRouter(
  pool: Pool,
  deps: CreateGameTeamRouterDeps = {},
): ExpressRouter {
  const router = Router();
...
// ─── Separat router for /api/game/invites/:token ────────────────────
//
...
export function createGameInviteAcceptRouter(
  pool: Pool,
  deps: CreateGameTeamRouterDeps = {},
): ExpressRouter {
  const router = Router();
  ...
  router.post('/:token/request-pin', async (req, res) => {
    ...
    const inviteUrl = `${_baseUrl}/game/invite/${encodeURIComponent(token)}`;
    try {
