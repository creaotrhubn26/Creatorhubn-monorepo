...
export function registerLeadgridEnturRoutes(deps: {
  ...
  // ────────────────────────────────────────────────────────────────
  // GET /geofencing?lat=&lon=  — scooter-slutt-soner (GBFS geofencing_zones)
  // klassifisert grønn/gul/rød for kart-overlay. To steg: finn operatører nær
  // punktet → hent deres soner.
  // ────────────────────────────────────────────────────────────────
  app.get("/api/leadgrid/entur/geofencing", async (req, res) => {
    const session = requireUserSession(req, res);
    if (!session) return;
    const lat = Number(req.query.lat);
    const lon = Number(req.query.lon);
    ...
    try {
      // 1) operatører (system-id-er) nær punktet
      const sysData = await graphql<{ vehicles?: any[] }>(MOBILITY, SYSTEMS_QUERY, { lat, lon });
      const ids = Array.from(
        new Set((sysData?.vehicles ?? []).map((v) => v?.system?.id).filter(Boolean)),
      ).slice(0, 8);
      if (ids.length === 0) {
        const body = { zones: [] };
        cacheSet(cacheKey, body, CACHE_TTL_VEHICLES);
        return res.json(body);
      }
      // 2) soner for de operatørene — men hvert system har tusenvis, så vi
      // filtrerer til soner nær punktet (~500 m bbox på geometrien).
      const zoneData = await graphql<{ geofencingZones?: any[] }>(MOBILITY, GEOFENCE_QUERY, { ids });
      const dLat = 500 / 111_000;
      const dLon = 500 / (111_000 * Math.max(0.2, Math.cos((lat * Math.PI) / 180)));
      const nearBbox = (geom: any): boolean => {
        ...
        while (stack.length) {
          ...
          if (Array.isArray(c) && typeof c[0] === "number" && typeof c[1] === "number") {
            if (Math.abs(c[1] - lat) <= dLat && Math.abs(c[0] - lon) <= dLon) return true;
          } else if (Array.isArray(c)) {
            for (const x of c) stack.push(x);
          }
        }
        return false;
      };
      const zones: any[] = [];
      for (const gz of zoneData?.geofencingZones ?? []) {
        let gj: any = gz?.geojson;
        if (typeof gj === "string") { try { gj = JSON.parse(gj); } catch { gj = null; } }
        for (const f of gj?.features ?? []) {
          if (!nearBbox(f?.geometry)) continue;
          const rules: any[] = f?.properties?.rules ?? [];
          const endAllowed = rules.every((r) => r?.rideEndAllowed !== false);
          const throughAllowed = rules.every((r) => r?.rideThroughAllowed !== false);
          const maxSpeed = rules.map((r) => Number(r?.maximumSpeedKph)).find((x) => Number.isFinite(x));
          let color = "green";                    // OK å parkere/avslutte
          if (!throughAllowed) color = "red";      // ikke lov å kjøre
          else if (!endAllowed) color = "red";     // ikke lov å avslutte (parkere)
          else if (Number.isFinite(maxSpeed)) color = "yellow"; // redusert fart
          zones.push({
            system: String(gz?.systemId ?? ""),
            name: String(f?.properties?.name ?? ""),
            color,
            max_speed_kph: Number.isFinite(maxSpeed) ? maxSpeed : null,
            geometry: f?.geometry ?? null,
          });
        }
      }
      const body = { zones: zones.slice(0, 60) };
      cacheSet(cacheKey, body, 5 * 60_000); // soner endres sjelden
      return res.json(body);
    } catch (err) {
      console.warn("[leadgrid-entur] geofencing failed:", (err as Error).message);
...
const SYSTEMS_QUERY = `
query Systems($lat: Float!, $lon: Float!) {
  vehicles(lat: $lat, lon: $lon, range: 700, count: 50) { system { id } }
}`;

const GEOFENCE_QUERY = `
query Geofence($ids: [ID!]) {
  geofencingZones(systemIds: $ids) {
    systemId
    geojson {
      features {
        ...
        properties {
          name
          rules { rideEndAllowed rideThroughAllowed maximumSpeedKph }
        }
      }
    }
  }
