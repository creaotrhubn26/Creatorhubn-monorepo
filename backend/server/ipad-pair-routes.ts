...
import { normalizeAuthSessionVersion } from "./auth-session-authority.js";
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        const lookup = pairToken
          ? await client.query<{ token: string; user_id: string; expires_at: string; used_at: string | null }>(
              `SELECT token, user_id, expires_at::text, used_at::text
        ...
        const userResult = await client.query<{
          ...
          `SELECT id::text, email::text, name::text,
