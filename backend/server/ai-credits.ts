...
export async function creditMove(pool: any, uid: string, type: string, amountUsd: number, ref: string | null, note: string): Promise<boolean> {
  ...
  if (amountUsd < 0) {
    ...
    if (!upd.rows.length) {
      // Utilstrekkelig saldo — rull tilbake ledger-oppføringen
      await pool.query(`DELETE FROM ai_credit_ledger WHERE id = $1`, [ledgerId]).catch(() => {});
      return false;
    }
  } else {
  ...
  await pool.query(`UPDATE ai_credit_ledger SET balance_after_usd = $1 WHERE id = $2`, [Number(upd.rows[0]?.balance_usd || 0), ledgerId]).catch(() => {});
  return true;
}
