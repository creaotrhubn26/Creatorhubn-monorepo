...
export function setupRoleRoomCallSheetRoutes({ app, pool, requireUserSession }: CallSheetRoutesDeps): void {
  app.get("/api/role-room/projects/:projectId/call-sheet-deliveries", async (req, res) => {
    ...
    res.json({ deliveries: rows.map((r) => ({ id: r.id, productionDayId: r.production_day_id, revision: r.revision, subject: r.subject, createdAt: r.created_at, total: r.total, sent: r.sent, failed: r.failed, acknowledged: r.acknowledged, recipients: recipientsByDelivery.get(r.id) ?? [] })) });
  });
  ...
  app.post("/api/role-room/call-sheet-deliveries/:deliveryId/remind", reminderLimit, async (req, res) => {
    ...
    try {
      ...
      const deliveryResult = await pool.query(`SELECT id,project_id,subject FROM role_room_call_sheet_deliveries WHERE id=$1 LIMIT 1`, [deliveryId]);
  ...
  app.get("/api/role-room/call-sheets/acknowledge/:token", receiptLimit, async (req, res) => {
    ...
    const { rows } = await pool.query(`WITH matched AS (SELECT r.id AS recipient_id,r.acknowledged_at,r.expires_at,d.subject FROM role_room_call_sheet_recipients r JOIN role_room_call_sheet_deliveries d ON d.id=r.delivery_id WHERE r.token_hash=$1 UNION ALL SELECT r.id AS recipient_id,r.acknowledged_at,t.expires_at,d.subject FROM role_room_call_sheet_recipient_tokens t JOIN role_room_call_sheet_recipients r ON r.id=t.recipient_id JOIN role_room_call_sheet_deliveries d ON d.id=r.delivery_id WHERE t.token_hash=$1 LIMIT 1) SELECT acknowledged_at,expires_at,subject FROM matched`, [hashToken(token)]);
  ...
  app.post("/api/role-room/call-sheets/acknowledge/:token", receiptLimit, async (req, res) => {
    const token = String(req.params.token || ""); if (!TOKEN_RE.test(token)) return res.status(404).type("html").send(receiptPage({ expired: true }));
    const { rows } = await pool.query(`WITH matched AS (SELECT r.id AS recipient_id,NULL::uuid AS token_id FROM role_room_call_sheet_recipients r WHERE r.token_hash=$1 AND r.expires_at>NOW() UNION ALL SELECT t.recipient_id,t.id AS token_id FROM role_room_call_sheet_recipient_tokens t WHERE t.token_hash=$1 AND t.expires_at>NOW() LIMIT 1),updated_recipient AS (UPDATE role_room_call_sheet_recipients r SET acknowledged_at=COALESCE(r.acknowledged_at,NOW()),updated_at=NOW() FROM matched m WHERE r.id=m.recipient_id RETURNING r.acknowledged_at),updated_token AS (UPDATE role_room_call_sheet_recipient_tokens t SET used_at=COALESCE(t.used_at,NOW()) FROM matched m WHERE t.id=m.token_id RETURNING t.id) SELECT acknowledged_at FROM updated_recipient`, [hashToken(token)]);
