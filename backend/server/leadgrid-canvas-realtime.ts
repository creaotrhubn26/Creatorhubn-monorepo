...
import {
  getCanvasAuthorization,
  isCanvasEntitledStrict,
} from "./leadgrid-canvas-authorization.js";
import {
...
export async function resolveCanvasRealtimeAccess(
  ...
  if (!(await isCanvasEntitledStrict(pool, userId))) return null;
  const organizationId = await resolveOrgIdForUser(pool, userId).catch(() => null);
...
export function createCanvasRealtimeServer(
  ...
  const resolveAccess = options.resolveAccess ?? ((userId: string, notatId: string) =>
    resolveCanvasRealtimeAccess(pool, userId, notatId));
  ...
  wss.on("connection", (ws, req) => {
    ...
    void (async () => {
      ...
      try {
        access = await resolveAccess(sesjon.userId, notatId);
      } catch {
  ...
  async function revalidate(klient: CanvasKlient): Promise<void> {
    ...
    try {
      ...
      const access = await resolveAccess(klient.userId, klient.notatId);
