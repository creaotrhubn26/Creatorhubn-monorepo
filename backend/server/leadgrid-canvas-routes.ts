...
export async function resolveCanvasRouteOrganization(
  ...
  try {
    // Resolve exactly the organization the persistence queries will use, then
    // re-prove current membership/status against that same id. This prevents a
    // caller-supplied org or the resolver's short cache from validating A while
    // the Canvas query reads/writes B.
...
export function registerLeadgridCanvasRoutes(deps: {
  ...
  app.get("/api/leadgrid/canvas", async (req, res) => {
    ...
    try {
      ...
      const orgId = await resolveCanvasRouteOrganization(pool, session.userId, false);
  ...
  app.post("/api/leadgrid/canvas", async (req, res) => {
    try {
      ...
      const orgId = await resolveCanvasRouteOrganization(pool, session.userId, true);
      await ensureSchema(pool);
