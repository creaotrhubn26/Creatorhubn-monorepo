...
const createPartnerSchema = z.object({
  ...
  referralCode: z.string().trim().min(3).max(80).regex(/^[A-Za-z0-9_-]+$/),
  commissionBasisPoints: z.number().int().min(0).max(10_000).default(1500),
  storageCommissionBasisPoints: z.number().int().min(0).max(10_000).default(500),
  commissionMonths: z.number().int().min(0).max(120).default(12),
  minimumPayoutMinor: z.number().int().min(0).max(100_000_000).default(DEFAULT_MINIMUM_PAYOUT_MINOR),
}).strict();
...
function publicOrigin(): string {
  const candidate = process.env.ROLE_ROOM_PUBLIC_URL?.trim() || DEFAULT_PUBLIC_ORIGIN;
  try {
...
function secureEqual(presented: unknown, expected: string | undefined): boolean {
...
function idFromExpandable(value: { id: string } | string | null | undefined): string | null {
...
function safeMinorAmount(value: string | number, label: string): number {
  ...
  if (!Number.isSafeInteger(parsed)) throw new Error(`${label}_outside_safe_integer_range`);
  return parsed;
}
...
export function readStripeInvoicePaymentReferences(invoice: Stripe.Invoice): {
  ...
  const stripeChargeId = idFromExpandable(directCharge)
    ?? (typeof paymentIntent === "object" ? idFromExpandable(paymentIntent.latest_charge) : null);
  return { stripePaymentIntentId, stripeChargeId };
}

function chargeReferences(charge: Stripe.Charge): { chargeId: string; paymentIntentId: string | null } {
  return {
    chargeId: charge.id,
...
function disputeReferences(dispute: Stripe.Dispute): { chargeId: string; paymentIntentId: string | null } {
  return {
...
function connectStatusFromAccount(account: Stripe.Account): "pending" | "restricted" | "complete" {
  const transferStatus = account.capabilities?.transfers;
  if (account.details_submitted && account.payouts_enabled && transferStatus === "active") {
    return "complete";
  }
...
async function findConnectAccountByPartnerMetadata(
  ...
  do {
    ...
    const match = page.data.find(
      (account) => account.metadata?.role_room_affiliate_partner_id === partnerId,
    );
...
async function readAffiliateBalance(queryable: Queryable, partnerId: string) {
  ...
  return {
    ...
    adjustmentMinor: safeMinorAmount(row?.adjustment_minor || 0, "affiliate_adjustment"),
    reservedOrTransferredMinor: safeMinorAmount(
      row?.reserved_or_transferred_minor || 0,
      "affiliate_allocated",
    ),
    availableMinor: safeMinorAmount(row?.available_minor || 0, "affiliate_available"),
    nextMaturityAt: row?.next_maturity_at?.toISOString?.() ?? row?.next_maturity_at ?? null,
  };
}

async function reconcileCommissionStatus(queryable: Queryable, commissionId: string): Promise<void> {
  await queryable.query(
    `WITH totals AS (
...
async function applyRefundAdjustments(
  ...
  const commissions = await commissionsForPayment(queryable, refs.chargeId, refs.paymentIntentId, true);
  for (const commission of commissions) {
    const original = safeMinorAmount(commission.commission_amount_minor, "affiliate_commission");
    const targetReversal = Math.min(
      original,
      Math.round(original * Math.min(charge.amount_refunded, charge.amount) / charge.amount),
    );
    ...
    const delta = targetReversal - safeMinorAmount(
...
async function applyChargebackAdjustment(
  ...
  for (const commission of commissions) {
    ...
    const currentNet = Math.max(0, safeMinorAmount(net.rows[0]?.net_minor || 0, "affiliate_net"));
    const sourcePaymentAmount = Math.max(
      1,
      safeMinorAmount(commission.source_payment_amount_minor || 0, "affiliate_source_payment"),
    );
    const proportionalTarget = Math.min(
      safeMinorAmount(commission.commission_amount_minor, "affiliate_commission"),
      Math.round(
        safeMinorAmount(commission.commission_amount_minor, "affiliate_commission")
    ...
    await queryable.query(
      ...
      [
        ...
        JSON.stringify({ stripeChargeId: refs.chargeId, stripePaymentIntentId: refs.paymentIntentId }),
      ],
    );
    await reconcileCommissionStatus(queryable, commission.id);
  }
...
async function applyChargebackRecovery(
  ...
  for (const commission of commissions) {
    ...
    const amount = safeMinorAmount(reversal.rows[0]?.amount_minor || 0, "affiliate_chargeback_recovery");
    if (amount <= 0) continue;
    await queryable.query(
      `INSERT INTO role_room_affiliate_commission_adjustments (
         commission_id, kind, source_object_id, stripe_event_id,
...
async function applyTransferState(queryable: Queryable, transfer: Stripe.Transfer): Promise<void> {
  ...
  for (const payout of result.rows) {
    ...
    for (const item of itemResult.rows) await reconcileCommissionStatus(queryable, item.commission_id);
  }
}
...
async function applyConnectedPayoutState(
  ...
  await queryable.query(
    ...
    [
      ...
      JSON.stringify({ automatic: payout.automatic, method: payout.method, type: payout.type }),
    ],
  );
  await queryable.query(
    `UPDATE role_room_affiliate_partners
...
async function eventMatchesAffiliate(pool: Pool, event: Stripe.Event): Promise<boolean> {
  ...
  if (event.type === "charge.refunded") {
    ...
    return (await commissionsForPayment(pool, refs.chargeId, refs.paymentIntentId)).length > 0;
  }
  if (
...
async function claimAffiliateEvent(pool: Pool, event: Stripe.Event): Promise<boolean> {
...
export async function handleRoleRoomAffiliateStripeEvent(
  ...
  if (!await claimAffiliateEvent(pool, event)) return { matched: true, duplicate: true };
  try {
    if (event.type === "account.updated") {
      ...
      const connectedAccountId = typeof event.account === "string" ? event.account : null;
      if (connectedAccountId) await applyConnectedPayoutState(pool, event.data.object, connectedAccountId);
      ...
      (event.type === "charge.dispute.closed" && event.data.object.status === "won")
      || event.type === "charge.dispute.funds_reinstated"
    ) {
      const client = await pool.connect();
      try {
    ...
    await pool.query(
      `UPDATE role_room_stripe_webhook_events
        ...
        WHERE stripe_event_id = $1`,
      [event.id, error instanceof Error ? error.message.slice(0, 1000) : "unknown_error"],
    ).catch(() => undefined);
    throw error;
  }
}
...
async function createMonthlyPayoutBatch(
  ...
  try {
    ...
    const ready = partner.status === "active"
    ...
    const minimumPayoutMinor = safeMinorAmount(partner.minimum_payout_minor, "affiliate_minimum_payout");
    ...
    for (const commission of commissionResult.rows) {
      const positiveAvailability = Math.max(
        0,
        safeMinorAmount(commission.available_minor, "affiliate_commission_available"),
      );
    ...
    if (remaining !== 0) throw new Error("affiliate_payout_allocation_mismatch");
    await client.query("COMMIT");
...
async function validateReservedPayout(pool: Pool, payout: PayoutRow): Promise<boolean> {
  const result = await pool.query<{
  ...
  const valid = result.rows.length > 0 && result.rows.every(
    (item) => safeMinorAmount(item.available_before_payout_minor, "affiliate_preflight_available")
  ...
  for (const item of released.rows) await reconcileCommissionStatus(pool, item.commission_id);
  return false;
}
...
async function sendPayoutTransfer(
  ...
  if (payout.status === "transferred" || payout.status === "reversed" || payout.status === "failed") {
    return { status: payout.status, transferId: payout.stripe_transfer_id || undefined };
  }
  await pool.query(
    `UPDATE role_room_affiliate_payouts
  ...
  try {
    ...
    if (payout.stripe_transfer_id) {
      ...
      transfer = existing.data.find(
        (candidate) => candidate.metadata?.role_room_affiliate_payout_id === payout.id,
      );
      if (!transfer) {
    ...
    return { status: transfer.reversed ? "reversed" : "transferred", transferId: transfer.id };
  } catch (error) {
...
export async function runRoleRoomAffiliatePayoutBatch(
  ...
  for (const partner of partners.rows) {
    const batch = await createMonthlyPayoutBatch(pool, partner.id, asOf, input.initiatedBy);
    if (!batch.payout) {
      results.push({ partnerId: partner.id, status: "skipped", reason: batch.reason });
      continue;
    }
    ...
    results.push({
      ...
      amountMinor: safeMinorAmount(batch.payout.amount_minor, "affiliate_payout"),
...
export function registerRoleRoomAffiliatePayoutRoutes({
  ...
  app.get("/api/role-room/storage/affiliate/connect/status", async (req, res) => {
    ...
    if (!parsed.success) return res.status(400).json({ error: "ugyldig_organization_id" });
    try {
      const access = await readRoleRoomOrganizationAccess(pool, parsed.data, session.userId);
      if (!access) return res.status(403).json({ error: "ingen_organisasjonstilgang" });
      if (!access.canAdminister) return res.status(403).json({ error: "kun_organisasjonsadmin" });
      let partner = await readConnectPartnerByOrganization(pool, parsed.data);
      if (!partner) return res.status(404).json({ error: "affiliate_partner_ikke_funnet" });
      ...
      return res.status(503).json({ error: "affiliate_payout_schema_unavailable" });
    }
  });

  app.post("/api/role-room/storage/affiliate/connect/onboarding", async (req, res) => {
    const session = getSession(req, activeSessions);
    if (!session) return res.status(401).json({ error: "krever_innlogging" });
    ...
    try {
      const access = await readRoleRoomOrganizationAccess(pool, parsed.data.organizationId, session.userId);
      if (!access) return res.status(403).json({ error: "ingen_organisasjonstilgang" });
      if (!access.canAdminister) return res.status(403).json({ error: "kun_organisasjonsadmin" });
      const partner = await readConnectPartnerByOrganization(pool, parsed.data.organizationId);
      if (!partner || partner.status !== "active") {
        return res.status(404).json({ error: "aktiv_affiliate_partner_ikke_funnet" });
      }
      let accountId = partner.stripe_connect_account_id;
      if (!accountId) {
        const account = await findConnectAccountByPartnerMetadata(stripe, partner.id)
          ?? await stripe.accounts.create({
            type: "express",
            country: partner.stripe_connect_country || "NO",
            default_currency: partner.payout_currency,
            email: partner.billing_email || partner.contact_email || session.email || undefined,
            business_type: "company",
            business_profile: {
              name: partner.organization_name,
              product_description: "Affiliate marketing services for The Role Room",
            },
      ...
      return res.status(502).json({ error: "stripe_connect_onboarding_failed" });
    }
  });
  ...
  app.post("/api/admin/role-room/affiliates/partners", async (req, res) => {
    ...
    try {
      ...
      const code = error && typeof error === "object" && "code" in error
        ...
        : "";
      if (code === "23505") return res.status(409).json({ error: "affiliate_partner_eller_kode_finnes" });
      if (code === "23503") return res.status(404).json({ error: "organisasjon_ikke_funnet" });
  ...
  const runBatch = async (initiatedBy: string, partnerId?: string) => {
    ...
    return runRoleRoomAffiliatePayoutBatch(pool, stripe, { partnerId, initiatedBy });
  };
  ...
  app.post("/api/admin/role-room/affiliates/payouts/run", async (req, res) => {
    ...
    if (partnerId && !partnerId.success) return res.status(400).json({ error: "ugyldig_partner_id" });
    try {
      const result = await runBatch(`admin:${admin.userId}`, partnerId?.data);
      if ("disabled" in result) return res.status(409).json({ error: "affiliate_payouts_deaktivert" });
      if ("stripeMissing" in result) return res.status(503).json({ error: "stripe_ikke_konfigurert" });
  ...
  app.post("/api/internal/role-room/affiliate-payouts/run", async (req, res) => {
    const secret = process.env.ROLE_ROOM_AFFILIATE_PAYOUT_CRON_SECRET;
    if (!secret) return res.status(503).json({ error: "affiliate_payout_cron_secret_mangler" });
