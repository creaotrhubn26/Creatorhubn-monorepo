...
/**
 * Post-commit delivery boundary. This module deliberately has no default email
 * or network implementation. The application may inject an approved adapter;
 * otherwise the personal link is returned only to the authorized manager.
 */
...
function configuredWorkspaceParticipantDocumentTrustedProxyHops(): number {
  const raw =
    process.env.WORKSPACE_PARTICIPANT_DOCUMENT_TRUST_PROXY_HOPS?.trim() ?? "";
...
export function resolveWorkspaceParticipantDocumentClientIp(
  req: Request,
): string {
  const socketAddress =
    normalizeWorkspaceParticipantDocumentIp(req.socket?.remoteAddress) ??
    "unknown";
  const trustedHops = configuredWorkspaceParticipantDocumentTrustedProxyHops();
  ...
  const forwarded = (
    ...
    .split(",")
    .map(normalizeWorkspaceParticipantDocumentIp)
    .filter((value): value is string => Boolean(value));
  const clientIndex = forwarded.length - trustedHops;
  return clientIndex >= 0 ? forwarded[clientIndex] : socketAddress;
}

function publicClientIp(req: Request): string | null {
  const value = resolveWorkspaceParticipantDocumentClientIp(req);
...
export function createWorkspaceParticipantDocumentPublicRateLimit(
  limit = 30,
): RequestHandler {
  return rateLimit({
    windowMs: 15 * 60_000,
    ...
    keyGenerator: (req) => {
      const ip = ipKeyGenerator(
        resolveWorkspaceParticipantDocumentClientIp(req),
      );
      const credential = documentTokenOf(req);
      const credentialDigest = crypto
        .createHash("sha256")
        .update(
          isWorkspaceParticipantDocumentToken(credential)
            ? credential
            : "missing",
          "utf8",
        )
      ...
      return crypto
        .createHash("sha256")
        .update(
          `${ip}:${String(req.params.documentId || "missing")}:${credentialDigest}`,
          "utf8",
        )
        .digest("hex");
    },
...
export function createWorkspaceParticipantDocumentIpRateLimit(
  limit = 120,
): RequestHandler {
  return rateLimit({
    windowMs: 15 * 60_000,
    ...
    keyGenerator: (req) =>
      crypto
        .createHash("sha256")
        .update(
          ipKeyGenerator(resolveWorkspaceParticipantDocumentClientIp(req)),
          "utf8",
        )
        .digest("hex"),
    handler: (_req, res) =>
...
function validatedTokenHash(req: Request): string {
  const token = documentTokenOf(req);
  if (!isWorkspaceParticipantDocumentToken(token)) {
...
async function deliver(
  ...
  try {
    return await deps.deliveryAdapter(request);
  } catch {
...
export function setupWorkspaceParticipantDocumentRoutes(
  ...
  const ipLimit = createWorkspaceParticipantDocumentIpRateLimit(120);
  const viewLimit = createWorkspaceParticipantDocumentPublicRateLimit(60);
  const mutationLimit = createWorkspaceParticipantDocumentPublicRateLimit(15);
      ...
      try {
        const issued = await runTransaction(async (client) => {
          ...
          if (!access.canManage) {
            throw new WorkspaceParticipantDocumentError(
              403,
              "participant_document_manage_denied",
              "Du kan se, men ikke administrere dokumenter.",
            );
          }
          await ensureScope(client, access, session.userId);
