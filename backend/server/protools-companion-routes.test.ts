...
vi.mock("./sound-room-storage-service.js", async () => {
  ...
  return { ...actual, initiateSoundRoomUpload: mocks.initiateUpload };
});
...
describe("Pro Tools Companion EaseVerse bridge", () => {
  beforeEach(() => {
    ...
    mocks.initiateUpload.mockResolvedValue({ objectId: "object-1", strategy: "multipart", uploadId: "upload-1", partSize: 16_777_216 });
  });
  ...
  it("sends Pro Tools bounces through the shared AWS checksum and multipart service", async () => {
    const pool = createPool();
    const app = express();
    app.use(express.json());
    setupProToolsCompanionRoutes({ app, pool, requireUserSession: vi.fn(() => null) });

    const checksum = "b".repeat(64);
    const response = await request(app)
      .post("/api/protools/sessions/session-1/bounce/presign")
      .set("authorization", "Bearer trr_desk_test")
      .send({ fileName: "Final Mix.wav", sizeBytes: 180_000_000, mimeType: "audio/wav", checksumSha256: checksum, clientEventId: "bounce-42" });

    expect(response.status).toBe(200);
    expect(response.body).toMatchObject({ objectId: "object-1", strategy: "multipart", storageKey: null, audioReviewProjectId: "review-1" });
    expect(mocks.initiateUpload).toHaveBeenCalledWith(pool, expect.objectContaining({
      userId: "user-1", projectId: "review-1", fileName: "Final_Mix.wav",
      sizeBytes: 180_000_000, checksumSha256: checksum, channel: "protools",
      sessionId: "session-1", clientEventId: "bounce-42",
    }));
  });
});
