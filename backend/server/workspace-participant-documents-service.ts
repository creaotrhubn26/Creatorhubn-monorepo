...
export function generateWorkspaceParticipantDocumentToken(
  ...
  const raw = (runtime?.randomBytes ?? crypto.randomBytes)(32).toString(
    "base64url",
  );
...
export async function issueWorkspaceParticipantDocument(
  ...
  const rawToken = generateWorkspaceParticipantDocumentToken(input.runtime);
...
export async function reissueWorkspaceParticipantDocumentToken(
  ...
  await db.query(
    ...
    [
      ...
      signer.id,
      input.actorUserId,
      JSON.stringify(compensationEventPayload(snapshot.compensation)),
      now.toISOString(),
    ],
  );
  const scope = {
  ...
  if (locked.document.status === "issued") {
    ...
    await db.query(
      `INSERT INTO workspace_participant_events
       ...
       VALUES ($1,$2,$3::uuid,$4::uuid,$5::uuid,'document_viewed','participant',$6::jsonb,$7)`,
      [
        ...eventScope(locked),
        JSON.stringify({
          ip: input.ip,
...
async function recordGuardianApprovalFromSignature(
  ...
  await db.query(
    `INSERT INTO workspace_participant_events
       (organization_id, project_id, participant_id, document_id, signer_id,
        event_type, actor_type, payload, occurred_at)
     VALUES ($1,$2,$3::uuid,$4::uuid,$5::uuid,'guardian_approval_recorded','system',$6::jsonb,$7)`,
    [
      ...eventScope(locked),
      JSON.stringify({
...
export async function signWorkspaceParticipantDocument(
  ...
  await db.query(
    `INSERT INTO workspace_participant_events
       (organization_id, project_id, participant_id, document_id, signer_id,
        event_type, actor_type, payload, occurred_at)
     VALUES ($1,$2,$3::uuid,$4::uuid,$5::uuid,'document_signed','participant',$6::jsonb,$7)`,
    [
      ...eventScope(locked),
      JSON.stringify({
        signatureMethod: input.signatureMethod,
...
export async function withdrawWorkspaceParticipantMediaConsent(
  ...
  await db.query(
    `INSERT INTO workspace_participant_events
       (organization_id, project_id, participant_id, document_id, signer_id,
        event_type, actor_type, payload, occurred_at)
     VALUES ($1,$2,$3::uuid,$4::uuid,$5::uuid,'media_consent_withdrawn','participant',$6::jsonb,$7)`,
    [
      ...eventScope(locked),
      JSON.stringify({
