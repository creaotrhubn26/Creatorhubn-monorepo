/**
 * Game team service — custom-rolle-system for spillstudioets team-stack.
 *
 * Modell:
 *   • Studio-eier oppretter et team. Team-id = Studio-eierens user_id.
 *   • Custom roller (game_team_role) defineres per studio med capability-sett.
 *   • Default 4 roller seedes når team først opprettes (Eier/Lærer/Koreograf/Danser).
 *   • Team-medlemmer er rader i enterprise_team_members med org_kind='game_studio'
 *     og game_role_id pekende på en custom rolle.
 *   • Invites går via game_team_invite (separat fra dance_tester_invite).
 *
 * Capability-katalog er definert nederst i denne filen — mirror i
 * frontend/client/src/components/role-room/game/gameTeamService.ts.
 */
...
export const CAPABILITIES = {
  ...
  story: [
    'story.edit',          // episoder, spørsmål, kilder, karakterer, lokasjoner, brett
    'scenes.edit',         // scenekort, replikker, gater, oppgaver
    'scenes.delete',
  ],
  review: [
    'review.request',
...
export const DEFAULT_ROLE_TEMPLATES: readonly DefaultRoleTemplate[] = [
  { label: 'Eier', isOwnerRole: true, isDefaultForInvite: false, displayOrder: 0, capabilities: ALL_CAPS_TRUE },
  {
    label: 'Produsent', isOwnerRole: false, isDefaultForInvite: false, displayOrder: 1,
    capabilities: capsOn('team.invite_members', 'billing.view', 'story.edit', 'scenes.edit', 'scenes.delete', 'review.request', 'review.decide', 'plan.edit', 'platform.edit', 'exports.use'),
  },
  {
    label: 'Narrativ designer', isOwnerRole: false, isDefaultForInvite: true, displayOrder: 2,
    capabilities: capsOn('story.edit', 'scenes.edit', 'review.request', 'exports.use'),
  },
  {
    label: 'Reviewer', isOwnerRole: false, isDefaultForInvite: false, displayOrder: 3,
    capabilities: capsOn('review.decide'),
  },
];
...
function mapRoleRow(row: Record<string, unknown>): GameTeamRole {
  return {
    id: String(row.id),
...
function mapMemberRow(row: Record<string, unknown>): GameTeamMember {
  const status = String(row.status ?? 'pending') as GameTeamMember['status'];
  return {
    ...
    gameRoleId: row.game_role_id ? String(row.game_role_id) : null,
    gameRoleLabel: row.game_role_label ? String(row.game_role_label) : null,
    invitedAt: isoTs(row.invited_at),
    joinedAt: isoTsOrNull(row.joined_at),
  };
}

function mapInviteRow(row: Record<string, unknown>): GameTeamInvite {
  return {
 ...
 * lander i spillstudio-flyten. Eier blir også registrert som medlem av eget team
 * (status='active', game_role_id = Eier-rolle).
 */
export async function ensureTeamForOwner(
  pool: Pool,
  ownerUserId: string,
  ownerEmail: string,
): Promise<GameTeamSummary> {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO enterprise_team_members
         (organization_id, user_id, email, role, status, joined_at, org_kind, game_role_id)
       VALUES ($1, $1, $2, 'admin', 'active', now(), 'game_studio', $3)
       ON CONFLICT (organization_id, email)
       DO UPDATE SET
         org_kind = 'game_studio',
         game_role_id = COALESCE(enterprise_team_members.game_role_id, EXCLUDED.game_role_id),
         status = 'active',
...
export async function getTeamSummary(
  ...
  const counts = await pool.query(
    `SELECT
     ...
     WHERE org_kind = 'game_studio' AND organization_id = $1`,
    [teamOrgId],
  );
...
export async function listMembershipsForUser(
  pool: Pool,
  userId: string,
): Promise<GameTeamMembershipDetail[]> {
  const r = await pool.query(
    `SELECT
       m.id, m.organization_id, m.user_id, m.email, m.status, m.role,
       m.game_role_id, m.invited_at, m.joined_at, m.upgrade_offer_seen_at,
       rl.label AS game_role_label
     FROM enterprise_team_members m
     LEFT JOIN game_team_role rl ON rl.id = m.game_role_id
     WHERE m.org_kind = 'game_studio'
       AND m.user_id = $1
  ...
  const out: GameTeamMembershipDetail[] = [];
  for (const row of r.rows) {
    const member = mapMemberRow(row);
    const team = await getTeamSummary(pool, member.teamOrganizationId);
    let role: GameTeamRole | null = null;
    if (member.gameRoleId) {
      const rr = await pool.query(`SELECT * FROM game_team_role WHERE id = $1`, [member.gameRoleId]);
...
export async function markUpgradeOfferSeen(
  ...
  const r = await pool.query(
    `UPDATE enterprise_team_members
      ...
      WHERE id = $1 AND user_id = $2 AND org_kind = 'game_studio'
      RETURNING id`,
    [memberRowId, userId],
  );
...
export async function getMembershipForUser(
  pool: Pool,
  userId: string,
): Promise<{ team: GameTeamSummary; member: GameTeamMember; role: GameTeamRole | null } | null> {
  const r = await pool.query(
    `SELECT
       m.id, m.organization_id, m.user_id, m.email, m.status, m.role,
       m.game_role_id, m.invited_at, m.joined_at,
       r.label AS game_role_label
     FROM enterprise_team_members m
     LEFT JOIN game_team_role r ON r.id = m.game_role_id
     WHERE m.org_kind = 'game_studio'
       AND m.user_id = $1
       AND m.status IN ('active','pending')
     ORDER BY m.joined_at DESC NULLS LAST, m.invited_at DESC
  ...
  if (member.gameRoleId) {
    const rr = await pool.query(
      `SELECT * FROM game_team_role WHERE id = $1`,
      [member.gameRoleId],
    );
    if (rr.rowCount) role = mapRoleRow(rr.rows[0]);
  }
...
export async function listRoles(pool: Pool, teamOrgId: string): Promise<GameTeamRole[]> {
  const r = await pool.query(
    `SELECT * FROM game_team_role
...
export async function getRole(pool: Pool, roleId: string): Promise<GameTeamRole | null> {
  const r = await pool.query(`SELECT * FROM game_team_role WHERE id = $1`, [roleId]);
...
export async function createRole(
  ...
  try {
    ...
    if (input.isDefaultForInvite) {
      await client.query(
        `UPDATE game_team_role SET is_default_for_invite = FALSE
          WHERE team_organization_id = $1 AND is_default_for_invite = TRUE`,
        [teamOrgId],
      );
    }
...
export async function updateRole(
  ...
  try {
    ...
    vals.push(roleId);
    const r = await client.query(
      `UPDATE game_team_role SET ${fields.join(', ')} WHERE id = $${i} RETURNING *`,
      vals,
    );
    await client.query('COMMIT');
    return mapRoleRow(r.rows[0]);
...
export async function deleteRole(pool: Pool, roleId: string): Promise<{ deleted: boolean; reason?: string }> {
  ...
  const inUse = await pool.query(
    `SELECT COUNT(*)::int AS n FROM enterprise_team_members
      WHERE game_role_id = $1 AND status IN ('active','pending')`,
    [roleId],
  );
  ...
  const pendingInvites = await pool.query(
    `SELECT COUNT(*)::int AS n FROM game_team_invite
  ...
  await pool.query(`DELETE FROM game_team_role WHERE id = $1`, [roleId]);
  return { deleted: true };
}
...
export async function listMembers(pool: Pool, teamOrgId: string): Promise<GameTeamMember[]> {
  const r = await pool.query(
    `SELECT m.*, r.label AS game_role_label
       FROM enterprise_team_members m
       LEFT JOIN game_team_role r ON r.id = m.game_role_id
      WHERE m.org_kind = 'game_studio' AND m.organization_id = $1
...
export async function updateMemberRole(
  ...
  const r = await pool.query(
    `UPDATE enterprise_team_members
        SET game_role_id = $1, updated_at = now()
      WHERE id = $2 AND organization_id = $3 AND org_kind = 'game_studio'
      RETURNING *`,
...
export async function removeMember(
  ...
  const r = await pool.query(
    `SELECT m.*, r.is_owner_role
       FROM enterprise_team_members m
       LEFT JOIN game_team_role r ON r.id = m.game_role_id
      WHERE m.id = $1 AND m.organization_id = $2 AND m.org_kind = 'game_studio'`,
...
export async function listInvites(pool: Pool, teamOrgId: string): Promise<GameTeamInvite[]> {
  const r = await pool.query(
    `SELECT i.*, r.label AS invited_role_label
       FROM game_team_invite i
       LEFT JOIN game_team_role r ON r.id = i.invited_role_id
...
export async function getInviteByToken(pool: Pool, token: string): Promise<GameTeamInvite | null> {
  const r = await pool.query(
    `SELECT i.*, r.label AS invited_role_label
       FROM game_team_invite i
       LEFT JOIN game_team_role r ON r.id = i.invited_role_id
...
): Promise<{ accepted: boolean; reason?: string; teamOrganizationId?: string; gameRoleId?: string }> {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    const inv = await client.query(
      `SELECT * FROM game_team_invite WHERE token = $1 FOR UPDATE`,
      [token],
    );
    if (!inv.rowCount) {
      await client.query('ROLLBACK');
    ...
    await client.query(
      `INSERT INTO enterprise_team_members
         (organization_id, user_id, email, role, status, joined_at, org_kind, game_role_id)
       VALUES ($1, $2, $3, 'member', 'active', now(), 'game_studio', $4)
       ON CONFLICT (organization_id, email)
       DO UPDATE SET
    ...
    await client.query('COMMIT');
    return { accepted: true, teamOrganizationId: teamOrgId, gameRoleId: roleId };
  } catch (err) {
    await client.query('ROLLBACK');
    throw err;
  } finally {
...
// ═══════════════════════════════════════════════════════════════════════
//  CAPABILITIES (resolver — brukes av useGamePlanGate)
// ═══════════════════════════════════════════════════════════════════════

/**
 * Resolver capabilities for en user via deres team-membership.
 * Returnerer tom Set hvis user ikke er i et game-team.
 */
export async function resolveUserCapabilities(
  ...
  const r = await pool.query(
    `SELECT r.capabilities, r.is_owner_role
       FROM enterprise_team_members m
       JOIN game_team_role r ON r.id = m.game_role_id
      WHERE m.org_kind = 'game_studio'
        AND m.user_id = $1
        AND m.status = 'active'
      LIMIT 1`,
...
// ═══════════════════════════════════════════════════════════════════════
//  SEATS — utleder fra game_subscription.plan.limits.maxTeamSeats
// ═══════════════════════════════════════════════════════════════════════
...
//
// Flyt:
//   1. Mottaker klikker /game/invite/<token>-lenken (magic link)
...
interface GameMailerHandle {
  user: string;
  transporter: Transporter;
}

let mailerCache: GameMailerHandle | null = null;
function getGameMailer(): GameMailerHandle | null {
...
export async function requestPinForInvite(
  ...
  const teamLabel = `spillstudio`;          // Vi har ikke separate team-navn — bruk generisk
...
export async function acceptInviteWithPin(
  ...
  try {
    ...
    if (!verifyInvitePin(input.token, input.pin, String(row.pin_hash))) {
      ...
      await client.query(
        `UPDATE game_team_invite SET pin_attempts = $1 WHERE token = $2`,
        [newAttempts, input.token],
      );
      await client.query('COMMIT');
    ...
    const teamOrgId = String(row.team_organization_id);
    const gameRoleId = String(row.invited_role_id);
    ...
    await client.query(
      `INSERT INTO enterprise_team_members
       ...
       DO UPDATE SET
         ...
         game_role_id = EXCLUDED.game_role_id,
         org_kind = 'game_studio'`,
      [teamOrgId, userId, email, gameRoleId],
    );
...
export async function getMaxTeamSeats(
  ...
  const r = await pool.query(
    `SELECT p.limits
       FROM game_subscription s
       JOIN game_plan p ON p.slug = s.plan_slug
      WHERE s.user_id = $1
  ...
  if (!r.rowCount) {
    // Uten abonnement (Solo): bare eieren selv.
    return 1;
  }
  const limits = asJson<Record<string, unknown>>(r.rows[0].limits, {});
  const raw = limits.seats;
...
/** Capabilities for en bruker i ett bestemt studio-team (eier = alle). */
export async function resolveUserCapabilitiesForTeam(
  pool: Pool,
  userId: string,
  teamOrgId: string,
): Promise<Set<string>> {
  if (userId === teamOrgId) return new Set(ALL_CAPABILITY_KEYS);
  const r = await pool.query(
    `SELECT r.capabilities, r.is_owner_role
       FROM enterprise_team_members m
       JOIN game_team_role r ON r.id = m.game_role_id
  ...
  if (!r.rowCount) return new Set();
  if (asBoolFlag(r.rows[0].is_owner_role)) return new Set(ALL_CAPABILITY_KEYS);
  const caps = asJson<Record<string, boolean>>(r.rows[0].capabilities, {});
  return new Set(Object.keys(caps).filter((k) => caps[k] === true));
}
