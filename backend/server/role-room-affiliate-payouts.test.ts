...
function buildRouteApp(pool: Pool, stripe: Stripe, membershipRole = "admin"): Express {
  const app = express();
...
describe("Role Room affiliate Connect onboarding", () => {
  ...
  it("creates one organization-owned Express account with only transfers requested", async () => {
    ...
    const response = await request(buildRouteApp({ query } as unknown as Pool, stripe))
      .post("/api/role-room/storage/affiliate/connect/onboarding")
      .set("Authorization", "Bearer token")
      .send({ organizationId: ORG_ID });

    expect(response.status).toBe(200);
    expect(response.body.onboardingUrl).toBe("https://connect.stripe.test/onboard");
    ...
    const createParams = vi.mocked(stripe.accounts.create).mock.calls[0][0] as Stripe.AccountCreateParams;
    ...
    expect(stripe.accountLinks.create).toHaveBeenCalledWith(expect.objectContaining({
      ...
      return_url: expect.stringContaining("https://theroleroom.com/role-room?affiliate_connect=return"),
    }));
    expect(statements.some(({ sql }) => sql.includes("stripe_connect_requirements"))).toBe(true);
  });
  ...
  it("does not expose onboarding to a non-admin organization member", async () => {
    ...
    const stripe = buildStripe();
    const response = await request(buildRouteApp({ query } as unknown as Pool, stripe, "member"))
      .post("/api/role-room/storage/affiliate/connect/onboarding")
      .set("Authorization", "Bearer token")
      .send({ organizationId: ORG_ID });
...
describe("Role Room affiliate refund and chargeback ledger", () => {
  it("writes the cumulative proportional refund once and marks the event processed", async () => {
    ...
    const query = vi.fn(async (sqlValue: unknown, params: unknown[] = []) => {
      const sql = String(sqlValue);
      statements.push({ sql, params });
      if (sql.includes("FROM role_room_affiliate_commissions") && sql.includes("stripe_charge_id")) {
    ...
    await expect(handleRoleRoomAffiliateStripeEvent(
      transactionalPool(query),
      event,
  ...
  it("reverses a chargeback and restores the same commission only when Stripe reports it won", async () => {
    ...
    const query = vi.fn(async (sqlValue: unknown, params: unknown[] = []) => {
      ...
      if (sql.includes("INSERT INTO role_room_stripe_webhook_events")) {
        eventClaimCount += 1;
        return { rows: [{ stripe_event_id: `evt_${eventClaimCount}` }], rowCount: 1 };
      }
      if (sql.includes("AS net_minor")) return { rows: [{ net_minor: "15000" }], rowCount: 1 };
      if (sql.includes("kind = 'chargeback'") && sql.includes("AS amount_minor")) {
...
describe("Role Room affiliate payout batches", () => {
  function buildBatchPool(options?: {
    ...
    const queryImpl = vi.fn(async (sqlValue: unknown, params: unknown[] = []) => {
      const sql = String(sqlValue);
      statements.push({ sql, params });
      if (sql.includes("SELECT id FROM role_room_affiliate_partners") && sql.includes("ORDER BY")) {
        return { rows: [{ id: PARTNER_ID }], rowCount: 1 };
      }
      if (sql.includes("stripe_connect_onboarding_status") && sql.includes("FOR UPDATE")) {
        return {
          rows: [{
            id: PARTNER_ID,
      ...
      if (sql.includes("available_minor") && sql.includes("FOR UPDATE OF commission")) {
        return options?.belowMinimum
          ? { rows: [{ id: COMMISSION_ID, available_minor: "99999" }], rowCount: 1 }
          : {
              rows: [
                { id: COMMISSION_ID, available_minor: "150000" },
                { id: "55555555-5555-4555-8555-555555555555", available_minor: "-20000" },
              ],
              rowCount: 2,
            };
      }
      ...
      if (sql.includes("available_before_payout_minor")) {
        return {
          rows: [{
            ...
            amount_minor: "130000",
            available_before_payout_minor: options?.ledgerChangedBeforeTransfer ? "0" : "150000",
          }],
          rowCount: 1,
        };
      }
      if (sql.includes("UPDATE role_room_affiliate_payouts") && sql.includes("RETURNING id")) {
        return { rows: [{ id: generatedPayoutId }], rowCount: 1 };
      }
      if (sql.includes("SELECT commission_id FROM role_room_affiliate_payout_items")) {
  ...
  it("nets negative carry-forward, reserves line items and transfers only the payable balance", async () => {
    ...
    const stripe = buildStripe();
    vi.mocked(stripe.transfers.create).mockImplementationOnce(async (params) => ({
      id: "tr_affiliate",
      object: "transfer",
    ...
    expect(result.results[0]).toMatchObject({ amountMinor: 130000, status: "transferred" });
    expect(stripe.transfers.create).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ idempotencyKey: expect.stringContaining("rr-affiliate-payout:") }),
    );
    const item = statements.find(({ sql }) => sql.includes("INSERT INTO role_room_affiliate_payout_items"));
  ...
  it("does not create a batch below the 1,000 NOK minimum", async () => {
    ...
    expect(result.results[0]).toMatchObject({ status: "skipped", reason: "below_minimum" });
    expect(stripe.transfers.create).not.toHaveBeenCalled();
  });

  it("cancels reserved line items if a refund changes the ledger before Stripe transfer", async () => {
    const { pool, statements } = buildBatchPool({ ledgerChangedBeforeTransfer: true });
    const stripe = buildStripe();
    const result = await runRoleRoomAffiliatePayoutBatch(pool, stripe, {
      asOf: new Date("2026-10-01T08:00:00.000Z"),
    ...
    expect(stripe.transfers.create).not.toHaveBeenCalled();
    expect(statements.some(({ sql }) =>
      sql.includes("SET status = 'released'") && sql.includes("role_room_affiliate_payout_items"),
    )).toBe(true);
  });
  ...
  it("keeps the cron endpoint fail-closed until both secret and payout flag are valid", async () => {
    process.env.ROLE_ROOM_AFFILIATE_PAYOUT_CRON_SECRET = "a-secure-monthly-cron-secret";
...
describe("Role Room connected payout webhooks", () => {
  it("keeps bank payout status separate from the internal transfer batch", async () => {
    ...
    const query = vi.fn(async (sqlValue: unknown, params: unknown[] = []) => {
      ...
      if (sql.includes("INSERT INTO role_room_stripe_webhook_events")) {
        return { rows: [{ stripe_event_id: "evt_payout_failed" }], rowCount: 1 };
      }
