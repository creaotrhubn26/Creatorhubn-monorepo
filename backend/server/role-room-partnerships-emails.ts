...
<html><body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 580px; margin: 0 auto; padding: 32px 20px; background: #0a0515; color: #f5f3ff;">
  <div style="background: #18122b; border: 1px solid rgba(136, 117, 235,0.18); border-radius: 16px; padding: 32px;">
    ...
    <p style="color: #8b7ec4; font-size: 12px; margin-top: 32px;">
      The Role Room · Creatorhub AS · <a href="https://theroleroom.com/privacy" style="color: #9e8cf8;">Personvern</a>
    </p>
  </div>
</body></html>`;
}

const PRIMARY_BTN = `display: inline-block; padding: 14px 28px; background: linear-gradient(135deg, #8875eb 0%, #6249df 100%); color: #fff; text-decoration: none; border-radius: 10px; font-weight: 700; font-size: 16px; margin: 12px 0;`;
...
export async function sendPartnershipProposed(
  ...
  const body = `
    ...
    ${
      args.message
        ? `<div style="margin: 20px 0; padding: 14px 18px; background: #18122b; border-left: 3px solid #8875eb; border-radius: 0 8px 8px 0;">
...
export async function sendProjectInvitationSent(
  ...
  const body = `
    ...
    <div style="margin: 16px 0; padding: 14px 16px; background: #18122b; border-radius: 8px;">
    ...
    ${
      args.notes
        ? `<div style="margin: 16px 0; padding: 14px 18px; background: #18122b; border-left: 3px solid #8875eb; border-radius: 0 8px 8px 0;">
