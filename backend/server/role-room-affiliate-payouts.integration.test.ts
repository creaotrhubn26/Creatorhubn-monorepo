...
describeWithDatabase("Role Room affiliate payout PostgreSQL integration", () => {
  ...
  it("records a full post-payout refund as negative carry-forward without mutating history", async () => {
    ...
    await expect(handleRoleRoomAffiliateStripeEvent(pool, event)).resolves.toEqual({ matched: true });
