...
function mergeReferences(
  ...
  for (const candidate of [
    ...submitted,
    ...authoritative.filter((value): value is ProductionReference =>
      Boolean(value),
    ),
  ]) {
...
function rowMatchesScene(
  ...
  if (
    assigned.some(
      (value) =>
        value === sceneId || normalizeKey(value) === normalizeKey(sceneId),
    )
  ) {
    return true;
  }
...
function referenceAssetMatchesScene(
  asset: StoryboardReferenceAsset,
  sceneId: string,
  identityKeys: ReadonlySet<string>,
): boolean {
  if (asset.sceneIds.includes(sceneId)) return true;
  return (
    identityKeys.has(normalizeKey(asset.entityId)) ||
    identityKeys.has(normalizeKey(asset.name))
  );
}

async function approvedProjectReferences(
  pool: Pool,
  projectId: string,
): Promise<StoryboardReferenceAsset[]> {
  try {
    return await listApprovedStoryboardReferenceAssets(pool, projectId);
  } catch (error) {
    // Zero-downtime deploy safety: a new server may briefly run before the
    // migration job. Existing Prompt Engine behavior remains available.
...
export async function hydrateStoryboardProductionContext(
  ...
  const [
    ...
    approvedProjectReferences(pool, input.projectId),
  ]);

  const sceneRow = record(sceneResult.rows[0]);
  const breakdown = record(
    sceneRow.production_breakdown ?? sceneRow.productionBreakdown,
  );
  const sceneCharacters = strings(sceneRow.characters);
  const submittedCharacterKeys = new Set(
    input.context.scene.characters.map(normalizeKey),
  );
  const sceneCharacterKeys = new Set(
    [...sceneCharacters, ...input.context.scene.characters].map(normalizeKey),
  );
  const candidates = candidatesResult.rows.map(record);
  const approvedCharacterReferences = approvedReferences
    .filter(
      (asset) =>
        asset.entityType === "character" &&
        referenceAssetMatchesScene(asset, input.sceneId, sceneCharacterKeys),
    )
    .map((asset) =>
      reference({
        id: asset.entityId || asset.id,
        name: asset.name,
        description: asset.description,
        images: [libraryReferenceId(asset.id)],
        locked: asset.locked,
      }),
    );

  const characterReferences = [
    ...rolesResult.rows.map(record).flatMap((role) => {
      const roleId = clean(role.id, 200);
      const roleName = clean(role.name, 300);
      const matches =
        sceneCharacterKeys.has(normalizeKey(roleId)) ||
      ...
      const candidate =
        ...
        candidates.find((item) =>
          strings(item.assigned_roles ?? item.assignedRoles).some(
            (id) => id === roleId,
          ),
        );
  ...
  const wardrobeReferences = [
    ...costumeBreakdown.map((costume, index) =>
      reference({
        ...
        images: [
          costume.images,
          costume.referenceImageIds,
          costume.reference_image_ids,
        ],
      }),
    ),
    ...costumeProps.map((prop) =>
      reference({
        id: prop.id,
        name: prop.name,
  ...
  const locationReferences = [
    ...breakdownLocations.map((location) => {
      const catalog = locationsResult.rows
        .map(record)
        .find(
          (candidate) =>
      ...
      return reference({
        ...
        description: [
          namedDescription(location),
          catalog ? namedDescription(catalog) : "",
        ]
          .filter(Boolean)
          .join(" · "),
        images: [location.photos, location.images, catalog?.photos],
      });
    }),
    ...
    ...approvedReferences
      .filter(
        (asset) =>
          asset.entityType === "location" &&
          referenceAssetMatchesScene(asset, input.sceneId, locationKeys),
      )
      .map((asset) =>
        reference({
          id: asset.entityId || asset.id,
    ...
    ...approvedReferences
      .filter(
        (asset) =>
          asset.entityType === "storyboard" &&
          (asset.entityId === input.sceneId ||
            asset.sceneIds.includes(input.sceneId)),
      )
      .map((asset) =>
        reference({
          id: asset.entityId || asset.id,
          name: "Visual continuity — " + asset.name,
          description: asset.description,
          images: [libraryReferenceId(asset.id)],
          locked: asset.locked,
        }),
  ...
  const breakdownPropKeys = new Set(
    breakdownProps
      .flatMap((prop) => [normalizeKey(prop.id), normalizeKey(prop.name)])
      .filter(Boolean),
  );
  const matchedProps = propsResult.rows
    .map(record)
    .filter(
      (prop) =>
        ...
        rowMatchesScene(
          prop,
          input.sceneId,
          input.context.scene.number ?? null,
        ),
    );
  const propReferences = [
    ...breakdownProps.map((prop) => {
      const catalog = propsResult.rows
        .map(record)
        .find(
          (candidate) =>
      ...
      return reference({
        ...
        description: [
          namedDescription(prop),
          catalog ? namedDescription(catalog) : "",
        ]
          .filter(Boolean)
          .join(" · "),
        images: [prop.images, catalog?.images],
      });
    }),
    ...
    ...approvedReferences
      .filter(
        (asset) =>
          asset.entityType === "prop" &&
          referenceAssetMatchesScene(asset, input.sceneId, breakdownPropKeys),
      )
      .map((asset) =>
        reference({
          id: asset.entityId || asset.id,
  ...
  return {
    ...input.context,
    production: {
      characters: mergeReferences(
        characterReferences,
        input.context.production.characters,
      ),
      wardrobe: mergeReferences(
        wardrobeReferences,
        input.context.production.wardrobe,
      ),
      locations: mergeReferences(
        locationReferences,
        input.context.production.locations,
      ),
    ...
    scene: {
      ...
      number:
        typeof sceneRow.scene_number === "number"
          ? sceneRow.scene_number
          : input.context.scene.number,
      ...
      timeOfDay:
        clean(sceneRow.time_of_day, 100) || input.context.scene.timeOfDay,
      action: clean(sceneRow.description, 4_000) || input.context.scene.action,
      characters: authoritativeCharacterNames.length
        ? authoritativeCharacterNames
        : input.context.scene.characters,
    },
  };
}
