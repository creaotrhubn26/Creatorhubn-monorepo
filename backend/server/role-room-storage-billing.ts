...
const checkoutSchema = z.object({
  ...
  (input) => input.addOns.extra100Gib + input.addOns.extra1Tib > 0,
  { message: "minst_ett_lagringstillegg_pakrevd", path: ["addOns"] },
);
...
function invoicePaymentReferences(invoice: Stripe.Invoice): {
  ...
  return {
    ...
    stripeChargeId: idFromExpandable(payment.payment.charge)
...
function commissionMaturesAt(paidAt: Date): Date {
  return new Date(paidAt.getTime() + AFFILIATE_COMMISSION_HOLD_DAYS * 86_400_000);
}
...
/**
 * Personal storage account used by workspace modules that are owned directly
 * by a CreatorHub user (Sound Room and desktop companions).  It intentionally
 * uses the same account/object/usage-ledger tables as organization uploads so
 * browser and native clients cannot drift into separate storage models.
 */
...
export async function syncRoleRoomCommercialStorageEntitlement(
  ...
  const planKey = input.active
    ? input.persona === "production_team" ? "agency" : "solo_pro"
    : "solo_free";
...
export async function accrueRoleRoomCommercialAffiliateCommission(
  ...
  const organizationNumber = input.organizationNumber.replace(/\D/g, "");
  if (!/^\d{9}$/.test(organizationNumber) || !input.stripeInvoiceId) return false;
  ...
  const commissionEndsAt = referral.commission_ends_at
    ?? new Date(paidAt.getTime() + referral.commission_months * 30.4375 * 86_400_000);
  const bonusEndsAt = new Date(
    paidAt.getTime() + referral.referred_org_bonus_months * 30.4375 * 86_400_000,
  );
  await pool.query(
  ...
  const subtotal = input.subtotalExcludingTaxMinor == null
    ? amountPaid
...
function publicOrigin(): string {
  ...
  try {
    ...
    if (parsed.protocol !== "https:" && !localDevelopment) return DEFAULT_PUBLIC_ORIGIN;
...
function billingPortalConfigurationId(): string | null {
  return process.env.ROLE_ROOM_STORAGE_BILLING_PORTAL_CONFIGURATION_ID?.trim() || null;
}
...
async function expirePaymentGrace(pool: Pool, storageAccountId: string): Promise<void> {
  await pool.query(
...
export async function readRoleRoomStorageContext(pool: Pool, storageAccountId: string) {
  ...
  return {
    ...
    availableBytes: Math.max(0, quotaBytes - usedBytes - Number(row.reserved_bytes)),
    fileCount: row.file_count,
    percentageUsed: quotaBytes > 0 ? Math.min(100, Math.round((usedBytes / quotaBytes) * 100)) : 0,
    status: row.status,
    billing: {
...
async function ensureStripeCustomer(
  ...
  const customer = await stripe.customers.create({
    ...
    email: organization.billingEmail || organization.contactEmail || session.email || undefined,
    metadata: {
...
async function syncSubscription(
  ...
  const accountStatus = addOnEntitlementsActive || withinBaseQuota
    ? "active" as const
    : "read_only" as const;
  ...
  for (const addOn of addOns) {
    ...
    if (quantity > 0) {
      await pool.query(
        ...
        [account.id, addOn.ref, addOn.bytes * quantity, JSON.stringify({ subscriptionId: subscription.id, quantity })],
      );
    } else {
      await pool.query(
...
async function accrueAffiliateCommission(
  ...
  const commissionEndsAt = referral.commission_ends_at
    ?? new Date(firstPaidAt.getTime() + referral.commission_months * 30.4375 * 86_400_000);
  const bonusEndsAt = new Date(firstPaidAt.getTime() + referral.referred_org_bonus_months * 30.4375 * 86_400_000);
  await pool.query(
    `UPDATE role_room_affiliate_referrals
...
async function eventMatchesStorage(pool: Pool, event: Stripe.Event): Promise<boolean> {
  if (
  ...
  if (event.type === "invoice.paid" || event.type === "invoice.payment_failed") {
    ...
    if (invoice.parent?.subscription_details?.metadata?.product_family === PRODUCT_FAMILY) return true;
...
async function claimWebhookEvent(pool: Pool, event: Stripe.Event): Promise<boolean> {
...
export async function handleRoleRoomStorageStripeEvent(
  ...
  if (!await claimWebhookEvent(pool, event)) return { matched: true, duplicate: true };
  try {
    if (
      event.type === "checkout.session.completed"
      || event.type === "checkout.session.async_payment_succeeded"
      ...
      if (!subscriptionId) throw new Error("storage_checkout_missing_subscription");
      ...
      const subscriptionId = subscriptionIdFromInvoice(invoice);
      if (!subscriptionId) throw new Error("storage_invoice_missing_subscription");
      const subscription = await stripe.subscriptions.retrieve(subscriptionId);
      const storageAccountId = await syncSubscription(pool, subscription);
      const organizationId = subscription.metadata.organization_id;
      if (organizationId) {
        await accrueAffiliateCommission(pool, organizationId, invoice, storageAccountId);
      }
...
export function registerRoleRoomStorageBillingRoutes({
  ...
  app.get("/api/role-room/storage/billing/context", async (req: Request, res: Response) => {
    ...
    try {
      const organization = await readRoleRoomOrganizationAccess(pool, parsed.data, session.userId);
      ...
      return res.json({
        ...
        checkoutAvailability: {
          ...
          extra1Tib: Boolean(catalog.addOns.extra1Tib.priceId) && oneTibCheckoutEnabled(),
      ...
      return res.status(503).json({ error: "storage_billing_schema_unavailable" });
    }
  });

  app.post("/api/role-room/storage/billing/checkout", async (req: Request, res: Response) => {
    const session = getSession(req, activeSessions);
    if (!session) return res.status(401).json({ error: "krever_innlogging" });
    ...
    try {
      ...
      if (!organization) return res.status(403).json({ error: "ingen_organisasjonstilgang" });
      if (!organization.canAdminister) return res.status(403).json({ error: "kun_organisasjonsadmin" });
      if (!stripe) return res.status(503).json({ error: "stripe_ikke_konfigurert" });
      const catalog = readRoleRoomStoragePriceCatalog();
      if (input.addOns.extra100Gib > 0 && !catalog.addOns.extra100Gib.priceId) {
        return res.status(503).json({ error: "pris_for_100_gib_ikke_konfigurert" });
      }
      if (input.addOns.extra1Tib > 0 && !catalog.addOns.extra1Tib.priceId) {
        return res.status(503).json({ error: "pris_for_1_tib_ikke_konfigurert" });
      }
      if (input.addOns.extra1Tib > 0 && !oneTibCheckoutEnabled()) {
        return res.status(409).json({ error: "1_tib_tillegg_ikke_aktivert" });
      }
      const account = await ensureRoleRoomOrganizationStorageAccount(pool, organization);
      if (account.stripe_subscription_id && !["canceled", "incomplete_expired"].includes(account.stripe_subscription_status || "")) {
      ...
      if (lock.rows.length === 0) {
        return res.status(409).json({ error: "checkout_pagar", action: "vent_eller_apne_portal" });
      }
      const customerId = await ensureStripeCustomer(pool, stripe, organization, session);
      const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = [];
      if (input.addOns.extra100Gib > 0) {
        lineItems.push({ price: catalog.addOns.extra100Gib.priceId!, quantity: input.addOns.extra100Gib });
      }
      if (input.addOns.extra1Tib > 0) {
        lineItems.push({ price: catalog.addOns.extra1Tib.priceId!, quantity: input.addOns.extra1Tib });
      }
      const metadata = {
        organization_id: organization.id,
      ...
      try {
        checkout = await stripe.checkout.sessions.create({
          ...
          idempotencyKey: [
            "rr-storage-checkout", organization.id,
            input.addOns.extra100Gib, input.addOns.extra1Tib, idempotencyBucket,
          ].join(":"),
        });
      } catch (error) {
        await pool.query(
  ...
  app.post("/api/role-room/storage/billing/portal", async (req: Request, res: Response) => {
    const session = getSession(req, activeSessions);
    if (!session) return res.status(401).json({ error: "krever_innlogging" });
    const parsed = organizationSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "ugyldig_organization_id" });
    try {
      const organization = await readRoleRoomOrganizationAccess(pool, parsed.data.organizationId, session.userId);
      if (!organization) return res.status(403).json({ error: "ingen_organisasjonstilgang" });
      if (!organization.canAdminister) return res.status(403).json({ error: "kun_organisasjonsadmin" });
      if (!stripe) return res.status(503).json({ error: "stripe_ikke_konfigurert" });
      if (!organization.stripeCustomerId) return res.status(404).json({ error: "stripe_kunde_mangler" });
      const configuration = billingPortalConfigurationId();
      if (!configuration) {
        return res.status(503).json({ error: "billing_portal_ikke_konfigurert" });
      }
      const account = await ensureRoleRoomOrganizationStorageAccount(pool, organization);
  ...
  app.post("/api/role-room/storage/affiliate/attribute", async (req: Request, res: Response) => {
    const session = getSession(req, activeSessions);
    if (!session) return res.status(401).json({ error: "krever_innlogging" });
    ...
    try {
      ...
      if (paidAccount.rows.length > 0) {
        return res.status(409).json({ error: "henvisning_ma_registreres_for_betaling" });
      }
      const partnerResult = await pool.query<{ id: string; organization_id: string }>(
      ...
      if (!partner) return res.status(404).json({ error: "henvisningskode_ikke_funnet" });
      if (partner.organization_id === organization.id) {
        return res.status(409).json({ error: "egen_organisasjon_kan_ikke_henvises" });
      }
      ...
      if (existing.rows[0] && existing.rows[0].affiliate_partner_id !== partner.id) {
        return res.status(409).json({ error: "organisasjonen_er_allerede_henvist" });
      }
      await pool.query(
        ...
        [partner.id, organization.id, JSON.stringify({ attributedByUserId: session.userId })],
      );
      return res.status(201).json({ ok: true, status: "attributed" });
    } catch (error) {
      console.error("[role-room-storage-billing] affiliate attribution failed", error);
