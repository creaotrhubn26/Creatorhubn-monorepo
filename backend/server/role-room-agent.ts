...
/** Per-source latency in milliseconds. Keys match the deterministic
 *  pipeline stage names so the UI can render a stable timeline. Optional
 *  on every key so a partial pipeline (e.g. orchestrator path with only
 *  some tools called) still produces a valid object. */
export type RoleRoomAgentServiceLatencies = {
  brreg?: number;
  website?: number;
...
/** Identifies which deterministic stage produced each top-level field.
 *  Used by the UI to render "Source attribution" tooltips and to drive
 *  the "Why this conclusion?" modal. The string values are stable
 *  user-facing labels; the agent path never invents new ones. */
export type RoleRoomAgentDataSource =
  | "brreg"
  | "website"
  | "google_places"
  | "claude_synthesis"
...
/** Per-field provenance returned by the synthesis model itself (item #6
 *  proper-fix). Replaces the heuristic confidence scoring on the frontend
 *  when the LLM populates this. Keys are dot-paths into the result
 *  (e.g. "companyProfile.industry", "planningDraft.brandGuide.toneOfVoice").
 *  Optional everywhere — when missing, the frontend falls back to
 *  heuristic provenance computed from data-source signals. */
export interface RoleRoomAgentFieldProvenanceEntry {
  /** 0-100 confidence as the model assessed it. Don't normalize on the
   *  client side — let the model's calibration speak for itself. */
  confidence?: number;
  /** Free-text "why this value" rationale. Powers the "Why this
   *  conclusion?" modal. Norwegian, 1-2 sentences. */
  rationale?: string;
  /** Which deterministic source(s) the value came from, in order of
   *  influence. The model says "brreg → website → me" if it took the
   *  Brreg name, cross-checked the website's og:site_name, then
   *  rephrased it for tone. */
  sourceChain?: RoleRoomAgentDataSource[];
}
export type RoleRoomAgentFieldProvenance = Record<string, RoleRoomAgentFieldProvenanceEntry>;

export type RoleRoomAgentNormalizedPayload = {
  generatedAt: string;
  provider: "openai" | "anthropic" | "fallback";
  model: string;
  /** Stable UUID for this research run. Stays the same across retries
   *  of the same bootstrap call; changes on a fresh research. Used by
   *  the UI for share-links, debug references, and version comparison. */
  researchId?: string;
  /** Wall-clock ISO timestamp when the bootstrap pipeline started.
   *  Always set when the new instrumentation is active; older cached
   *  results may lack it. */
  bootstrapStartedAt?: string;
  /** Wall-clock ISO timestamp when the pipeline finished. Combined with
   *  bootstrapStartedAt this gives total wall time; serviceLatencies
   *  gives the breakdown per source. */
  bootstrapFinishedAt?: string;
  /** Per-service latency map. UI renders this as a timeline. */
  serviceLatencies?: RoleRoomAgentServiceLatencies;
  /** Human-readable labels for fallbacks that fired during this run.
   *  Examples: "cohere_skipped_no_api_key", "google_places_no_results",
  ...
  /** 2-3 sentence pitch-ready executive summary, generated by Claude
   *  Haiku after the bootstrap completes. Item #42. Falls back to
   *  companyProfile.summary in the UI when missing. */
  executiveSummary?: string | null;
  /** Quality-assurance flags from role-room-research-validation.ts
   *  (items #51-#75). Sorted critical → warning → info. UI groups them
   *  per category in the overlay. Empty array = nothing to flag. */
...
function extractProbableLogoUrl(html: string, websiteUrl: string): string | null {
  // Prioritized scoring beats first-match. Old regex picked favicon over
  // a real <img class="logo">, then Printful rendered the 180×180 favicon
  // stretched across a 1800×2400 print area and the result looked broken.
  // Order: JSON-LD Organization.logo → <link rel="image_src"> →
  // <img alt|class|id="logo"> → header/nav imgs → apple-touch-icon (large) →
  // favicon. Tracking pixels, data: URLs and .gif files are filtered out
  // because they either can't be fetched by Printful or look wrong.
  const candidates: Array<{ url: string; score: number }> = [];

  const consider = (raw: string | null | undefined, score: number): void => {
    if (!hasText(raw)) return;
    const trimmed = raw.trim();
    if (/^data:/i.test(trimmed)) return;
    if (/\.gif(\?|#|$)/i.test(trimmed)) return;
    if (/(?:1x1|tracking|pixel|gtm|googletag|google-?analytics|hotjar|facebook\.com\/tr)/i.test(trimmed)) return;
    const resolved = resolveUrl(websiteUrl, trimmed);
    if (!resolved) return;
  ...
  try {
    ...
    return extractProbableLogoUrlRegexFallback(html, websiteUrl);
  }
  ...
  $('script[type="application/ld+json"]').each((_, el) => {
    try {
      ...
      for (const item of items) {
        ...
        if (!type.includes("organization") && !type.includes("localbusiness") && !type.includes("brand")) continue;
        const logoRaw = record.logo;
        if (typeof logoRaw === "string") {
          consider(logoRaw, 100);
        } else if (logoRaw && typeof logoRaw === "object" && !Array.isArray(logoRaw)) {
          const url = (logoRaw as Record<string, unknown>).url;
          if (typeof url === "string") consider(url, 100);
        }
      }
    } catch {
      // broken JSON-LD is common; ignore and keep scoring
    }
  });

  $('link[rel="image_src"]').each((_, el) => {
    consider($(el).attr("href"), 90);
  });
  ...
  $("img").each((_, el) => {
    ...
    if (!src) return;
    if (alt.includes("logo")) consider(src, 80);
    else if (cls.includes("logo") || id.includes("logo")) consider(src, 70);
  });
  ...
  $("header img, nav img, [class*='header'] img, [class*='nav'] img").each((_, el) => {
    ...
    if (!src) return;
    const alt = String($(el).attr("alt") || "").toLowerCase();
    // header/nav imgs that aren't obviously logos still beat favicon
    consider(src, alt.includes("logo") ? 75 : 50);
  });

  $('link[rel="apple-touch-icon"], link[rel="apple-touch-icon-precomposed"]').each((_, el) => {
    const sizes = String($(el).attr("sizes") || "");
    const sizeNum = Number.parseInt(sizes.split("x")[0] || "0", 10);
    const score = sizeNum >= 180 ? 40 : sizeNum >= 120 ? 30 : 20;
    consider($(el).attr("href"), score);
  });

  $('link[rel~="icon"], link[rel="shortcut icon"], link[rel="mask-icon"]').each((_, el) => {
    consider($(el).attr("href"), 10);
  });
...
function extractProbableLogoUrlRegexFallback(html: string, websiteUrl: string): string | null {
...
/**
 * Extract a brand-color palette directly from the customer's logo. The
 * feed planner's posts looked off-brand because `brandGuide.colors` was
 * always `[]` even when the logo URL had been scraped — Claude was never
 * told to derive colors from the image, and the deterministic fallback
 * didn't run a palette extractor. Now we fetch the logo here and feed
 * the bytes through node-vibrant (already a dependency, used in
...
export async function extractBrandColorsFromLogo(
  ...
  if (!hasText(logoUrl)) return [];
  if (!/^https?:\/\//i.test(logoUrl)) return [];

  try {
    const response = await fetch(logoUrl, {
      signal: AbortSignal.timeout(8_000),
      headers: { "User-Agent": "Mozilla/5.0 (RoleRoomAgent BrandPalette)" },
    });
    ...
    const primary = palette.Vibrant?.hex || palette.DarkVibrant?.hex || palette.Muted?.hex;
    if (primary) {
      colors.push({
        label: "Primær",
        hex: primary,
        usage: "Hovedfarge for overskrifter, CTA og brand-aksenter.",
      });
    }
    const dark = palette.DarkVibrant?.hex || palette.DarkMuted?.hex;
    if (dark && dark !== primary) {
      colors.push({
        label: "Mørk",
        hex: dark,
        usage: "Bakgrunner og kontrast-flater.",
      });
    }
    const light = palette.LightVibrant?.hex || palette.LightMuted?.hex;
    if (light && light !== primary && light !== dark) {
      colors.push({
        label: "Lys",
        hex: light,
        usage: "Highlights, sekundære flater og luftige seksjoner.",
      });
    }
    return colors;
  } catch (error) {
    console.error("[role-room-agent] brand color extraction failed", {
      logoUrl,
...
export async function fetchGooglePlacesCompetitorAnalysis(
  input: RoleRoomAgentProducerBootstrapInput,
  websiteInsights: RoleRoomAgentWebsiteInsights,
  businessSignals: RoleRoomAgentBusinessSignals | null,
  brregCompany: RoleRoomAgentBrregCompany | null,
...
export async function fetchGooglePlacesLocalPresencePlan(
  input: RoleRoomAgentProducerBootstrapInput,
  websiteInsights: RoleRoomAgentWebsiteInsights,
  businessSignals: RoleRoomAgentBusinessSignals | null,
  brregCompany: RoleRoomAgentBrregCompany | null,
...
function buildFallbackBootstrap(
  ...
  localPresencePlan: RoleRoomAgentLocalPresencePlan,
  merchSuppliers?: RoleRoomAgentMerchSuppliers | null,
  /** Brand palette extracted from the logo upstream (extractBrandColorsFromLogo).
   *  Defaults to empty so callers that haven't been updated still compile,
...
function normalizeBootstrapPayload(
  ...
  return {
    ...
    fieldMetadata: normalizeFieldMetadata(record.fieldMetadata),
  };
}

/** Validates and sanitizes the fieldMetadata blob the synthesis model
 *  returned. Drops entries with wrong shape, clamps confidence to [0,100],
 *  filters sourceChain to the documented enum, and trims rationale.
 *  Returns undefined when nothing usable was provided so the frontend
 *  falls back to heuristic provenance. */
const ALLOWED_SOURCES: ReadonlySet<RoleRoomAgentDataSource> = new Set<RoleRoomAgentDataSource>([
  "brreg",
  "website",
  "google_places",
  "claude_synthesis",
...
function normalizeFieldMetadata(value: unknown): RoleRoomAgentFieldProvenance | undefined {
  ...
  for (const [key, raw] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (typeof entry.confidence === "number" && Number.isFinite(entry.confidence)) {
      sanitized.confidence = Math.max(0, Math.min(100, Math.round(entry.confidence)));
    }
    if (hasText(entry.rationale)) {
      sanitized.rationale = normalizeWhitespace(String(entry.rationale)).slice(0, 400);
    }
    if (Array.isArray(entry.sourceChain)) {
      const chain = entry.sourceChain
        .filter((v): v is RoleRoomAgentDataSource => typeof v === "string" && ALLOWED_SOURCES.has(v as RoleRoomAgentDataSource));
      if (chain.length > 0) sanitized.sourceChain = chain;
    }
    if (sanitized.confidence !== undefined || sanitized.rationale || sanitized.sourceChain) {
      out[key] = sanitized;
    }
  }
 ...
 *  fallbacks bag is appended to when the call resolves with a falsy or
 *  empty result so the UI can show "we tried Brreg but found nothing"
 *  instead of silently dropping the stage. When `onProgress` is set,
 *  emits start/done/error events the SSE route forwards to the client. */
async function withTiming<T>(
  key: keyof RoleRoomAgentServiceLatencies,
  latencies: RoleRoomAgentServiceLatencies,
  fallbacks: string[],
  fn: () => Promise<T>,
  options?: {
  ...
  options?.onProgress?.({ type: "stage_start", stage: key });
  try {
    const result = await fn();
    const ms = Date.now() - startedAt;
    latencies[key] = ms;
    let fallback: string | undefined;
    if (options?.emptyFallbackLabel && options.isEmpty?.(result)) {
    ...
    const ms = Date.now() - startedAt;
    latencies[key] = ms;
    fallbacks.push(`${String(key)}_threw_${error instanceof Error ? error.name : "error"}`);
    options?.onProgress?.({
      type: "stage_error",
      stage: key,
      ms,
...
export async function generateRoleRoomAgentProducerBootstrap(
  ...
  // Behaves identically to withTiming otherwise.
  const time = <T>(
    key: keyof RoleRoomAgentServiceLatencies,
    fn: () => Promise<T>,
    opts?: { emptyFallbackLabel?: string; isEmpty?: (v: T) => boolean },
  ): Promise<T> =>
    withTiming(key, serviceLatencies, fallbacksUsed, fn, {
      emptyFallbackLabel: opts?.emptyFallbackLabel,
  ...
  const finalize = <T extends Omit<RoleRoomAgentNormalizedPayload, "researchId" | "bootstrapStartedAt" | "bootstrapFinishedAt" | "serviceLatencies" | "fallbacksUsed">>(
    ...
    try {
      const datapoints =
        (finalPayload.competitorAnalysis?.competitors?.length ?? 0)
        + (finalPayload.localPresencePlan?.nearbyOpportunities?.length ?? 0)
        + (finalPayload.socialProfileCandidates?.length ?? 0)
        + (finalPayload.merchSuppliers?.suppliers?.length ?? 0);
      console.log("[role-room-agent:telemetry] research_completed", JSON.stringify({
        researchId,
        projectId: input.projectId,
        totalMs: serviceLatencies.totalMs,
