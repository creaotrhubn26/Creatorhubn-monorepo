...
function buildPool(options: {
  ...
  const query = vi.fn(async (sqlValue: unknown, params: unknown[] = []) => {
    ...
    if (sql.includes("FROM organizations o") && sql.includes("membership_role")) {
      return {
        rows: [{
          ...
          billing_email: null,
          stripe_customer_id: options.customerId === undefined ? "cus_org" : options.customerId,
          plan: "solo_free",
          owner_user_id: options.ownerUserId ?? null,
          membership_role: options.membershipRole === undefined ? "admin" : options.membershipRole,
    ...
    if (sql.includes("INSERT INTO role_room_storage_accounts")) {
      return {
        rows: [accountRow({
          ...
          stripe_subscription_status: options.storageSubscriptionId ? "active" : null,
        })],
        rowCount: 1,
      };
    }
    ...
    if (sql.includes("UPDATE organizations") && sql.includes("RETURNING stripe_customer_id")) {
    ...
    if (sql.includes("SET checkout_lock_token") && sql.includes("RETURNING id")) {
      return { rows: [{ id: ACCOUNT_ID }], rowCount: 1 };
    }
    if (sql.includes("FROM role_room_affiliate_partners")) {
      return {
        rows: [{ id: "33333333-3333-4333-8333-333333333333", organization_id: options.affiliatePartnerOrgId ?? "org-partner" }],
        rowCount: 1,
      };
    }
    if (sql.includes("SELECT affiliate_partner_id")) return { rows: [], rowCount: 0 };
    ...
    if (sql.includes("FROM role_room_storage_accounts") && sql.includes("billing_grace_until")) {
      return {
        rows: [{
          id: ACCOUNT_ID,
          billing_grace_until: null,
...
function buildStripe() {
  return {
    ...
    checkout: {
      sessions: {
        create: vi.fn(async () => ({ id: "cs_test", url: "https://checkout.stripe.test/session" })),
      },
    },
    billingPortal: {
      sessions: {
        create: vi.fn(async () => ({ id: "bps_test", url: "https://billing.stripe.test/session" })),
      },
    },
    subscriptions: {
      retrieve: vi.fn(),
...
function buildApp(pool: Pool, stripe: Stripe | null): Express {
  ...
  registerRoleRoomStorageBillingRoutes({
    ...
    pool,
    stripe,
    activeSessions: new Map([["token", { userId: "user-1", email: "owner@example.test" }]]),
  });
  return app;
}
...
describe("Role Room organization storage billing routes", () => {
  beforeEach(() => {
    process.env.ROLE_ROOM_STORAGE_PRICE_EXTRA_100_GIB_MONTHLY = "price_storage_100";
    process.env.ROLE_ROOM_STORAGE_PRICE_EXTRA_1_TIB_MONTHLY = "price_storage_1t";
    process.env.ROLE_ROOM_STORAGE_1_TIB_CHECKOUT_ENABLED = "true";
    process.env.ROLE_ROOM_STORAGE_BILLING_PORTAL_CONFIGURATION_ID = "bpc_storage";
  ...
  it("requires an authenticated organization administrator for checkout", async () => {
    ...
    const response = await request(buildApp(pool, stripe))
      ...
      .send({ organizationId: ORG_ID, addOns: { extra100Gib: 1, extra1Tib: 0 } });
  ...
  it("uses only server-side Price IDs and server-controlled return URLs", async () => {
    ...
    expect(stripe.checkout.sessions.create).toHaveBeenCalledWith(
      expect.objectContaining({
        customer: "cus_org",
        line_items: [
          { price: "price_storage_100", quantity: 2 },
        ],
        success_url: "https://theroleroom.com/role-room?storage_billing=success&session_id={CHECKOUT_SESSION_ID}",
        cancel_url: "https://theroleroom.com/role-room?storage_billing=cancelled",
      ...
      expect.objectContaining({ idempotencyKey: expect.stringContaining(`rr-storage-checkout:${ORG_ID}`) }),
    );
  });
  ...
  it("releases the checkout lock when Stripe returns no hosted URL", async () => {
    ...
    expect(statements.some(({ sql }) =>
      sql.includes("checkout_lock_token = NULL") && sql.includes("checkout_lock_until = NULL"),
    )).toBe(true);
  });
  ...
  it("does not create a second base-plan subscription without a storage add-on", async () => {
    ...
    const response = await request(buildApp(pool, stripe))
      .post("/api/role-room/storage/billing/checkout")
      .set("Authorization", "Bearer token")
      .send({ organizationId: ORG_ID, addOns: { extra100Gib: 0, extra1Tib: 0 } });
  ...
  it("keeps the 1 TiB add-on unavailable until the egress safeguard is enabled", async () => {
    ...
    const checkout = await request(buildApp(pool, stripe))
      .post("/api/role-room/storage/billing/checkout")
      .set("Authorization", "Bearer token")
      .send({ organizationId: ORG_ID, addOns: { extra100Gib: 0, extra1Tib: 1 } });
  ...
  it("creates a portal session with a fixed same-site return URL", async () => {
    ...
    expect(stripe.billingPortal.sessions.create).toHaveBeenCalledWith({
      ...
      return_url: "https://theroleroom.com/role-room?storage_billing=portal_return",
      flow_data: {
  ...
  it("attributes an active partner code to the organization before payment", async () => {
    ...
    expect(statements.some(({ sql }) => sql.includes("INSERT INTO role_room_affiliate_referrals"))).toBe(true);
  });
...
describe("Role Room commercial storage entitlement", () => {
  it("maps the content-producer subscription to the included 25 GiB plan", async () => {
    ...
    await expect(syncRoleRoomCommercialStorageEntitlement(pool, {
    ...
    expect(sync?.params).toEqual(["937518684", "solo_pro", "sub_main", "cus_main"]);
  });
...
describe("Role Room commercial affiliate commission", () => {
  it("accrues 15 percent on ex-tax core revenue and grants the organization bonus once", async () => {
    ...
    await expect(accrueRoleRoomCommercialAffiliateCommission(pool, {
      organizationNumber: "937 518 684",
    ...
    const bonus = calls.find(({ sql }) => sql.includes("INSERT INTO role_room_storage_grants"));
    const commission = calls.find(({ sql }) => sql.includes("INSERT INTO role_room_affiliate_commissions"));
    expect(bonus?.sql).toContain("ON CONFLICT (storage_account_id, source_type, source_ref) DO NOTHING");
    ...
    expect(commission?.params[10]).toContain('"commissionScope":"core_subscription"');
  });
  ...
  it("refuses to attribute a commercial invoice when the organization number is ambiguous", async () => {
    ...
    await expect(accrueRoleRoomCommercialAffiliateCommission(
...
describe("Role Room storage Stripe webhook", () => {
  ...
  it("does not process the same signed event twice", async () => {
    ...
    const event = {
      ...
      data: {
        object: {
          id: "sub_storage",
          metadata: { product_family: "role_room_storage", organization_id: ORG_ID },
        },
      },
    } as unknown as Stripe.Event;

    await expect(handleRoleRoomStorageStripeEvent(pool, stripe, event)).resolves.toEqual({
      matched: true,
      duplicate: true,
    });
  ...
  it("syncs plan state and marks a new subscription event processed", async () => {
    ...
    await expect(handleRoleRoomStorageStripeEvent(pool, stripe, event)).resolves.toEqual({ matched: true });
    expect(statements.some(({ sql }) => sql.includes("UPDATE role_room_storage_accounts"))).toBe(true);
    expect(statements.some(({ sql, params }) =>
      sql.includes("status = 'processed'") && params[0] === "evt_subscription",
    )).toBe(true);
  });
  ...
  it("derives add-on entitlement from the configured Stripe Price instead of editable metadata", async () => {
    ...
    await expect(handleRoleRoomStorageStripeEvent(pool, stripe, event)).resolves.toEqual({ matched: true });
    const grantUpsert = statements.find(({ sql, params }) =>
