...
2. `showcase-*-routes.ts` (60 endpoints, ~2800 linjer 105258-108050) — **kartlagt 2026-05-10:** ~40 unike sub-segmenter, krever splitt i ~8 sub-moduler. Sub-modul-progresjon:
   ...
   - `showcase-client-routes.ts` (8) — klient-portal-funksjoner: send-overage-email, client-session/:id, client-selections (GET/POST/DELETE), public/:userId, client-submissions, project-state PUT ✅ **gjort** (commit pending push). Deps: 5 (app, pool, compatStoreGet, compatStoreSet, showcaseCompatKey). Importerer broadcastChatEventToUser direkte fra ./websocket-chat.js for WS-broadcast ved klientvalg.
