...
4. `evendi-*-routes.ts` (~63 endpoints, ikke 50 som memory.md sa) — kartlagt 2026-05-10: 39 unike sub-segmenter. Sub-modul-progresjon:
   ...
   - `evendi-conversations-routes.ts` (3) — vendor-side chat (list, hent meldinger, send) ✅ **gjort** (commit pending push). Auth distinkt: bruker activeSessions-Map direkte med vendor-lookup via email. Deps: 3 (app, pool, getSessionByToken-lambda).
