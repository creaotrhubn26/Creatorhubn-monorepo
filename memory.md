...
2. `showcase-*-routes.ts` (60 endpoints, ~2800 linjer 105258-108050) — **kartlagt 2026-05-10:** ~40 unike sub-segmenter, krever splitt i ~8 sub-moduler. Sub-modul-progresjon:
   ...
   - `showcase-batch-operations-routes.ts` (2) — masseoperasjoner (delete/archive/publish/feature/bulk-photo-enhance) + undo med compat-store-state ✅ **gjort** (commit pending push). Deps: 8 (app, pool, 3× compatStore-funksjoner, updateShowcaseItemRecord, createShowcaseItemRecord, showcaseBatchUndoKey).
