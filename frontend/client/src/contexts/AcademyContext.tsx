...
export interface Course {
  ...
  googleProductionWorkflow?: AcademyGoogleProductionWorkflow;
...
export type AcademyGoogleProductionRecordingMode =
  | "camera"
  | "camera-and-screen"
  | "screen"
  | "voiceover";

export type AcademyGoogleProductionSceneStatus =
  | "planned"
  | "recording"
  | "ready"
  | "done";

export interface AcademyGoogleProductionSceneDraft {
  id: string;
  title: string;
  objective: string;
  recordingMode: AcademyGoogleProductionRecordingMode;
  durationMinutes: number;
  visualCue: string;
  whyThisMode: string;
  teleprompterScript: string;
  notes?: string;
  status?: AcademyGoogleProductionSceneStatus;
}

export interface AcademyGoogleProductionWorkflow {
  googleRequired?: boolean;
  googleConnectedEmail?: string;
  sceneDrafts?: AcademyGoogleProductionSceneDraft[];
  recordingStartedAt?: string;
  recordingCompletedAt?: string;
  ctaApprovedAt?: string;
  proofApprovedAt?: string;
  runtimeApprovedAt?: string;
  lastVidsSyncAt?: string;
  lastNotebookLmSyncAt?: string;
  driveExportFileId?: string;
  driveExportName?: string;
  driveExportMimeType?: string;
  driveExportUrl?: string;
  driveExportPlaybackUrl?: string;
  driveExportVersion?: number;
  driveExportModifiedAt?: string;
  videoBoundAt?: string;
  updatedAt?: string;
}
...
const isAcademyPathname = (pathname: string): boolean => {
  ...
  return (
    /^\/academy(?:$|[/-])/.test(pathname) || pathname === "/academy-dashboard"
  );
};
...
const normalizeAcademyNavigationTarget = (
  ...
  try {
    ...
    if (
      parsed.searchParams.has("courseId") ||
      parsed.searchParams.has("course_id")
    ) {
      return target;
    }
...
const normalizeInstructorProfile = (
  ...
  const id =
    String(input.id || fallbackId || generatedId).trim() || generatedId;
...
const normalizeCourseResourceRecord = <
  T extends CourseResource | LessonResource,
>(
  resource: T,
): T => {
  ...
  const mediaUpdatedAt =
    String(resource.mediaUpdatedAt || "").trim() || undefined;
  ...
  return {
    ...
    size: Number.isFinite(Number(resource.size))
...
const normalizeAcademyVideoSourceType = (
  value: unknown,
): Course["videoSourceType"] | Lesson["videoSourceType"] | undefined => {
  const normalized = String(value || "").trim();
  return normalized === "google-drive-video" ||
...
const inferLinkedVideoFromResources = (
  ...
  return {
    videoSourceType:
      match.videoSourceType === "google-drive-video" ||
      match.videoSourceType === "google-vids" ||
      match.videoSourceType === "external-link" ||
      match.videoSourceType === "direct"
        ? match.videoSourceType
        : undefined,
    googleDriveFileId:
      String(match.googleDriveFileId || "").trim() || undefined,
    googleDriveMimeType:
      String(match.googleDriveMimeType || "").trim() || undefined,
    googleDriveWebViewLink:
      String(match.googleDriveWebViewLink || "").trim() || undefined,
    googleDrivePlaybackUrl:
      String(match.googleDrivePlaybackUrl || "").trim() || undefined,
...
const normalizeCourseRecord = (course: Course): Course => {
  ...
  const leadInstructorId =
    String(
      course.competencyLeadInstructorId || safeInstructor.id || "",
    ).trim() || safeInstructor.id;
  const normalizedResources = Array.isArray(course.resources)
    ? course.resources.map((resource) =>
        normalizeCourseResourceRecord(resource),
      )
    : [];
  const normalizedLessons = Array.isArray(course.lessons)
    ? course.lessons.map((lesson) => {
        ...
        const videoSourceType = normalizeAcademyVideoSourceType(
          lesson.videoSourceType || inferredLinkedVideo.videoSourceType,
        );
        ...
        return {
          ...
          videoSourceType,
          googleDriveFileId:
            String(lesson.googleDriveFileId || "").trim() ||
            inferredLinkedVideo.googleDriveFileId ||
            undefined,
          googleDriveMimeType:
            String(lesson.googleDriveMimeType || "").trim() ||
            inferredLinkedVideo.googleDriveMimeType ||
            undefined,
          googleDriveWebViewLink:
            String(lesson.googleDriveWebViewLink || "").trim() ||
            inferredLinkedVideo.googleDriveWebViewLink ||
            undefined,
          googleDrivePlaybackUrl:
            String(lesson.googleDrivePlaybackUrl || "").trim() ||
            inferredLinkedVideo.googleDrivePlaybackUrl ||
            undefined,
  ...
  const courseVideoSourceType = normalizeAcademyVideoSourceType(
    course.videoSourceType || inferredCourseLinkedVideo.videoSourceType,
  );
  ...
  return {
    ...
    videoSourceType: courseVideoSourceType,
    googleDriveFileId:
      String(course.googleDriveFileId || "").trim() ||
      inferredCourseLinkedVideo.googleDriveFileId ||
      undefined,
    googleDriveMimeType:
      String(course.googleDriveMimeType || "").trim() ||
      inferredCourseLinkedVideo.googleDriveMimeType ||
      undefined,
    googleDriveWebViewLink:
      String(course.googleDriveWebViewLink || "").trim() ||
      inferredCourseLinkedVideo.googleDriveWebViewLink ||
      undefined,
    googleDrivePlaybackUrl:
      String(course.googleDrivePlaybackUrl || "").trim() ||
      inferredCourseLinkedVideo.googleDrivePlaybackUrl ||
      undefined,
...
const buildInstructorRoster = (
  ...
  const upsert = (
    candidate: Partial<AcademyInstructor>,
    fallbackId?: string,
  ) => {
  ...
  courses.forEach((course) => {
    ...
    const leadInstructorId =
      String(course.competencyLeadInstructorId || courseInstructor.id).trim() ||
      courseInstructor.id;
  ...
  switch (action.type) {
    ...
    case "UPDATE_COURSE": {
      ...
      const shouldSyncCurrentCourse =
        state.currentCourse &&
        String(state.currentCourse.id) === String(action.payload.id);
      const nextCurrentCourse = shouldSyncCurrentCourse
        ? action.payload
        : state.currentCourse;
    ...
    case "DELETE_COURSE": {
      const nextCourses = state.courses.filter(
        (course) => course.id !== action.payload,
      );
      const currentCourseRemoved =
        state.currentCourse &&
        String(state.currentCourse.id) === String(action.payload);
      const nextCurrentCourse = currentCourseRemoved
        ? nextCourses[0] || null
        : state.currentCourse;
      const nextCurrentLesson = (() => {
        if (!nextCurrentCourse) return null;
    ...
    case "SET_FILTER_PRICE":
      return {
        ...state,
        filterPrice: action.payload as "free" | "paid" | "all",
      };
    default:
      return state;
  }
...
export const AcademyProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const enhancedProfessionConfig =
    apiProfessionConfigs?.[currentProfession] || professionConfig;
  const professionColor =
    getUserProfessionColor(currentProfession) || "#FF6B35";
  const professionMeta = useMemo(() => {
    const iconElementType = professionIcon?.type;
    const iconName =
      typeof iconElementType === "string"
        ? iconElementType
        : (iconElementType as { displayName?: string; name?: string })
            ?.displayName ||
  ...
  const writeAcademyKV = useCallback(
    async (key: string, value: unknown): Promise<boolean> => {
      ...
      try {
        ...
        if (!response.ok) {
          console.warn("Failed to persist academy KV key", {
            key,
  ...
  const syncCurrentCourseSelection = useCallback(
    (courses: Course[], persistedCourseId?: string | null) => {
      ...
      void writeAcademyKV(
        ACADEMY_CURRENT_COURSE_STORAGE_KEY,
  ...
  useEffect(() => {
    ...
    const courseId = String(
      state.currentCourse?.id || routeCourseId || "",
    ).trim();
    if (!courseId) return;
    ...
    const patchedPushState: History["pushState"] = (data, unused, url) =>
      originalPushState(
        data,
        unused,
        normalizeTarget(url) as string | URL | null | undefined,
      );
  ...
  const loadCourses = useCallback(async () => {
    ...
    try {
      ...
      const [dbCoursesRaw, dbCurrentCourseIdRaw, dbInstructorsRaw] =
        await Promise.all([
      ...
      const dbCourses = Array.isArray(dbCoursesRaw)
        ? (dbCoursesRaw as Course[])
        : null;
      ...
      if (dbCourses) {
        const normalizedCourses = dbCourses.map((course) =>
        ...
        await Promise.all([
          ...
          writeAcademyKV(
            ACADEMY_INSTRUCTORS_STORAGE_KEY,
      ...
      debugging.logIntegration(
        "info",
        "Academy courses initialized in database",
        {
          courseCount: 0,
          instructorCount: normalizedInstructors.length,
        },
      ...
      try {
        const newCourse: Course = normalizeCourseRecord({
          ...
          competencyLeadInstructorId:
            String(
              courseData.competencyLeadInstructorId ||
        ...
        const nextInstructors = buildInstructorRoster(
          nextCourses,
          state.instructors,
        );
  ...
  const updateCourse = useCallback(
    async (course: Course): Promise<void> => {
      ...
      try {
        ...
        if (
          state.currentCourse &&
          String(state.currentCourse.id) === String(updatedCourse.id)
        ) {
          await writeAcademyKV(
            ACADEMY_CURRENT_COURSE_STORAGE_KEY,
  ...
  const deleteCourse = useCallback(
    async (courseId: string): Promise<void> => {
      ...
      try {
        ...
        if (
          state.currentCourse &&
          String(state.currentCourse.id) === String(courseId)
        ) {
          if (nextCourses[0]) {
            await writeAcademyKV(
              ACADEMY_CURRENT_COURSE_STORAGE_KEY,
  ...
  const getInstructor = useCallback(
    (instructorId: string): AcademyInstructor | null => {
      return (
  ...
  const deleteInstructor = useCallback(
    async (instructorId: string): Promise<void> => {
      ...
      const nextCourses = state.courses.map((course) => {
        ...
        const nextCourseInstructor =
          shouldReassignLead && fallbackInstructor
            ? {
  ...
  const createLesson = useCallback(
    async (lessonData: Omit<Lesson, "id">): Promise<Lesson> => {
      ...
      try {
        ...
        const newLesson: Lesson = {
          ...
          videoInstructorId:
            String(
              lessonData.videoInstructorId ||
                course?.competencyLeadInstructorId ||
                "",
            ).trim() || undefined,
          id: `lesson-${Date.now()}`,
        };
        ...
        if (course) {
          ...
          const updatedCourse = normalizeCourseRecord({
            ...
            lessons: course.lessons.map((l) =>
              l.id === lesson.id ? normalizedLesson : l,
            ),
            updatedAt: new Date().toISOString(),
          });
          const nextCourses = state.courses.map((existingCourse) =>
  ...
  const unenrollFromCourse = useCallback(
    async (courseId: string): Promise<void> => {
      ...
      try {
        ...
        await writeAcademyKV(
          ACADEMY_ENROLLMENTS_STORAGE_KEY,
          updatedEnrollments,
        );
  ...
  const getEnrollment = useCallback(
    (courseId: string): Enrollment | null => {
  ...
  const addBookmark = useCallback(
    async (
      ...
      try {
        ...
        const nextProgress = upsertProgressEntry(
          state.progress,
          updatedProgress,
        );
    ...
    async (
      ...
      try {
        ...
        if (existingProgress) {
          const updatedProgress: StudentProgress = {
            ...existingProgress,
            bookmarks: existingProgress.bookmarks.filter(
              (t) => t !== timestamp,
            ),
          };
  ...
  const addNote = useCallback(
    async (courseId: string, lessonId: string, note: string): Promise<void> => {
  ...
  const updateSettings = useCallback(
    async (settings: Partial<AcademySettings>): Promise<void> => {
  ...
  const filteredCourses = useCallback(() => {
    ...
    if (state.filterLevel !== "all") {
      filtered = filtered.filter(
        (course) => course.level === state.filterLevel,
      );
    }
      ...
      if (course?.id) {
        ...
        const matchedLesson =
          (state.currentLesson
            ? course.lessons.find(
                (lesson) =>
                  String(lesson.id) === String(state.currentLesson?.id),
              )
            : null) ||
          course.lessons[0] ||
          null;
  ...
  useEffect(() => {
    ...
    const loadPersistedState = async () => {
      try {
        const [dbSettingsRaw, dbProgressRaw, dbEnrollmentsRaw] =
          await Promise.all([
        ...
        if (dbSettingsRaw && typeof dbSettingsRaw === "object") {
          dispatch({
            type: "SET_SETTINGS",
            payload: dbSettingsRaw as AcademySettings,
          });
        }
        ...
        if (Array.isArray(dbProgressRaw)) {
          dispatch({
            type: "SET_PROGRESS",
            payload: dbProgressRaw as StudentProgress[],
          });
        }
        ...
        if (Array.isArray(dbEnrollmentsRaw)) {
          dispatch({
            ...
            payload: dbEnrollmentsRaw as Enrollment[],
          });
        }
      } catch (error) {
...
export function useAcademy() {
  ...
  if (!context) {
    console.warn(
      "useAcademy called outside AcademyProvider, using default values",
    );
