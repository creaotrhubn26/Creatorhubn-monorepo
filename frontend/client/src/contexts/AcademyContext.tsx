...
type AcademyAction =
  ...
  | { type: 'SET_FILTER_PRICE'; payload: string };
...
function academyReducer(state: AcademyState, action: AcademyAction): AcademyState {
  switch (action.type) {
    ...
    case 'UPDATE_PROGRESS':
      return {
        ...
        progress: state.progress.map(p =>
          p.courseId === action.payload.courseId && p.lessonId === action.payload.lessonId
            ? action.payload
            : p
        ),
    };
    ...
    case 'SET_SORT_BY':
      return { ...state, sortBy: action.payload as any };
...
export const AcademyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadCourses = useCallback(async () => {
    ...
    try {
      ...
      const mockCourses: Course[] = [
        {
          ...
          thumbnail: '/thumbnails/photography-masterclass.jpg',
          videoUrl: '/videos/photography-masterclass-intro.mp4',
          duration: 7200, // 2 hours
          level: 'intermediate',
          category: 'photography',
          tags: ['photography','lighting','composition','portraits'],
          price: 299,
          isFree: false,
          isPublished: true,
          ...
          prerequisites: [],
          learningOutcomes: [
            'Master advanced lighting techniques','Understand composition principles','Learn post-processing workflows'
          ],
          resources: []
      },
        {
          id: '2',
          title: 'Wedding Videography Complete Guide',
          description: 'Complete guide to wedding videography from planning to delivery',
          instructor: {
            id: '2',
            name: 'CreatorHub Academy',
            avatar: '/avatars/creatorhub-academy.jpg',
            bio: 'Professional video production team',
            profession: 'videographer'
        },
          thumbnail: '/thumbnails/wedding-videography.jpg',
          videoUrl: '/videos/wedding-videography-intro.mp4',
          duration: 10800, // 3 hours
          level: 'beginner',
          category: 'wedding',
          tags: ['wedding','videography','planning','editing'],
          price: 0,
          isFree: true,
          isPublished: true,
          ...
          prerequisites: [],
          learningOutcomes: [
            'Plan wedding video shoots','Capture key moments','Edit professional wedding videos'
          ],
          resources: []
      }
      ];
      ...
      debugging.logIntegration('info','Courses loaded successfully', {
        courseCount: mockCourses.length
    });
...
}, [getCourse, analytics, performance, debugging]);
...
}, [state.courses, analytics, performance, debugging]);
...
}, [state.enrollments, analytics, performance, debugging]);
...
}, [state.progress, analytics, performance, debugging]);
...
}, [state.settings, analytics, performance, debugging]);
  ...
  const setFilterPrice = useCallback((price: string) => {
