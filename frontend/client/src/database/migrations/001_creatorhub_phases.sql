...
('total_phases', 7, 'count', 'Total number of phases in the system'),
('completed_phases', 7, 'count', 'Number of completed phases'),
('total_components', 17, 'count', 'Total number of components'),
('total_icons', 37, 'count', 'Total number of icons (custom + material-ui)'),
('active_users', 1247, 'count', 'Number of active users'),
('system_health', 100, 'percentage', 'System health percentage'),
('documentation_sections', 7, 'count', 'Number of documentation sections'),
('custom_icons', 28, 'count', 'Number of custom SVG icons'),
('material_ui_icons', 9, 'count', 'Number of Material-UI icons used')
...
CREATE INDEX IF NOT EXISTS idx_components_phase_id ON components(phase_id);
...
CREATE INDEX IF NOT EXISTS idx_documentation_level ON documentation_sections(level);
...
CREATE TRIGGER update_documentation_sections_updated_at BEFORE UPDATE ON documentation_sections
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
SELECT 
    ...
    array_agg(c.name) as components
FROM phases p
LEFT JOIN components c ON p.phase_id = c.phase_id
GROUP BY p.phase_id, p.name, p.description, p.status, p.progress
ORDER BY p.phase_id;

CREATE OR REPLACE VIEW system_overview AS
SELECT 
    (SELECT stat_value FROM system_stats WHERE stat_name = 'total_phases') as total_phases,
    (SELECT stat_value FROM system_stats WHERE stat_name = 'completed_phases') as completed_phases,
    (SELECT stat_value FROM system_stats WHERE stat_name = 'total_components') as total_components,
    (SELECT stat_value FROM system_stats WHERE stat_name = 'total_icons') as total_icons,
    (SELECT stat_value FROM system_stats WHERE stat_name = 'active_users') as active_users,
    (SELECT stat_value FROM system_stats WHERE stat_name = 'system_health') as system_health;

-- Grant permissions (adjust as needed for your setup)
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO creatorhub_user;
-- GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO creatorhub_user;
