              ...
              {isOppstartstilskuddOne && <Box data-testid="oppstartstilskudd-1-playbook" sx={{ mt: 1, p: 1, borderRadius: 1.5, bgcolor: 'rgba(249,115,22,0.08)', border: `1px solid ${BRAND.border}` }}>
                ...
                {plan ? (
                  <Stack direction={{ xs: 'column', sm: 'row' }} gap={0.75} sx={{ mt: 1 }}>
                    <Button data-testid="oppstartstilskudd-open-application" size="small" variant="contained" onClick={() => window.location.assign(`/admin-workspace?view=documents&product=${parentProduct}&document=${encodeURIComponent(plan.documentId)}`)} sx={{ bgcolor: '#c2410c' }}>Skriv søknaden</Button>
                  ...
                  <Button data-testid="oppstartstilskudd-start-plan" size="small" variant="contained" onClick={() => void startApplicationPlan(item)} disabled={startingPlanId === item.id} sx={{ mt: 1, bgcolor: '#c2410c' }}>{startingPlanId === item.id ? 'Oppretter…' : 'Start søknadsløp'}</Button>
                )}
              </Box>}
