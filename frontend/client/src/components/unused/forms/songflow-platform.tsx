...
const queryClient = useQueryClient();
  ...
  const { data: projects, isLoading } = useQuery({
    ...
    queryFn: () => const auth = await getAuthHeader();
 return apiRequest(['/api/songflow-projects']{
   headers: auth,
      headers: {
  }
  }),
  ...
  return (
    <Box sx={{ p:  3 }}>
                  ...
                  <Grid container spacing={2} alignItems="center">
                    ...
                    <Grid size={{ xs: 12 }} md={3}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap:  1 }}>
                        ...
                        <Typography variant="caption">
                          Google Drive backup: {new Date(track.lastBackup!).toLocaleString('')}
                        </Typography>
                      </Box>
      ...
      <TabPanel value={activeTab} index={1}>
        <Typography variant="h6" sx={{  mb:  2  }}>Musikkprosjekter</Typography>
      ...
      <TabPanel value={activeTab} index={3}>
        <Typography variant="h6" sx={{  mb:  2  }}>Aktive Samarbeidsprosjekter</Typography>
        <Grid container spacing={2}>
          <Grid size={{ xs: 12 }}>
            <MuiCard>
              <CardContent sx={theming.getThemedCardSx()}>
                ...
                <Typography color="text.secondary" sx={{ mb:  2 }}>
                  Del og samarbeid på musikkprosjekter i sanntid med Google Drive og Docs integration
                </Typography>
                <Alert severity="info">
                  Song Flow er spesielt designet for musikkprodusenter med full Google services integrasjon
                </Alert>
              </CardContent>
            </MuiCard>
          </Grid>
