...
export default function PhotographerOnboardingWizard() {
const queryClient = useQueryClient();
  
  // Theming system
  const theming = useTheming('photographer');
  
  // Database connection for PhotographerOnboardingWizard
  ...
  const updatePhotographerOnboardingWizard = useMutation({
  ...
  return (
    <Box sx={{ p:  2 }}>
      <Typography variant="h6" sx={{ ...{}, color: theming.colors.primary }}>
        Photographer Onboarding Wizard
      </Typography>
    </Box>
  );
}