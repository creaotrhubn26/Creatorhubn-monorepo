...
interface FirmwareUpdate {
  ...
  deviceBrand: string;
  deviceModel: string;
  deviceType: 'camera' | 'lens' | 'audio' | 'software' | 'accessory' | 'lighting' | 'memory' | 'tripod' | 'slider' | 'flash';
...
interface DeviceInfo {
  ...
  brand: string;
  model: string;
  type: 'camera' | 'lens' | 'audio' | 'software' | 'accessory' | 'lighting' | 'memory' | 'tripod' | 'slider' | 'flash';
...
const DEVICE_CATEGORIES = {
  photographer: [
    { 
      type: 'camera', 
      brands: ['Alpa','Canon','Fujifilm','GoPro','Hasselblad','Leica','Nikon','OM SYSTEM','Panasonic Lumix','Pentax','Phase One','Polaroid','Ricoh','Sigma','Sony','Zeiss'] 
  },
    { 
      type: 'lens', 
      brands: ['Canon','Fujinon','Irix','Laowa','Leica','Lensbaby','Meike','Nikon','Samyang','Schneider-Kreuznach','Sigma','Sirui','Sony','Tamron','Tokina','Viltrox','Voigtländer','Zeiss'] 
  },
    { 
      type: 'software', 
    ...
    { 
      type: 'flash', 
      brands: ['Broncolor','Canon Speedlite','Elinchrom','Godox','Hensel','Interfit','Jinbei','Metz','Neewer','Nikon Speedlight','Nissin','OM SYSTEM','Pentax','Phottix','Profoto','Sony HVL','Visico','Westcott'] 
  },
    { 
      type: 'lighting', 
      brands: ['Aladdin','Aputure','Astera','Creamsource','Dedolight','Dracast','FalconEyes','Fiilex','Godox','GVM','Kino Flo','Lupo','Luxli','Nanlite','Neewer','Rotolight','SmallRig','Westcott','Zhiyun','Quasar Science','COLBOR'] 
  },
    { 
      type: 'memory', 
      brands: ['ADAT','Angelbird','Delkin Devices','Gigastone','Hoodman','Integral','Kioxia','Kingston','Lexar','Panasonic','Patriot','PNY','ProGrade Digital','Samsung','SanDisk','Silicon Power','Sony','Team Group','Transcend','Verbatim','Wise Advanced'] 
  },
    { 
      type: 'tripod', 
      brands: ['3 Legged Thing','Benro','Cullmann','Feisol','Gitzo','Joby','K&F Concept''Leofoto','Manfrotto','Neewer','Peak Design','Really Right Stuff','Rollei','Sirui','SmallRig','Ulanzi','Vanguard','Velbon'] 
  },
    { 
      type: 'accessory', 
  ...
  videographer: [
    { 
      type: 'camera', 
      brands: ['ARR','AJA Video Systems','Atomos','Blackmagic Design','Canon Cinema EOS','DJI','Ikegami','JVC','Kinefinity','Panasonic','RED Digital Cinema','Sharp','SmallHD','Sony','Z CAM'] 
  },
    { 
      type: 'lens', 
      brands: ['Angénieux','ARRI','Atlas Lens Co.','Canon','Cooke','DZOFilm','Fujinon','Laowa','Leica','Samyang','Sigma','Sony','Tokina','Vazen','Zeiss'] 
  },
    { 
      type: 'audio', 
      brands: ['AK','Audio-Technica','BOYA','Countryman','Deity','DJI','DPA Microphones','Hollyland','K-Tek''Lectrosonics','Movo','Neumann','RØDE','Rycote','Sanken','Sennheiser','Shure','Schoeps','Sound Devices','Sony','Tascam','Tentacle Sync','Zoom','Zaxcom','Ambient Recording'] 
  },
    { 
      type: 'software', 
    ...
    { 
      type: 'lighting', 
      brands: ['Aladdin','Aputure','ARRI','Astera','Creamsource','Dedolight','Dracast','FalconEyes','Fiilex','Godox','GVM','Kino Flo','Lupo','Luxli','Nanlite','Neewer','Rotolight','SmallRig','Westcott','Zhiyun','Quasar Science','COLBOR'] 
  },
    { 
      type: 'slider', 
      brands: ['Edelkrone','GVM','iFootage','Kessler','Konova','Manfrotto','Moza','Neewer','Rhino','Smartta','YC Onion','Zeapon','Cinevate'] 
  },
    { 
      type: 'tripod', 
      brands: ['3 Legged Thing','Benro','Cartoni','Cullmann','E-Image''Feisol','Gitzo','iFootage','K&F Concept','Leofoto','Libec','Manfrotto','Miller','Neewer','OConnor','Sachtler','Sirui','SmallRig','Ulanzi','Vinten'] 
  },
    { 
      type: 'memory', 
      brands: ['ADAT','Angelbird','Delkin Devices','Gigastone','Hoodman','Integral','Kioxia','Kingston','Lexar','Panasonic','Patriot','PNY','ProGrade Digital','Samsung','SanDisk','Silicon Power','Sony','Team Group','Transcend','Verbatim','Wise Advanced'] 
  ...
  musicproducer: [
    { 
      type: 'audio', 
      brands: ['AK','Audio-Technica','DPA Microphones','Focusrite','Neumann','PreSonus','RME','RØDE','Sennheiser','Shure','Sound Devices','Steinberg','Universal Audio','MOTU','Zoom','Tascam'] 
  },
    { 
      type: 'software', 
      brands: ['Ableton','Logic Pro','Pro Tools','Cubase','FL Studio','Reaper','Steinberg','Native Instruments'] 
  },
    { 
      type: 'accessory', 
      brands: ['Akai','Native Instruments','Novation','Arturia','Roland','Focusrite','PreSonus','Universal Audio','MOTU'] 
  }
  ],
  vendor: [
    { 
    ...
    { 
      type: 'memory', 
      brands: ['ADAT','Angelbird','Delkin Devices','Kingston','Lexar','ProGrade Digital','Samsung','SanDisk','Sony','Transcend','Wise Advanced'] 
  }
  ]
};
...
export default function FirmwareUpdateManager({
  ...
  const { data: devicesList, isLoading: isLoadingDevices } = useQuery({
    ...
    queryFn: () => apiRequest(['/api/firmware/devices', profession],
    staleTime: 1000 * 60 * 10, {
      headers: {
  }
  }), // 10 minutes
  ...
  const priorityConfig = {
    critical: { color: 'error', icon: theming.getThemedIcon(', '), urgency: 'Kritisk - Installer umiddelbart',},
    high: { color: 'warning', icon: theming.getThemedIcon(', '), urgency: 'Høy - Installer innen 24 timer',},
    medium: { color: 'info', icon: theming.getThemedIcon(', '), urgency: 'Middels - Installer innen en uke',},
    low: { color: 'success', icon: theming.getThemedIcon(', '), urgency: 'Lav - Installer ved bekvemmelighet',}
};
  ...
  const startUpdate = async (update: FirmwareUpdate) => {
    ...
    try {
      ...
      setAvailableUpdates(prev => 
        prev.map(u => u.id === update.id ? completedUpdate : u)
      );
  ...
  const renderUpdateChecker = () => (
    <Paper sx={{ p:  3 ,  ...theming.getThemedCardSx() }}>
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12 }} md={6}>
          ...
          <Alert severity="info" sx={{ mb:  2 }}>
            <Typography variant="body2">
              Siste sjekk: {new Date().toLocaleDateString('no-N', { 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric',
                hour: '2-digit',
        ...
        <Grid size={{ xs: 12 }} md={6}>
          <Typography variant="subtitle1" sx={{ mb:  2 }}>Oppdateringsstatistikk</Typography>
      ...
      {availableUpdates.length > 0 && (
        <Box sx={{ mt:  3 }}>
          <Divider sx={{ mb:  2 }} />
          <Typography variant="subtitle1" sx={{ mb:  2 }}>Tilgjengelige oppdateringer</Typography>
          
          <List>
            {availableUpdates.map((update) => (
              <ListItem key={update.id} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, mb:  1 }}>
                <ListItemIcon>
                  <Avatar sx={{ bgcolor: `${priorityConfig[update.priority].color}.main` }}>
                    {update.deviceType === 'camera' ? <CameraAlt /> :
                     update.deviceType === 'lens' ? <Lens /> :
                     update.deviceType === 'software' ? theming.getThemedIcon('settings') :
                     update.deviceType === 'audio' ? <MusicNote /> :
                     update.deviceType === 'lighting' ? <Lightbulb /> :
                     update.deviceType === 'flash' ? <FlashOn /> :
                     update.deviceType === 'memory' ? <SdCard /> :
                     update.deviceType === 'tripod' ? <CameraStand /> :
                     update.deviceType === 'slider' ? <LinearScale /> : theming.getThemedIcon('business')}
                  </Avatar>
                </ListItemIcon>
                <ListItemText
                  primary={
                    <Box sx={{ display: 'flex', alignItems: 'center', gap:  1 }}>
                      ...
                      {update.securityUpdates.length > 0 && (
                        <Chip size="small" label="SIKKERHET" color="error" icon={<Shield />} />
                      )}
                    </Box>
                }
                  secondary={
  ...
  const renderUpdatePrioritization = () => (
    <Paper sx={{ p:  3 ,  ...theming.getThemedCardSx() }}>
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12 }} md={6}>
          <Typography variant="subtitle1" sx={{ mb:  2 }}>Prioritetsnivåer</Typography>
          
          <List>
            {Object.entries(priorityConfig).map(([level, config]) => (
              <ListItem key={level} sx={{ border: 1, borderColor: 'divider', borderRadius: 1, mb:  1 }}>
                <ListItemIcon>
        ...
        <Grid size={{ xs: 12 }} md={6}>
          <Typography variant="subtitle1" sx={{ mb:  2 }}>Automatiseringsregler</Typography>
          ...
          <Alert severity="warning" sx={{ mt:  2 }}>
            <Typography variant="body2">
              Kritiske oppdateringer installeres utenfor arbeidstid (22: 00-06:00) for å unngå avbrudd
            </Typography>
          </Alert>
        </Grid>
      </Grid>
      ...
      <Typography variant="subtitle1" sx={{ mb:  2 }}>Planlegging av oppdateringer</Typography>
  ...
  const renderUpdateAutomation = () => (
    <Paper sx={{ p:  3 ,  ...theming.getThemedCardSx() }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12 }} md={6}>
          <Typography variant="subtitle1" sx={{ mb:  2 }}>Monteringsfilter</Typography>
          
          <List>
            {DEVICE_CATEGORIES[profession].map((category, index) => (
              <ListItem key={index}>
                <ListItemIcon>
                  {category.type === 'camera' ? <CameraAlt /> :
                   category.type === 'lens' ? <Lens /> :
                   category.type === 'software' ? theming.getThemedIcon('settings') :
                   category.type === 'audio' ? <MusicNote /> :
                   category.type === 'lighting' ? <Lightbulb /> :
                   category.type === 'flash' ? <FlashOn /> :
                   category.type === 'memory' ? <SdCard /> :
                   category.type === 'tripod' ? <CameraStand /> :
                   category.type === 'slider' ? <LinearScale /> : theming.getThemedIcon('business')}
                </ListItemIcon>
                <ListItemText
  ...
  const renderUpdateHistory = () => (
    <Paper sx={{ p:  3 ,  ...theming.getThemedCardSx() }}>
      ...
      {updateHistory.length > 0 ? (
        ...
        <Alert severity="info">
          <Typography variant="body2">
            Ingen oppdateringshistorikk ennå
          </Typography>
        </Alert>
      )}
    </Paper>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb:  2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap:  2 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  Automatisk sjekking og prioritering av oppdateringer for {profession === 'photographer' ? 'fotografer' : profession === 'videographer' ? 'videografer' : profession === 'musicproducer' ? 'musikkprodusenter' : 'leverandører'}
                </Typography>
              </Box>
            </Box>
            <Box sx={{ display: 'flex', gap:  1 }}>
              ...
              <Chip
                ...
                color={availableUpdates.filter(u => u.priority === 'critical').length > 0 ? 'error' : 'success'}
                size="small"
              />
            </Box>
          </Box>
          ...
          {selectedUpdate && (
            <Box>
              ...
              <Grid container spacing={2} sx={{ mb:  3 }}>
                <Grid size={{ xs:  6 }}>
                  <Typography variant="body2" color="text.secondary">Nåværende versjon</Typography>
                  <Typography variant="body1">{selectedUpdate.currentVersion}</Typography>
                </Grid>
                <Grid size={{ xs:  6 }}>
                  <Typography variant="body2" color="text.secondary">Ny versjon</Typography>
                  <Typography variant="body1" sx={{ fontWeight: 600,color }}>{selectedUpdate.latestVersion}</Typography>
                </Grid>
                <Grid size={{ xs:  6 }}>
                  <Typography variant="body2" color="text.secondary">Nedlastingsstørrelse</Typography>
                  <Typography variant="body1">{selectedUpdate.updateSize} MB</Typography>
                </Grid>
                <Grid size={{ xs:  6 }}>
                  <Typography variant="body2" color="text.secondary">Estimert tid</Typography>
                  <Typography variant="body1">{selectedUpdate.installationTime} minutter</Typography>
                </Grid>
              </Grid>
              ...
              {selectedUpdate.backupRequired && (
                <Alert severity="warning" sx={{ mb:  2 }}>
                  <Typography variant="body2">
                    Denne oppdateringen krever sikkerhetskopi. All data vil bli sikkerhetskopiert automatisk.
                  </Typography>
                </Alert>
              )}
      ...
      >
        ...
        <DialogContent>
          {currentReleaseNotes && (
            <Box>
              <Typography variant="h6" sx={{  mb:  2  }}>
                {currentReleaseNotes.deviceBrand} {currentReleaseNotes.deviceModel} v{currentReleaseNotes.latestVersion}
              </Typography>
                ...
                <Box sx={{ mb:  3 }}>
                  ...
                  <List dense>
                    {currentReleaseNotes.securityUpdates.map((security, index) => (
                      <ListItem key={index}>
                        <ListItemIcon>
                          <Shield sx={{ color:'error.main', fontSize: 16}} />
                        </ListItemIcon>
