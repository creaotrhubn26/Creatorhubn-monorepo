...
export default function EmailChatIntegration() {
  ...
  // Database connection for EmailChatIntegration
  ...
  const updateEmailChatIntegration = useMutation({
  ...
  return (
    <Box sx={{ p:  2 }}>
      <Typography variant="h6" sx={{ ...{}, color: theming.colors.primary }}>
        {professionConfig ? `${professionConfig.displayName} - E-post Chat Integration` :'Email Chat Integration'}
      </Typography>
      {professionConfig && (
        <Typography variant="body1" color="text.secondary" sx={{ mt:  2 }}>
          E-post integrasjon tilpasset {professionConfig.displayName.toLowerCase()}-kommunikasjon
        </Typography>
      )}
    </Box>
  );
