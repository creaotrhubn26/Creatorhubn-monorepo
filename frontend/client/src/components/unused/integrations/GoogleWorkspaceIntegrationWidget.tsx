...
export default function GoogleWorkspaceIntegrationWidget() {
const queryClient = useQueryClient();
  
  // Theming system
  const theming = useTheming('prototype_tester');
  
  // Database connection for GoogleWorkspaceIntegrationWidget
  ...
  const updateGoogleWorkspaceIntegrationWidget = useMutation({
  ...
  return (
    <Box sx={{ p:  2 }}>
      <Typography variant="h6" sx={{ color: theming.colors.primary }}>
        GoogleWorkspaceIntegrationWidget
      </Typography>
    </Box>
  );
}