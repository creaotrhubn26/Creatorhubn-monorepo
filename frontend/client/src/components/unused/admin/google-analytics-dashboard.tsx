...
interface AnalyticsData {
  ...
  trends: {
    ...
    pageviewsLastWeek: Array<{ date: string; pageviews: number; sessions: number }>;
  ...
  content: {
    topPages: Array<{ page: string; pageviews: number; uniquePageviews: number; avgTimeOnPage: number }>;
    topProfessions: Array<{ profession: string; users: number; sessions: number; conversionRate: number }>;
};
  acquisition: {
    channels: Array<{ channel: string; users: number; sessions: number; conversionRate: number }>;
    campaigns: Array<{ campaign: string; clicks: number; impressions: number; cost: number; conversions: number }>;
};
  goals: {
    completions: Array<{ goal: string; completions: number; conversionRate: number; value: number }>;
...
export default function GoogleAnalyticsDashboard() {
  ...
  const { data: analyticsData, isLoading, error, refetch } = useQuery<AnalyticsData>({
    ...
    queryFn: () => apiRequest(`/api/google-analytics/data?dateRange=${dateRange}&property=${selectedProperty}`),
  ...
  React.useEffect(() => {
    componentRegistry.registerComponent('GoogleAnalyticsDashboard', {
      type: 'google-service',
      capabilities: ['google-analytics', 'data-visualization','reporting'],
      dataFlow: {
        sources: ['analytics-data', 'realtime-data','audience-data'],
        destinations: ['admin-dashboard','user-interface'],
        processors: ['analytics-processing', 'data-processing']
    }
  });

    // Set up data flow nodes
    dataFlow.registerNode('analytics-data', {
      type: 'source',
      data: analyticsData,
      metadata: { component: 'GoogleAnalyticsDashboard', type: 'analytics-data',}
  });

    dataFlow.registerNode('realtime-data', {
      type: 'source',
      data: realtimeData,
      metadata: { component: 'GoogleAnalyticsDashboard', type: 'realtime-data',}
  });

    dataFlow.registerNode('audience-data', {
      type: 'source',
      data: audienceData,
      metadata: { component: 'GoogleAnalyticsDashboard', type: 'audience-data',}
  });

    // Listen for Google Analytics events
    communication.subscribe('google-analytics: refresh', () => {
      handleRefresh();
  });

    communication.subscribe('google-analytics: date-range-change', (data) => {
      if (data.dateRange) {
        setDateRange(data.dateRange);
    }
  });

    communication.subscribe('google-analytics: property-change', (data) => {
      if (data.property) {
    ...
    return () => {
      componentRegistry.unregisterComponent('GoogleAnalyticsDashboard');
      dataFlow.unregisterNode('analytics-data');
      dataFlow.unregisterNode('realtime-data');
      dataFlow.unregisterNode('audience-data');
  };
}, [analyticsData, realtimeData, audienceData, componentRegistry, dataFlow, communication, handleRefresh]);
        ...
        <Alert severity="error" sx={{ mb:  3, display: 'flex', alignItems: 'center'}}>
          ...
          <Box sx={{ flex:  1 }}>
            ...
            <Typography variant="body2">
              For å aktivere Google Analytics dashboard må du konfigurere Google Analytics API-nøkler i Secrets-fanen.
              Kontakt administrator for å få tilgang til Google Analytics data.
            </Typography>
            <Button 
    ...
    return (
      <Box sx={{ p:  3 }}>
        <Alert severity="warning">
          ...
          <Typography>
            Google Analytics API må konfigureres for å vise data. 
            Kontakt administrator for å sette opp API-tilgang.
          </Typography>
        </Alert>
      </Box>
    );
          ...
          <MuiCard>
            <CardContent sx={theming.getThemedCardSx()}>
              <Typography variant="h6" gutterBottom sx={{  display: 'flex', alignItems: 'center', gap:  1  }}>
                <ShowChart />
                Brukertrender (Siste {dateRange === '7d' ? '7 dager' : dateRange === '30d' ? '30 dager' : dateRange})
              </Typography>
              <Box sx={{ height: 300}}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data.trends.usersLastWeek}>
