...
export default function PhotoDeliveryGalleryManager() {
  ...
  const createGalleryMutation = useMutation({
    mutationFn: (data: Partial<Gallery>) =>
      ...
      return apiRequest('/api/photo-showcase/create', {
        headers: auth,
        headers: {
    },
  ...
  const updateGalleryMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<Gallery> }) =>
      const auth = await getAuthHeader();
      return apiRequest(`/api/photo-showcase/galleries/${id}`, {
        headers: auth,
        headers: {
    },
  ...
  const getVisibilityIcon = (visibility: string) => {
    switch (visibility) {
      ...
      case 'client_only': return <Group fontSize="small" />;
        ...
        <Grid container spacing={2} sx={{ mb:  3 }}>
          <Grid size={{ xs: 12 }} sm={3}>
            <Card sx={{ 
              ...
              color: 'white',
              boxShadow: 3 ,  ...theming.getThemedCardSx() }}>
