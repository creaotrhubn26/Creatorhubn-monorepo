...
// ============================================================================
// THE ROLE ROOM → NEXT ROLE CV-INTEGRASJON
// Hjelpere som mapter DTO-en fra GET /api/nextrole/cv/me inn i Resume-strukturen,
// og merger ny profil-data inn i en eksisterende CV UTEN å slette brukerens
// egne oppføringer (erfaring/prosjekter de selv har lagt inn forblir).
// ============================================================================

function rrGenId(): string {
  return `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
}

function rrCvDtoToResumePatch(cv: any): Partial<Resume> {
  const pi = cv?.personalInfo ?? {};
  const skills: ResumeSkill[] = (cv?.skills ?? []).map((s: any, i: number) => ({
    id: rrGenId(),
    resumeId: '',
    name: s.name,
    category: s.category,
    proficiencyLevel: Math.max(1, Math.min(5, Math.round((s.proficiencyLevel ?? 80) / 20))),
    isEndorsed: false,
    displayOrder: i + 1,
    isVisible: true,
  }));
  const languages: ResumeLanguage[] = (cv?.languages ?? []).map((l: any, i: number) => ({
    id: rrGenId(),
    resumeId: '',
    name: l.name,
    proficiencyLevel: l.proficiencyLevel ?? (l.isNative ? 100 : 80),
    levelLabel: l.isNative ? 'Morsmål' : 'God',
    isNative: !!l.isNative,
    displayOrder: i + 1,
    isVisible: true,
  }));
  const certifications: ResumeCertification[] = (cv?.certifications ?? []).map((c: any, i: number) => ({
    id: rrGenId(),
    resumeId: '',
    name: c.name,
    issuer: c.issuer ?? 'The Role Room',
    issueDate: c.issueDate ?? '',
    displayOrder: i + 1,
    isVisible: true,
  }));
  const projects: ResumeProject[] = (cv?.projects ?? []).map((pr: any, i: number) => ({
    id: rrGenId(),
    resumeId: '',
    title: pr.title,
    ...
    autoGenerated: !!pr.autoGenerated,
    displayOrder: i + 1,
    isVisible: true,
  }));
  return {
...
function rrMergeIntoResume(resume: Resume, patch: Partial<Resume>): Partial<Resume> {
  const nameKey = (x: { name?: string }) => String(x?.name ?? '').toLowerCase().trim();

  const existingSkills = resume.skills ?? [];
  const skillNames = new Set(existingSkills.map(nameKey));
  const skills = [...existingSkills, ...(patch.skills ?? []).filter((s) => !skillNames.has(nameKey(s)))];

  const existingLangs = resume.languages ?? [];
  const langNames = new Set(existingLangs.map(nameKey));
  const languages = [...existingLangs, ...(patch.languages ?? []).filter((l) => !langNames.has(nameKey(l)))];

  const existingCerts = resume.certifications ?? [];
  const certNames = new Set(existingCerts.map(nameKey));
  const certifications = [...existingCerts, ...(patch.certifications ?? []).filter((c) => !certNames.has(nameKey(c)))];

  const existingProjects = resume.projects ?? [];
  const projectIds = new Set(existingProjects.map((p) => p.projectId).filter(Boolean));
  const projects = [
    ...existingProjects,
    ...(patch.projects ?? []).filter((p) => !projectIds.has(p.projectId)),
  ];

  return { personalInfo: patch.personalInfo, skills, languages, certifications, projects };
}
...
export default function ResumeBuilder() {
  ...
  const [syncingRoleRoom, setSyncingRoleRoom] = useState(false);
...
<body>
  ...
  /**
   * Bygg/synkroniser CV-en fra The Role Room-profilen (sømløs integrasjon).
   *  • Ingen CV → oppretter én med Role Room-malen og profil-data.
   *  • Eksisterende CV → merger inn nye ferdigheter/språk/sert./prosjekter
   *    fra profilen, uten å slette det brukeren selv har lagt til.
   *  • Ingen ferdig onboarding-profil → veiledende melding.
   */
  const handleBuildFromRoleRoom = useCallback(async () => {
    if (!user?.id || syncingRoleRoom) return;
    setSyncingRoleRoom(true);
    try {
      const res = await apiRequest('/api/nextrole/cv/me', {
        headers: { 'x-user-id': user.id, 'Content-Type': 'application/json' },
      });
      const cv = res?.cv;
      if (!cv) {
        setSnackbar({
          open: true,
          severity: 'warning',
          message: 'Fullfør onboarding i The Role Room først — derfra bygges CV-en automatisk.',
        });
        return;
      }
      const patch = rrCvDtoToResumePatch(cv);
      if (selectedResume) {
        const merged = rrMergeIntoResume(selectedResume, patch);
        await updateResumeMutation.mutateAsync({ id: selectedResume.id, data: merged });
        setSelectedResume({ ...selectedResume, ...merged });
        setSnackbar({
          open: true,
          severity: 'success',
          message: 'CV-en er synkronisert med The Role Room-profilen — prosjekter og ferdigheter oppdatert.',
        });
      } else {
        const created = await createResumeMutation.mutateAsync({
          title: 'Min CV',
          templateId: 'role-room',
          colorScheme: 'role-room-purple',
          status: 'draft',
          language: 'no',
          ...patch,
        });
        if (created) setSelectedResume(created);
        setSnackbar({
          open: true,
          severity: 'success',
          message: 'CV-en er bygget fra The Role Room-profilen. Rediger den videre i Next Role.',
        });
      }
    } catch (error) {
      console.error('Bygg CV fra The Role Room feilet:', error);
      setSnackbar({
        open: true,
        severity: 'error',
        message: 'Kunne ikke bygge CV fra The Role Room-profilen. Prøv igjen.',
      });
    } finally {
      setSyncingRoleRoom(false);
    }
  }, [user?.id, selectedResume, updateResumeMutation, createResumeMutation, syncingRoleRoom]);
              ...
              <Box>
                ...
                >
                  <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ flexWrap: 'wrap' }}>
                    <Button
                      variant="contained"
                      disabled={syncingRoleRoom || !user?.id}
                      startIcon={syncingRoleRoom ? <CircularProgress size={20} color="inherit" /> : <GroupsIcon />}
                      onClick={handleBuildFromRoleRoom}
                      sx={{
                        background: 'linear-gradient(135deg, #1E1A2E, #2D1B4E)',
                        '&:hover': { background: 'linear-gradient(135deg, #2D1B4E, #3B2170)' },
                      }}
                      title="Bygg CV-en fra The Role Room-profilen — ferdigheter, erfaring og prosjekter synkroniseres automatisk"
                    >
                      Bygg CV fra profilen
                    </Button>
                    <Button
                      variant="contained"
