  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', md: 'repeat(6, 1fr)' }, gap: 1.5, mb: 3 }}>
        ...
        <Kpi label="Aktive" valueMain={String(aggregate?.activeCount ?? 0)} valueSub={`+${aggregate?.trialingCount ?? 0} trial`} accent="#93a4dc" />
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1.5 }}>
          ...
          <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', rowGap: 1 }}>
            ...
            <Button
              ...
              sx={{ textTransform: 'none', fontWeight: 700, color: '#c4b5fd', borderColor: 'rgba(147, 164, 220,0.5)' }}
            >
              Seed standard-rader
            </Button>
            <Button size="small" variant="contained" startIcon={<AddIcon />} onClick={() => setCostDialog({ open: true, initial: null })} sx={{ textTransform: 'none', fontWeight: 700, bgcolor: '#4b3d8f' }}>
              Ny kostnad
            </Button>
          </Stack>
        </Stack>
        ...
        {aggregate ? (
          <Stack direction="row" spacing={1.5} sx={{ flexWrap: 'wrap', rowGap: 1, mb: 2 }}>
            ...
            {Object.entries(aggregate.fixedCostsByCategoryUsd).map(([cat, amount]) => (
              <Chip key={cat} label={`${PLATFORM_COST_CATEGORY_LABELS[cat as PlatformCostCategory] ?? cat}: ${usd(amount)}`} size="small" sx={{ bgcolor: 'rgba(147, 164, 220,0.15)', color: '#c4b5fd', fontWeight: 600, fontSize: '0.72rem' }} />
            ))}
          </Stack>
        ) : null}
