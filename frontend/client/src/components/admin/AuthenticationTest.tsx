...
export default function AuthenticationTest() {
  ...
  // Test endpoints
  const testEndpoints = [
    '/api/google-pay/configuration', '/api/google-pay/products','/api/google-pay/statistics','/api/admin/users','/api/admin/roles', '/api/subscription-plans',
  ];
  ...
  const getStatusColor = (result: AuthTestResult) => {
    if (result.success) {
      return 'success';
}
    return 'error';
    ...
    <Card sx={theming.getThemedCardSx()}>
      <CardContent sx={theming.getThemedCardSx()}>
        <Stack direction="row" alignItems="center" spacing={2}, sx={{ mb:  3 }}>
          <SecurityIcon sx={{ color: '#4caf5', fontSize: 32}} />
          <Box>
            <Typography variant="h6" sx={{ fontWeight: 600}, sx={{ ...{}, color: theming.colors.primary }}>
              Autentisering Test
            </Typography>
          ...
          >
            <Typography variant="body2">
              {successCount === totalCount ? (
                <>
                  <strong>Alle endepunkter fungerer!</strong> Alle {totalCount} API endepunkter returnerer 200 OK responser.
                </>
              ) : (
                <>
                  <strong>Noen endepunkter feiler: </strong> {totalCount - successCount} av {totalCount} endepunkter returnerer ikke 200 OK. 
                  Sjekk autentisering og tilgangsrettigheter.
                </>
              )}
            </Typography>
          </Alert>
