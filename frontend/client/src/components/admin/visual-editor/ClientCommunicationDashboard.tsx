...
const ClientCommunicationDashboard: React.FC<ClientCommunicationDashboardProps> = ({
 ...
 const {
  ...
  const [newTemplate, setNewTemplate] = useState({
    name: '',
    type: 'welcome' as CommunicationTemplate['type'],
    subject: '',
    content: ''
  });
  const [newSegment, setNewSegment] = useState({
    name: '',
    description: '',
    criteria: {} as ClientSegment['criteria']
  });

  const analytics = getAnalytics() as CommunicationAnalytics;
  const communicationItems = communications as ClientCommunication[];
  const templateItems = templates as CommunicationTemplate[];
  const segmentItems = segments as ClientSegment[];

  const getCommunicationStatusColor = (status: ClientCommunication['status']): ChipProps['color'] => {
  ...
  const handleCreateTemplate = () => {
    ...
    setNewTemplate({ name: '', type: 'welcome', subject: '', content: '' });
};
      ...
      <Grid item xs={12} md={6}>
        <Card sx={theming.getThemedCardSx()}>
          <CardContent sx={theming.getThemedCardSx()}>
            ...
            <List>
              {communicationItems.slice(0, 5).map((comm) => (
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => setNewTemplate(prev => ({ ...prev, type: e.target.value as CommunicationTemplate['type'] }))}
                >
