...
const FilesTab: React.FC<FilesTabProps> = ({ 
  ...
  // Zero Toast Compliance - using console logging instead of toasts
  ...
  const syncFoldersMutation = useMutation({
    mutationFn: () => apiRequest('/api/google-drive/sync', {
      headers: {
  ...
  const trackFolderChangesMutation = useMutation({
    mutationFn: () => apiRequest('/api/google-drive/track-folder-changes', {
      headers: {
        "Content-Type" : "application/json"
    ,},
      method: 'POS',
    ...
    onSuccess: (data) => {
      if (data.validation && !data.validation.isValid) {
        console.log(`📝 Mappeendringer oppdaget: ${data.validation.deviations?.length || 0} avvik fra standard`);
  ...
  const undoRedoMutation = useMutation({
    mutationFn: () => apiRequest('/api/google-drive/undo-folder-changes', {
      headers: {
        "Content-Type" : "application/json"
    ,},
      method: 'POS',
    ...
    onSuccess: (data) => {
      console.log(`${data.result?.action === 'undo' ? '↩️ Angret' : '↪️ Gjentatt'} ${data.result?.changesReverted || 0} endringer`);
      queryClient.invalidateQueries({ queryKey: ['/api/google-drive/folders', ],});
}
});
  ...
  const restoreStandardMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setTimeout(() => {
        ...
        setChangeHistory(null); // Clear change history after restoration
  ...
  // Backup current structure mutation
  const backupStructureMutation = useMutation({
    mutationFn: () => apiRequest('/api/google-drive/backup-folder-structure', {
      headers: {
        "Content-Type" : "application/json"
    ,},
      method: 'POS',
    ...
    onSuccess: (data) => {
      console.log(`💾 Backup opprettet: ${data.backup?.folderStructure?.length || 0} mapper sikkerhetskopiert`);
}
});

  // Custom folder creation removed - enforces system standardization
  ...
  // Get profession config with dynamic branding (auto-scalable)
  const getProfessionConfig = () => {
    ...
    // Fallback to local configs if dynamic data not available (will be overridden by dynamic profession data)
    const fallbackConfigs: Record<string, { color: string; label: string }> = {
      photographer: { color: '#ff8c00', label: 'Fotograf' }, // Fallback - overridden by dynamic profession data
      videographer: { color: '#e53e3e', label: 'Videograf' }, // Fallback - overridden by dynamic profession data
      music_producer: { color: '#9f7aea', label: 'Musikkprodusent' },
      vendor: { color: '#27ae60', label: 'Leverandør' },
    };
    
    const fallback = fallbackConfigs[profession] || fallbackConfigs.photographer;
    
    return {
      color: professionColor || fallback.color,
      label: displayName || fallback.label,
    };
};
  ...
  return (
    ...
    ,  ...theming.getThemedCardSx() }}>
        ...
        <Box sx={{ p:  3, pt:  2 }}>
            ...
            {(folderSettings?.customStructure || [
              ...
              <Grid size={{ xs: 12 }} sm={6} md={4} key={index}>
                <Box sx={{ 
                  ...
                  <FolderOpen sx={{ fontSize:  16, color: '#1a73e8' }} />
          ...
          {changeHistory && (
            <Box sx={{ 
              ...
              <Box sx={{ 
                ...
                <Search sx={{ fontSize:  16, color: '#4caf50' }} />
              </Box>
              <Box sx={{ flex:  1 }}>
                <Typography variant="subtitle2" sx={{ 
                  fontWeight: 600,
          ...
          <Box sx={{ 
            ...
            <Box sx={{ 
              p: 0,
              borderRadius:  1,
              bgcolor: 'rgba(25, 1240.2)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              mt: 0.5
          }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="#f57c00">
        ...
        <Grid container spacing={3}>
          {folders?.map((folder: DriveFolder) => (
            <Grid size={{ xs: 12 }} sm={6} md={4} key={folder.id}>
              <MuiCard sx={{ 
                height: '100%',
                transition: 'transform 0.2','&:hover': { transform: 'translateY(-4px)' },
                border: `1px solid ${config.color}20`
          }}>
                <CardContent sx={theming.getThemedCardSx()}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                    <FolderOpen sx={{ color: config.color, fontSize: 32}} />
                    <Box sx={{ flexGrow:  1 }}>
                      <Typography variant="h6" sx={{  fontWeight: 600}}>
                        {folder.name}
                      </Typography>
      ...
      {/* Gallery Management Section - Only for photographers (photographer-specific feature) */}
      {profession === 'photographer' && (
        <Box sx={{ mt:  4 }}>
          <Paper sx={{ 
            p:  0,
            borderRadius:  3,
            background: 'linear-gradient(135deg, rgba(1, 5, 6, 39, 176, 0.1) 0%, rgba(1, 5, 6, 39, 176, 0.05) 100%)',
            border: '1px solid rgba(16, 39, 176, 0.2)',
            overflow: 'hidden'
        ,  ...theming.getThemedCardSx() }}>
            {/* Header */}
            <Box sx={{ 
            ...
            <Box sx={{ p:  3, pt:  2 }}>
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12 }} sm={4}>
                  <Box sx={{ 
                    p: 2, borderRadius:  2,
                    bgcolor: 'rgba(16, 39, 176, 0.1)',
                    border: '1px solid rgba(16, 39, 176, 0.1)',
                    textAlign: 'center'
                }}>
              ...
              <List>
                {[
                  { name: 'Bryllup - Anna & Ole', status: 'Aktiv', views: 127,},
                  { name: 'Bedriftsfotografering - TechCorp', status: 'Fullført', views: 89,},
                  { name: 'Familie Hansen', status: 'Utkast', views:  0 }
                ].map((gallery, index) => (
                  <ListItem key={index} sx={{ 
                    borderRadius:  2,
                    mb:  1,
                    bgcolor: 'rgba(16, 39, 176, 0.05)',
                    border: '1px solid rgba(16, 39, 176, 0.1)'
              }}>
                    <ListItemIcon>
                      <PhotoLibrary sx={{ color: '#9c27b0' }} />
                    </ListItemIcon>
                    <ListItemText
        ...
        <DialogTitle sx={{ 
          ...
          {!isRestoring && !restoreComplete && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap:  1 }}>
              ...
              <RestoreFromTrash sx={{ fontSize: 20}} />
            </Box>
          )}
          {isRestoring && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap:  1 }}>
          ...
          {!isRestoring && !restoreComplete && (
            <>
              ...
              <Alert 
                ...
                sx={{ 
                  mb:  3,
                  backgroundColor: 'rgba(3, 150, 243, 0.08)',
                  border: '1px solid rgba(3, 150, 243, 0.15)',
                  borderRadius: 2
              }}
                icon={<Lightbulb sx={{ color: '#1976d2' }} />}
              >
                <AlertTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, fontWeight: 700}}>
              ...
              <Box sx={{ 
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                  <Construction sx={{ color: '#f57c00', fontSize: 28}} />
              ...
              <Box sx={{ 
                ...
                <Typography variant="h6" sx={{  
                  ...
                  <AutoFixHigh sx={{ color: config.color }} />
                ...
                <Grid container spacing={1.5}>
                  {[
                    ...
                    const iconComponents: { [key: string]: any } = {
                      PhotoCamera: <PhotoCamera sx={{ fontSize: 18, color: config.color }} />,
                      Palette: <Palette sx={{ fontSize: 18, color: config.color }} />,
                      LocalShipping: <LocalShipping sx={{ fontSize: 18, color: config.color }} />,
                      Assignment: <Assignment sx={{ fontSize: 18, color: config.color }} />,
                      Chat: <Chat sx={{ fontSize: 18, color: config.color }} />,
                      EventNote: <EventNote sx={{ fontSize: 18, color: config.color }} />,
                      Security: <Security sx={{ fontSize: 18, color: config.color }} />,
                      AutoFixHigh: <AutoFixHigh sx={{ fontSize: 18, color: config.color }} />
                };
        ...
        {!isRestoring && !restoreComplete && (
          <DialogActions sx={{ p:  3, gap:  2 }}>
            ...
            <Button onClick={handleConfirmedRestore}
              ...
              startIcon={<RestoreFromTrash />}
              sx={{ 
                background: 'linear-gradient(90deg, #f57c00 0%, #ff9800 100%)','&:hover': {
