...
const WarmupDialog: React.FC<{ open: boolean; projectId: string; onClose: () => void }> = ({ open, projectId, onClose }) => {
  ...
  const delSound = async (id: string) => { try { await apiRequest(`/api/audio-showcase/my-sounds/${id}`, { method: 'DELETE' }); } catch { /* */ } await loadSounds(); };
  const saveTemplate = async () => {
    if (!title.trim() || steps.length === 0) return; setSavingTpl(true);
    try { await apiRequest('/api/audio-showcase/warmup-templates', { method: 'POST', body: { title: title.trim(), target, note: note.trim() || undefined, steps } }); await loadTemplates(); }
    catch { /* */ } finally { setSavingTpl(false); }
  };
  const delTemplate = async (id: string) => { try { await apiRequest(`/api/audio-showcase/warmup-templates/${id}`, { method: 'DELETE' }); } catch { /* */ } await loadTemplates(); };
              ...
              <Stack spacing={0.5}>
                ...
                {steps.filter((s: any) => String(s.type || '').startsWith('custom')).map((s: any) => (
                  <Stack key={s.sourceId} direction="row" alignItems="center" spacing={1} sx={{ bgcolor: 'rgba(255,255,255,0.04)', borderRadius: '8px', px: 1, py: 0.5 }}>
                    ...
                    {s.audioUrl && <IconButton size="small" onClick={() => togglePreview(s.audioUrl)} sx={{ color: previewUrl === s.audioUrl ? ACCENT : FAINT, p: 0.25 }}>{previewUrl === s.audioUrl ? <Stop sx={{ fontSize: 15 }} /> : <PlayArrow sx={{ fontSize: 15 }} />}</IconButton>}
                    ...
                    {sounds.map((snd) => {
                      ...
                      return (
                        <Stack key={snd.id} direction="row" alignItems="center" spacing={1} sx={{ px: 0.75, py: 0.4, borderRadius: '8px', '&:hover': { bgcolor: 'rgba(255,255,255,0.04)' } }}>
                          ...
                          <IconButton size="small" onClick={() => togglePreview(snd.url)} sx={{ color: previewUrl === snd.url ? ACCENT : FAINT, p: 0.25 }}>{previewUrl === snd.url ? <Stop sx={{ fontSize: 15 }} /> : <PlayArrow sx={{ fontSize: 15 }} />}</IconButton>
                          <IconButton size="small" onClick={() => addSoundAsStep(snd)} disabled={inRoutine} sx={{ color: inRoutine ? GREEN : ACCENT, p: 0.25 }}>{inRoutine ? <CheckCircle sx={{ fontSize: 15 }} /> : <Add sx={{ fontSize: 15 }} />}</IconButton>
                          <IconButton size="small" onClick={() => delSound(snd.id)} sx={{ color: FAINT, p: 0.25 }}><DeleteOutline sx={{ fontSize: 15 }} /></IconButton>
                        </Stack>
                      );
                    })}
                  </Box>
                ...
                <>
                  ...
                  {templates.map((t) => (
                    <Stack key={t.id} direction="row" alignItems="center" spacing={0.5} sx={{ bgcolor: 'rgba(255,255,255,0.04)', borderRadius: '10px', px: 1.25, py: 0.75 }}>
                      ...
                      <Button size="small" onClick={() => applyTemplate(t)} sx={{ color: ACCENT, textTransform: 'none', fontSize: '0.68rem', minWidth: 0, px: 0.75 }}>Bruk</Button>
                      <IconButton size="small" onClick={() => delTemplate(t.id)} sx={{ color: FAINT, p: 0.25 }}><DeleteOutline sx={{ fontSize: 15 }} /></IconButton>
                    </Stack>
                  ))}
