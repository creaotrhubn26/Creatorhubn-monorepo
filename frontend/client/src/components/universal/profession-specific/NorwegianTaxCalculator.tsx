...
const EXPENSE_CATEGORIES: { [key: string]: ExpenseCategory[, ],} = {
  photographer: [
    {
      ...
      beskrivelse: 'Kameraer, objektiver, blitz',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Datautstyr',
      beskrivelse: 'C, skjermer, lagring',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Programvare',
      beskrivelse: 'Lightroom, Photoshop, Capture One',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Reise',
      beskrivelse: 'Transport til oppdrag',
      maksGradFradrag: 10,
      MVAFradragsrett: false,
  },
    {
      kategori: 'Markedsføring',
      beskrivelse: 'Annonser, nettsider',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Kontor',
      beskrivelse: 'Leie, strøm, telefon',
      maksGradFradrag:  50,
      MVAFradragsrett: true,
  },
  ],
  videographer: [
    {
      kategori: 'Videoutstyr',
      beskrivelse: 'Kameraer, objektiver, lyd',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Redigeringsutstyr',
      beskrivelse: 'C, programvare, lagring',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Produksjonsutstyr',
      beskrivelse: 'L, ysstativer, tilbehør',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Musikklisenser',
      beskrivelse: 'TOO, GRAMO, bibliotek',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
  ...
  musicproducer: [
    {
      kategori: 'Studioutstyr',
      beskrivelse: 'Mikrofoner, lydkort, monitorer',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Instrumenter',
      beskrivelse: 'Keyboard, gitar, synthesizere',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Programvare',
      beskrivelse: 'DW, plugins, bibliotek',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Studiorom',
      beskrivelse: 'Leie, akustikk, strøm',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'TONO/GRAM',
      beskrivelse: 'Medlemskap og avgifter',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Markedsføring',
      beskrivelse: 'Promotion, streaming',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
  ],
  vendor: [
    {
      kategori: 'Varelager',
      beskrivelse: 'Innkjøp av produkter',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Lagring',
      beskrivelse: 'Lager, hyller, sikkerhet',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Transport',
      beskrivelse: 'Frakt, levering, bil',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
      kategori: 'Markedsføring',
      beskrivelse: 'Annonser, messer, brosjyrer',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
    {
    ...
    {
      kategori: 'Forsikring',
      beskrivelse: 'Produkt- og yrkesansvar',
      maksGradFradrag: 10,
      MVAFradragsrett: true,
  },
  ],
...
export default function NorwegianTaxCalculator({
  ...
  const [externalTaxRates, setExternalTaxRates] = useState<any>(null);
  const [externalFuelPrices, setExternalFuelPrices] = useState<any>(null);
  const queryClient = useQueryClient();
  
  // Theming system
  const theming = useTheming('photographer');
  
  // External data service for tax rates and fuel prices
  const { getTaxRates, getFuelPrices } = useExternalData();
  
  // Fetch external tax and fuel data
  const fetchExternalData = async () => {
    try {
      const [taxRates, fuelPrices] = await Promise.all([
        getTaxRates(),
        getFuelPrices()
      ]);
  ...
  const performTaxCalculation = async () => {
    ...
    try {
      ...
      // Use real MVA rates from external data or fallback to standard rates
      const mvaRate25 = externalTaxRates?.mva_standard_rate || 25;
      const mvaRate15 = externalTaxRates?.mva_reduced_rate || 15;
      const mvaRate12 = externalTaxRates?.mva_food_rate || 12;
      
      // Calculate MVA breakdown
      const mva25 = invoices.reduce((sum, invoice) => {
        if (invoice.mvaSats === mvaRate25 && invoice.status === 'paid') {
          return sum + calculateMVA(invoice.belop, mvaRate25);
      }
        return sum;
    }, 0);

      const mva15 = invoices.reduce((sum, invoice) => {
        if (invoice.mvaSats === mvaRate15 && invoice.status === 'paid') {
          return sum + calculateMVA(invoice.belop, mvaRate15);
      }
        return sum;
    }, 0);

      const mva12 = invoices.reduce((sum, invoice) => {
        if (invoice.mvaSats === mvaRate12 && invoice.status === 'paid') {
          return sum + calculateMVA(invoice.belop, mvaRate12);
      }
        return sum;
    }, 0);
      ...
      const expenseMVA = expenses.reduce((sum, expense) => {
        const category = EXPENSE_CATEGORIES[profession].find(
          (cat) => cat.kategori === expense.kategori,
        );
      ...
      // Use real tax rate from external data or fallback to 22%
      const incomeTaxRate = externalTaxRates?.income_tax_rate || 0.22;
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell align="right">
                    {currentCalculation.omsetning.toLocaleString('no-NO')}
                  </TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>- MVA {externalTaxRates?.mva_standard_rate || 25}%</TableCell>
                  <TableCell align="right" sx={{ color: 'error.main' }}>
                    -{currentCalculation.mva25.toLocaleString('no-NO')}
                  </TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>- MVA {externalTaxRates?.mva_reduced_rate || 15}%</TableCell>
                  <TableCell align="right" sx={{ color: 'error.main' }}>
                    -{currentCalculation.mva15.toLocaleString('no-NO')}
                  </TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>- MVA {externalTaxRates?.mva_food_rate || 12}%</TableCell>
                  <TableCell align="right" sx={{ color: 'error.main' }}>
            ...
            {EXPENSE_CATEGORIES[profession].map((category, index) => (
              ...
              >
                <ListItemIcon>
                  <CheckCircle
                    sx={{
                      color: category.MVAFradragsrett ? 'success.main' : 'warning.main' }}
                  />
                </ListItemIcon>
                <ListItemText
                  ...
                  secondary={
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        Fradrag: {category.maksGradFradrag}% • MVA-fradrag: {''}
                        {category.MVAFradragsrett ? 'Ja' : 'Nei'}
                      </Typography>
                    </Box>
                }
                />
              ...
              <TableBody>
                {expenses.map((expense, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell align="right">
                      {calculateMVA(expense.belop, expense.mvaSats).toLocaleString('no-NO')} NOK
                    </TableCell>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={6}>
          ...
          {calculations.length > 0 ? (
            ...
            <Alert severity="info">
              <Typography variant="body2">Ingen beregninger utført ennå</Typography>
            </Alert>
          )}
        </Grid>
      </Grid>
        ...
        <Grid item xs={12} md={6}>
          ...
          <List>
            ...
            >
              ...
              <ListItemText primary="MVA-oppgave Q4 2024" secondary="Frist: 31. januar 2025" />
            </ListItem>
            ...
            >
              ...
              <ListItemText primary="Årsregnskap 2024" secondary="Frist: 31. mai 2025" />
            </ListItem>
            ...
            >
              ...
              <ListItemText primary="MVA-oppgave Q3 2024" secondary="Levert: 28. oktober 2024" />
            </ListItem>
          </List>
        </Grid>
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap:  2 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  MVA-beregning og skatteadministrasjon for{', '}
                  {profession === 'photographer'
                    ? 'fotografer'
                    : profession === 'videographer'
                      ? 'videografer'
            ...
            <Box sx={{ display: 'flex', gap:  1 }}>
              <Chip icon={<Euro />} label="NOK" color="primary" size="small" />
              <Chip icon={<AccountBalance />} label="Skatteetaten" color="success" size="small" />
            </Box>
          </Box>
