      ...
      if (response.ok) {
        ...
        return {
          ...
          crossComponent: {
            ...
            connectedComponents: [
              'google_meet', 'project_admin', 'wedding_timeline', 'document_manager', 'client_portal',
            ],
        },
      };
    }
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap:  2 }}>
            ...
            <Box>
              ...
              <Typography variant="subtitle1" sx={{ color: 'white', opacity: 0.9}}>
                {getDynamicProfessionName(profession)} • Google Meet ↔ Project Administration
              </Typography>
            </Box>
          </Box>
          <Box sx={{ display: 'flex', alignItems: 'center', gap:  2 }}>
            <Chip
              ...
              color={getIntegrationStatusColor(
                integration?.crossComponent.synchronization || 'syncing',
              )}
              icon={<SyncIcon />}
      ...
      <Grid container spacing={3} sx={{ mb:  3 }}>
          ...
          <MuiCard>
            <CardContent sx={theming.getThemedCardSx()}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                <Badge badgeContent={integration?.googleMeet.activeMeetings || 0} color="error">
        ...
        <Grid item xs={12} md={4}>
          <MuiCard>
            <CardContent sx={theming.getThemedCardSx()}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                <Badge badgeContent={integration?.projectAdmin.pendingTasks || 0} color="warning">
                  <ProjectIcon
                    color={integration?.projectAdmin.connected ? 'primary' : 'disabled'}
                  />
                </Badge>
                  ...
                  secondary={
                    <Box>
                      <Typography variant="body2">
                        {project.clientName} • {project.documents} dokumenter • {project.contracts}{', '}
                        kontrakter
                      </Typography>
                      <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                        <Chip
                          ...
                          color={getIntegrationStatusColor(project.status)}
                        />
                        {project.timeline && (
                          <Chip
                            size="small"
            ...
            <List>
              {notifications.slice(0, 5).map((notification, index) => (
                <ListItem key={index} sx={{ backgroundColor: 'grey.5', borderRadius: 1, mb:  1 }}>
                  <ListItemIcon>
                    {notification.metadata?.crossSync ? (
