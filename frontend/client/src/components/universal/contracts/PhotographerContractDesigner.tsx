...
export default function UniversalContractDesigner({
  ...
  const backupContractMutation = useMutation({
    mutationFn: (contractId: string) =>
      apiRequest(`/api/contracts/${contractd}/backup-drive`, {
        method: 'POS' }),
    onSuccess: (response) => {
          ...
          <Grid container spacing={2}>
            {steps.map((step, index) => (
              <Grid item xs={12} sm={3} key={index}>
                <Card
                  sx={{
                    border: activeStep  sx={theming.getThemedCardSx()}>= index ? '2px solid #4caf50' : '1px solid #e0e0e0',
                ...
                >
                  <CardContent sx={{ textAlign: 'center', py:  2 ,  ...theming.getThemedCardSx() }}>
                    ...
                    {activeStep > index && <CheckCircleIcon sx={{ color: '#4caf50', mt:  1 }} />}
                  </CardContent>
                </Card>
              </Grid>
            ))}
                ...
                <Box>
                  <Box
                    sx={{
                      ...
                      mb:  3}}
                  >
                    <Typography variant="h6" sx={{ color: theming.colors.primary }}>Administrer Kontraktseksjoner</Typography>
                    <Box>
                      <input
                        accept=".pdf,.doc,.docx"
                        style={{ display: 'none' }}
                      ...
                      <Accordion key={section.id} sx={{ mb:  1 }}>
                        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                          ...
                          >
                            ...
                            />
                            {section.required && (
                              <Chip label="Påkrevd" size="small" color="error" sx={{ mr:  2 }} />
                            )}
                            <IconButton
                              onClick={(e) => {
                                e.stopPropagation();
              ...
              {activeStep === 3 && (
                <Box>
                  ...
                  <Alert severity="info" sx={{ mb:  2 }}>
                    Kontrakten er opprettet og klar for gjennomgang. Send til klient for
                    godkjenning.
                  </Alert>
                  <Button variant="contained"
                    startIcon={<EmailIcon />}
              ...
              {activeStep === 4 && (
                <Box>
                  ...
                  <TextField
                    ...
                    sx={{ mb:  2 }}
                  />
                  <Button variant="contained" startIcon={<CheckCircleIcon />} sx={{ mt:  2 }}>
                    Fullfør Kontrakt
                  </Button>
                </Box>
              )}
              ...
              <Paper sx={{ p:  3, bgcolor: '#fafafa', minHeight: 400,  ...theming.getThemedCardSx() }}>
                ...
                <Typography variant="body2" gutterBottom>
                  ...
                  {contractData.totalAmount
                    ? `${contractData.totalAmount} NOK (inkl. 25% MVA)`
                    : 'Ikke utfylt'}
                </Typography>
      ...
      <FormControl fullWidth sx={{ mb:  2 }}>
        ...
        <Select
          ...
          onChange={(e) =>
            setFormData({
              ...formData,
              type: e.target.value as ContractSection[''],
          })
        }
        >
