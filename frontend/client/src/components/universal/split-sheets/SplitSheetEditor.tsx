...
export default function SplitSheetEditor({
  ...
  const [title, setTitle] = useState(splitSheet?.title || '');
  ...
  const { getUserProfessionColor, getProfessionDisplayName, getProfessionCategory } = useDynamicProfessions();
  ...
  const getAvailableRoles = (prof: string): ContributorRole[] => {
    switch(prof) {
      ...
      case 'videographer':
        return [
          'video_editor','colorist','sound_engineer','cinematographer','grip','gaffer','collaborator','other'
        ];
      default:
                  ...
                  {percentageError && (
                    <Box sx={{ mt: 1 }}>
                      <Typography variant="caption" sx={{ display: 'block', mb: 1 }}>
                        ...
                        {totalPercentage > 100 
                          ? `Totalen er ${totalPercentage.toFixed(2)}% - må være nøyaktig 100% (${(totalPercentage - 100).toFixed(2)}% for mye)`
                          : `Totalen er ${totalPercentage.toFixed(2)}% - må være nøyaktig 100% (${(100 - totalPercentage).toFixed(2)}% mangler)`
                        }
                      </Typography>
                            ...
                            <AccordionDetails>
                              <Grid container spacing={2}>
                                <Grid item xs={12} sm={6}>
                                  <TextField
                                    label="Telefon"
                                    value={contributor.custom_fields?.phone || ''}
                                    onChange={(e) => handleCustomFieldChange(index, 'phone', e.target.value)}
                                    fullWidth
                                    size="small"
                                    InputProps={{
                                      startAdornment: <Phone sx={{ mr: 1, fontSize: 18, color: 'text.secondary' }} />
                                    }}
                                  />
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                  <TextField
                                    label="PRO (f.eks. TONO, STIM)"
                                    value={contributor.custom_fields?.pro || ''}
                                    onChange={(e) => handleCustomFieldChange(index, 'pro', e.target.value)}
                                    fullWidth
                                    size="small"
                                    InputProps={{
                                      startAdornment: <Business sx={{ mr: 1, fontSize: 18, color: 'text.secondary' }} />
                                    }}
                                  />
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                  <TextField
                                    label="IPI-nummer"
                                    value={contributor.custom_fields?.ipi_number || ''}
                                    onChange={(e) => handleCustomFieldChange(index, 'ipi_number', e.target.value)}
                                    fullWidth
                                    size="small"
                                    InputProps={{
                                      startAdornment: <Badge sx={{ mr: 1, fontSize: 18, color: 'text.secondary' }} />
                                    }}
                                  />
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                  <TextField
                                    ...
                                    value={contributor.custom_fields?.organization_number || ''}
                                    onChange={(e) => handleCustomFieldChange(index, 'organization_number', e.target.value)}
                                    fullWidth
                                    size="small"
                                  />
                                </Grid>
                                ...
                                <Grid item xs={12}>
                                  <TextField
                                    ...
                                    multiline
                                    rows={2}
                                    placeholder="Ekstra notater om bidragsyteren..."
                                  />
                                </Grid>
                              </Grid>
                            </AccordionDetails>
