...
export interface HomeKpi {
  key: 'scenes' | 'gates' | 'tasks' | 'reviews' | 'lines' | 'platform' | 'guardian' | 'playtest';
  label: string;
  value: string;
  sub: string;
  pct: number | null;
...
export function homeKpis(o: NarrativeProjectOverview): HomeKpi[] {
  ...
  return [
    ...
    // Fase 8e: spilltest siste 7 dager — økter + scenen flest økter slutter i.
    { key: 'playtest', label: 'Spilltest', value: o.playtest ? String(o.playtest.sessions7d) : '–', sub: o.playtest ? (o.playtest.sessions7d ? (o.playtest.worstDropOff ? `økter siste 7 d · verste drop-off: ${o.playtest.worstDropOff.sceneCode} (${o.playtest.worstDropOff.sessions})` : 'økter siste 7 d · alle fullførte') : 'ingen økter siste 7 d') : 'ikke koblet', pct: null, tab: 'scenes', tone: o.playtest?.worstDropOff ? 'warning' : 'neutral' },
