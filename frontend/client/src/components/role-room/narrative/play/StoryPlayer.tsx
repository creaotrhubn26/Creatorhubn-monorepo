/**
 * StoryPlayer — presentasjonell spiller: ett element per tur, valg, TTS,
 * valgfri debugger og valgfri «Rediger element». Brukes av Play-fanen
 * (NarrativePlayPanel) og av den offentlige delingssiden /story/:token.
 */
...
export interface StoryPlayerProps {
  ...
  /** Når satt: «Rediger element»-knapp (kun i arbeidsflaten). */
  onEditElement?: (elementId: string) => void;
  /** Vis debugger-panelet (variabler, besøk, logg). */
...
export function StoryPlayer({ graph, onEditElement, showDebugger = true, minHeight = 'calc(100vh - 150px)', initialLocale = null }: StoryPlayerProps) {
  ...
  return (
    <Box data-testid="narrative-play-panel" sx={{ display: 'flex', minHeight, color: narrativeColors.text }}>
      ...
      {showDebugger ? (
        <Box sx={{ width: 320, flex: '0 0 320px', borderLeft: `1px solid ${narrativeColors.borderStrong}`, bgcolor: narrativeColors.bgPanel, display: { xs: 'none', md: 'block' } }} data-testid="narrative-play-debugger">
