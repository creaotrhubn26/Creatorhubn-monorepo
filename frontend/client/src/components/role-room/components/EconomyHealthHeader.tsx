  ...
  return (
    ...
    >
      ...
      <Box sx={{ mt: 1.5, display: 'grid', gap: 1.5, gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' } }}>
        ...
        <Metric label="Brukt" value={formatKr(spent, currency)} sub={hasRamme ? `${pct}% av ramme` : 'faktisk forbruk'} valueColor={overBudget ? '#fca5a5' : '#f7f9ff'} />
...
function Metric({ label, value, sub, valueColor = '#f7f9ff' }: { label: string; value: string; sub: string; valueColor?: string }) {
  return (
