  ...
  return (
    ...
    >
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
        <CircularProgress size={16} sx={{ color: 'var(--role-cyan, #5d76cb)' }} />
        <Typography sx={{ color: '#c3cbe6', fontWeight: 800, fontSize: '0.9rem' }}>
          {title}
        </Typography>
      </Stack>
      <LinearProgress sx={{ height: 2, mb: 1.2, bgcolor: 'rgba(93, 118, 203,0.12)', '& .MuiLinearProgress-bar': { bgcolor: 'var(--role-cyan, #5d76cb)' } }} />
      <Stack spacing={0.55}>
          ...
          return (
            <Stack key={stage} direction="row" spacing={0.8} alignItems="center">
              {isDone ? (
                ...
                <CircularProgress size={12} sx={{ color: 'var(--role-cyan, #5d76cb)' }} />
              ) : (
