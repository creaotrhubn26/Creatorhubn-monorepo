...
function HeadlinerIcon({ size, style, title }: { size: number; style?: CSSProperties; title: string }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" role="img" aria-label={title} style={style}>
      <title>{title}</title>
      {/* Marquee frame */}
      <rect x="8" y="22" width="48" height="26" rx="4" fill="#0f172a" stroke="#7666e6" strokeWidth="2" />
      {/* Inner frame */}
      <rect x="11" y="25" width="42" height="20" rx="2" fill="none" stroke="#7666e6" strokeWidth="1" opacity="0.5" />
      {/* Bottom stand */}
      <rect x="28" y="48" width="8" height="8" fill="#7666e6" opacity="0.75" />
      <rect x="22" y="56" width="20" height="3" rx="1" fill="#7666e6" />
      {/* Headline text proxy */}
      <rect x="16" y="31" width="32" height="3" rx="1" fill="#7666e6" opacity="0.9" />
      <rect x="18" y="37" width="28" height="2.5" rx="1" fill="#7666e6" opacity="0.55" />
      {/* Top bulbs with glow */}
