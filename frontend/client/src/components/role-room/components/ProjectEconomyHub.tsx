    ...
    <Stack spacing={2}>
        ...
        <Stack spacing={1.5}>
          <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1} justifyContent="space-between" alignItems={{ lg: 'center' }}>
            <Box>
              ...
              <Typography sx={{ color: 'rgba(226,232,240,0.78)', maxWidth: 760 }}>
                Samler prosjektramme, fasebudsjetter, teamkostnader, tilbud, kontrakter og budsjettgodkjenning i ett arbeidsområde.
              </Typography>
            </Box>
            <Stack direction="row" spacing={1} flexWrap="wrap">
              <Chip size="small" label={`Prosjekt ${project.name}`} sx={{ bgcolor: 'rgba(59,130,246,0.18)', color: '#93c5fd' }} />
              <Chip size="small" label={`Valuta ${currency}`} sx={{ bgcolor: 'rgba(52,211,153,0.16)', color: '#86efac' }} />
            </Stack>
          </Stack>
          ...
          >
            {[
              {
                label: 'Prosjektramme',
                value: projectBudget > 0 ? formatCurrency(projectBudget, currency) : 'Ikke satt',
                detail: projectBudget > 0
                  ? `Faseestimat ${formatCurrency(phaseEstimateTotal, currency)}`
                  : 'Sett totalramme før du begynner å finjustere kostnadsfaser.',
                icon: <AttachMoneyIcon sx={{ color: '#fbbf24' }} />,
              },
              {
                label: 'Teamkostnad',
                value: formatCurrency(projectedCrewTotal, currency),
                detail: `${billableCrewMembers.length} med dagsats · ${plannedShootDays} opptaksdager`,
                icon: <GroupsIcon sx={{ color: '#60a5fa' }} />,
              },
              {
                label: 'Reise og logistikk',
                value: totalTravelCosts > 0 ? formatCurrency(totalTravelCosts, currency) : 'Ikke registrert',
                detail: totalTravelCosts > 0
                  ? `${normalizedTravelCosts.length} crewprofiler · ${travelDayCount} reisedager`
                  : 'Ingen reisekostnader er synkronisert til budsjett ennå.',
                icon: <SyncIcon sx={{ color: '#f97316' }} />,
              },
	              {
	                label: 'Klientstatus',
	                value: getProjectWorkflowStatusLabel(project.producerWorkflowStatus),
	                detail: projectWorkflowDetail,
	                icon: <FactCheckIcon sx={{ color: '#c084fc' }} />,
	              },
	              {
	                label: 'Tilbud og kontrakter',
	                value: projectAgreements.length > 0
	                  ? `${projectAgreements.length} juridiske avtaler`
	                  : `${syncedCrewItems.length + syncedTravelItems.length} synkede linjer`,
	                detail: projectAgreements.length > 0
	                  ? legalFlowSummary
	                  : `Godkjent ${formatCurrency(toNumber(totals.approved), currency)} · Faktisk ${formatCurrency(toNumber(totals.actual), currency)}`,
	                icon: <SyncIcon sx={{ color: '#34d399' }} />,
	              },
              {
                label: 'Produksjonsomfang',
                value: primaryFormatEstimate ? formatSeconds(primaryFormatEstimate.estimatedSeconds) : 'Ikke beregnet',
                detail: `${shotLists.length} shotlister · ${productionEstimate.formatEstimates.length} leveranseformat${productionEstimate.formatEstimates.length === 1 ? '' : 'er'}`,
                icon: <FactCheckIcon sx={{ color: '#38bdf8' }} />,
              },
              {
                label: 'Opptaksbelastning',
                value: formatMinutes(productionEstimate.totalRealisticFieldMinutes),
                detail: `${productionEstimate.suggestedShootDays} foreslåtte opptaksdager · ${productionEstimate.overloadedDayCount} overbooket`,
              ...
              >
                <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 0.75 }}>
                  {card.icon}
                  <Typography sx={{ color: 'rgba(226,232,240,0.82)', fontSize: '0.82rem', fontWeight: 700 }}>
                    {card.label}
                  </Typography>
                </Stack>
                <Typography sx={{ color: '#fff', fontSize: { xs: '1.1rem', lg: '1.25rem' }, fontWeight: 800 }}>
                  {card.value}
                </Typography>
                <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.4 }}>
                  {card.detail}
                </Typography>
              </Box>
            ))}
      ...
      >
        ...
        >
          ...
          <Tab value="budget" label="Budsjett og avtaler" />
          <Tab value="approvals" label="Godkjenning og milepæler" />
        </Tabs>
          ...
          {activeView === 'overview' ? (
            <Box
              sx={{
                ...
                gridTemplateColumns: {
                  xs: '1fr',
                  xl: 'minmax(0, 1.55fr) minmax(300px, 0.95fr)',
                },
                gap: 2,
                alignItems: 'start',
              }}
              ...
              <Stack spacing={2}>
                ...
                >
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          p: 1.5,
                          borderRadius: 2,
                          border: isActive ? '1px solid rgba(251,191,36,0.58)' : '1px solid rgba(148,163,184,0.16)',
                          background: isActive ? 'rgba(251,191,36,0.08)' : 'rgba(15,23,42,0.66)',
                          cursor: 'pointer',
                      ...
                      >
                        ...
                        <Typography sx={{ color: '#e2e8f0', fontWeight: 800, fontSize: '1.05rem' }}>
                          {formatCurrency(phaseCard.estimate, currency)}
                        </Typography>
                        <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.84rem', mt: 0.4 }}>
                          Godkjent {formatCurrency(phaseCard.approved, currency)} · Faktisk {formatCurrency(phaseCard.actual, currency)}
                        </Typography>
                        <Typography sx={{ color: 'rgba(148,163,184,0.84)', fontSize: '0.8rem', mt: 0.6 }}>
                          {phaseCard.syncedLineCount > 0
                            ? `${phaseCard.syncedLineCount} linjer kommer fra team/reise-synk`
                            : 'Ingen synkede team- eller reiselinjer i denne fasen ennå.'}
                        </Typography>
                      </Box>
                    );
                  })}
                  <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                    ...
                    <Typography sx={{ color: '#e2e8f0', fontWeight: 800, fontSize: '1.1rem' }}>
                      {projectBudget > 0 ? formatCurrency(projectBudget, currency) : 'Ikke satt'}
                    </Typography>
                    <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem', mt: 0.6 }}>
                      Faseestimat {formatCurrency(phaseEstimateTotal, currency)} · Godkjent {formatCurrency(toNumber(totals.approved), currency)}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                    ...
                    <Typography sx={{ color: '#e2e8f0', fontWeight: 800, fontSize: '1.1rem' }}>
                      {formatCurrency(projectedCrewTotal + totalTravelCosts, currency)}
                    </Typography>
                    <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem', mt: 0.6 }}>
                  ...
                  <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                    ...
                    <Stack spacing={0.75}>
                      <Chip
                        size="small"
                        label={crewSyncCoverage === null
                          ? 'Ingen dagsatser registrert'
                          : `Team synket ${Math.round(crewSyncCoverage * 100)}%`}
                        sx={{ alignSelf: 'flex-start', bgcolor: 'rgba(59,130,246,0.16)', color: '#bfdbfe' }}
                      />
                      <Chip
                        size="small"
                        label={travelSyncCoverage === null
                          ? 'Ingen reiser registrert'
                          : `Reiser synket ${Math.round(travelSyncCoverage * 100)}%`}
                  ...
                  <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                    ...
                    <Typography sx={{ color: '#e2e8f0', fontWeight: 800, fontSize: '1.1rem' }}>
                      {approvalPressure} å følge opp
                    </Typography>
                    <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem', mt: 0.6 }}>
                      {approvalPendingCount} venter · {approvalChangeRequestCount} krever endringer på tvers av leveranser, budsjett og avtaler
                    </Typography>
                  </Box>
                </Box>
                ...
                >
                  <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1} justifyContent="space-between" sx={{ mb: 1 }}>
                    <Box>
                      <Typography sx={{ color: '#fff', fontWeight: 800 }}>
                        Estimatgrunnlag for budsjett
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem' }}>
                        Samme estimat brukes nå i tidslinje, shotlist og økonomi slik at budsjettet bygges på reell produksjonslast.
                      </Typography>
                    </Box>
                    <Stack direction="row" spacing={1} flexWrap="wrap">
                      <Chip size="small" label={`${productionEstimate.suggestedShootDays} opptaksdager`} sx={{ bgcolor: 'rgba(16,185,129,0.16)', color: '#a7f3d0' }} />
                      <Chip size="small" label={`${suggestedCrewDays} crew-dager`} sx={{ bgcolor: 'rgba(59,130,246,0.16)', color: '#bfdbfe' }} />
                      <Chip size="small" label={`Hovedleveranse ${primaryFormatEstimate ? formatSeconds(primaryFormatEstimate.estimatedSeconds) : '0:00'}`} sx={{ bgcolor: 'rgba(251,191,36,0.16)', color: '#fde68a' }} />
                    </Stack>
                  </Stack>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mb: 1 }}>
                    ...
                    {onOpenDeliverableSource ? (
                      <Button
                        size="small"
                        variant="outlined"
                        onClick={() => onOpenDeliverableSource('shotlist')}
                        sx={{ textTransform: 'none', fontWeight: 700 }}
                      >
                        Åpne shotlist
                      </Button>
                  ...
                  >
                    <Box sx={{ p: 1.15, borderRadius: 1.4, border: '1px solid rgba(148,163,184,0.14)', background: 'rgba(2,6,23,0.54)' }}>
                      <Typography sx={{ color: 'rgba(226,232,240,0.78)', fontSize: '0.8rem', fontWeight: 700 }}>
                        Feltbelastning
                      </Typography>
                      <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.04rem', mt: 0.45 }}>
                        {formatMinutes(productionEstimate.totalRealisticFieldMinutes)}
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.45 }}>
                        {productionDays.length > 0
                          ? `${productionDays.length} planlagte produksjonsdager mot ${productionEstimate.suggestedShootDays} anbefalt.`
                          : 'Ingen produksjonsdager er lagt inn ennå. Estimatet foreslår dagsbehov automatisk.'}
                      </Typography>
                    </Box>
                    <Box sx={{ p: 1.15, borderRadius: 1.4, border: '1px solid rgba(148,163,184,0.14)', background: 'rgba(2,6,23,0.54)' }}>
                      <Typography sx={{ color: 'rgba(226,232,240,0.78)', fontSize: '0.8rem', fontWeight: 700 }}>
                        Output og distribusjon
                      </Typography>
                      <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.04rem', mt: 0.45 }}>
                        {primaryFormatEstimate ? primaryFormatEstimate.label : 'Ingen format valgt'}
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.45 }}>
                        {productionEstimate.formatEstimates.length} formater er beregnet fra samme manus- og shotlistgrunnlag.
                      </Typography>
                    </Box>
                    <Box sx={{ p: 1.15, borderRadius: 1.4, border: '1px solid rgba(148,163,184,0.14)', background: 'rgba(2,6,23,0.54)' }}>
                      <Typography sx={{ color: 'rgba(226,232,240,0.78)', fontSize: '0.8rem', fontWeight: 700 }}>
                      ...
                      <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.04rem', mt: 0.45 }}>
                        {suggestedCrewDays} crew-dager
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.45 }}>
                        Basert på {Math.max(1, billableCrewMembers.length || normalizedCrewMembers.length || 1)} bemannede roller og foreslått opptaksramme.
                      </Typography>
                    </Box>
                    <Box sx={{ p: 1.15, borderRadius: 1.4, border: '1px solid rgba(148,163,184,0.14)', background: 'rgba(2,6,23,0.54)' }}>
                      <Typography sx={{ color: 'rgba(226,232,240,0.78)', fontSize: '0.8rem', fontWeight: 700 }}>
                      ...
                      <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.04rem', mt: 0.45 }}>
                        {unscheduledLoad ? `${unscheduledLoad.shotListCount} shotlister uten opptaksdag` : 'Ingen shotlister uten opptaksdag'}
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.45 }}>
                        {unscheduledLoad
                          ? `${formatMinutes(unscheduledLoad.realisticFieldMinutes)} må fortsatt plasseres før budsjettet kan stoles på fullt ut.`
                          : 'Shotlistene er koblet til produksjonsdager, så budsjettscenariet er mer realistisk.'}
                      </Typography>
                    </Box>
                  </Box>
                </Box>
                ...
                >
                  <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1} justifyContent="space-between" sx={{ mb: 1 }}>
                    <Box>
                      <Typography sx={{ color: '#fff', fontWeight: 800 }}>
                        Retning og leveransegrunnlag
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem' }}>
                        Samme retning, aktivering og leveranser brukes nå i planlegging, klientsamarbeid, eksport og økonomi.
                      </Typography>
                    </Box>
                    <Stack direction="row" spacing={1} flexWrap="wrap">
                      <Chip size="small" label={`${deliveryManifest.deliveryItems.length} leveranser`} sx={{ bgcolor: 'rgba(59,130,246,0.16)', color: '#bfdbfe' }} />
                      <Chip size="small" label={`${pendingClientMoments.length} åpne klientpunkter`} sx={{ bgcolor: 'rgba(251,191,36,0.16)', color: '#fde68a' }} />
                    </Stack>
                  </Stack>
                  {producerStrategySnapshot.length > 0 ? (
                    <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mb: 1 }}>
                      {producerStrategySnapshot.map((item) => (
                        <Chip
                          key={item.label}
                          size="small"
                  ...
                  >
                    <Box sx={{ p: 1.15, borderRadius: 1.4, border: '1px solid rgba(148,163,184,0.14)', background: 'rgba(2,6,23,0.54)' }}>
                      ...
                      <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.04rem', mt: 0.45 }}>
                        {brandGuideReadyCount}/5 punkter klare
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.45 }}>
                        {brandGuideReadyCount === 5
                          ? 'Merkevaregrunnlaget er klart til å styre eksport og finaler.'
                          : 'Noen merkevarepunkter mangler før klientpakken er helt konsekvent.'}
                      </Typography>
                    </Box>
                    <Box sx={{ p: 1.15, borderRadius: 1.4, border: '1px solid rgba(148,163,184,0.14)', background: 'rgba(2,6,23,0.54)' }}>
                      <Typography sx={{ color: 'rgba(226,232,240,0.78)', fontSize: '0.8rem', fontWeight: 700 }}>
                      ...
                      <Typography sx={{ color: '#fff', fontWeight: 800, fontSize: '1.04rem', mt: 0.45 }}>
                        {deliveryWorkflowReadyCount}/6 regler satt
                      </Typography>
                      <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.45 }}>
                        Filnavn, versjoner, mapper og backup følger nå samme prosjektregler.
                      </Typography>
                    </Box>
                  </Box>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mt: 1.1 }}>
                    {onOpenReviews ? (
                      <Button
                        size="small"
                        variant="outlined"
                        onClick={() => onOpenReviews({ projectId: project.id, panel: 'reviews' })}
                        sx={{ textTransform: 'none', fontWeight: 700 }}
                      >
                    ...
                    {onOpenTimeline ? (
                      <Button
                        size="small"
                        variant="outlined"
                        onClick={() => onOpenTimeline({ projectId: project.id, panel: 'timeline' })}
                        sx={{ textTransform: 'none', fontWeight: 700 }}
                      >
                        Åpne tidslinje
                      </Button>
                  ...
                  <Stack spacing={1}>
                        ...
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} justifyContent="space-between">
                          <Box>
                            ...
                            <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.82rem', mt: 0.35 }}>
                              {moment.detail || 'Ingen ekstra detaljer ennå.'}
                            </Typography>
                          </Box>
                          <Stack spacing={0.4} alignItems={{ sm: 'flex-end' }}>
                            <Chip
                              size="small"
                            ...
                            />
                            <Typography sx={{ color: 'rgba(203,213,225,0.68)', fontSize: '0.78rem' }}>
                              {moment.date ? formatDateTime(moment.date) ?? moment.date : 'Dato ikke satt'}
                            </Typography>
                          </Stack>
                        </Stack>
                      </Box>
                    ...
                    {pendingClientMoments.length === 0 ? (
                      <Typography sx={{ color: 'rgba(203,213,225,0.72)', fontSize: '0.88rem' }}>
                        Ingen åpne klientpunkter i faseplan eller content-kalender ennå.
                      </Typography>
                    ) : null}
                    <Divider sx={{ borderColor: 'rgba(148,163,184,0.16)' }} />
                    {nextDeliveryItems.map((item) => (
                      <Box
                        key={item.id}
                        sx={{
                          p: 1,
                      ...
                      >
                        ...
                        <Typography sx={{ color: 'rgba(148,163,184,0.8)', fontSize: '0.78rem', mt: 0.25 }}>
                          {`Pakke ${item.packageName}`}
                        </Typography>
                        <Typography sx={{ color: 'rgba(148,163,184,0.8)', fontSize: '0.78rem', mt: 0.25 }}>
                          {`Versjon ${item.versionLabel} · ${item.deliveryStageLabel}`}
                          {item.publishDateLabel ? ` · Publisering ${item.publishDateLabel}` : ' · Publiseringsdato ikke satt'}
                        </Typography>
                      </Box>
                    ))}
                  </Stack>
                  ...
                  {statusTouchpoint ? (
                    <Stack spacing={0.8}>
                      ...
                      <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.85rem' }}>
                        {workflowDriverReview ? 'Denne saken driver prosjektstatusen akkurat nå.' : 'Siste sak i klientflyten.'}
                      </Typography>
                      ...
                      {statusTouchpointAgreement ? (
                        <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.85rem' }}>
                          {statusTouchpointAgreement.google_signature
                            ? 'Avtalestatus, signaturstatus og prosjektstatus er nå låst til samme juridiske flyt.'
                            : 'Avtalen følger foreløpig bare reviewstatus og ikke juridisk signaturflyt.'}
                        </Typography>
                      ) : null}
                      <Stack direction="row" spacing={1} flexWrap="wrap">
                        <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.85rem' }}>
                          Sendt {formatDateTime(statusTouchpoint.requested_at) ?? 'ukjent tidspunkt'}
                        </Typography>
                        <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.85rem' }}>
                          Kommentarer {statusTouchpoint.comments?.length ?? 0}
                        </Typography>
                      </Stack>
                      <Stack direction="row" spacing={1} flexWrap="wrap">
                        <Chip size="small" label={`Budsjett ${budgetReviewSummary.pending} venter`} sx={{ bgcolor: 'rgba(59,130,246,0.14)', color: '#bfdbfe' }} />
                        <Chip size="small" label={`Avtaler ${agreementReviewSummary.pending} venter`} sx={{ bgcolor: 'rgba(192,132,252,0.14)', color: '#e9d5ff' }} />
                        <Chip size="small" label={`Til signering ${agreementSignatureSummary.inFlight}`} sx={{ bgcolor: 'rgba(251,191,36,0.14)', color: '#fde68a' }} />
                        <Chip size="small" label={`Signert ${agreementSignatureSummary.signed}`} sx={{ bgcolor: 'rgba(16,185,129,0.16)', color: '#86efac' }} />
                      </Stack>
                      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                        {onOpenMedia && statusTouchpointAgreement ? (
                          <Button
                            size="small"
                            variant="outlined"
                            onClick={() => onOpenMedia(getAgreementMediaFocus(statusTouchpointAgreement))}
                            sx={{ textTransform: 'none', fontWeight: 700 }}
                          >
                            Åpne i klientflate
                          </Button>
                        ...
                        {onOpenReviews ? (
                          <Button
                            ...
                            onClick={() => openReviewFocus({
                              ...
                              linkedEntityType: statusTouchpoint.target_entity_type ?? undefined,
                              linkedEntityId: statusTouchpoint.target_entity_id ?? undefined,
                              agreementId: statusTouchpoint.target_entity_type === 'project_agreement'
                                ? statusTouchpoint.target_entity_id ?? undefined
                                : undefined,
                            })}
                            sx={{ textTransform: 'none', fontWeight: 700 }}
                          >
                ...
                >
                  ...
                  <Stack spacing={1}>
                    ...
                    <Button
                      ...
                      sx={{ justifyContent: 'flex-start', textTransform: 'none', fontWeight: 700, bgcolor: '#1d4ed8' }}
                    >
                      Gå til budsjett og avtaler
                    </Button>
                    <Button
                      variant="outlined"
          ...
          {activeView === 'budget' ? (
            <Box
              sx={{
                ...
                gridTemplateColumns: {
                  xs: '1fr',
                  xl: 'minmax(0, 1.75fr) minmax(300px, 0.9fr)',
                },
                gap: 2,
                alignItems: 'start',
              }}
            >
                ...
                >
                  ...
                  >
                    <Box>
                      ...
                      <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem' }}>
                        Fordeling, honorarer og rettigheter håndteres her sammen med resten av økonomien.
                      </Typography>
                    </Box>
                    <Stack direction="row" spacing={1} flexWrap="wrap">
              ...
              <Stack spacing={2}>
                <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                  ...
                  <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.9rem', mb: 1.2 }}>
                    Velg fasen du budsjetterer for, så holder arbeidsflaten kostlinjer, tilbud og oppfølging samlet rundt samme steg i produksjonen.
                  </Typography>
                ...
                <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                  ...
                  <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.9rem', mb: 1.2 }}>
                    Dagsatser fra teamoppsettet kan brukes direkte i produksjonsbudsjettet uten dobbel registrering.
                  </Typography>
                  <Stack spacing={0.75}>
                    <Typography sx={{ color: '#e2e8f0', fontWeight: 700 }}>
                      {formatCurrency(crewDailyTotal, currency)} per opptaksdag
                    </Typography>
                    <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem' }}>
                      {formatCurrency(projectedCrewTotal, currency)} for {plannedShootDays} planlagte opptaksdager.
                    </Typography>
                    ...
                    {topDepartments.map(([department, departmentRate]) => (
                      <Chip
                        key={department}
                        size="small"
                ...
                <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                  ...
                  <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.9rem', mb: 1.2 }}>
                    Reisekostnader fra produksjonsdagene samles her før de låses som budsjettlinjer.
                  </Typography>
                  <Stack spacing={0.75}>
                    <Typography sx={{ color: '#e2e8f0', fontWeight: 700 }}>
                      {totalTravelCosts > 0 ? formatCurrency(totalTravelCosts, currency) : 'Ingen registrerte reisekostnader'}
                    </Typography>
                    <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem' }}>
                      {normalizedTravelCosts.length > 0
                        ? `${normalizedTravelCosts.length} profiler · ${travelDayCount} reisedager`
                        : 'Reisedager blir tilgjengelige her når transportkostnader er registrert i produksjonsplanen.'}
                    </Typography>
              ...
              <Box sx={{ p: 1.5, borderRadius: 2, border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(15,23,42,0.66)' }}>
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} justifyContent="space-between" sx={{ mb: 1.25 }}>
                  <Box>
                    ...
                    <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.86rem' }}>
                      Samler budsjettpakker, endringsordre og klientgodkjenninger for økonomi i én arbeidsflate.
                    </Typography>
                  </Box>
                  <Stack direction="row" spacing={1} flexWrap="wrap">
                    <Chip size="small" label={`Leveranser ${deliverableReviewSummary.pending} venter`} sx={{ bgcolor: 'rgba(96,165,250,0.16)', color: '#bfdbfe' }} />
                    <Chip size="small" label={`Venter ${budgetReviewSummary.pending}`} />
                    <Chip size="small" label={`Godkjent ${budgetReviewSummary.approved}`} sx={{ color: '#34d399' }} />
                    <Chip size="small" label={`Endringer ${budgetReviewSummary.changesRequested}`} sx={{ color: '#fbbf24' }} />
                    <Chip size="small" label={`Avtaler ${agreementReviewSummary.pending} venter`} sx={{ bgcolor: 'rgba(192,132,252,0.14)', color: '#e9d5ff' }} />
                    <Chip size="small" label={`Til signering ${agreementSignatureSummary.inFlight}`} sx={{ bgcolor: 'rgba(251,191,36,0.14)', color: '#fde68a' }} />
                    <Chip size="small" label={`Signert ${agreementSignatureSummary.signed}`} sx={{ bgcolor: 'rgba(16,185,129,0.16)', color: '#86efac' }} />
                  </Stack>
                  ...
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} justifyContent="space-between" sx={{ mb: 1 }}>
                    <Box>
                      ...
                      <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.84rem' }}>
                        Prioriterer saken som styrer prosjektstatus først, deretter det som krever handling før neste klientbeslutning.
                      </Typography>
                    </Box>
                    <Stack direction="row" spacing={1} flexWrap="wrap">
                      <Chip
                ...
                {!readOnly ? (
                  ...
                  >
                    ...
                    <Typography sx={{ color: 'rgba(203,213,225,0.74)', fontSize: '0.84rem', mb: 1.2 }}>
                      Opprett klientgodkjenning for storyboard, manus eller shotlist direkte fra økonomiflyten. Leveransen blir sporet videre i klientsamarbeid og tidslinje.
                    </Typography>
                    <Stack spacing={1}>
                      <TextField
                        select
