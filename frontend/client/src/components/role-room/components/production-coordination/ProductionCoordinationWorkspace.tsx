...
const fieldSx = {
  ...
  '& fieldset': { borderColor: 'rgba(147, 164, 220,.25)' },
};
    ...
    <Box component="section" data-testid="production-coordination-workspace" sx={{ minHeight: '100%', overflowY: 'auto', bgcolor: '#06101b', color: '#f8fafc', p: { xs: 1.5, md: 3 } }}>
      <Box sx={{ maxWidth: 1540, mx: 'auto' }}>
        <Stack direction={{ xs: 'column', lg: 'row' }} justifyContent="space-between" gap={2} sx={{ mb: 2 }}>
          <Box>
            <Chip label="PRODUKSJONSKOORDINERING · DAGSFLYT" size="small" sx={{ color: '#c3cbe6', bgcolor: 'rgba(63, 81, 181,.12)', border: '1px solid rgba(93, 118, 203,.32)', fontWeight: 800 }} />
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} gap={1} alignItems={{ lg: 'center' }}>
            <Button variant="outlined" startIcon={<ScheduleIcon />} onClick={() => { if (confirmIfDirty()) onOpenSchedule(); }} sx={{ color: '#dfe4f3', borderColor: 'rgba(93, 118, 203,.35)' }}>Opptaksplan</Button>
            <Button variant="outlined" startIcon={<CrewIcon />} onClick={() => { if (confirmIfDirty()) onOpenCrew(); }} sx={{ color: '#dfe4f3', borderColor: 'rgba(93, 118, 203,.35)' }}>Team</Button>
            <Button variant="outlined" startIcon={<FullWorkspaceIcon />} onClick={() => { if (confirmIfDirty()) onOpenFullWorkspace(); }} sx={{ color: '#dfe4f3', borderColor: 'rgba(93, 118, 203,.35)' }}>Hele prosjektet</Button>
          </Stack>
        </Stack>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', lg: 'repeat(5, 1fr)' }, gap: 1.25, mb: 2 }}>
              {[
                ...
                <Card key={label} variant="outlined" sx={{ ...panelSx, p: 1.5 }}>
                  ...
                  <Typography sx={{ fontSize: '1.45rem', fontWeight: 850, color: label === 'Blokkeringer' && value !== '0' ? '#fca5a5' : '#93a4dc' }}>{value}</Typography>
                </Card>
              ))}
            </Box>
              ...
              <Stack spacing={2}>
                <Card variant="outlined" sx={{ ...panelSx, p: 2 }}>
                    ...
                    {operations.tasks.length === 0 ? <Typography sx={{ color: 'rgba(203,213,225,.58)', fontSize: '.82rem' }}>Ingen oppgaver er registrert.</Typography> : operations.tasks.map((task) => (
                      <Box key={task.id} sx={{ p: 1.25, borderRadius: 1.5, border: `1px solid ${task.status === 'blocked' ? 'rgba(248,113,113,.5)' : 'rgba(148,163,184,.14)'}` }}>
                        <Stack direction={{ xs: 'column', sm: 'row' }} gap={1} alignItems={{ sm: 'center' }}>
                          <Chip size="small" label={PRIORITY_LABELS[task.priority]} sx={{ color: task.priority === 'urgent' ? '#fecaca' : '#c3cbe6' }} />
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: '2fr 1fr 1fr', lg: '2fr 1fr 1fr 1.2fr 1.3fr auto' }, gap: 1, mt: 1.5 }}>
                    ...
                    <Button aria-label="Legg til oppgave" variant="outlined" disabled={readOnly || !taskDraft.title.trim()} onClick={addTask} sx={{ color: '#c3cbe6', borderColor: 'rgba(93, 118, 203,.35)', minWidth: 44 }}><AddIcon /></Button>
                  </Box>
                </Card>
