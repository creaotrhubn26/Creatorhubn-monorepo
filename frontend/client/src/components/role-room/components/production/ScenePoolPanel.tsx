            ...
            {onAssignRequest && (
              <Tooltip title="Tildel til dag">
                <IconButton
                  ...
                  sx={{ p: 0.375, bgcolor: alpha('#5446e1', 0.1), '&:hover': { bgcolor: alpha('#5446e1', 0.2) } }}
                >
                  <AssignIcon sx={{ fontSize: 14, color: '#5446e1' }} />
                </IconButton>
              </Tooltip>
            )}
          </Box>
...
const StatsBar: FC<{ stats: ScenePoolStats; isMobile: boolean }> = ({ stats, isMobile }) => (
  <Box sx={{ display: 'flex', gap: { xs: 1.5, sm: 2 }, flexWrap: 'wrap', alignItems: 'center' }}>
    {[
      { value: stats.totalScenes, label: 'Scener', color: '#5446e1' },
...
export const ScenePoolPanel: FC<ScenePoolPanelProps> = ({
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: 'background.paper' }}>
      ...
      {/* ── Header ─────────────────────────────────────────────────── */}
      <Box sx={{ p: { xs: 1.25, sm: 1.75 }, borderBottom: 1, borderColor: 'divider', bgcolor: alpha('#5446e1', 0.04) }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
          <Typography variant="subtitle1" fontWeight={700} sx={{ display: 'flex', alignItems: 'center', gap: 0.75, fontSize: responsive.fontSize.subtitle }}>
            <StarIcon sx={{ fontSize: 18, color: '#5446e1' }} />
            Scene Pool
      ...
      {pool.stats.totalScenes + (pool.allItems.length) > 0 && (
        <Box sx={{ p: { xs: 0.75, sm: 1 }, borderTop: 1, borderColor: 'divider' }}>
          ...
          <LinearProgress
            ...
            sx={{ height: 4, borderRadius: 2, mt: 0.25, bgcolor: alpha('#5446e1', 0.15),
                  '& .MuiLinearProgress-bar': { bgcolor: '#5446e1' } }}
          />
        </Box>
      )}
    </Box>
