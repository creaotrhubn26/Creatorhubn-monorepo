...
export const ScriptSupervisorNotes: React.FC<ScriptSupervisorNotesProps> = ({
  ...
  const getStatusConfig = (status: TakeLog['status']) => {
    const configs = {
      print: { label: 'PRINT', color: 'var(--role-violet, #5d76cb)', icon: <StarIcon /> },
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        ...
        <Grid size={{ xs: 4 }}>
          <Paper sx={{ p: 2, textAlign: 'center', bgcolor: 'rgba(93, 118, 203, 0.1)' }}>
            <Typography variant="h3" sx={{ color: 'var(--role-violet, #5d76cb)' }}>{printTakes}</Typography>
                    ...
                    {logs.sort((a, b) => a.takeNumber - b.takeNumber).map(log => {
                      ...
                      return (
                        <TableRow 
                          key={log.id}
                          sx={{ 
                            bgcolor: log.circled ? 'rgba(93, 118, 203, 0.1)' : 'transparent',
