...
const BrushButton = styled(Box, {
  ...
  cursor: 'pointer',
  transition: 'all 0.2s',
  backgroundColor: isSelected ? 'rgba(93, 118, 203, 0.3)' : 'transparent',
  border: isSelected ? '2px solid #5d76cb' : '2px solid transparent',
  '&:hover': {
...
const ColorCircle = styled(Box, {
  ...
  boxShadow: isSelected ? '0 0 0 2px #5d76cb' : 'none',
  transition: 'all 0.2s',
  '&:hover': {
    transform: 'scale(1.1)',
  },
  ...
  const renderTip = () => {
    switch (type) {
      ...
      case 'highlighter':
        return (
          <>
            <rect x="6" y="4" width="20" height="30" fill="#3e3180" rx="3" />
