...
const SymmetryButton = styled(IconButton, {
  ...
  borderRadius: 6,
  padding: 6,
  backgroundColor: active ? 'rgba(136, 117, 235, 0.3)' : 'transparent',
  border: active ? '1px solid rgba(136, 117, 235, 0.5)' : '1px solid transparent',
  '&:hover': {
    backgroundColor: active ? 'rgba(136, 117, 235, 0.4)' : 'rgba(255,255,255,0.1)',
  },
}));
...
export function drawSymmetryGuides(
  ...
  ctx.save();
  ctx.strokeStyle = 'rgba(136, 117, 235, 0.5)';
  ctx.lineWidth = 1;
  ctx.setLineDash([8, 4]);
    ...
    <ToolbarContainer>
      ...
      {settings.enabled && (
        ...
        >
          ...
          />
          <Typography variant="caption" sx={{ color: '#9e8cf8', fontSize: 10 }}>
            {settings.type === 'radial' 
