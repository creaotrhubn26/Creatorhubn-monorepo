  ...
  return (
    <Stack spacing={1.75}>
      ...
      <Box
        ...
        sx={{
          borderRadius: '14px', border: `1.5px dashed ${dragOver ? '#5d76cb' : 'rgba(148,163,184,0.32)'}`,
          background: dragOver ? 'rgba(93, 118, 203,0.08)' : 'rgba(255,255,255,0.015)',
          px: 2, py: { xs: 4, md: 5 }, textAlign: 'center', transition: 'all 0.15s',
        }}
      >
        <Box sx={{ width: 56, height: 56, mx: 'auto', mb: 1.5, borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg,#4b3d8f,#5d76cb)', boxShadow: '0 8px 24px rgba(75, 61, 143,0.4)' }}>
      ...
      {uploads.map((u) => (
        <Box key={u.tempId} sx={{ borderRadius: '10px', border: '1px solid rgba(148,163,184,0.16)', background: 'rgba(27, 18, 44,0.4)', p: 1.25 }}>
          ...
          {u.error ? (
            ...
            <LinearProgress variant="determinate" value={u.pct} sx={{ height: 6, borderRadius: 3, backgroundColor: 'rgba(148,163,184,0.16)', '& .MuiLinearProgress-bar': { borderRadius: 3, background: 'linear-gradient(90deg,#5d76cb,#4b3d8f)' } }} />
          )}
        </Box>
      ))}
    ...
    <Box sx={{ borderRadius: '10px', border: '1px solid rgba(148,163,184,0.14)', background: 'rgba(27, 18, 44,0.35)', p: 1.1, display: 'flex', alignItems: 'center', gap: 1.25 }}>
      ...
      <Box sx={{ minWidth: 0, flex: 1 }}>
        ...
        <Stack direction="row" spacing={0.75} alignItems="center" sx={{ mt: 0.2, flexWrap: 'wrap', rowGap: 0.2 }}>
          <Chip label={CATEGORY_LABEL[m.entryType] ?? 'Annet'} size="small" sx={{ height: 18, fontSize: '0.62rem', fontWeight: 700, color: 'rgba(195, 203, 230,0.95)', background: 'rgba(93, 118, 203,0.14)' }} />
