...
export default function VaultTrustCeremony({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog
      ...
      slotProps={{ paper: { sx: { background: 'linear-gradient(180deg,#0c0a18,#0a0a14)', border: '1px solid rgba(118, 102, 230,0.35)', color: '#e2e8f0', borderRadius: 3 } } }}
    >
      <Box sx={{ p: 3, textAlign: 'center' }}>
        <Stack spacing={0.5} alignItems="center" sx={{ mb: 1 }}>
          <KeyIcon sx={{ fontSize: 18, color: '#7666e6' }} />
        ...
        <Box sx={{ my: 2.5, display: 'flex', justifyContent: 'center' }}>
          <Box sx={{ width: 80, height: 80, borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(118, 102, 230,0.1)', border: '1px solid rgba(118, 102, 230,0.25)' }}>
            {s.icon}
          </Box>
        </Box>
        ...
        <MobileStepper
          ...
          sx={{ background: 'transparent', justifyContent: 'center', mt: 1.5, '& .MuiMobileStepper-dot': { background: 'rgba(148,163,184,0.3)' }, '& .MuiMobileStepper-dotActive': { background: '#7666e6' } }}
        ...
        <Stack direction="row" spacing={1} sx={{ mt: 1.5 }}>
          ...
          <Box sx={{ flex: 1 }} />
          {last ? (
            <Button onClick={finish} variant="contained" startIcon={<ShieldIcon />} sx={{ textTransform: 'none', fontWeight: 800, minHeight: 44, px: 3, color: '#fff', background: 'linear-gradient(135deg,#7666e6,#6f52e3)', '&:hover': { background: 'linear-gradient(135deg,#523ee0,#4926d3)' } }}>
              Legg inn første tilgang
            </Button>
          ) : (
            <Button onClick={() => setStep((v) => v + 1)} variant="contained" endIcon={<NextIcon />} sx={{ textTransform: 'none', fontWeight: 800, minHeight: 44, px: 3, color: '#fff', background: 'linear-gradient(135deg,#7666e6,#6f52e3)', '&:hover': { background: 'linear-gradient(135deg,#523ee0,#4926d3)' } }}>
              Videre
            </Button>
          )}
        </Stack>
