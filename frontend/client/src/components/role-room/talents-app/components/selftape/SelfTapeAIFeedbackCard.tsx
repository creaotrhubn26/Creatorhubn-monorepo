...
function gradeTone(grade: string | null | undefined): { bg: string; fg: string; label: string } {
  if (!grade) return { bg: 'rgba(75, 61, 143,0.16)', fg: palette.accentBright, label: '—' };
  ...
  return { bg: 'rgba(75, 61, 143,0.16)', fg: palette.accentBright, label: grade };
}
          ...
          {currentTake && feedback?.status !== 'generating' ? (
            <Box
              ...
              sx={{
                ...
                alignItems: 'center',
                gap: 0.4,
                '&:hover': canRegenerate ? { background: 'linear-gradient(135deg, #3e3180 0%, #32127a 100%)' } : undefined,
              }}
            >
              {busy
