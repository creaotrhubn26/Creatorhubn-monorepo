              ...
              <span>
                <Button
                  ...
                  sx={{
                    ...
                    '&:hover': {
                      background: 'linear-gradient(135deg, #3e3180 0%, #32127a 100%)',
                      boxShadow: '0 6px 18px rgba(75, 61, 143,0.52)',
                    },
      ...
      <Box sx={{ width: 320, flexShrink: 0, p: 3, pl: 0, display: { xs: 'none', lg: 'flex' }, flexDirection: 'column', gap: 2 }}>
        ...
        {overview ? (
          <Box sx={cardSx}>
            ...
            <Box sx={{ mt: 2, height: 60, borderRadius: radius.sm, bgcolor: 'rgba(75, 61, 143,0.04)', position: 'relative', overflow: 'hidden' }}>
            ...
            <Stack spacing={0.4} sx={{ flexGrow: 1 }}>
              ...
              <Stack direction="row" spacing={0.6} flexWrap="wrap" useFlexGap sx={{ mt: 0.6 }}>
                {p.requested_scopes.slice(0, 5).map((s) => (
                  <Chip key={s} label={s} size="small" sx={{ bgcolor: 'rgba(75, 61, 143,0.10)', color: palette.textMuted, height: 18, fontSize: '0.7rem' }} />
                ))}
              </Stack>
            </Stack>
...
function FeaturedCard({ talent }: { talent: TalentSearchHit }) {
  ...
  return (
    <Box sx={{ p: 1.2, borderRadius: radius.md, bgcolor: palette.bgCardElevated, border: `1px solid ${palette.borderSubtle}`, display: 'flex', gap: 1.2 }}>
      <Avatar src={talent.headshot_url ?? undefined} sx={{ width: 56, height: 56, borderRadius: 1, bgcolor: 'rgba(75, 61, 143,0.18)' }}>
...
function TalentCard({ talent }: { talent: TalentSearchHit }) {
  ...
  return (
    <Box sx={{ borderRadius: radius.lg, bgcolor: palette.bgCard, border: `1px solid ${palette.border}`, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ position: 'relative', aspectRatio: '1 / 1', bgcolor: 'rgba(75, 61, 143,0.08)' }}>
        {talent.headshot_url ? (
      ...
      <Box sx={{ p: 1.4, flexGrow: 1, display: 'flex', flexDirection: 'column', gap: 0.6 }}>
        ...
        <Stack direction="row" spacing={0.6} flexWrap="wrap" useFlexGap sx={{ mt: 0.6 }}>
          {skills.map((s, i) => (
            <Chip key={i} label={typeof s === 'string' ? s : s.label} size="small" sx={{ bgcolor: 'rgba(75, 61, 143,0.12)', color: palette.accentBright, fontSize: '0.7rem', height: 20 }} />
          ))}
        </Stack>
        <Box sx={{ flexGrow: 1 }} />
...
function SavedSearchRow({ search, onApply, onDelete }: { search: SavedSearch; onApply: () => void; onDelete: () => void }) {
  ...
  return (
    <Box onClick={onApply} sx={{ p: 1.2, borderRadius: radius.sm, cursor: 'pointer', '&:hover': { bgcolor: 'rgba(75, 61, 143,0.08)' }, display: 'grid', gridTemplateColumns: '32px 1fr 24px', gap: 1, alignItems: 'center' }}>
      <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: 'rgba(75, 61, 143,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  if (!data || data.length === 0) {
    return (
      <svg viewBox="0 0 320 60" width="100%" height="100%" preserveAspectRatio="none">
        <text x="160" y="34" textAnchor="middle" fill="rgba(75, 61, 143,0.45)" fontSize="11" fontFamily="-apple-system, sans-serif">
