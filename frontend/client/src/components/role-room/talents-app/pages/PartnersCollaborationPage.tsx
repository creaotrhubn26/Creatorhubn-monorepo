...
const AVATAR_COLORS = ['#6249df', '#ec4899', '#6249df', '#f59e0b', '#10b981', 'var(--role-cyan, #7dd3fc)', '#fb7185'];
...
function accessChip(access: PartnerOverviewRow['access_level']) {
  const map: Record<PartnerOverviewRow['access_level'], { label: string; fg: string; Icon: React.ComponentType<{ sx?: object }> }> = {
    ...
    custom:    { label: 'Tilpasset',        fg: '#9e8cf8', Icon: ShieldIcon },
        ...
        >
          ...
          <Button
            ...
            sx={{
              ...
              boxShadow: '0 8px 24px rgba(98, 73, 223, 0.28)',
                ...
                return (
                  <Box
                    ...
                    sx={{
                      ...
                      bgcolor: isSel ? 'rgba(98, 73, 223, 0.08)' : 'transparent',
                  ...
                  >
                    ...
                    <Box>
                      <Tooltip title={p.role_label === 'Casting Partner' ? 'Caster roller for film/TV/teater' : 'Tilbyr workshops, etterutdanning eller catalog'}>
                        <Chip label={p.role_label === 'Casting Partner' ? 'Casting' : 'Senter'} size="small" sx={{ bgcolor: 'rgba(98, 73, 223, 0.12)', color: palette.accentBright, fontWeight: 600, fontSize: '0.72rem', height: 22, cursor: 'help' }} />
                      </Tooltip>
                    </Box>
  ...
  return (
    <Box sx={cardSx}>
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
        ...
        <Box sx={{ width: 38, height: 38, borderRadius: radius.sm, bgcolor: 'rgba(98, 73, 223, 0.14)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function PageBtn({ children, active = false }: { children: React.ReactNode; active?: boolean }) {
  return (
    <Box sx={{ minWidth: 28, height: 28, px: 1, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: radius.xs, bgcolor: active ? palette.accent : 'transparent', color: active ? '#fff' : palette.textMuted, fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', '&:hover': { bgcolor: active ? palette.accent : 'rgba(98, 73, 223, 0.12)' } }}>
      {children}
    </Box>
  );
}
    ...
    <Box sx={{ p: 1.4, borderRadius: radius.md, bgcolor: palette.bgCardElevated, border: `1px solid ${palette.borderSubtle}`, display: 'flex', flexDirection: 'column', gap: 1 }}>
      <Stack direction="row" spacing={1} alignItems="flex-start">
        {event.kind === 'invite' && !event.display_name ? (
          <Box sx={{ width: 32, height: 32, borderRadius: radius.sm, bgcolor: 'rgba(98, 73, 223, 0.16)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
    ...
    <Box sx={{ ...cardSx, position: 'sticky', top: 16 }}>
      <Stack direction="row" spacing={1.4} alignItems="center" sx={{ mb: 2 }}>
        ...
        <Box>
          ...
          <Chip label={partner.role_label} size="small" sx={{ mt: 0.4, bgcolor: 'rgba(98, 73, 223, 0.12)', color: palette.accentBright, fontWeight: 600, fontSize: '0.7rem', height: 20 }} />
        </Box>
      </Stack>
      ...
      <Stack spacing={1}>
        <Button startIcon={<LockOutlinedIcon />} onClick={onEdit} sx={{ textTransform: 'none', fontWeight: 700, fontSize: '0.88rem', py: 1.2, borderRadius: radius.sm, background: palette.accentGradient, color: '#fff', '&:hover': { filter: 'brightness(1.08)' }, boxShadow: '0 6px 18px rgba(98, 73, 223, 0.32)' }}>
          Endre tilgang
        </Button>
        <Button startIcon={<PersonAddAltOutlinedIcon />} onClick={onInvite} sx={{ textTransform: 'none', fontWeight: 600, fontSize: '0.85rem', py: 1.1, borderRadius: radius.sm, color: palette.textPrimary, border: `1px solid ${palette.borderStrong}`, '&:hover': { bgcolor: 'rgba(98, 73, 223, 0.08)' } }}>
          Inviter ny partner
        </Button>
      </Stack>
    </Box>
          ...
          {scopeRows.map(({ key, label, desc, Icon }) => (
            <Box
              ...
              sx={{
                ...
                bgcolor: perms[key] ? 'rgba(98, 73, 223,0.08)' : 'transparent',
                '&:hover': { bgcolor: 'rgba(98, 73, 223,0.05)' },
              }}
            >
        ...
        renderOption={(props, option) => (
          <Box component="li" {...props} key={option.id}>
            <Stack direction="row" spacing={1.2} alignItems="center" sx={{ width: '100%' }}>
              <Avatar sx={{ width: 28, height: 28, bgcolor: 'rgba(98, 73, 223,0.18)', color: palette.accentBright, fontSize: '0.7rem', fontWeight: 700 }}>
...
function ScopePresetChip({ label, scopes, currentScopes, onSet }: {
  ...
  return (
    <Chip
      ...
      sx={{
        bgcolor: matches ? palette.accent : 'rgba(98, 73, 223,0.08)',
        color: matches ? '#fff' : palette.accentBright,
        border: `1px solid ${matches ? palette.accent : palette.borderStrong}`,
        fontWeight: 700,
        fontSize: '0.78rem',
        cursor: 'pointer',
        '&:hover': { bgcolor: matches ? palette.accent : 'rgba(98, 73, 223,0.18)' },
      }}
    />
  );
}
