    ...
    <Box sx={{ display: 'grid', gap: 2.5 }}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'flex-start' }} spacing={2}>
        <Stack direction="row" spacing={1.75} alignItems="flex-start">
          <Box sx={{ width: 50, height: 50, borderRadius: 3, bgcolor: 'rgba(93, 118, 203,0.16)', color: '#c3cbe6', display: 'grid', placeItems: 'center', flexShrink: 0 }}><AssessmentIcon /></Box>
          <Box>
      ...
      <Panel sx={{ p: 2.5 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={1} sx={{ mb: 2, flexWrap: 'wrap', gap: 1 }}>
          ...
          <ToggleButtonGroup size="small" exclusive value={filter} onChange={(_e, v: Filter | null) => { if (v) setFilter(v); }}
            sx={{ '& .MuiToggleButton-root': { color: 'rgba(255,255,255,0.75)', textTransform: 'none', px: 1.5, borderColor: 'rgba(255,255,255,0.12)' }, '& .Mui-selected': { bgcolor: 'rgba(93, 118, 203,0.28) !important', color: '#fff !important' } }}>
                    ...
                    <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1} flexWrap="wrap" useFlexGap>
                      <Box sx={{ minWidth: 0 }}>
                        ...
                        <Stack direction="row" spacing={1} sx={{ mt: 0.5 }} flexWrap="wrap" useFlexGap>
                          <Chip size="small" label={it.assignmentTitle} sx={{ height: 20, fontSize: 10, bgcolor: 'rgba(93, 118, 203,0.22)', color: '#dfe4f3' }} />
                      ...
                      {it.productionProjectId && (
                        <Button size="small" variant="outlined" startIcon={<OpenIcon />} onClick={() => openProductionInRoleRoom(it.productionProjectId as string)}
                          sx={{ borderColor: 'rgba(93, 118, 203,0.5)', color: '#dfe4f3', textTransform: 'none', whiteSpace: 'nowrap', '&:hover': { borderColor: ACCENT, bgcolor: 'rgba(93, 118, 203,0.08)' } }}>
                          Åpne produksjon
                        </Button>
                      )}
                        ...
                        <Tooltip title="Send karakteren rett til LMS-karakterboka via LTI (AGS). MVP: går til LMS-brukeren som åpnet Role Room fra LMS-en — per-student passback for hele kullet kommer med roster-sync.">
                          <span>
                            <Button size="small" variant="outlined"
                              ...
                              sx={{ borderColor: 'rgba(93, 118, 203,0.5)', color: pushedIds.has(it.submissionId) ? '#10b981' : '#dfe4f3', textTransform: 'none', whiteSpace: 'nowrap', '&:hover': { borderColor: ACCENT, bgcolor: 'rgba(93, 118, 203,0.08)' } }}>
...
function RubricScoring({ assignmentId, studentId, onError }: { assignmentId: string; studentId: string; onError: (m: string | null) => void }) {
  ...
  return (
    <Box sx={{ display: 'grid', gap: 1, p: 1.5, borderRadius: 2, bgcolor: 'rgba(93, 118, 203,0.06)', border: '1px solid rgba(93, 118, 203,0.2)' }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between">
        ...
        <Chip size="small" label={`${total} / ${max} poeng · ${pct}%`} sx={{ height: 22, fontSize: 11, fontWeight: 700, bgcolor: 'rgba(93, 118, 203,0.22)', color: '#dfe4f3' }} />
      </Stack>
      {criteria.map((c) => (
        <Stack key={c.id} direction={{ xs: 'column', sm: 'row' }} alignItems={{ sm: 'center' }} justifyContent="space-between" spacing={0.5}>
          ...
          <ToggleButtonGroup size="small" exclusive value={scores[c.id] ?? 0}
            ...
            sx={{ '& .MuiToggleButton-root': { color: 'rgba(255,255,255,0.75)', textTransform: 'none', fontSize: 11, py: 0.25, px: 1 }, '& .Mui-selected': { bgcolor: 'rgba(93, 118, 203,0.28) !important', color: '#fff !important' } }}>
