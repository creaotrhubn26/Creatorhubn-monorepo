...
export function PortfolioTab() {
  ...
  // Avgangs-pipeline (utdanning → Talents). Selve promoteringen (m/ samtykke-
  // attestasjon) skjer i Bransje-fanen; her viser vi kun status per student.
    ...
    <Box sx={{ display: 'grid', gap: 2.5 }}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'flex-start' }} spacing={2}>
        <Stack direction="row" spacing={1.75} alignItems="flex-start">
          <Box sx={{ width: 50, height: 50, borderRadius: 3, bgcolor: 'rgba(93, 118, 203,0.16)', color: '#c3cbe6', display: 'grid', placeItems: 'center', flexShrink: 0 }}><PortfolioIcon /></Box>
          <Box>
        ...
        <Stack direction="row" spacing={1.25} flexWrap="wrap" useFlexGap>
          ...
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => { if (students.length === 0) { setError('Legg til studenter i Kull & studenter først.'); return; } setNewOpen(true); }} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#4b3d8f' }, textTransform: 'none', fontWeight: 700, borderRadius: 2 }}>
      ...
      <Panel sx={{ p: 0, overflowX: 'auto' }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 150px', minWidth: 560, px: 2, py: 1.25, bgcolor: 'rgba(255,255,255,0.02)', borderTop: '1px solid rgba(255,255,255,0.08)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        {portfolios.length === 0 ? (
          ...
          const tMeta = tstatus === 'claimed'
            ...
            : tstatus === 'claimable'
              ? { title: 'Invitert til Talents — venter på studentens samtykke (styres i Bransje-fanen)', color: '#f59e0b', icon: <TalentIcon fontSize="small" /> }
              : { title: 'Ikke på Talents ennå — promoter fra Bransje-fanen', color: 'rgba(255,255,255,0.3)', icon: <TalentIcon fontSize="small" /> };
          return (
            <Box key={p.id} sx={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 150px', minWidth: 560, alignItems: 'center', px: 2, py: 1.5, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <Stack direction="row" alignItems="center" spacing={1.25} sx={{ minWidth: 0 }}>
                <Avatar sx={{ width: 32, height: 32, fontSize: 11.5, bgcolor: 'rgba(93, 118, 203,0.3)', color: '#dfe4f3' }}>{initials(p.studentName)}</Avatar>
      ...
      <Dialog open={newOpen} onClose={() => setNewOpen(false)} maxWidth="sm" fullWidth
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={handleCreate} disabled={!newStudentId || creating} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#4b3d8f' }, textTransform: 'none', fontWeight: 700 }}>{creating ? 'Oppretter…' : 'Opprett'}</Button>
        </DialogActions>
      </Dialog>
