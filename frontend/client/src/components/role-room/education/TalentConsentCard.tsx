...
export function TalentConsentCard({ onDone }: { onDone?: () => void }) {
  ...
  const p = pending[0];
  return (
    <Box sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(136, 117, 235,0.09)', border: '1px solid rgba(136, 117, 235,0.32)', color: '#fff' }}>
      <Stack direction="row" alignItems="center" spacing={1.25} sx={{ mb: 1 }}>
        <Box sx={{ width: 42, height: 42, borderRadius: 2.5, bgcolor: 'rgba(136, 117, 235,0.22)', color: '#c6bdf4', display: 'grid', placeItems: 'center', flexShrink: 0 }}><TalentIcon /></Box>
        <Box>
      ...
      {info && (
        <Box sx={{ mt: 1.5 }}>
          ...
          <Typography sx={{ fontSize: 11.5, fontWeight: 700, color: '#c6bdf4', textTransform: 'uppercase', letterSpacing: 0.5, mb: 0.5 }}>Hva deles</Typography>
          ...
          <Typography sx={{ fontSize: 11.5, fontWeight: 700, color: '#c6bdf4', textTransform: 'uppercase', letterSpacing: 0.5, mb: 0.5 }}>Dine rettigheter</Typography>
      ...
      {p.showreelUrl && (
        <Stack direction="row" alignItems="center" spacing={0.75} sx={{ mt: 1.5 }}>
          ...
          <Button size="small" href={p.showreelUrl} target="_blank" rel="noopener" startIcon={<ShowreelIcon sx={{ fontSize: '15px !important' }} />} sx={{ color: '#c6bdf4', textTransform: 'none', fontSize: 12 }}>Se</Button>
        </Stack>
      )}
