...
function QuickAction({ eid, icon, label, onClick }: { eid: string; icon: React.ReactNode; label: string; onClick?: () => void }) {
  return (
    <Stack direction="row" alignItems="center" spacing={1.25} onClick={onClick} sx={{ py: 1.1, cursor: 'pointer', color: 'rgba(255,255,255,0.85)', '&:hover': { color: '#fff' } }}>
      <Box sx={{ width: 30, height: 30, borderRadius: 2, display: 'grid', placeItems: 'center', bgcolor: 'rgba(117, 107, 231,0.16)', color: '#c4b5fd', flexShrink: 0, '& svg': { fontSize: 16 } }}>{icon}</Box>
...
export function CohortsTab({ onNavigate }: { onNavigate?: (t: EducationTabId) => void }) {
  ...
  const kpis = [
    { id: 'studenter', label: 'Studenter', value: totalStudents, hint: 'På tvers av alle kull', color: '#c4b5fd', bg: 'rgba(117, 107, 231,0.16)', icon: <CohortIcon /> },
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 300px' }, gap: 2.75, alignItems: 'start' }}>
      <Box sx={{ display: 'grid', gap: 2.5, minWidth: 0 }}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'flex-start' }} spacing={2}>
          <Stack direction="row" spacing={1.75} alignItems="flex-start">
            <Box sx={{ width: 50, height: 50, borderRadius: 3, bgcolor: 'rgba(117, 107, 231,0.16)', color: '#c4b5fd', display: 'grid', placeItems: 'center', flexShrink: 0 }}><CohortIcon /></Box>
            <Box>
        ...
        <Collapse in={creating}>
          <Panel sx={{ border: '1px solid rgba(117, 107, 231,0.35)' }}>
            ...
            <Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ mt: 1.5 }}>
              ...
              <Button variant="contained" onClick={handleCreate} disabled={!newName.trim() || busy} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none' }}>{busy ? 'Oppretter…' : 'Opprett'}</Button>
            </Stack>
          </Panel>
        </Collapse>
        ...
        <Collapse in={!!launchId && bannerOpen}>
          <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 2.25, p: 2.5, borderRadius: 3.5, background: 'linear-gradient(100deg, rgba(117, 107, 231,0.16), rgba(99,102,241,0.08))', border: '1px solid rgba(117, 107, 231,0.32)', flexWrap: 'wrap' }}>
            <Box sx={{ width: 58, height: 58, borderRadius: 3.5, bgcolor: 'rgba(117, 107, 231,0.22)', color: '#c4b5fd', display: 'grid', placeItems: 'center', flexShrink: 0 }}><CanvasIcon sx={{ fontSize: 28 }} /></Box>
            <Box sx={{ flex: 1, minWidth: 200 }}>
              ...
              <T eid="edu-ks-canvas-course" sx={{ fontSize: 15, color: '#dedbf9', fontWeight: 600, mt: 0.25 }}>
            ...
            {sections && sections.sections.length > 0 ? (
              <Tooltip title={`Oppretter ett kull per Canvas-seksjon (${sections.sections.map((s) => `${s.section}: ${s.studentCount}`).join(' · ')})${sections.unsectioned ? ` · ${sections.unsectioned} uten seksjon` : ''}`}>
                <Button variant="contained" startIcon={<CohortIcon />} disabled={importing} onClick={handleImportBySection} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none', fontWeight: 700, borderRadius: 2, whiteSpace: 'nowrap' }}>
              ...
              <Button variant="contained" startIcon={<InviteIcon />} disabled={importing} onClick={() => handleImportFromCanvas({ cohortName: canvasCohortName })} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none', fontWeight: 700, borderRadius: 2, whiteSpace: 'nowrap' }}>
        ...
        <Box sx={{ display: 'grid', gap: 2.5, gridTemplateColumns: { xs: '1fr', md: '300px 1fr' }, alignItems: 'start' }}>
          <Panel>
            ...
            {visibleCohorts.length === 0 ? (
              ...
              return (
                <Box key={c.id} sx={{ position: 'relative', mb: 1 }}>
                  <Card sx={{ bgcolor: active ? 'rgba(117, 107, 231,0.08)' : 'transparent', border: `1px solid ${active ? 'rgba(117, 107, 231,0.55)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 2.5 }}>
            ...
            <Button fullWidth startIcon={<ArchiveIcon />} onClick={() => setShowArchived((v) => !v)} sx={{ mt: 0.5, borderRadius: 2, textTransform: 'none', color: showArchived ? '#c4b5fd' : 'rgba(255,255,255,0.7)', border: `1px solid ${showArchived ? 'rgba(117, 107, 231,0.5)' : 'rgba(255,255,255,0.12)'}` }}>
          ...
          <Panel sx={{ p: 0, overflowX: 'auto' }}>
            ...
            {selected && (
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ px: 2, pb: 1.5 }}>
                ...
                <Button variant="outlined" startIcon={<InviteIcon />} onClick={handleAddStudent} disabled={!addName.trim() || busy} sx={{ borderColor: 'rgba(117, 107, 231,0.5)', color: '#dedbf9', textTransform: 'none', borderRadius: 2, whiteSpace: 'nowrap' }}>Legg til</Button>
              </Stack>
            )}
              ...
              return (
                <Box key={s.id} sx={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 40px', minWidth: 560, alignItems: 'center', px: 2, py: 1.25, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <Stack direction="row" alignItems="center" spacing={1.25} sx={{ minWidth: 0 }}>
                    <Avatar sx={{ width: 30, height: 30, fontSize: 11, bgcolor: 'rgba(117, 107, 231,0.3)', color: '#dedbf9' }}>{initials(s.name)}</Avatar>
                    <Box sx={{ minWidth: 0 }}>
        ...
        <Panel>
          ...
          <Tooltip title={launchId ? (selected ? `Synk rosteret inn i «${selected.name}»` : 'Synk rosteret til et nytt kull') : 'Åpne Role Room fra Canvas for å synkronisere'}>
            <span style={{ display: 'block' }}>
              <Button fullWidth variant="contained" startIcon={<SyncIcon />} disabled={!launchId || importing} onClick={() => handleImportFromCanvas(selected ? { cohortId: selected.id } : { cohortName: canvasCohortName })} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none', fontWeight: 700, borderRadius: 2 }}>
      ...
      <Dialog open={csvOpen} onClose={() => setCsvOpen(false)} maxWidth="sm" fullWidth
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={handleImportCsv} disabled={csvPreview.length === 0 || csvBusy} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none', fontWeight: 700 }}>{csvBusy ? 'Importerer…' : `Importer ${csvPreview.length || ''}`}</Button>
        </DialogActions>
      </Dialog>
