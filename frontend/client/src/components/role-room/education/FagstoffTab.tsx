...
export function FagstoffTab() {
  ...
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 300px' }, gap: 2.75, alignItems: 'start' }}>
      <Box sx={{ display: 'grid', gap: 2.5, minWidth: 0 }}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'flex-start' }} spacing={2}>
          <Stack direction="row" spacing={1.75} alignItems="flex-start">
            <Box sx={{ width: 50, height: 50, borderRadius: 3, bgcolor: 'rgba(93, 118, 203,0.16)', color: '#c3cbe6', display: 'grid', placeItems: 'center', flexShrink: 0 }}><LibraryIcon /></Box>
            <Box>
          ...
          <Panel sx={{ border: '1px solid rgba(93, 118, 203,0.35)' }}>
            <Stack spacing={1.5}>
              ...
              <Stack direction="row" justifyContent="flex-end" spacing={1}>
                ...
                <Button variant="contained" onClick={handleCreate} disabled={!f.title.trim() || busy} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#4b3d8f' }, textTransform: 'none' }}>{busy ? 'Lagrer…' : 'Legg til leksjon'}</Button>
              </Stack>
            </Stack>
          </Panel>
        </Collapse>
          ...
          <Box key={g.cat.key}>
            ...
            <Box sx={{ display: 'grid', gap: 1.5, gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' } }}>
              {g.items.map((r) => {
                ...
                return (
                  <Panel key={r.id} sx={{ p: 2, position: 'relative', '&:hover': { borderColor: 'rgba(93, 118, 203,0.4)' } }}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={1} sx={{ mt: 1.25 }}>
                      {r.url && <Button size="small" href={r.url} target="_blank" rel="noopener" startIcon={<LinkIcon sx={{ fontSize: '14px !important' }} />} sx={{ color: '#c3cbe6', textTransform: 'none', fontSize: 12 }}>Åpne</Button>}
                    </Stack>
      ...
      <Stack spacing={2}>
        <Panel sx={{ bgcolor: 'rgba(93, 118, 203,0.08)', border: '1px solid rgba(93, 118, 203,0.28)' }}>
          <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1 }}>
            <SuggestIcon sx={{ fontSize: 18, color: '#c3cbe6' }} />
          ...
          {suggestions.length === 0 ? (
            ...
            <Box key={i} sx={{ py: 1, borderTop: i > 0 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
              ...
              <Button size="small" startIcon={<AddIcon sx={{ fontSize: '14px !important' }} />} onClick={() => handleAddSuggested(s)} disabled={busy} sx={{ color: '#c3cbe6', textTransform: 'none', fontSize: 12, mt: 0.5, px: 0 }}>Legg til</Button>
            </Box>
          ))}
        </Panel>
