...
const THUMBS = ['linear-gradient(135deg,#1e3a5f,#2a4a6f)', 'linear-gradient(135deg,#1a4a2e,#2f6b3f)', 'linear-gradient(135deg,#3a3550,#4a4560)', 'linear-gradient(135deg,#261f5f,#362f6f)'];
  ...
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 300px' }, gap: 2.75, alignItems: 'start' }}>
      <Box sx={{ display: 'grid', gap: 2.5, minWidth: 0 }}>
        ...
        <Collapse in={creating}>
          <Panel sx={{ border: '1px solid rgba(117, 107, 231,0.35)' }}>
            ...
            <Stack direction="row" justifyContent="flex-end" spacing={1} sx={{ mt: 1.5 }}>
              ...
              <Button variant="contained" onClick={handleCreate} disabled={!title.trim() || busy} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none' }}>{busy ? 'Oppretter…' : 'Opprett'}</Button>
            </Stack>
          </Panel>
        </Collapse>
        ...
        <Box sx={{ display: 'grid', gap: 1.75, gridTemplateColumns: { xs: '1fr 1fr', lg: 'repeat(4, 1fr)' } }}>
          {kpis.map((k) => (
            <Panel key={k.id} sx={{ p: 2 }}>
              <Stack direction="row" alignItems="center" spacing={1.5}>
                <Box sx={{ width: 44, height: 44, borderRadius: 2.5, display: 'grid', placeItems: 'center', bgcolor: 'rgba(117, 107, 231,0.16)', color: '#c4b5fd', flexShrink: 0, '& svg': { fontSize: 22 } }}>{k.icon}</Box>
                <Box>
            ...
            return (
              <Stack key={p.id} direction="row" alignItems="center" spacing={2.25} sx={{ py: 2, borderTop: i > 0 ? '1px solid rgba(255,255,255,0.06)' : 'none', flexWrap: { xs: 'wrap', xl: 'nowrap' } }}>
                ...
                <Box sx={{ width: 110, flexShrink: 0 }}>
                  ...
                  <Box sx={{ height: 6, borderRadius: 3, bgcolor: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}><Box sx={{ height: '100%', width: `${pl.progress}%`, borderRadius: 3, background: `linear-gradient(90deg, ${pl.color}, #9d97ee)` }} /></Box>
                </Box>
        ...
        {/* Kom i gang-bånd */}
        <Stack direction={{ xs: 'column', md: 'row' }} alignItems={{ md: 'center' }} spacing={2} sx={{ p: 2.25, borderRadius: 3, border: '1px dashed rgba(117, 107, 231,0.35)', bgcolor: 'rgba(117, 107, 231,0.05)' }}>
          <Box sx={{ width: 44, height: 44, borderRadius: 2.5, bgcolor: 'rgba(117, 107, 231,0.2)', color: '#c4b5fd', display: 'grid', placeItems: 'center', flexShrink: 0 }}><RocketIcon /></Box>
          <Box sx={{ flex: 1 }}>
          ...
          <Button variant="contained" onClick={() => setCreating(true)} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none', fontWeight: 700, borderRadius: 2 }}>
          ...
          {upcoming.length === 0 ? (
            ...
            return (
              <Stack key={a.id} direction="row" spacing={1.25} sx={{ py: 1, cursor: a.productionProjectId ? 'pointer' : 'default' }}
                ...
                <Box sx={{ width: 30, height: 30, borderRadius: 2, display: 'grid', placeItems: 'center', bgcolor: soon ? 'rgba(236,72,153,0.16)' : 'rgba(117, 107, 231,0.16)', color: soon ? '#ec4899' : '#c4b5fd', flexShrink: 0 }}><DeliverIcon sx={{ fontSize: 15 }} /></Box>
                <Box sx={{ minWidth: 0 }}>
      ...
      <Dialog open={tmplOpen} onClose={() => setTmplOpen(false)} maxWidth="sm" fullWidth
        ...
        <DialogContent>
          <Box sx={{ display: 'grid', gap: 1, gridTemplateColumns: '1fr 1fr', mb: 2 }}>
            {PRODUCTION_TEMPLATES.map((t) => {
              ...
              return (
                <Card key={t.id} sx={{ bgcolor: active ? 'rgba(117, 107, 231,0.12)' : 'rgba(255,255,255,0.03)', border: `1px solid ${active ? 'rgba(117, 107, 231,0.55)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 2.5 }}>
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={handleCreateFromTemplate} disabled={!tmplTitle.trim() || tmplBusy} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#5446e1' }, textTransform: 'none', fontWeight: 700 }}>{tmplBusy ? 'Oppretter…' : 'Opprett fra mal'}</Button>
        </DialogActions>
      </Dialog>
          ...
          {membersBusy ? (
            ...
            <Stack key={m.studentId} direction="row" alignItems="center" justifyContent="space-between" spacing={1} sx={{ py: 1, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <Select size="small" value={m.assigned ? m.role : 'none'} onChange={(e) => setMemberRole(m.studentId, e.target.value as MemberRole | 'none')}
                sx={{ fontSize: 12.5, minWidth: 150, color: m.assigned ? '#dedbf9' : 'rgba(255,255,255,0.72)', '& .MuiOutlinedInput-notchedOutline': { borderColor: 'rgba(255,255,255,0.12)' }, '& .MuiSelect-select': { py: 0.75 } }}>
