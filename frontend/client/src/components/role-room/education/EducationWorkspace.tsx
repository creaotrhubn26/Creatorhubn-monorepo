  ...
  return (
    <Card sx={{ bgcolor: 'rgba(117, 107, 231,0.06)', border: '1px solid rgba(117, 107, 231,0.24)', borderRadius: 3 }}>
      <CardContent sx={{ p: 4, textAlign: 'center' }}>
        ...
        <Chip label="Kommer i neste skive" size="small" sx={{ mt: 2, bgcolor: 'rgba(117, 107, 231,0.22)', color: '#dedbf9', fontWeight: 600 }} />
      </CardContent>
    </Card>
  );
}
      ...
      {EDUCATION_TABS.map((t) => {
        ...
        return (
          <Box key={t.id} onClick={() => onNavigate(t.id)} data-edit-id={`edu-nav-${t.id}`} sx={{
            ...
            bgcolor: active ? 'rgba(117, 107, 231,0.16)' : 'transparent',
            boxShadow: active ? 'inset 3px 0 0 #756be7' : 'none',
            transition: 'background .15s, color .15s',
            '&:hover': { bgcolor: active ? 'rgba(117, 107, 231,0.2)' : 'rgba(255,255,255,0.05)', color: '#fff' },
...
function Sidebar({ activeTab, onNavigate }: { activeTab: EducationTabId; onNavigate: (t: EducationTabId) => void }) {
  return (
    <Box component="nav" sx={{
      ...
      background: 'linear-gradient(180deg, rgba(10,10,10,0.5) 0%, rgba(9, 8, 22,0.86) 100%), url(/trr-edu-sidebar-bg.png)',
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, px: 1, py: 1, mt: 0.5, borderTop: '1px solid rgba(255,255,255,0.07)', pt: 1.25 }}>
        <Avatar sx={{ width: 30, height: 30, bgcolor: 'rgba(117, 107, 231,0.3)', color: '#dedbf9', fontSize: 12, fontWeight: 700 }}>TRR</Avatar>
...
function TopBar({ onHelp, onMenu }: { onHelp: () => void; onMenu: () => void }) {
  ...
  return (
    <Box sx={{
      ...
      {/* Modus-velger */}
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.5, py: 0.75, borderRadius: 2, border: '1px solid rgba(255,255,255,0.12)', bgcolor: 'rgba(117, 107, 231,0.1)', flexShrink: 0 }}>
      ...
      <Stack direction="row" spacing={0.5} alignItems="center">
        ...
        <Stack direction="row" spacing={1} alignItems="center" sx={{ pl: 1 }}>
          ...
          <Avatar sx={{ width: 34, height: 34, bgcolor: 'rgba(117, 107, 231,0.35)', color: '#dedbf9', fontSize: 13, fontWeight: 700 }}>F</Avatar>
        </Stack>
      </Stack>
    </Box>
  );
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#0a0a0a', color: '#fff' }}>
      ...
      <Drawer open={mobileNavOpen} onClose={() => setMobileNavOpen(false)}
        ...
        sx={{ display: { xs: 'block', md: 'none' }, '& .MuiDrawer-paper': { width: 272, boxSizing: 'border-box', p: 1.75, bgcolor: '#090816', color: '#fff', borderRight: '1px solid rgba(255,255,255,0.08)' } }}>
