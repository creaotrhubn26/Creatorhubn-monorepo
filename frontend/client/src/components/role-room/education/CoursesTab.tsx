    ...
    <Box sx={{ display: 'grid', gap: 2.5 }}>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'flex-start' }} spacing={2}>
        <Stack direction="row" spacing={1.75} alignItems="flex-start">
          <Box sx={{ width: 50, height: 50, borderRadius: 3, bgcolor: 'rgba(93, 118, 203,0.16)', color: '#c3cbe6', display: 'grid', placeItems: 'center', flexShrink: 0 }}><CourseIcon /></Box>
          <Box>
        ...
        <Stack direction="row" spacing={1.25} flexWrap="wrap" useFlexGap>
          ...
          <Button variant="contained" startIcon={<AddIcon />} onClick={openNew} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#4b3d8f' }, textTransform: 'none', fontWeight: 700, borderRadius: 2 }}>
      ...
      <Collapse in={editing !== null}>
        <Panel sx={{ border: '1px solid rgba(93, 118, 203,0.35)' }}>
          ...
          <Stack spacing={1.5}>
            ...
            <T eid="edu-em-form-lu" sx={{ fontSize: 11.5, fontWeight: 700, color: '#c3cbe6', textTransform: 'uppercase', letterSpacing: 0.5, mt: 0.5 }}>Læringsutbytte (én linje per punkt)</T>
            ...
            <Stack direction="row" justifyContent="flex-end" spacing={1}>
              ...
              <Button variant="contained" onClick={save} disabled={!f.title.trim() || busy} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#4b3d8f' }, textTransform: 'none' }}>{busy ? 'Lagrer…' : 'Lagre emne'}</Button>
            </Stack>
          </Stack>
        </Panel>
      </Collapse>
                ...
                <Stack direction="row" alignItems="flex-start" justifyContent="space-between">
                  <Box sx={{ minWidth: 0, pr: 6 }}>
                    <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap" useFlexGap>
                      {c.code && <Chip label={c.code} size="small" sx={{ height: 20, fontSize: 10.5, fontWeight: 800, bgcolor: 'rgba(93, 118, 203,0.2)', color: '#c3cbe6' }} />}
                ...
                {luCount > 0 && (
                  <Box sx={{ mt: 1.5, display: 'grid', gap: 1, gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' } }}>
                    {([['Kunnskap', lu.knowledge], ['Ferdigheter', lu.skills], ['Generell kompetanse', lu.generalCompetence]] as [string, string[]][]).map(([label, items]) => (
                      <Box key={label}>
                        <Typography sx={{ fontSize: 10, fontWeight: 700, color: '#c3cbe6', textTransform: 'uppercase', letterSpacing: 0.5, mb: 0.5 }}>{label}</Typography>
      ...
      <Dialog open={packOpen} onClose={() => setPackOpen(false)} maxWidth="sm" fullWidth
        ...
        <DialogContent>
          ...
          <Stack spacing={1} sx={{ mb: 2 }}>
            {STUDY_PLAN_PACKS.map((p) => {
              ...
              return (
                <Card key={p.id} sx={{ bgcolor: active ? 'rgba(93, 118, 203,0.12)' : 'rgba(255,255,255,0.03)', border: `1px solid ${active ? 'rgba(93, 118, 203,0.55)' : 'rgba(255,255,255,0.08)'}`, borderRadius: 2.5 }}>
                  <CardActionArea onClick={() => setPackId(p.id)} sx={{ p: 1.75 }}>
                    ...
                    <Typography sx={{ fontSize: 11, color: '#c3cbe6', mt: 0.75, fontWeight: 600 }}>{p.courses.length} emner · {arbeidskrav} arbeidskrav · {p.exampleInstitutions}</Typography>
                  </CardActionArea>
                </Card>
              );
            })}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          <Button variant="contained" onClick={adoptPack} disabled={packBusy} sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#4b3d8f' }, textTransform: 'none', fontWeight: 700 }}>{packBusy ? 'Oppretter…' : 'Adopter'}</Button>
        </DialogActions>
      </Dialog>
