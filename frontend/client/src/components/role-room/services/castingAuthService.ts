...
export const castingAuthService = {
  ...
  async getUserRole(projectId: string, userId?: string): Promise<UserRole | null> {
    ...
    const accessPermissions = Object.fromEntries(
      Object.entries({ ...access.permissions, ...access.grants })
        .filter((entry): entry is [string, boolean] => typeof entry[1] === 'boolean'),
    ) as UserRole['permissions'];
    const additionalRoles = access.roles.filter((role) => role !== access.role);
    if (matched) {
      const effectiveRole = access.role as UserRoleType;
      return {
        ...matched,
        role: effectiveRole,
        additionalRoles,
        serverGrants: access.grants,
        permissions: {
          ...this.getDefaultPermissions(effectiveRole),
          ...(matched.permissions ?? {}),
          ...accessPermissions,
        },
      };
    ...
    return {
      ...
      serverGrants: access.grants,
      permissions: {
        ...this.getDefaultPermissions(access.role as UserRoleType),
        ...accessPermissions,
      },
      createdAt: now,
      updatedAt: now,
  ...
  async canViewEconomy(projectId: string, userId?: string): Promise<boolean> {
    const userRole = await this.getUserRole(projectId, userId);
    if (!userRole) return false;
    return ['director', 'producer', 'production_manager', 'content_producer', 'client_reviewer'].includes(userRole.role) ||
