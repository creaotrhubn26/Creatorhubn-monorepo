/**
 * GameInviteLandingPage — 2-stegs magic-link + PIN-bekreftelse for spillstudio-team-invite.
 *
 * GDPR/sikkerhet:
...
import type { InvitePublicInfo } from './gameTeamService';
...
export const GameInviteLandingPage: React.FC<Props> = ({ token }) => {
  ...
  const submitPin = async () => {
    ...
    try {
      ...
      setStep('done');
      setTimeout(() => {
        navigate(`/?mode=game_studio&team=${encodeURIComponent(r.teamOrganizationId)}`);
      }, 1400);
    } catch (e) {
      ...
      <Layout>
        <Stack spacing={3} alignItems="center" sx={{ textAlign: 'center', maxWidth: 460 }}>
          ...
          <Typography sx={{ fontSize: 14, color: TEXT_DIM }}>
            som <Box component="span" sx={{ color: PURPLE_LIGHT, fontWeight: 700 }}>{info.invitedRoleLabel}</Box> i et spillstudio (Story Graph) på CreatorHub.
          </Typography>

          <Box sx={{ width: '100%', mt: 2, p: 2, borderRadius: 2, bgcolor: 'rgba(34,197,94,0.06)', border: `1px solid ${PANEL_BORDER}`, display: 'flex', alignItems: 'center', gap: 1.5 }}>
  ...
  return (
    <Layout>
      <Stack spacing={2.5} sx={{ maxWidth: 440, width: '100%' }}>
        ...
        {/* GDPR consent */}
        <Box sx={{ p: 1.5, borderRadius: 1.5, bgcolor: 'rgba(34,197,94,0.04)', border: `1px solid ${PANEL_BORDER}` }}>
          <FormControlLabel
            control={
              <Checkbox
                checked={consent}
...
export default GameInviteLandingPage;
