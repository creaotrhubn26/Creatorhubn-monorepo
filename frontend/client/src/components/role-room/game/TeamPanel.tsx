/**
 * TeamPanel — Studio-eierens kontrollsenter for spillstudio-team:
 ...
 * Mountes via NarrativeWorkspace case 'team' (Studio-plan: team_seats).
 *
 * UI-skjema: glassmorphism + grønne Story Graph-tokens (kopi av dans sitt panel);
...
import * as svc from './gameTeamService';
import type { GameTeamRole, GameTeamMember, GameTeamInvite, GameTeamSummary, MyMembership } from './gameTeamService';

// ─── Branding tokens ────────────────────────────────────────────────────

const PURPLE_DEEP   = '#052e16';
const PURPLE_BRIGHT = narrativeColors.accentDark;
const PURPLE_LIGHT  = narrativeColors.accent;
const PURPLE_GLASS  = 'rgba(34,197,94,0.10)';
const TEXT_DIM      = 'rgba(229,231,235,0.65)';
const TEXT_MUTED    = 'rgba(229,231,235,0.45)';
const PANEL_BG      = narrativeColors.bgPanel;
const PANEL_BORDER  = narrativeColors.borderStrong;
...
export const TeamPanel: React.FC = () => {
  ...
  const [roles, setRoles] = React.useState<GameTeamRole[]>([]);
  const [members, setMembers] = React.useState<GameTeamMember[]>([]);
  const [invites, setInvites] = React.useState<GameTeamInvite[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [error, setError] = React.useState<string | null>(null);

  // UI state
  const [inviteOpen, setInviteOpen] = React.useState(false);
  const [editingRole, setEditingRole] = React.useState<GameTeamRole | null>(null);
    ...
    try {
      ...
      if (!m || !m.team) {
        try {
          ...
          // ignore — kan være medlem uten eget team
        }
      }
      setMembership(m);
      if (m && m.team) {
    ...
    return (
      <Box sx={{ p: 3, textAlign: 'center' }}>
        <Typography sx={{ color: TEXT_DIM }}>
          Du har ikke tilgang til team-administrasjon her. Team eies av studioets
          prosjekteier — be eieren invitere deg.
        </Typography>
      </Box>
    );
  }
        ...
        <InviteDialog
          ...
          onCreated={async (token) => {
            ...
            if (typeof window !== 'undefined') {
              ...
              w.dataLayer.push({ event: 'game_invite_created', invite_token: token });
            }
            // Kopier invite-link til clipboard som UX-bonus
            const link = `${window.location.origin}/game/invite/${token}`;
            try {
...
const TeamHeader: React.FC<{
  ...
  >
    <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'center' }} justifyContent="space-between">
      <Stack spacing={0.5}>
        <Typography sx={sectionLabel}>Spillstudio · Team</Typography>
...
const MemberRow: React.FC<{
  ...
  const role = roles.find((r) => r.id === member.gameRoleId);
        ...
        <>
          <TextField
            ...
            onChange={async (e) => {
              ...
              if (!newRoleId || newRoleId === member.gameRoleId) return;
              try {
...
const RolesList: React.FC<{
  roles: GameTeamRole[];
  canManage: boolean;
  onEdit: (r: GameTeamRole) => void;
  onDelete: (r: GameTeamRole) => void;
        ...
        <Stack key={r.id} direction="row" alignItems="center" spacing={2} sx={{ py: 1.25 }} data-testid={`team-role-row-${r.id}`}>
          <Box sx={{ flexGrow: 1, minWidth: 0 }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              {r.isOwnerRole ? (
                <Chip label="Eier · låst" size="small" sx={{ bgcolor: 'rgba(245,158,11,0.18)', color: narrativeColors.warning, fontWeight: 700, height: 18, fontSize: 10 }} />
              ) : null}
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md" PaperProps={{ sx: { bgcolor: '#0f0a1c', border: `1px solid ${PANEL_BORDER}` }, 'data-testid': 'team-role-dialog' } as never}>
      <DialogTitle sx={{ color: 'rgba(237,233,254,0.95)' }}>
        ...
        {isOwner ? <Chip label="Eier — capabilities er låst til alt" size="small" sx={{ ml: 2, bgcolor: 'rgba(245,158,11,0.18)', color: narrativeColors.warning, fontWeight: 700 }} /> : null}
      </DialogTitle>
      <DialogContent>
        <Stack spacing={2.5} sx={{ pt: 1 }}>
          {err ? <Alert severity="error">{err}</Alert> : null}
