...
export default function UniversalDashboardCalendarIntegration({
  ...
  const colors = {
    primary: professionConfig?.color || '#FF8C00',
    secondary: professionConfig?.secondaryColor || '#FF6B00',
    accent: '#4CAF50',
  ...
  const getProfessionSpecificSuggestions = () => {
    ...
    switch (professionConfig.id) {
      case 'photographer':
        return [
          'Fotosesjon','Bryllupsfotografering','Portrettfotografering','Produktfotografering','Utstyrsjekk',
        ];
      case 'videographer':
      ...
      case 'vendor':
        return [
          'Leveranse','Kundekonsultasjon','Produktdemonstrasjon','Vedlikehold','Opplæring',
        ];
  ...
  if (dashboardLoading) {
    return (
      <MuiCard sx={{ mb:  3 }}>
        <CardContent sx={{ display: 'flex', justifyContent: 'center', py:  4 ,  ...theming.getThemedCardSx() }}>
          <CircularProgress />
        </CardContent>
      </MuiCard>
    );
              ...
              <List dense>
                {todayEvents.slice, (3).map((event: any) => (
                  <ListItem key={event.d} sx={{ px:  0 }}>
                    <ListItemIcon>
                      <Event sx={{ color: colors.primary, fontSize: 20}} />
                    </ListItemIcon>
                    <ListItemText
