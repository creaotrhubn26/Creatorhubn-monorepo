...
const fallbackCourses: Course[] = [
  {
    ...
    description: 'Advanced directing for cinematic productions.',
    instructor: {
      id: 'analytics-instructor-1',
      name: 'Sarah Johnson',
      avatar: '',
      bio: 'Film director',
  ...
  {
    id: 'analytics-course-2',
    title: 'Lighting Techniques',
    description: 'Practical cinematic lighting workflows.',
    instructor: {
      id: 'analytics-instructor-2',
      name: 'Adrian Berglund',
      avatar: '',
      bio: 'Lighting specialist',
      profession: 'videographer',
    },
    thumbnail: '',
    videoUrl: '/assets/academy/intro-video.mp4',
  ...
  {
    id: 'analytics-course-3',
    title: 'Advanced Cinematography',
    description: 'Composition and camera language at pro level.',
    instructor: {
      id: 'analytics-instructor-3',
      name: 'Lisa Holden',
      avatar: '',
      bio: 'Cinematographer',
      profession: 'videographer',
...
const defaultTopStudents: TopStudentRow[] = [
  { id: 's1', name: 'Sarah Johnson', role: 'Filmmaker', growth: 9.5, progress: 58, avatarTheme: 0 },
  { id: 's2', name: 'Adrian Berglund', role: 'Cinematographer', growth: 30, progress: 66, avatarTheme: 1 },
  { id: 's3', name: 'Lisa Holden', role: 'Drone Pilot', growth: 32, progress: 41, avatarTheme: 2 },
];
...
const buildSeries = (start: number, end: number, points: number, jitter = 0.08): number[] => {
...
const pathFromSeries = (values: number[], width: number, height: number, pad = 12): string => {
...
function AcademyAnalyticsStudio({ courseId }: AcademyAnalyticsStudioProps) {
  ...
  const filteredCourses = useMemo(() => {
    ...
    return courses.filter((course) => String(course.id) === String(selectedCourseId));
  ...
  }, [courseId, courses, filteredCourses, getCourse, selectedCourseId, state.currentCourse]);

  const filteredEnrollments = useMemo(() => {
    if (selectedCourseId === 'all') return enrollments;
    return enrollments.filter((enrollment) => String(enrollment.courseId) === String(selectedCourseId));
  }, [enrollments, selectedCourseId]);

  const enrolledCount = useMemo(() => {
    if (filteredEnrollments.length > 0) return filteredEnrollments.length;
    const fromStudents = filteredCourses.reduce((sum, course) => sum + Number(course.studentCount || 0), 0);
    return Math.max(1250, fromStudents || 0);
  }, [filteredCourses, filteredEnrollments.length]);

  const activeStudents = useMemo(() => {
    if (filteredEnrollments.length > 0) {
      const unique = new Set(filteredEnrollments.map((row) => String(row.studentId || 'guest')));
      return unique.size;
    }
  ...
  const engagementRate = useMemo(() => {
    if (filteredEnrollments.length > 0) {
      const values = filteredEnrollments.map((row) => progressFromEnrollment(row));
      return clamp(Math.round(values.reduce((sum, value) => sum + value, 0) / Math.max(values.length, 1)), 0, 100);
    }
    return 78;
  }, [filteredEnrollments]);

  const revenueTotal = useMemo(() => {
    const fallback = Math.max(92450, filteredCourses.reduce((sum, course) => sum + Number(course.price || 0) * Math.max(1, Number(course.studentCount || 1)) * 0.21, 0));
    if (filteredEnrollments.length === 0) return Math.round(fallback);

    const byCourse = filteredCourses.reduce((sum, course) => {
      const count = filteredEnrollments.filter((row) => String(row.courseId) === String(course.id)).length;
  ...
  const revenueBreakdown = useMemo(() => {
    ...
    return [
      { id: 'sales', label: 'Course Sales', value: courseSales, color: '#f8b321' },
      { id: 'subscriptions', label: 'Subscriptions', value: subscriptions, color: '#8ea6d6' },
      { id: 'affiliates', label: 'Affiliates', value: affiliates, color: '#f5d47a' },
    ];
  }, [revenueTotal]);

  const topCourses = useMemo<TopCourseRow[]>(() => {
    const rows = filteredCourses.map((course, index) => {
      const enrollmentCountForCourse = filteredEnrollments.filter((entry) => String(entry.courseId) === String(course.id)).length;
      const fallbackEnrollments = Number(course.studentCount || 0);
      const effectiveEnrollments = Math.max(enrollmentCountForCourse, fallbackEnrollments, 1);
      const revenue = Math.round(effectiveEnrollments * Number(course.price || 0));
  ...
  const topStudents = useMemo<TopStudentRow[]>(() => {
    ...
    const grouped = new Map<string, { progress: number[]; courses: Set<string> }>();
    filteredEnrollments.forEach((entry) => {
      const key = String(entry.studentId || 'guest');
      const bucket = grouped.get(key) || { progress: [], courses: new Set<string>() };
    ...
    return Array.from(grouped.entries())
      .map(([studentId, values], index) => {
        const progress = Math.round(values.progress.reduce((sum, value) => sum + value, 0) / Math.max(values.progress.length, 1));
        return {
  ...
  const sourceRows = useMemo<SourceRow[]>(() => {
    const base = [
      { id: 'src-1', label: 'Instagram Ad', secondaryLabel: 'Community Email', weight: 0.34, change: 11.6 },
      { id: 'src-2', label: 'YouTube Video', secondaryLabel: 'YouTube Video', weight: 0.31, change: 9.4 },
      { id: 'src-3', label: 'Creator Referral', secondaryLabel: 'Podcast Episode', weight: 0.22, change: 8.2 },
    ];
  ...
  const growthPoints = useMemo(() => {
    ...
    const enrollSeries = buildSeries(Math.round(enrolledCount * 0.28), enrolledCount, points, 0.12);
    const activeSeries = buildSeries(Math.round(activeStudents * 0.34), activeStudents, points, 0.1);
    return { enrollSeries, activeSeries };
  }, [activeStudents, enrolledCount, growthMode]);

  const engagementPoints = useMemo(() => {
    const completionSeries = buildSeries(Math.max(35, engagementRate - 22), engagementRate, 8, 0.06);
    const watchTimeSeries = buildSeries(Math.max(30, engagementRate - 26), Math.max(50, engagementRate - 2), 8, 0.08);
    return { completionSeries, watchTimeSeries };
  }, [engagementRate]);

  const growthEnrollPath = useMemo(() => pathFromSeries(growthPoints.enrollSeries, 680, 260), [growthPoints.enrollSeries]);
  const growthActivePath = useMemo(() => pathFromSeries(growthPoints.activeSeries, 680, 260), [growthPoints.activeSeries]);
  ...
  const revenueConic = useMemo(() => {
    const total = Math.max(1, revenueBreakdown.reduce((sum, row) => sum + row.value, 0));
    let cursor = 0;
    ...
    >
      <Box
        sx={{
          ...
          pointerEvents: 'none',
          background:
            'radial-gradient(circle at 76% 13%, rgba(248,179,33,0.25), rgba(5,8,13,0) 42%), radial-gradient(circle at 15% 80%, rgba(82,121,204,0.13), rgba(6,8,14,0) 44%), linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0) 30%)',
        }}
      />
          ...
          >
            ...
            <Stack direction="row" spacing={1.2} alignItems="center">
              ...
              <Avatar sx={{ width: 34, height: 34, bgcolor: '#f8b321', color: '#111' }}>
                {String(activeCourse?.instructor?.name || 'C').charAt(0).toUpperCase()}
              </Avatar>
            </Stack>
          </Box>
          ...
          <Box sx={{ flex: 1, minHeight: 0, p: 2, overflow: 'auto' }}>
            ...
            <Box sx={{ mt: 1.1, display: 'grid', gridTemplateColumns: { xs: '1fr', xl: 'minmax(0, 1fr) 520px' }, gap: 1 }}>
              <Box sx={{ ...panelSx, p: 1 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.8 }}>
                  ...
                  <Stack direction="row" spacing={0.8}>
                    <Select
                      size="small"
                      value={growthMode}
                      onChange={(event) => setGrowthMode(event.target.value as 'daily' | 'weekly')}
                      sx={{
                        minWidth: 100,
                        color: '#edf0f7',
                        '& .MuiOutlinedInput-notchedOutline': { borderColor: 'rgba(255,255,255,0.16)' },
                ...
                >
                  <Box component="svg" viewBox="0 0 680 260" sx={{ width: '100%', height: 220, display: 'block' }}>
                    <defs>
                      <pattern id="grid-growth" width="68" height="52" patternUnits="userSpaceOnUse">
                        <path d="M 68 0 L 0 0 0 52" fill="none" stroke="rgba(255,255,255,0.07)" strokeWidth="1" />
                      </pattern>
                    </defs>
                    <rect x="0" y="0" width="680" height="260" fill="url(#grid-growth)" />
                    <path d={growthEnrollPath} fill="none" stroke="#f8b321" strokeWidth="3" strokeLinecap="round" />
                    <path d={growthActivePath} fill="none" stroke="#8ea6d6" strokeWidth="3" strokeLinecap="round" />
                  </Box>
                </Box>
                  ...
                  {sourceRows.map((source) => (
                    ...
                    >
                      <Box>
                        ...
                        <Typography sx={{ fontSize: 12, color: 'rgba(237,240,247,0.6)' }}>{source.secondaryLabel}</Typography>
                      </Box>
                      <Stack direction="row" spacing={1} alignItems="center">
                        <Typography sx={{ color: 'rgba(237,240,247,0.8)' }}>{formatInteger(source.count)}</Typography>
                        <Typography sx={{ color: '#9acd6f', fontWeight: 700 }}>▲ {source.changePct}%</Typography>
                      </Stack>
                    </Stack>
                  ))}
                </Stack>
                ...
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.8 }}>
                  ...
                  <Stack direction="row" spacing={0.8}>
                    ...
                    <Select
                      size="small"
                      value={revenueRange}
                      onChange={(event) => setRevenueRange(event.target.value as RangeFilter)}
                      sx={{
                        minWidth: 130,
                        color: '#edf0f7',
                        '& .MuiOutlinedInput-notchedOutline': { borderColor: 'rgba(255,255,255,0.16)' },
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.2}>
                  ...
                  <Stack spacing={0.6} sx={{ flex: 1 }}>
                    {revenueBreakdown.map((row) => {
                      const ratio = revenueTotal === 0 ? 0 : (row.value / revenueTotal) * 100;
                      return (
                        <Box key={row.id}>
                          <Stack direction="row" justifyContent="space-between" alignItems="center">
                            ...
                            <Typography sx={{ fontWeight: 700 }}>{formatNok(row.value)}</Typography>
                          </Stack>
                          <LinearProgress
                            variant="determinate"
                            value={ratio}
                      ...
                      {topCourses.map((course, index) => (
                        ...
                        >
                          ...
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            <Typography sx={{ fontWeight: 600 }} noWrap>{course.title}</Typography>
                            <Typography sx={{ fontSize: 12, color: 'rgba(237,240,247,0.64)' }} noWrap>
                              {formatNok(course.revenue)}
                            </Typography>
                          </Box>
                          <Typography sx={{ color: '#f8d56f', fontWeight: 700 }}>▲ {course.growth}%</Typography>
                        </Stack>
                      ))}
                    </Stack>
                  </Box>
                    ...
                    <Stack spacing={0.6}>
                      {topStudents.map((student, index) => (
                        ...
                        >
                          ...
                          <Box sx={{ flex: 1, minWidth: 0 }}>
                            <Typography sx={{ fontWeight: 600 }} noWrap>{student.name}</Typography>
                            <Typography sx={{ fontSize: 12, color: 'rgba(237,240,247,0.64)' }} noWrap>{student.role}</Typography>
                          </Box>
                          <Typography sx={{ color: '#9acd6f', fontWeight: 700 }}>▲ {student.growth}%</Typography>
                        </Stack>
                      ))}
                    </Stack>
                  </Box>
            ...
            <Box sx={{ mt: 1, display: 'grid', gridTemplateColumns: { xs: '1fr', xl: 'minmax(0, 1fr) 520px' }, gap: 1 }}>
              <Box sx={{ ...panelSx, p: 1 }}>
                ...
                >
                  <Box component="svg" viewBox="0 0 680 220" sx={{ width: '100%', height: 200, display: 'block' }}>
                    <defs>
                      <pattern id="grid-engagement" width="68" height="44" patternUnits="userSpaceOnUse">
                        <path d="M 68 0 L 0 0 0 44" fill="none" stroke="rgba(255,255,255,0.07)" strokeWidth="1" />
                      </pattern>
                    </defs>
                    <rect x="0" y="0" width="680" height="220" fill="url(#grid-engagement)" />
                    <path d={engagementWatchPath} fill="none" stroke="#f8b321" strokeWidth="3" strokeLinecap="round" />
                    <path d={engagementCompletionPath} fill="none" stroke="#8ea6d6" strokeWidth="3" strokeLinecap="round" />
                  </Box>
                </Box>
              ...
              <Box sx={{ ...panelSx, p: 1 }}>
                ...
                <Stack spacing={0.8} sx={{ mt: 1 }}>
                  ...
                  <Stack direction="row" spacing={0.7} alignItems="center">
                    ...
                    <Typography>Lesson completion rate improved by 10% across active cohorts.</Typography>
                  </Stack>
                  <Stack direction="row" spacing={0.7} alignItems="center">
                    <Assessment sx={{ color: '#8ea6d6' }} />
                    <Typography>
                      Consider adding bonus content or community Q&amp;A sessions to further boost engagement.
                    </Typography>
                  </Stack>
                </Stack>
                ...
                >
                  <Typography sx={{ color: '#f8d56f', fontWeight: 700 }}>Next Best Action</Typography>
                  <Typography sx={{ mt: 0.4, color: 'rgba(237,240,247,0.82)' }}>
                    Promote the top performing module and schedule one live feedback session this week.
                  </Typography>
                </Box>
              </Box>
            </Box>
...
export default withUniversalIntegration(AcademyAnalyticsStudio, {
  ...
  featureIds: ['analytics-dashboard', 'revenue-analytics', 'engagement-tracking', 'academy-growth-metrics'],
});
