...
const TUTORIAL_KV_KEY = `academy_tutorial_pref_${TUTORIAL_ID}`;
...
const readTutorialFallbackFromLocal = (): TutorialPreference => {
  const localDismissed = localStorage.getItem(
    `${TUTORIAL_ID}-tutorial-dismissed`,
  );
...
const readTutorialFallbackFromDb =
  async (): Promise<TutorialPreference | null> => {
    try {
      const response = await fetch(
        `/api/user/kv/${encodeURIComponent(TUTORIAL_KV_KEY)}`,
        {
          credentials: "include",
        },
      );
      ...
      const parsed =
        typeof value === "string"
          ? (JSON.parse(value) as Partial<TutorialPreference>)
          : (value as Partial<TutorialPreference>);
      return {
        tutorialId: TUTORIAL_ID,
...
const writeTutorialFallbackToDb = async (
  value: Partial<TutorialPreference>,
): Promise<void> => {
  try {
    ...
    // Keep local fallback even when DB write is unavailable.
  }
};

const clearTutorialFallbackFromDb = async (): Promise<void> => {
  try {
    await fetch(`/api/user/kv/${encodeURIComponent(TUTORIAL_KV_KEY)}`, {
      method: "DELETE",
      credentials: "include",
    });
  } catch {
...
export const AcademyTutorial: React.FC<AcademyTutorialProps> = ({
    ...
    queryFn: async () => {
      try {
        return await apiRequest(
          `/api/user/preferences/tutorial/${TUTORIAL_ID}`,
        );
      } catch {
        // Fallback to DB KV, then localStorage.
        const dbFallback = await readTutorialFallbackFromDb();
        if (dbFallback) return dbFallback;
        const localFallback = readTutorialFallbackFromLocal();
        void writeTutorialFallbackToDb(localFallback);
  ...
  const saveProgressMutation = useMutation({
    mutationFn: async (steps: number[]) => {
      return await apiRequest(
        `/api/user/preferences/tutorial/${TUTORIAL_ID}/progress`,
        {
          method: "PATCH",
    ...
    onSuccess: (_, steps) => {
      ...
      localStorage.setItem(
        `${TUTORIAL_ID}-completed-steps`,
        JSON.stringify(steps),
      );
      void writeTutorialFallbackToDb({
        completedSteps: steps,
        dismissed: tutorialPrefs?.dismissed ?? false,
        dismissedAt: tutorialPrefs?.dismissedAt ?? null,
        profession: tutorialPrefs?.profession ?? profession ?? null,
      });
      // Update cache
      queryClient.setQueryData(
        ["tutorialPreferences", TUTORIAL_ID],
        (old: TutorialPreference | undefined) => ({
          ...old,
          completedSteps: steps,
        }),
      );
    ...
    onError: (_, steps) => {
      // Fallback to localStorage + DB KV
      localStorage.setItem(
        `${TUTORIAL_ID}-completed-steps`,
        JSON.stringify(steps),
      );
  ...
  const resetTutorialMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void clearTutorialFallbackFromDb();
      setCompletedSteps([]);
      setActiveStep(0);
      queryClient.invalidateQueries({
        queryKey: ["tutorialPreferences", TUTORIAL_ID],
  ...
  const instructorSteps = [
    {
      ...
      icon: <VideoLibrary />,
      content:
        'Start by clicking "Create Course" to build your educational content.',
      example:
        'Click the "+ Create Competency" button on the dashboard to open the Skills Builder.',
    ...
    },
    {
      ...
      icon: <Assignment />,
      content:
        "Break your course into modules. Each module groups related lessons together.",
      example:
        'Create modules like "Equipment Basics", "Lighting Techniques", and "Post-Processing".',
    ...
    },
    {
      ...
      example:
        "Upload a 20-minute tutorial and add chapters at key moments (2:00 - Intro, 5:30 - Setup).",
    ...
    },
    {
      ...
      icon: <AutoAwesome />,
      content:
        "Add text overlays, CTAs, lower thirds, and use AI-powered features.",
    ...
    },
    {
      ...
      icon: <Publish />,
      content:
        "Choose free or paid access. Set your price and publish to the community.",
    ...
    },
    {
      ...
      example:
        "Check your Instructor Revenue Dashboard for earnings and payout requests.",
  ...
  const studentSteps = [
    {
      ...
      example:
        'Filter courses by "Photography" to find relevant content for your skill level.',
    ...
    },
    {
      ...
      icon: <CheckCircle />,
      content:
        'Click "Enroll" to join free courses or complete payment for paid ones.',
      example:
        'Click "Enroll Now" and follow the payment flow for premium courses.',
    ...
    },
    {
      ...
      icon: <PlayArrow />,
      content:
        "Navigate lessons using the sidebar. Use chapters to jump to specific sections.",
      example:
        'Click on "Module 2: Lighting" then select "Lesson 3: Natural Light".',
    ...
    },
    {
      ...
      icon: <Schedule />,
      content:
        "Your progress is saved automatically. Resume where you left off.",
      example:
        "Check the progress bar to see how much of the course you've completed.",
    ...
    },
    {
      ...
      icon: <Analytics />,
      content:
        "Ask questions, share your work, and connect with other students.",
      example:
        "Post your assignment work in the course community for feedback.",
    ...
    },
    {
      ...
      icon: <Groups />,
      content:
        "Access downloadable resources, templates, and connect with the community.",
      example:
        "Click the download icon next to resources like presets, LUTs, or PDFs.",
  ...
  const instructorMistakes = [
    {
      ...
      solution:
        "Keep lessons under 15 minutes. Split longer content into multiple lessons.",
    },
    {
      mistake: "Skipping chapters and annotations",
      solution:
        "Add chapters every 2-3 minutes. Students will thank you for easy navigation.",
    },
    {
      mistake: "No preview content",
      solution:
        "Offer at least one free lesson so students can try before buying.",
    },
    {
  ...
  const studentMistakes = [
    ...
    },
    {
      ...
      solution:
        "Check each lesson for downloadable files—they enhance learning.",
    },
    {
      mistake: "Passive watching",
      solution:
        "Practice alongside tutorials. Active learning beats passive watching.",
    },
    {
      mistake: "Not asking questions",
      solution:
        "Use the community features. Instructors and peers are here to help.",
    },
  ];

  const commonMistakes =
    userRole === "instructor" ? instructorMistakes : studentMistakes;
  ...
  const checklist =
    userRole === "instructor" ? instructorChecklist : studentChecklist;
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ pt: 3 }}>
        ...
        >
          ...
          >
            ...
            What You'll Learn as{" "}
            {userRole === "instructor" ? "an Instructor" : "a Student"}
          </Typography>
          <List dense>
        ...
        >
          <Typography variant="body2" sx={{ fontWeight: 600, mb: 0.5 }}>
            Perfect for: {getProfessionDisplayName()}er{" "}
            {userRole === "instructor" ? "som vil lage kurs" : "som vil lære"}
          </Typography>
        ...
        <Typography variant="h6" sx={{ fontWeight: 700, mb: 2 }}>
          ...
          {userRole === "instructor" ? "Instructors" : "Students"}
        </Typography>
                  ...
                  <Box
                    sx={{
                      ...
                      border:
                        activeStep >= index
                          ? "none"
                          : `2px solid ${theming.colors.primary}30`,
          ...
          <Box sx={{ mt: 3 }}>
                ...
                <TableBody>
                  {keyboardShortcuts.map((shortcut) => (
                    <TableRow key={shortcut.action}>
                      <TableCell
                        sx={{ fontWeight: 600, border: "none", py: 1 }}
                      >
                        {shortcut.action}
                      </TableCell>
            ...
            >
              ...
              {userRole === "instructor" ? (
                ...
                >
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    <strong>Goal:</strong> Create and publish a professional
                    course in one week
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2, pl: 2 }}>
                    • Outline your course (5-10 modules)
                    <br />
                    • Write lesson titles and descriptions
                    <br />• Gather resources (presets, templates, PDFs)
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2, pl: 2 }}>
                    • Record screen or camera tutorials
                    <br />
                    • Keep lessons under 15 minutes each
                    <br />• Aim for 3-5 lessons per module
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2, pl: 2 }}>
                    • Upload videos to Skills Builder
                    <br />
                    • Add chapters (use Auto-Detect for speed)
                    <br />• Add CTAs and lower thirds
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ pl: 2 }}>
                    • Set pricing and preview settings
                    <br />
                    • Write a compelling course description
                    <br />• Publish to the community!
                  </Typography>
                </Box>
              ) : (
                <Box
                ...
                >
                  <Typography variant="body2" sx={{ mb: 2 }}>
                    <strong>Goal:</strong> Complete a course and apply what you
                    learned
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2, pl: 2 }}>
                    ...
                    <br />• Watch free preview lessons first
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2, pl: 2 }}>
                    ...
                    <br />• Bookmark key moments for reference
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ mb: 2, pl: 2 }}>
                    ...
                    <br />
                    • Complete assignments if provided
                    <br />• Share your work in the community
                  </Typography>
                  ...
                  <Typography variant="body2" sx={{ pl: 2 }}>
                    ...
                    <br />• Download your completion certificate!
                  </Typography>
                </Box>
              )}
            ...
            >
              {resetTutorialMutation.isPending
                ? "Tilbakestiller..."
                : "Start Over"}
            </Button>
          </Box>
      ...
      >
        ...
        >
          Access this guide anytime from the Academy Dashboard → Help icon (?)
          or Settings → Tutorials
        </Typography>
      </DialogActions>
    </Dialog>
  );
