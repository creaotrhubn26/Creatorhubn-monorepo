...
const TeamWorkspacePage: React.FC = () => {
  ...
  const content = useMemo(() => {
    switch (tab) {
      ...
      case 'chat':            return <Box sx={{ height: 'calc(100dvh - 160px)', maxWidth: 760, mx: 'auto' }}><WorkspaceChatPanel projectId={projectId} category={workspaceCategory} presence={presenceMembers} /></Box>;
      default:                return <ComingTab label={navItem?.label || tab} />;
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, projectId, presenceMembers, user?.id, user?.email, user?.profession, user?.firstName, projectProfession, workspaceCategory, (project as any)?.title, (project as any)?.name, (realProject as any)?.updatedAt, navItem?.label, participantAccessState]);
