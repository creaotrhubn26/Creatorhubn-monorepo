...
const WORKSPACE_EVENTS_WS_BASE = typeof import.meta !== 'undefined' && import.meta.env?.VITE_WS_BASE ? import.meta.env.VITE_WS_BASE : 'wss://creatorhub-backend-rtbl.onrender.com';
...
function applyNavOverrides(items: WsNavItem[], ov: Record<string, any> | null | undefined): WsNavItem[] {
  ...
  return kept
    .map((n, i) => ({
      n,
      i,
      ord: typeof ov[n.key]?.order === 'number' ? ov[n.key].order : 1000 + i
    }))
...
const TeamWorkspacePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user?.profession === 'enterprise' || (user as any)?.role === 'instructor') {
      setIsMentor(true);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!token) return;
    apiRequest(`/api/projects/team/accept/${encodeURIComponent(token)}`, {
      method: 'POST'
    })
      .then((r: any) => {
  ...
  useEffect(() => {
    ...
    apiRequest(`/api/projects/${encodeURIComponent(projectId)}`)
      .then((r: any) => {
        ...
        const date = rawDate
          ? new Date(rawDate).toLocaleDateString('nb-NO', {
              day: 'numeric',
              month: 'short',
              year: 'numeric'
            })
        ...
        setRealProject({
          id: p.id,
          name: p.title || p.name || 'Uten tittel',
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      clearTimeout(retry);
      try {
        sockEvt = new WebSocket(`${WORKSPACE_EVENTS_WS_BASE}/api/ipad/ws/events?token=${encodeURIComponent(token)}`);
      } catch {
        retry = setTimeout(connect, 8000);
        return;
      }
      sockEvt.onclose = () => {
        if (alive) retry = setTimeout(connect, 8000);
      };
      sockEvt.onerror = () => {
        try {
          sockEvt && sockEvt.close();
        } catch {
          /* */
        }
      };
  ...
  const workspaceCategory = workspaceCategoryFor(user?.profession, categoryOverrides);
  ...
  useEffect(() => {
    ...
    fetch('/api/design/tokens?ws=creatorhub', { credentials: 'same-origin' })
      ...
      .then((d) => {
        if (live && d && d.tokens && d.tokens.nav && typeof d.tokens.nav === 'object') setNavOv(d.tokens.nav);
      })
      .catch(() => {});
    return () => {
      live = false;
  ...
  const nav = useMemo(() => {
    const professionNav = localizeNav(navForProfession(user?.profession, { isMentor, categoryOverrides }), wsLocale).map((item) =>
      item.capability === WORKSPACE_PROJECT_PARTICIPANTS_FEATURE_ID
        ? {
            ...item,
            locked: !participantCapabilityAllowed,
            lockReason: participantCapabilityAllowed ? undefined : participantAccessState.loading ? 'Kontrollerer prosjekttilgang' : participantAccessState.error?.code === 'enterprise_required' ? 'Krever Enterprise-tilgang' : participantAccessState.error?.message || 'Krever tilgang til prosjektets medvirkende'
          }
        : item
    );
    return applyNavOverrides(professionNav, navOv);
  }, [user?.profession, isMentor, categoryOverrides, wsLocale, navOv, participantCapabilityAllowed, participantAccessState.loading, participantAccessState.error?.code, participantAccessState.error?.message]);
  ...
  const content = useMemo(() => {
    switch (tab) {
      case 'oversikt':
        return <OversiktTab projectId={projectId} profession={user?.profession} />;
      case 'prosjektplan':
        return <ProsjektplanTab projectId={projectId} profession={user?.profession} />;
      case 'produksjonskart':
        return <ProduksjonskartTab projectId={projectId} />;
      case 'shotlist':
        return <ShotlistTab projectId={projectId} />;
      case 'laater':
        return <LaaterTab projectId={projectId} />;
      case 'sesjoner':
        return <SesjonerTab projectId={projectId} />;
      case 'oppdrag':
        return <OppdragTab projectId={projectId} />;
      case 'bookinger':
        return <BookingerTab projectId={projectId} />;
      case 'academy':
        return (
          <AcademyProvider>
      ...
      case 'community':
        return <CommunityHub userId={user?.id} userEmail={user?.email} profession={user?.profession || undefined} />;
      case 'moodboard':
        return <MoodboardTab projectId={projectId} profession={user?.profession} />;
      case 'media':
        return <MediaTab projectId={projectId} />;
      case 'utstyr':
        return <UtstyrTab projectId={projectId} profession={user?.profession} userId={user?.id} />;
      case 'leveranser':
        return <LeveranserTab projectId={projectId} />;
      case 'oppgaver':
        return <OppgaverTab projectId={projectId} profession={user?.profession} />;
      case 'avtaler':
        return <AvtalerTab projectId={projectId} />;
      case 'foresporsler':
        return <ForesporslerTab projectId={projectId} profession={user?.profession} userId={user?.id} userName={user?.firstName || (user as any)?.name || user?.email} />;
      case 'kundevisning':
        return <KundevisningTab projectId={projectId} />;
      case 'team':
        return <TeamTab projectId={projectId} profession={user?.profession} userId={user?.id} projectName={(project as any)?.title || (project as any)?.name} lastUpdated={(realProject as any)?.updatedAt || null} />;
      case 'medvirkende':
        return <WorkspaceParticipantsTab projectId={projectId} accessState={participantAccessState} />;
      case 'sound-room':
        return <SoundRoomTab projectId={projectId} />;
      case 'video-room':
        return <VideoRoomTab projectId={projectId} />;
      case 'photo-room':
        return <PhotoRoomTab projectId={projectId} />;
      case 'chat':
        return (
      ...
      default:
        return <ComingTab label={navItem?.label || tab} />;
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, projectId, user?.id, user?.email, user?.profession, user?.firstName, (project as any)?.title, (project as any)?.name, (realProject as any)?.updatedAt, navItem?.label, categoryOverrides, participantAccessState]);
  ...
  return (
    <WsLocaleProvider value={wsLocale}>
      <WorkspaceShell
        ...
        navItems={nav}
        badges={{
          foresporsler: inboundCount,
          kundevisning: clientActivityUnseen,
          'sound-room': bandUnseen
        }}
      ...
      >
        ...
        <Snackbar open={!!accepted} autoHideDuration={5000} onClose={() => setAccepted(null)} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}>
          <Alert severity="success" variant="filled" onClose={() => setAccepted(null)}>
            {accepted}
          </Alert>
        </Snackbar>
        ...
        {isPrototypeTester && <UniversalPrototypeFeedback profession={user?.profession} dashboardType="workspace" component="workspace" currentTab={tab} userEmail={user?.email} floatingPosition={{ bottom: 96, right: 24, zIndex: 1200 }} />}
