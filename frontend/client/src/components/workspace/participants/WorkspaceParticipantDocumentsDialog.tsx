...
const T: WsDict = {
  ...
  viewOnly: {
    no: "Du kan lese dokumentstatus. Bare en prosjektadministrator kan utstede dokumenter eller fornye lenker.",
    en: "You can view document status. Only a project administrator can issue documents or renew links.",
  },
  ...
  renew: { no: "Forny personlig lenke", en: "Renew personal link" },
  renewing: { no: "Fornyer…", en: "Renewing…" },
  copy: { no: "Kopier lenke", en: "Copy link" },
  copied: { no: "Lenken er kopiert.", en: "Link copied." },
  manualCopy: {
    no: "Automatisk kopiering feilet. Marker lenken og kopier manuelt.",
    en: "Automatic copying failed. Select the link and copy it manually.",
  },
  noDelivery: {
    no: "E-postutsending er ikke koblet til. Kopier den personlige lenken og send den via en trygg kanal.",
    en: "Email delivery is not connected. Copy the personal link and send it through a secure channel.",
  },
...
interface Props {
  ...
  access: Pick<WorkspaceParticipantAccess, "canView" | "canManage">;
}
...
function deliveryReason(reason: string | null, locale: "no" | "en"): string {
  if (reason === "recipient_missing")
    return locale === "en"
      ? "Recipient email is missing"
      : "Mottaker mangler e-post";
  if (reason === "delivery_failed")
    return locale === "en" ? "Delivery failed" : "Utsending feilet";
  return locale === "en"
    ? "Email is not connected"
    : "E-post er ikke koblet til";
}
...
export default function WorkspaceParticipantDocumentsDialog({
  ...
  const [deliveryFallbackId, setDeliveryFallbackId] = useState<string | null>(
    null,
  );
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [manualCopyId, setManualCopyId] = useState<string | null>(null);

  const clearPortalCredentials = useCallback(() => {
    setIssueType(null);
    setPortalLinks({});
    setDeliveryFallbackId(null);
    setCopiedId(null);
  ...
  const issueDocument = async () => {
    ...
    try {
      ...
      setPortalLinks((current) => ({
        ...current,
        [result.document.id]: result.portalUrl,
      }));
      setDeliveryFallbackId(result.delivery.sent ? null : result.document.id);
      setIssueType(null);
      await loadDocuments();
    } catch (error) {
  ...
  const renewLink = async (document: WorkspaceParticipantDocumentSummary) => {
    ...
    try {
      ...
      setPortalLinks((current) => ({
        ...current,
        [document.id]: result.portalUrl,
      }));
      setDeliveryFallbackId(result.delivery.sent ? null : document.id);
      await loadDocuments();
    } catch (error) {
      setMutationError(workspaceParticipantDocumentsError(error).message);
    } finally {
  ...
  const copyLink = async (documentId: string) => {
    const url = portalLinks[documentId];
    if (!url) return;
    setCopiedId(null);
    setManualCopyId(null);
    try {
      ...
      <DialogContent dividers>
        {!access.canView ? (
            ...
            {participant?.isMinor && (
              ...
              >
                <Stack
                  ...
                  sx={{ mb: 1.5 }}
                >
                  <VerifiedUserOutlined sx={{ color: ws.amber }} />
          ...
          <Stack spacing={1.5}>
            ...
            {mutationError && <Alert severity="error">{mutationError}</Alert>}
            {deliveryFallbackId && (
              <Alert severity="warning">{t("noDelivery")}</Alert>
            )}
            {loading ? (
              ...
              documents.map((document) => {
                const portalUrl = portalLinks[document.id];
                return (
                  <Box
                    key={document.id}
                    sx={{
                      ...
                      <Stack spacing={0.75} sx={{ mt: 1.5 }}>
                        ...
                        <Stack direction="row" spacing={1} alignItems="center">
                          <Button
                            ...
                            onClick={() => void copyLink(document.id)}
                          >
                            {t("copy")}
                          </Button>
                          {copiedId === document.id && (
                            <Typography
                              sx={{ color: ws.green, fontSize: 11.5 }}
                            >
                              {t("copied")}
                            </Typography>
                          )}
                        </Stack>
                        {manualCopyId === document.id && (
                          <Alert severity="info">{t("manualCopy")}</Alert>
                        )}
                      </Stack>
                    )}
                  </Box>
        ...
        {issueType ? (
          <>
            ...
            <Button
              ...
              disabled={busy || !access.canManage || compensationBlocksContract}
