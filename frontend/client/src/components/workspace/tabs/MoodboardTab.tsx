...
import { WsCard, WsSectionTitle, WsStat, WsPills, WsTag, WsImageGrid, WsModal, WsPageTitle, wsAlert } from '../ui';
...
const MoodboardTab: React.FC<{ projectId: string; profession?: string }> = ({ projectId, profession }) => {
  ...
  const saveNotesAll = (next: string[], noteCatsExtra?: Record<string, string>) => {
    ...
    notesTimer.current = setTimeout(() => { apiRequest(`/api/projects/${encodeURIComponent(projectId)}/moodboard-meta`, { method: 'PUT', body }).catch((e: any) => { wsAlert(e?.message || t('couldNotSave')); loadMeta(); }); }, 500);
  };
  ...
  const savePalette = (next: any[]) => {
    ...
    palSaveTimer.current = setTimeout(() => {
      ...
      }).catch((e: any) => { wsAlert(e?.message || t('couldNotSave')); loadMeta(); });
    }, 400);
  };
  ...
  const setSelCat = (k: string | null) => {
    ...
    apiRequest(`/api/projects/${encodeURIComponent(projectId)}/images/${encodeURIComponent(selIm.id)}`, { method: 'PATCH', body: { category: k } })
      .then(() => mood.reload()).catch((e: any) => wsAlert(e?.message || t('couldNotSave')));
  };
  const toggleSelFlag = () => {
    if (!selIm || !isReal) return;
  ...
  const addImgComment = async () => {
    ...
    setImgComment('');
    if (isReal) { await apiRequest(`/api/projects/${encodeURIComponent(projectId)}/images/${encodeURIComponent(selIm.id)}`, { method: 'PATCH', body: { comments: next } }).catch((e: any) => wsAlert(e?.message || t('couldNotSave'))); mood.reload(); }
        ...
        <WsCard sx={{ mb: 2 }}>
          ...
          {subTab === 'shared' && (<>
            ...
            <WsImageGrid
              ...
              onRemove={(id: string) => { apiRequest(`/api/projects/${encodeURIComponent(projectId)}/images/${encodeURIComponent(id)}`, { method: 'DELETE' }).then(() => shared.reload()).catch((e: any) => wsAlert(e?.message || t('couldNotSave'))); }}
              actions={(im: any) => (
                <>
                  ...
                  {isReal && (
                    <Box onClick={() => apiRequest(`/api/projects/${encodeURIComponent(projectId)}/images/${encodeURIComponent(im.id)}`, { method: 'PATCH', body: { panel: 'moodboard' } }).then(() => { shared.reload(); mood.reload(); }).catch((e: any) => wsAlert(e?.message || t('couldNotSave')))} title="Flytt til moodboard" sx={{ px: 0.6, py: 0.15, borderRadius: 1, bgcolor: 'rgba(0,0,0,.55)', color: '#fff', fontSize: 9.5, fontWeight: 800, cursor: 'pointer', '&:hover': { bgcolor: ws.accent, color: ws.accentContrast } }}>→ Moodboard</Box>
                  )}
                </>
              )}
            />
