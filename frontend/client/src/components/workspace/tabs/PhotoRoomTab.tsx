...
const PhotoRoomTab: React.FC<{ projectId: string; readOnly?: boolean }> = ({ projectId, readOnly = false }) => {
  ...
  const renderTile = (asset: PhotoAsset, compact: boolean) => {
    ...
    return <Box key={asset.id} sx={{ position: 'relative', flex: compact ? '0 0 112px' : undefined, minWidth: 0, contentVisibility: compact ? 'visible' : 'auto', containIntrinsicSize: compact ? undefined : '168px 150px' }}>
      <Box component="button" type="button" aria-label={`${asset.filename}, ${statusLabel(asset.reviewStatus)}`}
        aria-current={active ? 'true' : undefined} aria-selected={checked}
        ...
        {!compact && <Box sx={{ p: 1 }}><Typography noWrap sx={{ fontSize: 12, fontWeight: 700 }}>{asset.filename}</Typography><Typography noWrap sx={{ fontSize: 10.5, color: ws.textFaint }}>{asset.folderName || t('unassigned')} · {asset.sourceName || 'Capture'}</Typography></Box>}
      </Box>
  ...
  const renderInspector = () => {
    ...
    return <Stack spacing={1.25}>
      <Box><Typography noWrap sx={{ fontSize: 13, fontWeight: 800 }}>{selectedAsset.filename}</Typography><Typography sx={{ fontSize: 11, color: selectedAsset.reviewStatus ? STATUS_COLOR[selectedAsset.reviewStatus] : ws.textFaint }}>{statusLabel(selectedAsset.reviewStatus)}</Typography></Box>
  ...
  return <Box ref={roomRef} sx={{ minWidth: 0, bgcolor: ws.bg }}>
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr)', lg: `210px minmax(0,1fr) ${inspectorOpen ? '330px' : '42px'}` }, gap: 1.5, alignItems: 'start' }}>
      <WsCard pad={1} sx={{ display: { xs: 'none', lg: 'block' }, position: 'sticky', top: 12, maxHeight: 'calc(100vh - 32px)', overflowY: 'auto' }}>
        <Typography sx={{ px: 1, mb: 0.5, fontSize: 10.5, fontWeight: 800, color: ws.textFaint, textTransform: 'uppercase' }}>{t('smartViews')}</Typography>
        ...
        <Divider sx={{ my: 1.25 }} /><Typography sx={{ px: 1, mb: 0.5, fontSize: 10.5, fontWeight: 800, color: ws.textFaint, textTransform: 'uppercase' }}>{t('sources')}</Typography>
        ...
        <Divider sx={{ my: 1.25 }} /><Typography sx={{ px: 1, mb: 0.5, fontSize: 10.5, fontWeight: 800, color: ws.textFaint, textTransform: 'uppercase' }}>{t('collections')}</Typography>
        ...
        <Divider sx={{ my: 1.25 }} /><Typography sx={{ px: 1, mb: 0.5, fontSize: 10.5, fontWeight: 800, color: ws.textFaint, textTransform: 'uppercase' }}>{t('deliveries')}</Typography>
      ...
      <Box sx={{ minWidth: 0 }}>
        ...
        <Stack direction="row" justifyContent="center" alignItems="center" spacing={1} sx={{ py: 1 }}><Typography sx={{ fontSize: 11, color: ws.textFaint }}>{t('showing')} {assets.length} {t('of')} {data?.pageInfo?.total || assets.length}</Typography>{data?.pageInfo?.hasMore && <Button size="small" disabled={loadingMore} onClick={() => void loadPhotos(assets.length)}>{loadingMore ? <CircularProgress size={16} /> : t('loadMore')}</Button>}</Stack>
      </Box>
