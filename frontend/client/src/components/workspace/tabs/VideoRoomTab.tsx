...
const promptAvidFrameRate = (): string | null => {
  let saved = "25";
  try { saved = window.localStorage.getItem("creatorhub:avid-frame-rate") || saved; } catch { /* Storage can be disabled. */ }
  const candidate = window.prompt(
    "Bildefrekvens i Avid-prosjektet (f.eks. 25 eller 24000/1001):",
    saved,
  );
  if (candidate === null) return null;
  const value = candidate.trim();
  const match = value.match(/^(\d+(?:\.\d+)?)(?:\/(\d+(?:\.\d+)?))?$/);
  const rate = match ? Number(match[1]) / Number(match[2] || 1) : NaN;
  if (!Number.isFinite(rate) || rate < 1 || rate > 240) {
    window.alert("Ugyldig bildefrekvens. Bruk et tall eller en brøk, for eksempel 25 eller 24000/1001.");
    return null;
  }
  try { window.localStorage.setItem("creatorhub:avid-frame-rate", value); } catch { /* Optional convenience only. */ }
  return value;
};
...
const VideoRoomTab: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const exportMarkers = async (editor: string, format?: string) => {
    ...
    if (format === "avid") {
      const frameRate = promptAvidFrameRate(); if (!frameRate) return;
      const anchor = document.createElement("a");
      anchor.href = `${base}&format=avid&frameRate=${encodeURIComponent(frameRate)}&track=V1`;
      anchor.download = `creatorhub-${current.id}-avid-markers.txt`;
  ...
  const importMarkers = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (/\.txt$/i.test(file.name)) {
        const frameRate = promptAvidFrameRate(); if (!frameRate) return;
        const result: any = await apiRequest(`/api/projects/${encodeURIComponent(projectId)}/video-marker-sync/avid`, {
          method: "POST", body: { versionId: current.id, avidText: content, frameRate },
        });
        const rejected = Array.isArray(result.rejected) ? result.rejected.length : 0;
        window.alert(`${result.imported || 0} Avid-markører synkronisert${rejected ? `, ${rejected} linjer avvist` : ""}.`);
        await Promise.all([load(current.id, true), loadCollaboration(current.id)]);
        return;
      } else if (/\.(?:fcpxml|xml)$/i.test(file.name) || content.trimStart().startsWith("<")) {
        editor = "final_cut";
        ...
        const suggestedEditor = ["resolve", "premiere", "final_cut", "avid"].includes(parsed.editor) ? parsed.editor : "resolve";
        const selected = window.prompt("Kilde: resolve, premiere, final_cut eller avid", suggestedEditor);
        if (selected === null) return;
        if (!["resolve", "premiere", "final_cut", "avid"].includes(selected)) throw new Error("Ukjent NLE-kilde");
        editor = selected;
      }
            ...
            <WsCard sx={{ mb: 2 }}>
              <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" gap={1} sx={{ mb: 1.25 }}>
                ...
                <Stack direction="row" spacing={0.5} flexWrap="wrap" useFlexGap>
                  ...
                  <Button size="small" startIcon={<SyncAlt />} onClick={() => exportMarkers("avid", "avid")}>Avid TXT ut</Button>
                  ...
                  <Box component="input" ref={markerImportInput} type="file" accept=".json,.xml,.fcpxml,.txt,application/json,application/xml,text/xml,text/plain" onChange={importMarkers} sx={{ display: "none" }} />
                </Stack>
              </Stack>
