...
const ShotlistTab: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const toggleChk = async (item: any, idx: number) => {
    if (isRealP && item.id) {
      ...
      catch (e: any) { wsAlert(e?.message || 'Kunne ikke oppdatere'); }
    } else {
  ...
  const addChk = async (label: string = '', opts?: { category?: string; critical?: boolean; assignedTo?: string; color?: string }) => {
    ...
    } catch (e: any) { wsAlert(e?.message || 'Kunne ikke legge til'); }
  ...
  const assignChk = async (item: any, idx: number, name: string) => {
    ...
    if (isRealP && item.id) {
      ...
      catch (e: any) { wsAlert(e?.message || 'Kunne ikke oppdatere ansvar'); }
    } else {
      setChk(prev => setter(prev));
    }
  };
  ...
  const delChk = async (item: any, idx: number) => {
    if (isRealP && item.id) {
      ...
      catch (e: any) { wsAlert(e?.message || 'Kunne ikke slette'); }
    } else {
  ...
  const sendShotComment = async () => {
    ...
    } catch (e: any) { wsAlert(e?.message || 'Kunne ikke sende kommentar'); }
  };
  ...
  const saveComments = async (shotId: string, comments: any[]) => {
    ...
    try {
      await apiRequest(`/api/projects/${encodeURIComponent(projectId)}/shot-list`, { method: 'POST', body: { shots: next } });
      loadShotList();
    } catch (e: any) { wsAlert(e?.message || 'Kunne ikke lagre endringen'); }
  };
  ...
  const toggleDone = async () => {
    ...
    try {
      ...
      loadShotList();
    } catch (e: any) { wsAlert(e?.message || 'Kunne ikke oppdatere status'); }
  ...
  const useCatChoice = (m: { item: any; idx: number; quick: boolean }, k: string, color?: string) => {
    ...
    if (isRealP && m.item?.id) {
      apiRequest(`/api/projects/${encodeURIComponent(projectId)}/checklist/${encodeURIComponent(m.item.id)}`, { method: 'PATCH', body: { category: k, ...(color ? { color } : {}) } }).catch((e: any) => wsAlert(e?.message || 'Kunne ikke endre kategori'));
    }
  };
