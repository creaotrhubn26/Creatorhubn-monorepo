...
import { WsCard, WsSectionTitle, WsRing, WsPills, WsTag, WsImageGrid, WsModal, WsErrorState, WsPageTitle, wsAlert, wsConfirm } from '../ui';
...
const LeveranserTab: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const patchFiles = (next: any[]) => {
    ...
    apiRequest(`/api/projects/${encodeURIComponent(projectId)}/deliverables/${encodeURIComponent(sel.id)}`, { method: 'PATCH', body: { files: next } })
      .catch((e: any) => { wsAlert(e?.message || t('couldNotAttach')); setDlFiles(dlFiles); });
  };
  ...
  const attachPicked = async () => {
    ...
    catch (e: any) { setDlFiles(dlFiles); wsAlert(e?.message || t('couldNotAttach')); }
  };
  ...
  const markDeliverableDone = async () => {
    if (!isReal) return;
    const id = sel?.id;
    if (!id || ['delivered', 'done', 'completed'].includes(sel?.statusKey)) { wsAlert(t('noOpenDeliverables')); return; }
    if (!await wsConfirm(t('deliverConfirm').replace('{title}', sel?.title || '?'))) return;
