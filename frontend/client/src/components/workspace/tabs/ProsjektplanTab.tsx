...
const ProsjektplanTab: React.FC<{ projectId: string; profession?: string }> = ({ projectId, profession }) => {
  ...
  const addMilestone = async () => {
    if (!isReal) return;
    const title = await wsPrompt(t('promptMsTitle'));
    ...
    try {
      ...
      wsAlert(err?.message || t('addMsFailed'));
    }
  };
  ...
  const importPlan = async (file: File) => {
    try {
      ...
      const err = e as Error;
      wsAlert(err?.message || t('importFailed'));
    }
  };
...
const ListView: React.FC<ListViewProps> = ({ ms, phases, t, dloc, projectId, isReal, navigate }) => {
  ...
  const bulkAction = async (action: 'delete' | 'move' | 'status', value?: string) => {
    if (action === 'delete' && !await wsConfirm(t('confirmDelete'))) return;
    if (!isReal) {
      setSelected(new Set());
      setReload((r) => r + 1);
      return;
