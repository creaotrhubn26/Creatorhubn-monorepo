...
import { WsCard, WsTag, WsImageGrid, WsModal, WsErrorState, WsPageTitle, wsAlert, wsConfirm } from '../ui';
...
const MediaTab: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const moveAssetFolder = async (id: string, folderId: string | null) => {
    ...
    catch (e: any) { wsAlert(e?.message || t('error')); }
  };
  ...
  const cloneRef = async () => {
    ...
    try {
      ...
      if (r?.copied === 0) wsAlert('Ingen nye referanser — samlingsnavnet finnes kanskje allerede.');
    } catch (e: any) { wsAlert(e?.message || t('error')); }
  };
  ...
  const deleteAsset = async (id: string) => {
    if (!isReal || !id) return;
    if (!await wsConfirm(t('deleteAssetConfirm'))) return;
    try {
      ...
      wsAlert(`${t('deleteAssetFailed')}${e?.message ? ` (${e.message})` : ''}`);
    }
  };
  ...
  const bulkDelete = async () => {
    ...
    if (!await wsConfirm(t('deleteBulkConfirm').replace('{n}', String(ids.length)))) return;
    endBulk();
    for (const id of ids) {
      try { await apiRequest(`/api/projects/${encodeURIComponent(projectId)}/media/assets/${encodeURIComponent(id)}`, { method: 'DELETE' }); }
      catch (e: any) { if (e?.status === 404) continue; wsAlert(`${t('deleteAssetFailed')}: ${String(ids.length)}`); break; }
    }
    reloadMedia();
  };
  ...
  const downloadOne = async (im: any, mime?: string) => {
    ...
    try {
      ...
      if (!blob) { wsAlert(t('dlFailed')); return; }
