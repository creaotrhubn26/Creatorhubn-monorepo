...
import { WsCard, WsSectionTitle, WsRing, WsPills, WsTag, WsTable, WsImageGrid, WsPageTitle, wsAlert } from '../ui';
...
const ProduksjonskartTab: React.FC<{ projectId: string }> = ({ projectId }) => {
  ...
  const createTeamChat = async () => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke opprette chatten');
    } finally {
      setChatBusy(false);
    }
  };
  ...
  const addNote = async () => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke lagre notat');
    } finally {
      setSavingNote(false);
    }
  };
  const deleteNote = async (id: string) => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke slette notat');
    }
  };
  ...
  const changeStatus = async (row: PlanRow, next: PlanStatus) => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke endre status');
    }
  };
  ...
  const saveEvent = async () => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke lagre hendelse');
    } finally {
  ...
  const deleteEvent = async () => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke slette hendelse');
    } finally {
      setSavingEvent(false);
    }
  };
...
const syncOwner = isReal ? (sync?.owner || null) : SAMPLE_SYNC.owner;
  ...
  const toggleShare = () => {
    ...
    if (!myCrewName.trim()) {
      wsAlert('Velg din crew-profil først (nedtrekksmenyen over kartet).');
      return;
    }
    if (!navigator.geolocation) { wsAlert('Geolokasjon støttes ikke i denne nettleseren.'); return; }
    setShareLoc(true);
  ...
  const toggleCheckin = async (loc: LocationInfo, member: CrewMember, checked: boolean) => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke oppdatere innsjekking');
    }
  };
  ...
  const saveLocEdit = async () => {
    ...
    try {
      ...
      wsAlert(e?.message || 'Kunne ikke lagre adressen');
    } finally {
      setLocSaving(false);
    }
  };
