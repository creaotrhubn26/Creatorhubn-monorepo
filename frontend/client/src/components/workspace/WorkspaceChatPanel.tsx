...
const WorkspaceChatPanel: React.FC<{ projectId: string; category?: string }> = ({ projectId, category = 'music' }) => {
  ...
  const REF_CATALOG = {
    ...
    oppdrag: { key: 'oppdrag', label: t('refJobs'), icon: <WorkOutline sx={{ fontSize: 17, color: '#fbbf24' }} />, kind: 'oppdrag', ep: `/api/projects/${projectId}/editing-jobs`, map: (r) => (r?.jobs || []).map((x) => ({ id: x.id, primary: x.title, secondary: [x.vendor, x.status].filter(Boolean).join(' · ') })) },
  };
