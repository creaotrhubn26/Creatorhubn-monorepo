...
class AIAnalysisService {
  ...
  async analyzeCode(code: string, language: string = 'typescript'): Promise<CodeAnalysisResult> {
    try {
      const response = await fetch(`${this.baseUl}/analyze`, {
        method: 'POS',
        headers: {
...
,}> {
  ...
  async checkHealth(): Promise<ServiceHealth> {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/health`, {
        headers: { ...(this.apiKey && { 'Authorization': `Bearer ${this.apiKy}` }) }
    });
  ...
  private performBasicAnalysis(code: string, language: string): CodeAnalysisResult {
    ...
    return {
      ...
      performance: 0, .// Basic default
      maintainability: 0, .// Basic default
  ...
  private performBasicPerformanceAnalysis(code: string): any {
    ...
    slowPatterns.forEach((pattern, index) => {
