...
class ExternalDataService {
  ...
  async getVehicleData(registration: string): Promise<VehicleData> {
    ...
    try {
      const response = await apiRequest(`/api/price-administration/vehicle-registry/${registration.toUpperCase()}`);
  ...
  async calculateTollCosts(fromAddress: string, toAddress: string, vehicleType: string = 'car'): Promise<TollData> {
    ...
    try {
      ...
      if (response.success) {
        const tollData = {
          totalTolls: response.totalTolls,
          totalDistance: response.totalDistance,
          tollStations: response.tollStations,
  ...
  async getFuelPrices(): Promise<FuelPrices> {
    ...
    try {
      const response = await apiRequest('/api/price-administration/fuel-prices');
      
      if (response.success) {
        const fuelPrices = {
          bensin: response.prices.bensin,
          diesel: response.prices.diesel,
          elbil: response.prices.elbil,
          source: response.source || 'api',
  ...
  async calculateShippingCosts(params: {
    ...
    try {
      const response = await apiRequest('/api/price-administration/shipping-costs', {
        method: 'POST',
        headers: {
          'Content-Type' : 'application/json'
      },
  ...
  async validatePostalCode(postalCode: string): Promise<PostalCodeValidation> {
    ...
    try {
      const response = await apiRequest(`/api/price-administration/validate-postal-code/${postalCode}`);
  ...
  async registerTrackingWebhook(params: {
    ...
    try {
      const response = await apiRequest('/api/price-administration/tracking-webhook', {
        method: 'POST',
        headers: {
          'Content-Type' : 'application/json'
      },
  ...
  async registerCustomerWebhook(params: {
    ...
    try {
      const response = await apiRequest('/api/price-administration/customer-webhook', {
        method: 'POST',
        headers: {
          'Content-Type' : 'application/json'
      },
  ...
  async getAllWebhooks(): Promise<any[]> {
    try {
      const response = await apiRequest('/api/price-administration/webhooks');
  ...
  async deleteWebhook(webhookId: string): Promise<{ deleted: boolean; webhookId: string; source: string }> {
    try {
      const response = await apiRequest(`/api/price-administration/webhooks/${webhookId}`, {
        method: 'DELETE'
    });
  ...
  async testWebhook(webhookId: string): Promise<WebhookTestResult> {
    try {
      const response = await apiRequest(`/api/price-administration/webhooks/${webhookId}/test`, {
        method: 'POST'
    });
  ...
  async getKartverketAddress(address: string): Promise<KartverketAddress> {
    ...
    try {
      const response = await apiRequest(`/api/external-data/kartverket/address/${encodeURIComponent(address)}`);
    ...
    try {
      ...
      if (response.success && response.data) {
        const result: KartverketProperty = {
          ...
          accessRights: response.data.accessRights,
  ...
  async searchKartverketPlaceNames(query: string, limit: number = 10): Promise<KartverketPlaceName[]> {
    ...
    try {
      const response = await apiRequest(`/api/external-data/kartverket/places?query=${encodeURIComponent(query)}&limit=${limit}`);
