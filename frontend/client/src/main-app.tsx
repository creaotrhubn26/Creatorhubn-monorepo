// Full authenticated application bootstrap. The tiny main.tsx route selector
// imports this module only outside the private participant-document portal.
import './lib/workspaceParticipantDocumentCredential';

// Buffer polyfill for browser compatibility (first after credential scrub)
...
import { withInternalApiAuthorization } from './lib/internalApiFetchAuth';
import { isLeadgridStandalonePath } from './lib/leadgridNavigation';
import {
...
const shouldUseRoleRoomDedicatedHostBootstrap = (
  ...
  // /leadgrid/* eies av casting-main også på localhost og hoveddomenet.
  // Uten denne path-gaten startet localhost CreatorHub-routeren, som ikke har
  // Leadgrid-rutene og derfor skjulte rute-/CTA-feil bak feil landingsside.
  if (isLeadgridStandalonePath(pathname)) {
    return true;
  }
  if (
...
(() => {
  ...
  window.fetch = (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    try {
      init = withInternalApiAuthorization(
        input,
        init,
        window.location,
