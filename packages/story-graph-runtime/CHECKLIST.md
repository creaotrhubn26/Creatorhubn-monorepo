# Manuell sjekkliste — C#/GDScript-lasterne

CI kompilerer ikke C# eller GDScript. Kjør denne lista lokalt ved endringer i
`unity/StoryGraphRuntime.cs`, `godot/story_graph_runtime.gd`, motoren
(`frontend/shared/narrative-runtime`, `narrative-script`) eller fixtures.
...
- Advarsler er IKKE del av transkriptet, men `Session.Warnings` skal være tom for fixture-en.

## 4. Kjente avvik (dokumentert, ikke feil)

- `show()`/`reset*()` og komponent-attributter støttes ikke i C#/GDScript (advarsel).
- Flyttall formateres med motorens `ToString`/`str()` — små avvik utover 3 desimaler kan
  forekomme; fixture-en bruker heltall.
